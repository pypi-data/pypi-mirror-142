...
class DrbQueryVisitor(XQueryParserVisitor):
    ...
    def visitModuleImport(self, ctx: XQueryParser.ModuleImportContext):
        ...
        if uri_of_lib is not None:
            node_module = DrbFileFactory().create(uri_of_lib)
            if node_module.has_impl(io.RawIOBase):
                ...
                visitor = DrbQueryVisitor(None)
                output = visitor.visit(tree)

                for key in visitor.static_context.in_scope_variables.keys():
                    var = visitor.static_context.in_scope_variables[key]
                    if var.namespace is not None \
                            and var.namespace != visitor.prefix_module:
                        continue
                        # TODO raise error
                ...
                for key in visitor.static_context.\
                        namespace_builtin_func.keys():
                    namespace, func_name = DrbQueryFuncUtil.\
                        split_namespace_name(key)
                    ...
                    if namespace is not None and \
                        ...
                        # TODO raise error
                    namespace = module_context
                    func = visitor.static_context.namespace_builtin_func[key]

                    self.static_context.namespace_builtin_func[namespace
                                                               + ':' +
                                                               func_name] = \
                        func
        else:
    ...
    def visitFunctionDecl(self, ctx: XQueryParser.FunctionDeclContext):
        if ctx.eqName() is not None:
            ...
            # (namespace, name) = DrbQueryFuncUtil.split_namespace_name()
    ...
    def call_built_in_func(self,
                           name_func: str,
                           built_in_func: XQueryParser.FunctionDeclContext,
                           argument_list: list):
            ...
            for index in range(len(list_def_args)):
                ...
                if def_arg.typeDeclaration() is not None:
                    ...
                    value_arg = self.get_value_from_definition_type(
                        value_arg,
                        type_var_full,
                        name_func + ' arg index ' + str(index + 1))
    ...
    def visitFunctionCall(self, ctx: XQueryParser.FunctionCallContext):
        ...
        if func_to_call is not None:
            return self.call_built_in_func(name_full,
                                           func_to_call, argument_list)
    ...
    def visitComparisonExpr(self, ctx: XQueryParser.ComparisonExprContext):
        ...
        for left_item in list_left:
            for right_item in list_right:
                if op == 'eq' or op == '=':
                    if DrbQueryFuncUtil.get_value(left_item) == \
                            DrbQueryFuncUtil.get_value(right_item) \
                            or str(DrbQueryFuncUtil.get_value(left_item)) \
                            == str(DrbQueryFuncUtil.get_value(right_item)):
                        test_ok = True
                        break
                elif op == 'ne' or op == '!=':
                    if DrbQueryFuncUtil.get_value(left_item) != \
    ...
    def get_elem_cons_name_and_attributes(self, ctx, q_name: str):
        ...
        if namespace is not None:
            ...
            namespace = self.dynamic_context.namespace_default_elt

        node = DrbXqueryItem(None, name, namespace)
    ...
    def visitCompAttrConstructor(self,
        ...
        return DynamicContext(node=name, value=value_attr,
                              namespace=namespace,
                              attribute=True)
        ...
        for scope in self.flwor_scope_list:
            ...
            if isinstance(ret_partial, list):
                ...
                for item in ret_partial:
                    ...
                    current_scope[var_name] = XqueryVariable(var_name,
                                                             value=item,
                                                             type_var=type_var)
                    if var_position is not None:
                        current_scope[var_position] = XqueryVariable(
...
class DrbXqueryParserErrorListener(ErrorListener):
    def syntaxError(self, recognizer, offendingSymbol, line, column, msg, e):
        raise StaticException(ErrorXQUERY.XPST0003,
                              "ERROR: when parsing line " + str(line) +
