...
class StaticContext:
    ...
    def get_var(self, key: str):
        ...
        if key not in self.in_scope_variables.keys():
            raise DynamicException(ErrorXQUERY.XPST0008, "Variable : " +
                                   key + " unknown.")
        return self.in_scope_variables[key]
    ...
    def add_func_build_in(self, func_name: str, func_exp):
        func_name = self.get_key_var(func_name)
        self.namespace_builtin_func[func_name] = func_exp

    def get_func_build_in(self, func_name: str):
        func_name = self.get_key_var(func_name)
        if func_name in self.namespace_builtin_func.keys():
            return self.namespace_builtin_func[func_name]
        return None
...
class DynamicContext:
    ...
    def namespace_uri(self) -> Optional[str]:
        if isinstance(self.node, DrbNode):
            return self.node.namespace_uri
