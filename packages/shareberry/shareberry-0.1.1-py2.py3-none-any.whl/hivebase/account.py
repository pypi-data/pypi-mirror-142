...
class Address(object):
    ...
    def __init__(self, address=None, pubkey=None, prefix="STM"):
        ...
        if pubkey is not None:
            ...
            raise Exception(
                "Address has to be initialized by either the "
...
class PublicKey(object):
    ...
    def compressed(self):
        ...
        p = ecdsa.VerifyingKey.from_string(
            compat_bytes(self), curve=ecdsa.SECP256k1
        ).pubkey.point
        x_str = ecdsa.util.number_to_string(p.x(), order)
        # y_str = ecdsa.util.number_to_string(p.y(), order)
        compressed = hexlify(
            compat_bytes(compat_chr(2 + (p.y() & 1)), "ascii") + x_str
        ).decode("ascii")
        return compressed
...
class PrivateKey(object):
    ...
    * ``PrivateKey("w-i-f").uncompressed.address``:
    ...
    def __init__(self, wif=None, prefix="SHARE"):
        if wif is None:
            import os
    ...
    def compressedpubkey(self):
        ...
        y_str = ecdsa.util.number_to_string(p.y(), order)
        compressed = hexlify(
            compat_chr(2 + (p.y() & 1)).encode("ascii") + x_str
        ).decode("ascii")
