...
class Key(DataDir):
    ...
    def create_table(self):
        ...
        """
        query = (
            "CREATE TABLE %s (" % self.__tablename__
    ...
    def getPrivateKeyForPublicKey(self, pub):
        ...
        """
        query = ("SELECT wif from %s " % (self.__tablename__) + "WHERE pub=?", (pub,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        cursor.execute(*query)
        key = cursor.fetchone()
    ...
    def updateWif(self, pub, wif):
        ...
        """
        query = (
            "UPDATE %s " % self.__tablename__ + "SET wif=? WHERE pub=?",
            (wif, pub),
        )
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
    ...
    def add(self, wif, pub):
        ...
        query = (
            "INSERT INTO %s (pub, wif) " % self.__tablename__ + "VALUES (?, ?)",
            (pub, wif),
        )
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
    ...
    def delete(self, pub):
        """ Delete the key identified as `pub`
           ...
           :param str pub: Public key
        """
        query = ("DELETE FROM %s " % (self.__tablename__) + "WHERE pub=?", (pub,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        cursor.execute(*query)
        connection.commit()
...
class Configuration(DataDir):
    ...
    def checkBackup(self):
        ...
        try:
            if (
                datetime.now()
                - datetime.strptime(configStorage["lastBackup"], timeformat)
            ).days > 7:
    ...
    def _haveKey(self, key):
        ...
        """
        query = ("SELECT value FROM %s " % (self.__tablename__) + "WHERE key=?", (key,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        cursor.execute(*query)
        return True if cursor.fetchone() else False
    ...
    def __setitem__(self, key, value):
        if self._haveKey(key):
            ...
            query = (
                "INSERT INTO %s " % self.__tablename__ + "(key, value) VALUES (?, ?)",
                (key, value),
            )
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
    ...
    def delete(self, key):
        ...
        """
        query = ("DELETE FROM %s " % (self.__tablename__) + "WHERE key=?", (key,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        cursor.execute(*query)
        connection.commit()
...
class KeyEncryptionKey(object):
    ...
    def newKEK(self):
        ...
        if self.config_key in configStorage and configStorage[self.config_key]:
            return
    ...
    def getEncryptedKEK(self):
        ...
        aes = AESCipher(self.userPassphrase)
        return "{}${}".format(
            self.deriveChecksum(self.decrypted_KEK), aes.encrypt(self.decrypted_KEK)
        )
