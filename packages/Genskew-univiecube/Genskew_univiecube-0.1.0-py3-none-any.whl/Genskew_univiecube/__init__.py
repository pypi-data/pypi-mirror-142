...
def plot_sequence(results, filelocation, outputfolder=None, out_file_type=None, dpi=None, skewi=None):
    ...
    if not skewi:
        ...
        ax.set(xlabel='Genome Position', ylabel='GC Skew')
        ax.axvline(x=int(results.max_cm_position), color="green", linewidth=0.3,
                   label="maximum, at " + str(results.max_cm_position))
        ax.axvline(x=int(results.min_cm_position), color="green", linewidth=0.3,
                   label="minimum, at " + str(results.min_cm_position))
...
# test
filename = '/home/julius/sequence2.fasta'

sequence = gen_sequence(filename)
skewi = True
nucleotide1 = 'G'
nucleotide2 = 'C'
stepsize = None
...
sequence_object = Object(sequence, nucleotide1, nucleotide2, stepsize, windowsize)

result = Object.gen_results(sequence_object)

output_folder = None
out_filetype = 'png'
dpi = None

plot_sequence(result, filename, output_folder, out_filetype, dpi, skewi )