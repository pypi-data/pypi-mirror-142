...
*** Version History:
* Version 2.0:
2021-09-15, bug fix in PairedCompSimExperiment.object_pairs()
...
class SubjectThurstone:
    ...
    def response(self, pair, tc_ind=0):
        ...
        return int(np.sum(abs(x) >= self.response_thresholds)) * (1 if x > 0 else -1)
...
class PairedCompSimPopulation:
    ...
    def _rvs_response_thresholds(self, attr, pcf):
        ...
        NOTE: *** For now, assumed same for all attributes ***
        :param attr: string label for desired Attribute
        :param pcf: PairedCompFrame object defining experiment
...
class PairedCompSimExperiment:
    ...
    def object_pairs(self):
        ...
        if self.pair_different:
            ...
            # no pairs presented with equal object indices, like (1, 1)
        else:
            return itertools.product(range(self.pcf.n_objects), repeat=2)
            # allow tuples with equal object indices, like (1, 1)
