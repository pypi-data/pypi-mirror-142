...
"""
# *** CHECK len(objects_alias) == len(objects) warning if mismatch ***
# *** FIX warning messages to display underlying exception message *********
...
class PairedCompFrame:
    ...
    def __init__(self,
        ...
        if objects_alias is not None:
            if len(objects_alias) < len(objects):
                logger.warning(f'objects_alias NOT USED. Must have >={len(objects)} elements.')
...
class PairedCompDataSet:
    ...
    def __init__(self, pcf, pcd):
        ...
        :param pcd: nested dict with elements (group_id: group_data), where
            ...
            *** res could be a pd.DataFrame instance instead ? *****
