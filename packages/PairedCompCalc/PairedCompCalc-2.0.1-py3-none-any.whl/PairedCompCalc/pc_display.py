...
Figures can be saved in various formats, as specified in pc_display_format.
...
*** Usage Example:
    ...
    pc_ds.quality.groups['NormalHearing']['population_mean']['Intelligibility'].range.percentile_plot.ax
...
2021-09-12, cleanup doc text; FMT['show_intevals'] moved to pc_display_format
...
# simplify class names xxxDisplay ?
...
def display(pc_result, **kwargs):  # *** not needed ? ***
    ...
    """
    return PairedCompDisplaySet.display(pc_result, **kwargs)
...
class PairedCompDisplaySet:
    ...
    """
    # *** separate top class for lr_test ? or remove altogether *****
    ...
    def display(cls, pcr, **kwargs):
        ...
        Arne Leijon, 2021-08-27, use pc_model.PairedCompResultSet as input
            and generate predictive models internally, as required
        """
    ...
    def save(self, path):
        ...
        if self.population_mean is not None:
            for (a, a_disp) in self.population_mean.items():
                a_disp.save(path / 'population_mean' / a)
        if self.population_individual is not None:
            for (a, a_disp) in self.population_individual.items():
                a_disp.save(path / 'population_individual' / a)
        if self.subjects is not None:
            for (a, a_disp) in self.subjects.items():
                a_disp.save(path / 'subject_group' / a)
        if self.attr_corr is not None:
            self.attr_corr.save(path)

    @classmethod
    def display(cls, pcm_g):  # input pcf separately ? ****************
        """Generate all displays for ONE group
        :param pcm_g: dict with elements (a_label: pc_result.PairedCompGroupModel)
        :return: cls instance with all displays for this group

        Arne Leijon, 2021-08-29, changed signature, using pc_result.PairedCompGroupModel
            and generate predictive models internally, as required
        """
        pop_ind = None
        pop_mean = None
        ...
        if FMT['population_individual']:
            # *** warning about sufficient number of subjects ?
...
class AttributeDisplay:
    ...
    in ONE population or subject group representing a population

    2021-08-27, new class
    """
    def __init__(self,
                 range,
                 diff=None,
...
class TestCondEffectSet:
    ...
    def display(cls, a, pcf, q, thr, q_map=None):
        ...
        2021-08-28, simplified signature and function, using only model input
        2021-08-31, easier general signature, using only samples as input,
            for all distribution variants
        2021-09-07, using RangeDisplay.create for most data
        """
        q_samples = q.transpose((1, 0, 2))
        # = stored as q[test_cond, sample, system]
        cat_limits = _median_cat_limits(thr, pcf.forced_choice)
        ...
        if len(pcf.test_factors) > 1:
            ...
            perc_by_test_cond = fmt.tab_percentiles(perc_tc,  # skip first ref object ?
                                                    ...
                                                    y_label=a,
                                                    case_labels=[*pcf.test_factors.items(),
                                                                 (FMT['object'], pcf.objects_disp),
                                                                 ],
                                                    case_order=[FMT['object'],
...
class GroupEffectSet:
    ...
    def __init__(self, pop_mean=None, pop_ind=None, subjects=None):  # *** skip subjects here?
        ...
        :param subjects: dict with elements (a_label, subject boxplots by groups???)
        """
    ...
    def display(cls, pcr):
        ...
        # g_keys = [k for k in pcr.models.keys() if type(k) is not tuple]
        pcm = {a: {g: g_models[a]
                   for (g, g_models) in pcr.models.items()
                   if type(g) is not tuple}  # skip merged group, if any
               for a in pcr.pcf.attributes}
        # pcm[a_label][group] = single pc_result.PairedCompGroupModel instance
        pop_mean = None
        pop_ind = None
        if FMT['population_individual']:
            pop_ind = {a: AttributeGroupEffects.display(a, pcr.pcf,  # skip merged group
...
class AttributeGroupEffects:
    ...
    def display(cls, attr, pcf, ag_models):
        ...
        """
        # **** input samples instead, to allow subject_group data ???
        # **** use RangeDisplay.create() instead ???

        q_samples = [m.quality_samples for m in ag_models.values()]
        # stored as q_samples[group][sample, test-cond-tuple, object]
        # might have different n_samples if different n_subjects in groups
        ...
        # as needed by fig_percentiles and tab_percentiles
        q_cdf = np.array([_sample_cdf_0(q.reshape(-1, pcf.n_objects), axis=0)
                          for q in q_samples])
        # q_cdf[g, i] = cdf for i-th object in g-th group

        group_names = [g for g in ag_models.keys()]
        # *** can not display response-category thresholds,
        # *** because the groups have different intervals
        perc_by_group_fig = fmt.fig_percentiles(q_perc,
                                                y_label=attr,
                                                case_labels=[(FMT['group'], group_names),
                                                             (FMT['object'], pcf.objects_disp)]
                                                )
        # = overall perc plot objects by groups
        group_labels = [{FMT['group']: g} for g in ag_models.keys()]
        # q_perc[p, g, i] = p-th percentile for i-th object in g-th group
        # q_cdf[g, i] = cdf for i-th object in g-th group
        perc_by_group_tab = fmt.tab_percentiles(q_perc,  # [1:, ...],  # skip object[0] ???
                                                ...
                                                case_order=[FMT['object'], FMT['group']])
        ...
        q_samples = [qs[..., 1:].reshape(-1, len(tc_s_labels))
                     for qs in q_samples]
        # q_samples[group][n, t_s] = n-th sample for case t_s = index in tc_s_labels
        d = cred_group_diff(q_samples, sample_axis=0, case_axis=1,
                            p_lim=FMT['credibility_limit'])
        group_diff = fmt.tab_credible_diff_3d(d,
                                              y_label=attr,
                                              diff_head=[FMT['group']],  # must be list ???
                                              diff_list=group_labels,
                                              case_head=tc_s_heads,
                                              case_list=tc_s_labels)
...
class RangeDisplay:
    ...
    def create(cls, a_label, q_samples, object_labels,
        ...
        :param q_map: (optional) 2D or 3D array with quality MAP estimates
            q_map[n, i] = n-th quality point estimate of i-th object, OR
            q_map[n, c, i] = n-th quality point estimate of i-th object in c-th case
        :param object_labels: sequence of string labels for tested objects
            len(object_tuple) == q.shape[-1]
        ...
        :return: a cls instance
        """
        # *** generalize with input case_list <- any combination of objects and cases ???
        if q_samples.ndim == 3:
        ...
        range_perc = fmt.fig_percentiles(q_perc,
                                         cat_limits=thr,
                                         y_label=a_label,
        ...
        if q_perc.ndim == 3:
            ...
            case_labels = [case_tuple,
                           (FMT['object'], object_labels)]
            case_order = [FMT['object'], case_tuple[0]]
        else:
        ...
        if q_map is not None:  # *** -> separate classmethod create_with_box
            if q_map.ndim == 3:
                q_map = q_map.transpose((1, 0, 2))
                # q_map[c, n, i] = n-th value for i-th object in c-th case,
