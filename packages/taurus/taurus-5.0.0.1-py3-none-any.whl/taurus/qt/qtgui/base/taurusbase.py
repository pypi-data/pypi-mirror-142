...
class TaurusBaseComponent(TaurusListener, BaseConfigurableClass):
    ...
    def get_registered_formatters():
        ...
        for ep in pkg_resources.iter_entry_points("taurus.qt.formatters"):
            try:
                ...
                taurus.warning(
                    'Cannot load "%s" formatter. Reason: %r', ep.name, e
                )
    ...
    def getParentTaurusComponent(self):
        ...
        raise RuntimeError(
            "Not allowed to call "
            + "TaurusBaseComponent::getParentTaurusComponent()"
        )
    ...
    def setEventBufferPeriod(self, period):
        ...
        if period == 0:
            ...
            self._bufferedEventsTimer = Timer(
                period, self.fireBufferedEvents, self
            )
    ...
    def getModelObj(self):
        """Returns the taurus model obj associated with this component or None
    ...
    def displayValue(self, v):
        ...
        if self._format is None:
            try:
                ...
                self.warning(
                    (
                        "Cannot update format. Reverting to default."
                        + " Reason: %r"
                    ),
                    e,
                )
        ...
        try:
            ...
            self.debug(
                "Invalid format %r for %r. Using '{0}'", self._format, v
            )
            fmt_v = "{0}".format(v)
    ...
    def _updateFormat(self, dtype, **kwargs):
        ...
        if self.FORMAT is None or isinstance(self.FORMAT, str):
            ...
            self._format = self.FORMAT(
                dtype=dtype, basecomponent=self, **kwargs
            )
    ...
    def getFormat(self):
        ...
        if isinstance(self.FORMAT, str):
            ...
            formatter = "{0}.{1}".format(
                self.FORMAT.__module__, self.FORMAT.__name__
            )
        return formatter
    ...
    def _attach(self):
        ...
        if cls is None:
            ...
            try:
                ...
                self._attached = False
                self.debug(
                    "Exception occured while trying to attach '%s'"
                    % self.modelName
                )
                self.traceback()
    ...
    def setModelCheck(self, model, check=True):
        ...
        if model is None:
            model = ""
        model = str(model)
        if check and model == self._localModelName:
            return
...
class TaurusBaseWidget(TaurusBaseComponent):
    ...
    def setUseParentModel(self, yesno):
        ...
        if not is_same:
            ...
            if (
                ...
                and not TaurusBaseWidget._UseParentMsg
            ):
    ...
    def setDisconnectOnHide(self, disconnect):
        ...
        if not self.visible() and not disconnect:
            self.info(
                "Ignoring setDisconnectOnHide to False: widget is not visible"
            )
            return
    ...
    def mouseMoveEvent(self, event):
        ...
        if (
            ...
            return self.getQtClass().mouseMoveEvent(self, event)
        ret = self.getQtClass().mouseMoveEvent(
            self, event
        )  # call the superclass
...
class TaurusBaseWritableWidget(TaurusBaseWidget):
    ...
    def __init__(
        self, name="", taurus_parent=None, designMode=False, **kwargs
    ):
        self.call__init__(
    ...
    def forceApply(self):
        ...
        try:
            v = self.getValue()
            op = WriteAttrOperation(
                self.getModelObj(), v, self.getOperationCallbacks()
            )
