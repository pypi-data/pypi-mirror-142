...
class DockWidgetPanel(Qt.QDockWidget, TaurusBaseWidget):
    ...
    def __init__(self, parent, widget, name, mainwindow):
        ...
        # self._widget = self.widget()  # keep a pointer
        name = str(name)
    ...
    def setWidgetFromClassName(self, classname, modulename=None):
        if ":" in classname:
            # class name as a pkg_resources-like spec ("mod:class"')
            modulename, classname = classname.strip().split(":")
            klass = getattr(importlib.import_module(modulename), classname)
        elif "." in classname:
            self.deprecated(
                dep="specifying classname as 'mod.class'",
                alt="classname='mod:class'",
                rel="5.0.0",
            )
            mn, classname = classname.rsplit(".", 1)
            modulename = ("{}.{}".format(modulename or "", mn)).strip(". ")
        elif not modulename:
            # use TaurusWidgetFactory, deprecated
            from taurus.qt.qtgui.util import TaurusWidgetFactory

            _qt_widgets = TaurusWidgetFactory()._qt_widgets
            modulename, _ = _qt_widgets[self.classname]
            self.deprecated(
                dep="not providing module info for {}".format(classname),
                alt="classname='{}:{}'".format(modulename, self.classname),
                rel="5.0.0",
            )

        klass = getattr(importlib.import_module(modulename), classname)

        # do nothing if the current widget is already of the specified class
        if type(self.widget()) == klass:
            return

        try:
            w = klass()
        except Exception as e:
            m = "Can't create widget for '{}:{}'".format(modulename, classname)
    ...
    def applyConfig(self, configdict, depth=-1):
        # create the widget
        try:
            self.setWidgetFromClassName(
                configdict.get("widgetClassName"),
                modulename=configdict.get("widgetModuleName", None),
            )
    ...
    **Example 3: mixing declarative and programmatic ways**
    ...
    def loadConfiguration(self, confname):
        ...
        try:
            ...
            msg = (
                "Error loading configuration: %s" % traceback.format_exc()
            )  # repr(e)
            self.error(msg)
            Qt.QMessageBox.critical(
                self, "Initialization error", msg, Qt.QMessageBox.Abort
    ...
    def setAllInstrumentAssociations(
