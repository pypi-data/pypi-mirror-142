...
class TaurusForm(TaurusWidget):
    ...
    widget, a units widget and an "extra" widget (some of them may not be
    shown) which are vertically aligned with their counterparts from other
    items.
    ...
    You can also see some code that exemplifies the use of TaurusForm in
    :ref:`Taurus coding examples <examples>`
    """
    ...
    def __getitem__(self, key):
        """provides a list-like interface: items of the form can be accessed
    ...
    def _splitModel(self, modelNames):
        ...
        as separators)
        """
        if isinstance(modelNames, bytes):
    ...
    def chooseModels(self):
        """launches a model chooser dialog to modify the contents of the
        form"""
        if self.__modelChooserDlg is None:
            ...
            self.__modelChooserDlg.modelChooser.updateModels.connect(
                self.setModel
            )
    ...
    def chooseAttrs(self):
        self.info(
            "TaurusForm.chooseAttrs() is deprecated. "
            + "Use TaurusForm.chooseModels() instead"
        )
        self.chooseModels()
    ...
    def getFormWidget(self, model=None):
        ...
        try:
            ...
            try:
                key = (
                    obj.getDeviceProxy().info().dev_class
                )  # TODO: Tango-centric
            except Exception:
                return self._defaultFormWidget, (), {}
            ...
            try:
                ...
                self.warning(
                    'Cannot import "%s". Using default widget for "%s".'
                    % (name, model)
                )
                return self._defaultFormWidget, (), {}
        ...
        for i, model in enumerate(self.getModel()):
            ...
            for ep in self._itemFactories:
                ...
                if widget is not None:
                    self.debug(
                        "widget for '%s' provided by '%s'", model, ep.name
                    )
                    break
            ...
            # todo: UGLY... See if this can be done in other ways...
            #       (this causes trouble with widget that need more vertical
            #       space , like PoolMotorTV)
            ...
            try:
                ...
                self.debug(
                    "Cannot set format %s to child %s", self.getFormat(), model
                )
    ...
    def getItemByModel(self, model, index=0):
        """returns the child item with given model. If there is more than one
        item with the same model, the index parameter can be used to
        distinguish among them Please note that his index is only relative to
        same-model items!
        """
    ...
    def getItems(self):
        """returns a list of the objects that have been created as childs of
        the form"""
    ...
    def onChangeLabelsAction(self):
        ...
        labelConfig, ok = Qt.QInputDialog.getItem(
            self, "Change Label", msg, keys, 0, True
        )
        if ok:
            for item in self.getItems():
    ...
    model = Qt.pyqtProperty(
        "QStringList", getModel, TaurusWidget.setModel, resetModel
    )
    ...
    withButtons = Qt.pyqtProperty(
        "bool", isWithButtons, setWithButtons, resetWithButtons
    )

    modifiableByUser = Qt.pyqtProperty(
        "bool",
        TaurusWidget.isModifiableByUser,
        setModifiableByUser,
...
class TaurusCommandsForm(TaurusWidget):
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        self._operatorViewFilter = (
            lambda x: x.disp_level == DisplayLevel.OPERATOR
        )
    ...
    def _updateCommandWidgets(self, *args):
        """Inserts command buttons and parameter widgets in the layout,
        according to the commands from the model
        """
        ...
        try:
            ...
            self.warning(
                "Problem querying commands from %s. Reason: %s", dev, e
            )
            self._clearFrame()
            return
            ...
            if in_type is not None:
                ...
                if c.cmd_name.lower() in self._defaultParameters:
                    for par in self._defaultParameters.get(
                        ...
                        pwidget.addItem(par)
                    if (self._defaultParameters[c.cmd_name.lower()] or [""])[
                        0
                    ] == "":
                ...
                pwidget.currentIndexChanged["QString"].connect(
                    button.setParameters
                )
    ...
    def _onCommandExecuted(self, result):
        ...
        output = (
            "<i>%s</i><br>" % timestamp.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def setDefaultParameters(self, params):
        """sets the values that will appear by default in the parameters combo
        box, the command combo box for the command will be editable only if the
