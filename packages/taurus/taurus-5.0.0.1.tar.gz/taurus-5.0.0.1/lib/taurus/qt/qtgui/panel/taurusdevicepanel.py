...
def get_eqtype(dev):  # TODO: Tango-centric
    """It extracts the eqtype from a device name like
    domain/family/eqtype-serial
    """
    try:
...
class TaurusDevicePanel(TaurusWidget):
    ...
    def __init__(self, parent=None, model=None, palette=None, bound=True):
        ...
        self._header.layout().addWidget(
            self._image, 0, 0, 2, 1, Qt.Qt.AlignCenter
        )
        self._header.layout().addWidget(self._label, 0, 1, Qt.Qt.AlignLeft)
        self._header.layout().addWidget(
            self._stateframe, 1, 1, 1, 2, Qt.Qt.AlignLeft
        )
    ...
    def loadConfigFile(self, ifile=None):
        ...
        if isinstance(ifile, str) and ifile.endswith(".py"):
            import importlib

            spec = importlib.util.spec_from_file_location("config_file", ifile)
    ...
    def setModel(self, model, pixmap=None):
        ...
        self.trace(
            "In TaurusDevicePanel.setModel(%s(%s),%s)"
        ...
        try:
            ...
            if self.getModel():
                # Do not dettach previous model before pinging the new one
                # (fail message will be shown at except: clause)
                self.detach()
    ...
    def get_attrs_form(self, device, form=None, filters=None, parent=None):
        ...
        self.trace(
            "In TaurusDevicePanel.get_attrs_form(%s,%s)" % (device, filters)
        )
        allattrs = sorted(
            str(a)
            for a in taurus.Device(device).get_attribute_list()
        ...
        if attrs:
            ...
            try:
                ...
                self.warning(
                    "TaurusDevicePanel.ERROR: Unable to setModel "
                    + "for TaurusDevicePanel.attrs_form!!: %s",
    ...
    def get_comms_form(self, device, form=None, parent=None):
        ...
        if TaurusDevicePanel._command_filter and not params:
            self.debug(
                "TaurusDevicePanel.get_comms_form(%s): "
                + "By default an unknown device type will display no commands",
                device,
            )
            return None
        if not form:
        ...
        try:
            ...
            if params:
                form.setSortKey(
                    lambda x, vals=[s[0].lower() for s in params]: vals.index(
                        x.cmd_name.lower()
                    )
                    if str(x.cmd_name).lower() in vals
                    else 100
                )
                form.setViewFilters(
                    [
                        lambda c: str(c.cmd_name).lower()
                        not in ("state", "status")
                        and any(
...
class TaurusDevPanel(TaurusGui):
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        # @todo: This should be done in the ui file when the
        #        TaurusDatabaseTree Designer plugin is available
...
@click.option(
    ...
    "--filter",
    help=(
        "regexp to filter for attributes to show "
...
@click.option(
    "-t", "--trend", is_flag=True, help="Create a temporal trend widget"
)
