...
class TaurusReadWriteSwitcher(TaurusWidget):
    ...
    constructor of TaurusReadWriteSwitcher:
        ...
        - `enterEditTriggers` is a tuple containing one or more of the
          following:

            - key shortcut (either a Qt.Qt.Key or a QKeySequence)
            - event type on the read widget (a Qt.QEvent.Type)
            - signal from the read widget (a str representing a Signal
              signature)

        - `exitEditTriggers` is a tuple containing one or more of the
          following:

            - key shortcut (either a Qt.Qt.Key or a QKeySequence)
            - event type on the write widget (a Qt.QEvent.Type)
            - signal from the write widget (a str representing a Signal
              signature)
    ...
    def __init__(
        ...
        self.enterEditAction.triggered.connect(
        ...
        for sig in self.exitEditSignals:
            try:
                ...
                if isinstance(e, AttributeError) and hasattr(Qt, "SIGNAL"):
                    ...
                    self.connect(
                        self.writeWidget, Qt.SIGNAL(sig), self.exitEdit
                    )
                    self.debug(
                        "Cannot connect %s using new style signal."
    ...
    def _updateSizePolicy(self):
        """Update the size policy of the switcher widget to the most
        restrictive combination of the policies of the read and write widgets
        """
        policy = None
            ...
            if w is not None:
                ...
                if policy is None:
                    ...
                    policy = Qt.QSizePolicy(
                        Qt.QSizePolicy.Policy(h), Qt.QSizePolicy.Policy(v)
                    )
        if policy is not None:
            self.setSizePolicy(policy)

    def setModel(self, model):
        """This implementation propagates the model to the read and write
        widgets.You may reimplement it to do things like passing different
        models to each.
        """
        if self.readWidget is not None:
...
def demo1():
    ...
    w = TaurusReadWriteSwitcher(
        readWClass=TaurusLabel, writeWClass=TaurusValueLineEdit
    )
...
def demo3():
    ...
    w1 = TaurusReadWriteSwitcher(
        readWClass=TaurusLabel, writeWClass=TaurusValueLineEdit
    )
    w1.model = "sys/tg_test/1/long_scalar"

    w2 = TaurusReadWriteSwitcher(
        readWClass=TaurusLed, writeWClass=TaurusValueCheckBox
    )
