...
from taurus.qt.qtgui.extra_guiqwt.styles import (
    TaurusCurveParam,
    TaurusTrendParam,
)
...
class TaurusCurveItem(CurveItem, TaurusBaseComponent):
    """A CurveItem that autoupdates its values & params when x or y components
    change"""
...
class TaurusTrendItem(CurveItem, TaurusBaseComponent):
    """A CurveItem that listens to events from a Taurus scalar attribute and
    appends new values to it"""
    ...
    def handleEvent(self, evt_src, evt_type, evt_value):
        ...
        if self.__xBuffer is None:
            self.__xBuffer = ArrayBuffer(
                numpy.zeros(
                    min(128, self.taurusparam.maxBufferSize), dtype="d"
                ),
...
def taurusTrendMain():
    ...
    plot = w.get_plot()
    for a in args:
        item = TaurusTrendItem(
            stackMode=stackMode, buffersize=int(options.max_buffer_size)
        )
        plot.add_item(item)
        item.setModel(a)
