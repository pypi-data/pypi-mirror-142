...
The files in the configuration dir determine the default, permanent,
pre-defined contents of the GUI. While the user may add/remove more elements at
run time and those customizations will also be stored, this file defines what a
...
class IntroPage(BasePage):
    ...
    def _setupUI(self):
        ...
        self.setPixmap(
            Qt.QWizard.WatermarkPixmap,
            Qt.QIcon.fromTheme("document-properties").pixmap(120, 120),
        )
    ...
    def getIntroText(self):
        text = (
            ...
            + "TaurusGui-based applications are very customizable. "
            + "The user can add/remove elements at run time and store those "
            + "customizations. So with this wizard you will define "
            + "just the default contents of the GUI."
        )
        return text
...
class ProjectPage(BasePage):
    def __init__(self, parent=None):
        ...
        self.setSubTitle(
            "Choose a location for the application files "
    ...
    def _setupUI(self):
        ...
        self._projectDirLE.setToolTip(
            "This directory will be used to store all files "
            + "needed by the application."
        )
        self._projectDirBT = Qt.QPushButton(
            Qt.QIcon.fromTheme("document-properties"), "..."
        )
        ...
        if len(configs_found) == 1:
            ...
            if option == Qt.QMessageBox.Yes:
                try:
                    ...
                    Qt.QMessageBox.warning(
                        self,
                        "Error loading project configuration",
...
class GeneralSettings(BasePage):
    ...
    def initializePage(self):
        ...
        self.wizard().__setitem__(
            "organizationName", self._getOrganizationName
        )
    ...
    def fromXml(self, xml):
        ...
        self._organizationCombo.setEditText(
            AppSettingsWizard.getValueFromNode(
                xml, "ORGANIZATION", default="Taurus"
            )
        )
    ...
    def _setupUI(self):
        ...
        self._layout.addWidget(
            self._guiNameLabel, 1, 0, 1, 1, Qt.Qt.AlignRight
        )
        self._layout.addWidget(
            self._guiNameLineEdit, 1, 1, 1, 1, Qt.Qt.AlignRight
        )
        ...
        self._layout.addWidget(
            self._organizationNameLabel, 2, 0, 1, 1, Qt.Qt.AlignRight
        )
        self._layout.addWidget(
            self._organizationCombo, 2, 1, 1, 1, Qt.Qt.AlignRight
        )
...
class CustomLogoPage(BasePage):
    def __init__(self, parent=None):
        BasePage.__init__(self, parent)
        self._customLogoDefaultPath = getattr(
    ...
    def _setupUI(self):
        ...
        self._label = Qt.QLabel(
            "\nIf you want to have a custom logo inside your application "
            + "panel, please select the image file. \n"
        )
        ...
        self._customLogoDefaultButton.setIcon(
            Qt.QIcon("actions:edit-undo.svg")
        )
        ...
        self._layout.addWidget(
            self._customLogoLineEdit, 2, 1, Qt.Qt.AlignRight
        )
        ...
        self._layout.addWidget(
            self._customLogo, 4, 1, 1, 1, Qt.Qt.AlignHCenter
        )
    ...
    def _setImage(self, image):
        if type(image) == Qt.QPixmap:
            ...
            self._customLogo.setPixmap(
                Qt.QPixmap()
                .fromImage(image)
                .scaled(60, 200, Qt.Qt.KeepAspectRatio)
            )
        else:
            self._customLogo.setPixmap(
...
class SynopticPage(BasePage):
    ...
    def _setupUI(self):
        ...
        self._label = Qt.QLabel(
            "If you want to add one or more synoptic panels "
            + "(graphical views of instruments) select the corresponding "
            + "JDRAW files here\n"
        )
        self._label.setWordWrap(True)
    ...
    def _moveUp(self):
        if len(self._synopticList.selectedIndexes()) > 0:
            ...
            if self._synoptic_id > 0:
                ...
                self._refreshSynopticList()
                self._synopticList.setCurrentIndex(
                    self._synopticList.indexFromItem(
...
class MacroServerInfoPage(BasePage):
    ...
    def initializePage(self):
        ...
        self._label.setText(
            (
                "\n <b>%s</b> can communicate with a Sardana's Macro Server "
    ...
    def checkData(self):
        BasePage.checkData(self)
        if (self._macroGroupBox.isChecked()) and len(
            self._confWidget.macroServerComboBox.currentText()
        ):
...
class InstrumentsPage(BasePage):
    ...
    def initializePage(self):
        ...
        self._label.setText(
            (
                "<b>%s</b> can use instrument information stored in "
                + "the Sardana's Pool to create instrument panels."
            )
            % self.wizard().__getitem__("guiName")
        )
    ...
    def _setupUI(self):
        ...
        self._layout.addWidget(
            self._instrumentsLabel, 5, 0, 1, 1, Qt.Qt.AlignRight
        )
        self._layout.addWidget(
            self._intstrumentsBoolean, 5, 1, 1, 1, Qt.Qt.AlignRight
        )
...
class PanelsPage(BasePage):
    ...
    def _addPanel(self):
        ...
        if ok:
            _ = paneldesc.getWidget()  # TODO: is this needed?
    ...
    def _moveUp(self):
        if len(self._panelList.selectedIndexes()) > 0:
            ...
            if self._panel_id > 0:
                ...
                self._refreshPanelList()
                self._panelList.setCurrentIndex(
                    self._panelList.indexFromItem(
...
class ExternalAppEditor(Qt.QDialog):
    ...
    # @todo: this class should be made more generic
    #        (e.g. provide a getter for an ExternalAppAction) and then moved
    #        elsewhere
...
class ExternalAppPage(BasePage):
    ...
    def _setupUI(self):
        ...
        self._label = Qt.QLabel(
            "The GUI may include shortcuts to external applications. "
    ...
    def _moveUp(self):
        if len(self._externalAppList.selectedIndexes()) > 0:
            ...
            if self._app_id > 0:
                ...
                self._refreshApplicationList()
                self._externalAppList.setCurrentIndex(
                    self._externalAppList.indexFromItem(
...
class MonitorPage(BasePage):
    ...
    def _setupUI(self):
        ...
        self._label = Qt.QLabel(
            "\nIf you want to monitor some attributes, "
            + "add them to the monitor list. \n"
        )
        self._label.setWordWrap(True)
        self._layout.addWidget(self._label, 0, 0, 1, 4)
    ...
    def _selectMonitor(self):
        models, ok = TaurusModelChooser.modelChooserDlg(host=None)
        if ok:
...
class OutroPage(BasePage):
    def __init__(self, parent=None):
        ...
        self._substTable.setSizePolicy(
            Qt.QSizePolicy(Qt.QSizePolicy.Expanding, Qt.QSizePolicy.Expanding)
        )
    ...
    def createProject(self):
        ...
        logfile.write(
            "Project created by AppSettingsWizard on %s\n"
        ...
        for i in range(self._substTable.rowCount()):
            ...
            dst = os.path.join(
                install_dir, str(self._substTable.item(i, 1).text())
            )
        ...
        xmlcfgfilename = os.path.join(
            install_dir, self.wizard().getXmlConfigFileName()
        )
        ...
        pycfgfilename = os.path.join(
            install_dir, "%s.py" % self.wizard().getConfigFilePrefix()
        )
        ...
        # if all went ok...
        msg = (
            "Application project was successfully created."
...
class AppSettingsWizard(Qt.QWizard):
    ...
    The files in the configuration dir determine the default, permanent,
    pre-defined contents of the GUI. While the user may add/remove more
    elements at run time and those customizations will also be stored, this
    file defines what a user will find when launching the GUI for the first
    time."""
    ...
    def __init__(
        self, parent=None, jdrawCommand="jdraw", configFilePrefix="config"
    ):
    ...
    def generateXml(self):
        """returns the xml code corresponding to the options selected in the
        wizard and a dictionary representing the paths that have been
        substituted.
        ...
        mod_dir = os.path.join(
            self.__getitem__("projectDir"), "tgconf_" + guiName.text
        )
        ...
        if synopticList:
            ...
            for src in synopticList:
                ...
                etree.SubElement(
                    synoptics, "synoptic", str=os.path.basename(dst)
                )
                ...
                for ref in re.findall(r"file_name:\"(.+?)\"", contents):
                    ...
                    if ref != refdst:
                        ...
                        _long = (
                            'The synoptic file "%s" references a file that '
                            + "has been copied to the project dir in order to "
                            + "make the project portable. "
                            + 'Please edit "%s" and replace "%s" by "%s"'
                        ) % (dst, dst, ref, refdst)
