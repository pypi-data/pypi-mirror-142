...
class TaurusGuiComponentDescription(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.classname is None and (
            ...
            raise ValueError(
                "Module info must be given"
                + "(except if passing a Taurus class name)"
            )
    ...
    def fromXml(xmlstring):
        ...
        if (sharedDataWriteNode is not None) and (
            sharedDataWriteNode.text is not None
        ):
        ...
        if (sharedDataReadNode is not None) and (
            sharedDataReadNode.text is not None
        ):
            for child in sharedDataReadNode:
                if (child.get("datauid") is not None) and (
                    child.get("slotName") is not None
                ):
                    sharedDataRead[child.get("datauid")] = child.get(
                        "slotName"
                    )
