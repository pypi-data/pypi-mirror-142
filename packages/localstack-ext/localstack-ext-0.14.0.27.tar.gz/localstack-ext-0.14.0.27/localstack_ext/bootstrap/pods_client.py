...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:str=None,three_way:bool=False)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=True)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(len(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=len(zip_data_content)
  return result
 def pull(self,inject_version_state:bool=False,reset_state_before:bool=False,lazy:bool=False):
  ...
  rm_rf(zip_path)
 @staticmethod
 def presigned_url(pod_name:str,mode:str)->str:
  ...
  if response.status_code>=400:
   raise Exception("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise Exception("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:str=None,three_way:bool=False):
