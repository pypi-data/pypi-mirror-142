...
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=set())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=None,state_files=set(),version_number=0)
 ...
 with open(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with open(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with open(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:str,meta_archives:Dict[str,str],state_archives:Dict[str,str],remote_info:Dict[str,str],pod_name:str):
...
def merge_from_remote(version_space_archive:str,meta_archives:Dict[str,str],state_archives:Dict[str,str]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->bool:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:str):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:str)->bool:
 ...
 return True
def list_locally_available_pods(show_remote_or_local:bool=True)->Set[str]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:str,file_name:str,service:str,region:str,root:str,serialization:Serialization,account_id=TEST_AWS_ACCOUNT_ID)->str:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->str:
 ...
 return key
def _get_state_file_path(key:str)->str:
...
def list_state_files(key:str)->Optional[str]:
...
def get_version_info(version_no:int)->Dict[str,str]:
 ...
 return result
def commit(message:str=None)->Revision:
...
def create_version_space_archive()->str:
...
def get_max_version_no()->int:
 with open(config_context.get_max_ver_path())as fp:
...
def push_overwrite(version:int,comment:str)->bool:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=True,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return True
def push(comment:str=None,three_way:bool=False,include_assets:bool=True)->Version:
 ...
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,set())
 ...
 if include_assets:
  _add_assets_to_version_state_archive(new_version.version_number)
 ...
 return new_version
def _add_assets_to_version_state_archive(version_number:int,cleanup:bool=True):
 archive_path=config_context.get_version_state_archive_path(version=version_number)
 assets_path=config_context.get_assets_root_path()
 with zipfile.ZipFile(archive_path,"a")as archive:
  for folder_name,subfolders,file_names in os.walk(assets_path):
   for file in file_names:
    path=os.path.join(folder_name,file)
    archive.write(filename=path,arcname=os.path.relpath(path,start=config_context.get_pod_root_dir()))
 if cleanup:
  rm_rf(assets_path)
def _get_dst_path_for_state_file(version_state_dir:str,state_file:StateFileRef):
 ...
 mkdir(dst_path)
 return dst_path
def _create_state_directory(version_number:int,state_file_refs:Set[StateFileRef],delete_files=False,archive=True):
...
def set_active_version(version_no:int,commit_before=False)->bool:
 ...
 return False
def _set_active_version(key:str):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=set(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:int)->Version:
...
def load_version_references()->List[Tuple[int,str]]:
 result={}
 with open(config_context.get_known_ver_path(),"r")as vp:
...
def list_versions()->List[str]:
 ...
 return result
def list_version_commits(version_no:int)->List[str]:
 if version_no==-1:
...
def _add_known_ver(new_ver_no,new_ver_key)->str:
 with open(config_context.get_known_ver_path(),"a")as fp:
...
def merge_expansion_point_with_max(three_way=False):
 ...
 for serialization_mechanism,new_version_state_files_path in new_version_tmp_dirs.items():
  try:
   merge_manager=CPVCSMergeManager.get(serialization_mechanism,raise_if_missing=True)
...
def add_assets_to_pod(assets_root_paths:List[str]):
 for assets_root_path in assets_root_paths:
  pod_asset_root=os.path.join(config_context.get_assets_root_path(),os.path.basename(assets_root_path))
  try:
   cp_r(src=assets_root_path,dst=pod_asset_root)
  except Exception as e:
   LOG.warning("Failed to copy assets for %s: %s",assets_root_path,e)
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->str:
 try:
