...
def is_admin():
        async def predicate(ctx):
            return ctx.author.display_name in info['FullAccess'] and ctx.author.display_name in adminsss
...
class PartyBot(commands.Bot):
    ...
    async def event_ready(self) -> None:
        ...
        try:
            ...
            payload = f"api_key=u1655828-b1abae81aed8749790ded8a4&format=json&type=1&url=https://{os.getenv('REPL_SLUG')}.{os.getenv('REPL_OWNER').repl.co}&friendly_name={self.user.display_name}"
            headers = {
