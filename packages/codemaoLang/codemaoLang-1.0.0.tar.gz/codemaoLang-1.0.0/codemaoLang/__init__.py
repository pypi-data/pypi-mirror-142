...
class user:
    def __init__(self, identity: str, password: str):
        ...
        self.grade = user_info['grade']
        self.grade_desc = user_info['grade_desc']
    ...
    def logout(self):
        ...
        self.auth.data = self.auth.token = self.auth.phone_number = self.auth.email = self.auth.has_password = \
            self.auth.is_weak_password = self.data = self.id = self.nickname = self.avatar_url = \
            self.fullname = self.birthday = self.sex = self.qq = self.description = \
            self.grade = self.grade_desc = self.info.data = self.info.id = self.info.nickname = self.info.avatar = \
