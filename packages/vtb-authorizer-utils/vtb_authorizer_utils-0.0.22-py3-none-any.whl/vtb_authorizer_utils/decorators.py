...
def authorizer_resource_type(service: str,
                             ...
                             default_access_type: Optional[str] = 'public',
                             url_pattern_postfix: Optional[str] = '/', prefixes=None):
    """
    ...
    def decorator(view_class):
        if base_url_pattern:
            register_default_authorizer_resource_rules(view_class, base_url_pattern,
                                                       ...
                                                       url_pattern_postfix=url_pattern_postfix, prefixes=prefixes)

        AuthorizerSchemaBuilder().add_resource_type(service=service, name=name, title=title, description=description, cls=view_class)
        return view_class
...
def authorizer_resource_rule(http_method: str,
                             ...
                             context_types: Optional[Set[ContextType]] = None, prefixes=None):
    """
    ...
    * - указывает, что в этом месте может быть любое значение, например идентификатор сущности
    ...
    def decorator(view_method):
        ...
        AuthorizerSchemaBuilder().add_resource_rule(view_method, http_method.upper(), url_pattern,
                                                    code, access_type, operation_name,
                                                    view_method.__qualname__,
                                                    context_types=context_types, prefixes=prefixes)
...
def register_default_authorizer_resource_rules(view_class,
                                               ...
                                               detail_id_str: Optional[str] = '*', prefixes=None):
    ...
    _register_default_http_methods(view_class, methods, default_context_types, default_access_type, prefixes=prefixes)
...
def _register_default_http_methods(view_class,
                                   ...
                                   default_context_types: Optional[Set[ContextType]] = None,
                                   default_access_type: Optional[str] = 'public', prefixes=None):
    ...
    for method, info in methods.items():
        if hasattr(view_class, method):
            ...
            if callable(view_method):
                ...
                http_method = info[0]
                AuthorizerSchemaBuilder().add_resource_rule(view_method, http_method.upper(), info[1], method,
                                                            default_access_type,
                                                            operation_name, name,
                                                            context_types=default_context_types, prefixes=prefixes)
