...
class Api():
    ...
    """
    with open('janda/base/Base.json', 'r', encoding='utf-8') as f:
        nh = json.load(f)

    def __init__(self,
                 BASE_URL: str = nh['nhentai']['api'],
                 BASE_IMG: str = nh['nhentai']['img'],
                 BASE_PURURIN: str = nh['pururin']['api'],
                 BASE_HENTAIFOX: str = nh['hentaifox']['api'],
                 BASE_HENTAI2READ: str = nh['hentai2read']['api']):
    ...
    for image in raw_object['images']['pages']:
        if image['t'] == 'p':
            thumbnail_urls.append(
                f'{BASE_URL.img}/{media}/{len(thumbnail_urls) + 1}t.png')

        elif image['t'] == 'j':
            thumbnail_urls.append(
                f'{BASE_URL.img}/{media}/{len(thumbnail_urls) + 1}t.jpg')

        elif image['t'] == 'g':
            thumbnail_urls.append(
                f'{BASE_URL.img}/{media}/{len(thumbnail_urls) + 1}t.gif')
