...
def train(hyp, opt, device, callbacks):
    """
    hyp: 模型训练的超参数可以是 yaml 文件也可以是 dict 文件，path/to/hyp.yaml or hyp dictionary
    """
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze

    # save_dir: 模型训练阶段各种文件的保存路径: runs\train\exp4, Directories
    w = pathlib.Path.joinpath(save_dir, 'weights')  # 模型权重的保存地址-weights dir
    ...
    nc = 1 if single_cls else int(data_dict['nc'])  # 一共有多少类别
    single_cls = nc == 1  # 在数据集的类别只有一个的时候，为单分类
    names = ['item'] if single_cls and len(data_dict['names']) != 1 else data_dict['names']  # class names
    assert len(names) == nc, f'{len(names)} names found for nc={nc} dataset in {data}'  # check
    is_coco = isinstance(val_path, str) and val_path.endswith('coco/val2017.txt')  # 判断是否是 COCO dataset
    ...
    # 优化 BN 的权重, 无衰减
    if opt.optimizer == 'Adam' or opt.optimizer == 'AdamW':
    ...
    # 移动平均数, 模型更新的比较缓慢，Exponential Moving Average：EMA
    ema = ModelEMA(model) if RANK in [-1, 0] else None

    # 是否从上次继续训练， 最佳的拟合值是多少 - Resume
    start_epoch, best_fitness = 0, 0.0
    if pretrained:
        if ckpt['optimizer'] is not None:  # Optimizer
            optimizer.load_state_dict(ckpt['optimizer'])
            best_fitness = ckpt['best_fitness']

        if ema and ckpt.get('ema'):  # EMA
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, help='initial weights path',
                        default=os.path.join(ROOT, 'weights', 'yolov5s.pt'))  # 模型权重的选择
    parser.add_argument('--cfg', type=str, default=os.path.join(ROOT, 'models', 'yolov5s.yaml'),
                        help='model.yaml path')  # 模型的配置文件
