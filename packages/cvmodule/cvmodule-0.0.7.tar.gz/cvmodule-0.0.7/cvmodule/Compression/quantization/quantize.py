...
if __name__ == '__main__':
    ...
    if pruned:
        ...
        net.load_state_dict(
            torch.load(
                os.path.join(checkpoint, 'pruned_False_4_bits.pth'), map_location=torch.device('cpu')
            )
        )

    weights_data = OrderedDict()
    for k, v in net.state_dict().items():
        if 'classifier' not in k and 'num_batches' not in k and 'running' not in k:
            if 'weight' in k:
                bias_name = k.replace('weight', 'bias')
                weights_data[k] = v.char()
                weights_data[bias_name] = net.state_dict()[bias_name].char()
        else:
            weights_data[k] = v

    net.load_state_dict(weights_data)
    for k, v in net.state_dict().items():
        print(k, v.dtype, weights_data[k].dtype)
