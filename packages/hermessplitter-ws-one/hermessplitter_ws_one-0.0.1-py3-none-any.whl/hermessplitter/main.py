...
class HermesSplitter(WeightSplitter):
    ...
    def activate(self, carnum, kf=0.19, client=None):
        """ Активировать HERMES """
        if functions.check_hermes_active(self.hermes_sqlshell):
            kf = functions.get_kf(self.wdb_sqlshell, self.hermes_sqlshell,
                                  carrier=client)
    ...
    def make_magic(self, data):
        try:
            if self.active and data.isdigit() and self.avg_tara != 0 and \
