...
class Smart(Generic[T]):
    ...
    @property
    def register(self) -> SmartRegister:
    ...
    def init(
        ...
        if name is None:
            ...
            obj = self._init_object(
                obj=self.get(name),
                location=strutil.join_keys(self._location, name),
            )
    ...
    def _init_from_dict(
        ...
        if name in self.__register.origins:
            name = self.__register.origins[name]
        ...
        for i, param in enumerate(signature.parameters.values()):
            ...
            if not (i == 0 and annotation is param.empty and default is param.empty) or kind in (
                ...
                if annotation is Smart or isinstance(default, Smart) and default.type is dict:
                    ...
                    keyword = self.__register.aliases.get(keyword, keyword)
