...
class S3Path:
	def __init__(self, s3, path):
		...
		self._s3 = s3
		self._path = path
		self._name_and_extension = self._path.rstrip('/').split('/')[-1]
		if '.' in self.name_and_extension:
			self._name, self._extension = self.name_and_extension.rsplit('.')
		else:
			self._name = self.name_and_extension
