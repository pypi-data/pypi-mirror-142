...
class API:
    ...
    def connect_to_voice(self, user_id: str) -> bool:
        ...
        tmp = self.__wait_for_response()
        if not tmp["bool"]: print(tmp["Data"])
        return tmp["bool"]
    ...
    def play_file(self, path: str) -> bool:
        self.sending = True
        self.__send({"Command": "Play Audio File", "Value": path})
        tmp = self.__wait_for_response()
        return tmp == "True"
    ...
    def pause_currently_playing(self) -> bool:
        self.sending = True
        self.__send({"Command": "Pause Currently Playing", "Value":None})
        tmp = self.__wait_for_response()
        return tmp == "True"

    def resume_paused(self) -> bool:
        self.sending = True
        self.__send({"Command": "Resume Paused", "Value":None})
        tmp = self.__wait_for_response()
        return tmp == "True"

    def skip_currently_playing(self) -> bool:
        self.sending = True
        self.__send({"Command": "Skip Currently Playing", "Value":None})
        tmp = self.__wait_for_response()
        return tmp == "True"

    def stop_currently_playing(self) -> bool:
        self.sending = True
        self.__send({"Command": "Stop Currently Playing", "Value":None})
        tmp = self.__wait_for_response()
        return tmp == "True"
    ...
    def set_as_hook_for_track_finished(self, callable: Callable[[Message], None]) -> bool:
