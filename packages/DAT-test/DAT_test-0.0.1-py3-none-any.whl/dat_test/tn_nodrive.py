# -*- coding: utf-8 -*-
"""Traffic_Network_model_withoutDrive.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1wQfP1QFLyUSPieWA_4o8-c5Nkv13mZVi
...
class ConnectionInterPriorityError(AllMyError):
    """
    Error class generating error related to wrong list of pod for priority
    """

class NotImplementedError(AllMyError):
    """
    Error class generating all errors linked to an abstract class method not been proprely inmplemented
    """

class DecodingError(AllMyError):
    """
    Error class generating all errors linked to unabilty to imported a model
    """

class Error(AllMyError):
    """
...
class HiddenPrints:
    """
    Class define for hinding print in part of the code
    """
    def __enter__(self):
...
class Utilities:
    """
    Class used to stock all the utilities functions
    ...
    def show_information(self,function):
        ...
        Parameters
        ----------
            Function : the function about wich we want the doc
    ...
    def get_function_param(self,function):
        """
        return the list of the attribut and default value of a function

        Parameters
        ----------
            Function : the function about wich we want the param
    ...
    def show_all(self,more=False):
        ...
        for i in range(len(class_list)):
            ...
            for j in range(len(function_list_name)):
                ...
                s1+="\n"
                if (more):
                    arg_list=self.get_function_param(function_list[j])
    ...
    def get_car_type(self):
        ...
        for i in range(len(list_class)):
            class_object=list_class[i](0,0,0,0)
...
class Predifine(Cars):
    ...
    def get_path(self,previous_state):
        ...
        if len(previous_state.intersections[inter_num].roads_exit)==1:
            ...
            raise Error("no road exit after this one")
        else:
    ...
    def __create_car(self,number0,p,l,type0):
        ...
        try:
            ...
            with HiddenPrints(): #for blocking print in case two car type have the same number (one warning at the creation of the network is enough)
                type_car_dict=utilities.get_car_type()#get the dict of all the Car type and there key (type number)
            car_object=type_car_dict[type0](type_road,num_road,number0,p,l)#creation of the car
        except KeyError:
            raise ValidTypeNumberCarsError("number "+str(type0)+" doesn't match a type in the car type dict")
        except Exception as e:
            print("unespected error")
            raise e
        return car_object
...
class RoadNetwork:
    ...
    def __init__(self):
        ...
        self.__car_type_dict=utilities.get_car_type()#dict of all the car type (the key is the num of the car type, the value is the class.)
    ...
    def __repr__(self):
        ...
        for i in self.roads: #loop on all the road of the network (get all information about all the road)
            if (self.roads[i].type_road=="normal"):
                ...
                raise ValidTypeRoadError("unidentified road type found")
    ...
    def set_intersection_priority(self,num_inter,liste_priority):
        ...
        for i in range(k):
            ...
            if (weight<0):
                raise Error("invalid priority weight : weight can't be negative")
            total_weight+=weight
        ...
        if (k>n):
            ...
            raise ConnectionInterPriorityError(message)
    ...
    def get_next_road(self,car):
        ...
        if (n==1):
            ...
            raise Error("no available path toward exit for this car.")
...
class CreateNetwork:
    ...
    def add_intersection(self, label, cordX=0, cordY=0):
        ...
        if label in keys:
            raise Error("label already use please use another one")
    ...
    def switch_labels(self,prior_road_label_list=[],new_road_label_list=[],prior_intersection_label_list=[],new_intersection_label_list=[]):
        ...
        if (len(prior_road_label_list)!=len(new_road_label_list)) or (len(prior_intersection_label_list)!=len(new_intersection_label_list)):
            raise Error("prior and new label list must be of equal size")
    ...
    def linked_road(self,list_begin_intersection,list_exit_intersection,list_label,L=None,vmax=None,class_speed=None):
        if (len(list_begin_intersection)!=len(list_exit_intersection)):
            raise Error("list_begin_intersection and list_exit_intersection must be of equal size")
    ...
    def swap(self,axis,axis_pos):
        ...
        if (axis=="vertical"):
            ...
            raise Error("argument axis must be either vertical or horizontal")
    ...
    def create_grid(self,height_road_number,height_road_size,length_road_number=None,length_road_size=None,vm=20,vclass=LinearSpeed(),size=1):
        ...
        if (height_road_number<=0 or length_road_number<=0):
            raise Error("cannot create a network with negative or zero lenght")
    ...
    def create_traffic_circle(self,road_length,number_of_intersection,out_road_length,list_direction=None,vm=20,vclass=LinearSpeed(),coeff_size=1):
        ...
        try:
            ...
            print(message)
        except ConnectionInterPriorityError:
    ...
    def __transform_string_to_speed(self,string):
        ...
        if (string=="constant speed calculation function"):
            ...
            raise DecodingError(string+" is unknow speed class")
    ...
    def __transform_string_to_intersection(self,string):
        ...
        for i in range(len(entry_road_list)):
            ...
            if (len(key)!=2):
                raise DecodingError("wrong size of key for road in intersection creation")
    ...
    def __transform_string_to_road(self,string):
        ...
        if (boolean=="False"):
            ...
            raise DecodingError("non boolean found for road.has_priority")
    ...
    def __transform_string_to_model(self,string):
        ...
        if (len(new_list_string)!=5):
            raise DecodingError("error in decoding file, wrong splitting in different model part")
    
        #entry roads part
        if (entry_road_string[0:10]!="entry road"):
            raise DecodingError("file doesn't follow the correct configuration for model")
...
class SimulateNetwork:
        ...
        for i in range(len(types)):
            if types[i]>0:
                if k==0:
                    ...
                    raise Error("this functiononly work with two different type, not more.")
                k+=1
                n+=types[i]  
        ...
        while (inter_key_current not in inter_already_pass_by and i<n):
            if (len(self.__road_network.intersections[inter_key_current].roads_exit)>1):
                ...
                if (sum(list_list_choice[i])!=1):
                    raise Error("list_list_choice must have a sum either o or 1 has value and sum must be equal to one in a sublist ")
    ...
    def place_mix_controlled_cars(self,n,l,initial_road,tensor_car_decision):
        ...
        h=0
        if (n<k):
            raise Error("total number of cars can't be smaller than controlled cars.")
    ...
    def run_get_all_info(self,dt,Nmax,n_pass_over=1):
        ...
        if (type(Nmax)!=int or type(n_pass_over)!=int):
            ...
            raise Error("Nmax and n_pass_over msut be positive integer and not 0")
    ...
    def draw_density(self,label=True,cooef_size=2):
        ...
        if (label):
            ...
            for i in range(len(road_pos)):
                ...
                s+=str(1)
                s+='}'
                plt.annotate(road_label[i], ((2*ox[i_]+vx[i])/2+offset2*size_y, (2*oy[i]+vy[i])/2+offset2*size_y))
...
if __name__ == "__main__":
    ...
    crn.create_grid(height_road_number=2,height_road_size=100,length_road_number=2,length_road_size=100,vm=50,vclass=LinearSpeed(),size=2)
    ...
    # ROAD'S PARAMETERS
    crn.change_road_param(list_roads=[("normal",0),("normal",1)],L = 7,vmax=3,class_speed=LinearSpeed(1))
    crn.change_road_param(list_roads=[("normal",2),("normal",3)],L = 7,vmax=7,class_speed=LinearSpeed(1))
    crn.change_road_param(list_roads=[("normal",4)],L = 7,vmax=3,class_speed=LinearSpeed(1))
    crn.change_road_param(list_roads=[("normal",5)],L = 7,vmax=3,class_speed=LinearSpeed(1))
    #crn.change_road_param(list_roads=[("normal",6)],L = 7,vmax=7,class_speed=LinearSpeed(1))

    # PRIORITIES
    #crn.set_intersection_priority(2,[[("normal",6),0.9],[("normal",0),0.1]])
    crn.set_intersection_priority(3,[[("normal",3),0.9],[("normal",1),0.1]])
    ...
    # CARS POSITIONS
    rn.position_car_uniform(n = 7,type0=2)
    #rn.position_car_road(n=5,type_road="normal",road_num=1,type0=2)
    #rn.position_car_road(n=10,type_road="normal",road_num=6,type0=2)
    #rn.position_car_road(n=10,type_road="normal",road_num=0,type0=2)
    #rn.position_car_road(n=5,type_road="normal",road_num=3,type0=2)

    rn.compile_car_system()

    # NETWORK DISPLAY
    crn.draw_static()
    rn.draw_car(label=False)