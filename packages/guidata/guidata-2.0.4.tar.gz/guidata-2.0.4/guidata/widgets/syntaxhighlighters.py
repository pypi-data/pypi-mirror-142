...
class CythonSH(PythonSH):
    ...
    PROG = re.compile(
        make_python_patterns(ADDITIONAL_KEYWORDS, ADDITIONAL_BUILTINS), re.S
    )
    IDPROG = re.compile(r"\s+([\w\.]+)", re.S)
...
def make_generic_c_patterns(
    keywords, builtins, instance=None, define=None, comment=None
):
    "Strongly inspired from idlelib.ColorDelegator.make_pat"
            ...
            for key, value in list(match.groupdict().items()):
                if value:
                    ...
                    if key == "comment_start":
                        inside_comment = True
                        self.setFormat(
                            start, len(text) - start, self.formats["comment"]
                        )
...
def make_fortran_patterns():
    ...
    dqstring = r'(\b[rRuU])?"[^"\\\n]*(\\.[^"\\\n]*)*"?'
    string = any("string", [sqstring, dqstring])
    return "|".join([kw, comment, string, number, builtin, any("SYNC", [r"\n"])])
                ...
                if value:
                    ...
                    if value.lower() in ("subroutine", "module", "function"):
                        ...
                        if match1:
                            start1, end1 = match1.span(1)
                            self.setFormat(
                                start1, end1 - start1, self.formats["definition"]
                            )
...
def make_nsis_patterns():
    ...
    define = any("define", [r"\![^\n]*"])
    comment = any("comment", [r"\;[^\n]*", r"\#[^\n]*", r"\/\*(.*?)\*\/"])
    return make_generic_c_patterns(
        kwstr1 + " " + kwstr2 + " " + kwstr3,
        bistr,
        instance=instance,
        define=define,
        comment=comment,
...
def make_yaml_patterns():
    ...
    dqstring = r'(\b[rRuU])?"[^"\\\n]*(\\.[^"\\\n]*)*"?'
    string = any("string", [sqstring, dqstring])
    return "|".join([kw, string, number, links, comment, any("SYNC", [r"\n"])])
                ...
                if value:
                    ...
                    if previous_state == self.COMMENT:
                        if key == "multiline_comment_end":
                            ...
                            self.setFormat(end, len(text), self.formats["normal"])
                        else:
                        ...
                        if key == "multiline_comment_start":
                            self.setCurrentBlockState(self.COMMENT)
                            self.setFormat(start, len(text), self.formats["comment"])
                        else:
                            self.setCurrentBlockState(self.NORMAL)
                            try:
                                self.setFormat(start, end - start, self.formats[key])
