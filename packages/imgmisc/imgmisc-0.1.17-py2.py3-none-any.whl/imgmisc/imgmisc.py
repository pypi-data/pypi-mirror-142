...
def vector_mesh(centers, directions, scale=1, geom=None, scalars=None, scalar_tag=None):
    if geom is None:
        geom = pv.Arrow()
    vmesh = pv.PolyData()
    vmesh.points = centers - .5 * scale * directions 
...
def smooth_point_cloud(points, iters, r=30, weights='linear'):
    ...
    for ii in range(iters):
        ...
        dists = [[((points[ii] - pc)**2).sum()**.5 / r for pc in pts_coords[ii]]
                 for ii in range(len(pts))]

        def norm(x): return (x-np.min(x))/(np.max(x)-np.min(x))
        weights = [[fct(dist) for idx, dist in enumerate(dists[ii])]
                   for ii, pts_ii in enumerate(pts)]

        points = np.array(
            [np.average(points[pts[ii]], 0, weights=weights[ii]) for ii in range(len(pts))])

    return points

def smooth_point_cloud_scalars(points, scalars, iters, r=30, weights='linear', scalar_weights=True):
    if weights.lower() in ['linear', 'lin']:
        fct = lambda x: 1. / (1 + x)
    elif weights.lower() in ['quad', 'quadric', 'quadratic']:
        fct = lambda x: 1. / (1 + x)**2
    ...
    for ii in range(iters):
        ...
        pts = [pp for pp in pts]
        pts_coords = [points[pts[ii]] for ii in range(len(pts))]
        dists = [[((points[ii] - pc)**2).sum()**.5 / r for pc in pts_coords[ii]] for ii in range(len(pts))]
        
        norm = lambda x: (x-np.min(x))/(np.max(x)-np.min(x))
        norm_scalars = norm(scalars)
        if scalar_weights:
            weights = [[fct(dist) * norm_scalars[pts_ii[idx]]for idx, dist in enumerate(dists[ii])] for ii, pts_ii in enumerate(pts)]
        else:
            weights = [[fct(dist) for idx, dist in enumerate(dists[ii])] for ii, pts_ii in enumerate(pts)]
        
        scalars = np.array([np.average(scalars[pts[ii]], weights=weights[ii]) for ii in range(len(pts))])
        
    return scalars



def weighted_average_points(points, scalars, iters, r=30, weights='linear', scalar_weights=True):
    if weights.lower() in ['linear', 'lin']:
        fct = lambda x: 1. / (1 + x)
    elif weights.lower() in ['quad', 'quadric', 'quadratic']:
        fct = lambda x: 1. / (1 + x)**2
    ...
    for ii in range(iters):
        ...
        points = np.array([np.average(points[pts[ii]], 0, weights=weights[ii]) for ii in range(len(pts))])
        scalars = np.array([np.average(scalars[pts[ii]], weights=weights[ii]) for ii in range(len(pts))])
...
def weighted_variance_points(points, scalars, iters, r=30, weights='linear'):
    ...
    pts = [pp for pp in pts]
    pts_coords = [points[pts[ii]] for ii in range(len(pts))]
    dists = [[((points[ii] - pc)**2).sum()**.5 for pc in pts_coords[ii]] for ii in range(len(pts))]
    weights = [[fct(dist) for idx, dist in enumerate(dists[ii])] for ii, pts_ii in enumerate(pts)]
    
    std = [np.sqrt(np.cov(scalars[pts[ii]], aweights=weights[ii])) for ii in range(len(pts))]
    ...
    for ii in range(iters):
        ...
        points = [np.average(points[pts[ii]], 0, weights=weights[ii]) for ii in range(len(pts))]
...
def average_vectors(centers, directions, iters, r=30, weights='linear', scalars=None, normalise_scalars=True, point_scalar_weights=False, vector_scalar_weights=True):
    ...
    for ii in range(iters):
        ...
        dists = [np.array([((centers[ii] - pc)**2).sum()**.5 for pc in pts_coords[ii]]) for ii in range(len(pts))]
        if point_scalar_weights:
            pt_weights = [np.array([fct(dist) * scalars[pts_ii[idx]] for idx, dist in enumerate(dists[ii])]) for ii, pts_ii in enumerate(pts)]
        else:
            pt_weights = [np.array([fct(dist) for idx, dist in enumerate(dists[ii])]) for ii, pts_ii in enumerate(pts)]
        if vector_scalar_weights:
            vec_weights = [np.array([fct(dist) * scalars[pts_ii[idx]] for idx, dist in enumerate(dists[ii])]) for ii, pts_ii in enumerate(pts)]
        else:
            vec_weights = [np.array([fct(dist) for idx, dist in enumerate(dists[ii])]) for ii, pts_ii in enumerate(pts)]
            
        # Smoothen coordinates and directions        
        centers = np.array([np.average(centers[pts[ii]], 0, weights=pt_weights[ii]) for ii in range(len(pts))])
        directions = np.array([np.average(directions[pts[ii]], 0, weights=vec_weights[ii]) for ii in range(len(pts))])
        directions = np.array([directions[ii] / ((directions[ii]**2).sum()**.5) for ii in range(len(pts))])
        
        if scalars is not None:
            scalars = np.array([np.average(scalars[pts[ii]], weights=pt_weights[ii]) for ii in range(len(pts))])
...
def create_mesh(contour, resolution=[1, 1, 1]):
    v, f, _, _ = marching_cubes(
        contour > 0, 0.5, spacing=list(resolution), step_size=1, allow_degenerate=False)
...
def get_resolution(fname):
    ...
    if ext in ['.czi']:
        ...
        chosen = series[0]
        try:
            res = tuple([float(chosen.getMetadata().get(key)) for key in ('voxel_size_z', 'voxel_size_y', 'voxel_size_x')])
        except:
            res = 1., 1., 1.
        return res
