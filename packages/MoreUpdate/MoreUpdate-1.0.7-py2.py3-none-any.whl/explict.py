...
def secondsToStr(t):
    return "%02d:%02d:%02d.%03d" % reduce(lambda ll,b : divmod(ll[0],b) + ll[1:],
            [(round(t*1000),),1000,60,60])



MAX_INT = 999999999999999999999999
...
def debug(*argv,**kwargs):
	def inner(func):
		def wrapper():
			print("-----------------------------------------------------------------------------------")
			print(f"[INFO] Debug for Function : \"{func}\" Started at {secondsToStr(int(time.time()))}")
			...
			mul = 100000
			#print(stime,etime)
			tf = float("{0:.5f}".format(etime-stime))
			print(f"[INFO] First run time : {tf*1000}ms")
			try:
				base = base / ((etime-stime)*mul)
			except ZeroDivisionError:
				print("[INFO] The function to fast reduce to 500 base")
				base = 500
			if base == 0:
				print(f"[INFO] Function Took longer than expected so base will be 10")
				base = 10
			base = int(base)
			if error == "":
				print("[INFO] No error was found")
				print("[INFO] Benmarking Largest Time Took of function")
				print(f"[INFO] Benmarking started at {secondsToStr(int(time.time()))} for {base} times")
				for i in range(base):
					...
					if (et - st < largest):
						smallest = et - st
				print(f"[INFO] Benmarking STOPED at {secondsToStr(int(time.time()))}")
				print(f"[INFO] Last Longest function took is : {largest*1000}ms")
				print(f"[INFO] Last Fastest function took is : {smallest*1000}ms")
			if error != "":
				print(f"[ERROR] Function raise an error : {error}")
				#print(mess)
			etime = time.perf_counter()
			print(f"[INFO] Debug for Function : \"{func}\" STOPED at {secondsToStr(int(time.time()))}")
			print(f"[INFO] Debug for Function Took : {etime - stime}s To finish")
			print("-----------------------------------------------------------------------------------")
