...
parser.add_argument('-i','--inbam', metavar='FILE', type=str,help='bam sort by cellname')
...
def fraction_reads(outdir):
    ...
    stats_df.loc[1.0, "Sequencing Saturation"] = (1- (cellcount_df.filter(pl.col("Count") == 1).height)/(cellcount_df.height-1))*100
    ...
    for sampling_fraction in sampling_fractions:
        if sampling_fraction == 0.0:
            ...
            stats_df.loc[sampling_fraction, "Sequencing Saturation"] = (1- (cellcount_sampled.filter(pl.col("Count") == 1).height)/(cellcount_sampled.height-1))*100
