import os
from .utils import str_mkdir,logging_call
...
class Report:
    ...
    def run(self):
        ...
        generate_report_cmd = 'python %s/rna/generate_report.py --outPath %s --htmlTemplate %s/template/template.html --name %s --species %s' \
            %(_root_dir,self.outdir,_root_dir,self.name,self.species)
        generate_output_cmd = ['python %s/rna/report_output.py --indir %s'%(_root_dir,self.outdir)]
        if self.no_bam:
