...
class Count:
    ...
    def run(self):
        ...
        PISA_attrcnt_cmd = '%s/soft/PISA attrcnt -cb CB -tags UB,GN \
            ...
            %s'%(_root_dir,self.thread,self.outdir,self.cDNAbarcodeCount,self.bam)
        cellCalling_cmd = [
        ...
        logging_call(PISA_attrcnt_cmd,'count',self.outdir)
...
def parse_count(parser):
    ...
    parser.add_argument(
        '--bam',
        help='Bam file after star and anno, eg./01.data/test.final.bam'
    )
    parser.add_argument(
        '--cDNAbarcodeCount',
