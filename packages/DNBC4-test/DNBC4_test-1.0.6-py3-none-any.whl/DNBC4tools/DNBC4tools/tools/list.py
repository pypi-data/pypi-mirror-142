    ...
    def run(self):
        with open(self.list) as samplelist:
            for line in samplelist:
                ...
                cmd_line = ['DNBC4tools run --name %s --cDNAfastq1 %s --cDNAfastq2 %s --oligofastq1 %s --oligofastq2 %s --thread %s --starIndexDir %s --gtf %s --species %s'
                %(name,cDNAr1,cDNAr2,oligor1,oligor2,self.thread,self.starIndexDir,self.gtf,species)]
                if self.cDNAconfig:
                ...
                if self.lowqual:
                    cmd_line += ['--lowqual %s'%self.lowqual]
                if self.process:
                ...
                if self.no_bgifilter:
                    cmd_line += ['--no_bgifilter']               
                #cmd_line += ['--dry']
                cmd_line = ' '.join(cmd_line)
                python_path = os.environ.get("_")
                path = '/'.join(str(python_path).split('/')[0:-2]) + '/bin'
                shelllist.write('export PATH='+path+':$PATH'+'\n')
