...
from ..input.edit_resource_type_input import EditResourceTypeInput


# fmt: off
QUERY: List[str] = ["""
mutation editResourceType($input: EditResourceTypeInput!) {
...
class editResourceType:
    ...
    class editResourceTypeData(DataClassJsonMixin):
        @dataclass(frozen=True)
    ...
    # fmt: off
    @classmethod
    def execute(cls, client: Client, input: EditResourceTypeInput) -> editResourceTypeData.ResourceType:
        variables: Dict[str, Any] = {"input": input}
        new_variables = encode_variables(variables, custom_scalars)
        response_text = client.execute(
            gql("".join(set(QUERY))), variable_values=new_variables
        )
        res = cls.editResourceTypeData.from_dict(response_text)
        return res.editResourceType

    # fmt: off
    @classmethod
    async def execute_async(cls, client: Client, input: EditResourceTypeInput) -> editResourceTypeData.ResourceType:
        variables: Dict[str, Any] = {"input": input}
        new_variables = encode_variables(variables, custom_scalars)
