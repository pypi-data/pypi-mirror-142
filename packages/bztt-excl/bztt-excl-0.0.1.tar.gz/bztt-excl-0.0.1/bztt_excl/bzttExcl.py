...
class BzttExcl:  #工单处理
    ...
    def ifsData(self,jsonData):
        ...
        #print("start",jsonData["startRange"],"end",jsonData["startRange"] + ':' + rangeEndStr)
    ...
    def countyCountyNum(self,jsonData):
        ...
        for info in data:
            ...
            #sht.range( rang).api.NumberFormat = "0"  #单元格赋值
            


            #print("formula",str(dataValue))
            #dataValue = sht.range(rang).formula #读取工作表中的数据
            #sht.range( rang).value = str(dataValue) #将要读取的数据写入工作表
        ...
        sht = self.wb["解决"].sheets[0]
        sht.range('A4').formula = "=1"#单元格赋值
        print(1)
        sht.range('A2').formula = data["滨城区"]#单元格赋值
    ...
    def copyData(self,copyJson):
        ...
        #print("sourceTitle",copyJson["sourceTitle"])
        #print("targetTitle",copyJson["targetTitle"])
    ...
    def formatJson(self,formatJson):
        ...
        #for info in formatJson["formatVariable"]:
        ...
        for i in range(0,len(formatJson["formatVariable"])):
            ...
            if info["type"] == "input":
                ...
                print("strptime",startTimeStr,info)
