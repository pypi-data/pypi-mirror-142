...
Ext.ux.StartMenu = Ext.extend(Ext.menu.Menu, {
        ...
        if (this.useMenuSearch) {
            this.menuSearchField = new Ext.app.form.SearchField({
                ...
                onTrigger1Click: function () {
                    ...
                    this.startMenu.closeAllFloatMenu();
                    this.startMenu.clearMenuSearchFilter();
                },
                onTrigger2Click: function () {
                    ...
                    this.startMenu.showFilteredStartMenuItems(filterText);
                },
            });
            this.menuSearchField.on('keydown', function () {
                this.startMenu.closeAllFloatMenu();
            });
    ...
    /**
     * Закрывает все выпадающие меню, чтобы после фильтрации не возникали баги,
     * связанные с его расположением
     * @param complexMenuItem Элемент меню с вложенными элементами.
     * Если параметр не указан, выбирается основное меню.
     */
    closeAllFloatMenu: function (complexMenuItem) {
        var menuItem = complexMenuItem || this;
        menuItem.items.each(function (item) {
            var menu = item.menu;

            if (menu) {
                if (menu.ul && menu.ul.isVisible()) {
                    item.deactivate();
                }
                this.closeAllFloatMenu(menu);
            }
        }.bind(this));
    },
    ...
    applyMenuFilter: function (filterText, menuItem) {
        ...
        if (currentMenu) {
            var innerVisibleCount = this.showFilteredStartMenuItems(
                filterText, currentMenu)

            // Если у вложенного меню внутри есть хотя бы 1
            // видимый элемент, то это меню тоже видно
            if (!isVisible && innerVisibleCount > 0) {
                isVisible = true;
            }

            if (currentMenu.ul) {
                currentMenu.ul.setHeight('auto');
            }
        }
     ...
     * @param complexMenuItem Пункт меню со вложенными элементами для
     *  проверки (необязательный параметр). Если не задан, проверяются корневые
     *  и вложенные в них элементы меню.
     * @returns {number} Количество видимых элементов внутри данного пункта меню
     */
    showFilteredStartMenuItems: function (filterText, complexMenuItem) {
        var menuItem = complexMenuItem || this,
            innerVisibleCount = 0;
        ...
        menuItem.items.each(function (item){
            innerVisibleCount += Number(this.applyMenuFilter(filterText, item));
        }.bind(this));

        return innerVisibleCount
    },
    ...
    clearMenuItemSearchFilter: function (menuItem) {
        if (menuItem.menu) {
            this.clearMenuSearchFilter(menuItem.menu);

            if (menuItem.menu.ul) {
                menuItem.menu.ul.setHeight('auto');
            }
        }
     ...
     * @param complexMenuItem Пункт меню со вложенными элементами, для которого
     *  отключается фильтрация. Если не указан параметр, то обрабатываются
     *  все элементы меню.
     */
    clearMenuSearchFilter: function (complexMenuItem) {
        var menuItem = complexMenuItem || this;
        menuItem.items.each(this.clearMenuItemSearchFilter.bind(this));
    },
});
