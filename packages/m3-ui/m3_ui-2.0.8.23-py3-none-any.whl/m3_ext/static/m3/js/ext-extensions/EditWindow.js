...
Ext.m3.EditWindow = Ext.extend(Ext.m3.Window, {
	...
	/**
	 * Проверяет форму на наличие некорректных полей, отдает список label'ов этих полей с указанием места нахождения
	 */
        ...
        submittedForm.items.each(function(f){
            if(!f.validate()){
                ...
                while(isContinueSearch) {
                    ...
                    if (parentElement) {
                        parentComponent = Ext.getCmp(parentElement.id);

                        if (parentComponent) {
                            if (parentComponent.getXType() == 'panel') {
                                if (parentComponent.title) {
                                    panelTitles.push('"' + parentComponent.title + '"');
                                }
                            } else if (parentComponent.getXType() == 'm3-editwindow') {
                                isContinueSearch = false;
                            }

                            currentElement = parentComponent.el;
                        } else {
                ...
                invalidName = '<br>- ' + f.fieldLabel;

                if (panelTitles.length) {
                    invalidName = invalidName + ' (на панели ' + panelTitles.join(' - ') + ')';
                }

                invalidNames.push(invalidName)
            }
        });
