...
if __name__ == '__main__':
	...
	parser.add_argument('-m', '--missing', action="store", type=is_valid_file, default=None, help='Find repeats present in INFILE but not in MISSING')
	parser.add_argument('-o', '--outfile', action="store", default=sys.stdout, type=argparse.FileType('w'), help='where to write output [stdout]')
	args = parser.parse_args()

	header = ''
	dna = ''
	# load MISSING file if provided
	if args.missing:
		with open(args.missing) as fp:
			for line in chain(fp, '>'):
				if line.startswith(">"):
					if dna:
						missing = Locus(header, dna)
			...
			if line.startswith(">"):
				if dna:
					...
					for repeat in repeats:
						seq = locus.seq(repeat[0],repeat[1])
						if 1 + repeat[1] - repeat[0] >= args.len and 'n' not in seq:
							a,b,c,d = map(str, repeat)
							pairs = ((a,b),(c,d))
							if not args.missing:
								locus.add_feature('repeat_region', 0, pairs)
							elif missing.dna.count(seq) == 1:
								locus.add_feature('repeat_region', 0, pairs)
					locus.write()
				header = line[1:].rstrip()
				dna = ""
