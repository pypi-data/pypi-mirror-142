    ...
    def execute(self, context):
        try:
            ...
            if self.insert_table != None:
                self.log.info('Executing insert table : %s', self.insert_table)
...
class MergeOperator(BaseOperator):
    ...
    def execute(self, context):
        try:
            ...
            self.log.info('Executing inquery sql: %s', self.inquery_sql)
            inquery_data = hook.get_records(self.inquery_sql)
            self.log.info('Executing ms_data: %s', inquery_data)
            if len(inquery_data) > 0:
