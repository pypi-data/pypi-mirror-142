...
class ParserPSPICE:
    ...
    def read_netlist(self, full_path_file_name: str, verbose: bool = False):
        ...
        ** Reads a PSPICE netlist file **
        ...
        flag_read_global_parameters = False
        flag_read_parametrized_component = False
        flag_read_options = False
        flag_read_autoconverge = False
        flag_read_time_schedule = False
        ...
        with open(full_path_file_name) as file:
            for row, line in enumerate(file):
                ...
                if '.STMLIB' in line:
                    ...
                    self.circuit_data.Stimuli.stimulus_files.append(value)
                    continue
                ...
                if '.LIB' in line:
                    ...
                    self.circuit_data.Libraries.component_libraries.append(value)
                    continue

                # Read global parameters
                if '.PARAM' in line:
                    flag_read_global_parameters = True
                    continue
                if flag_read_global_parameters and '+ ' in line:
                    line_split = line.rstrip('\n').split(' ')
                    line_split = line_split[1].split('=')
                    name  = line_split[0].strip('"')
                    value = line_split[1].strip('{').strip('}')
                    self.circuit_data.Global_Parameters.global_parameters.append([name, value])
                    continue

                # Read options
                if '.OPTIONS' in line:
                    flag_read_options = True
                    continue
                if flag_read_options and '+ ' in line:
                    line_split = line.rstrip('\n').split(' ')
                    line_split = line_split[1].split('=')
                    name  = line_split[0].strip('"')
                    value = line_split[1].strip('{').strip('}')
                    self.circuit_data.Options.options_simulation.append([name, value])
                    continue

                # Read options
                if '.AUTOCONVERGE' in line:
                    flag_read_autoconverge = True
                    flag_read_options = False
                    continue
                if flag_read_autoconverge and '+ ' in line:
                    line_split = line.rstrip('\n').split(' ')
                    line_split = line_split[1].split('=')
                    name  = line_split[0].strip('"')
                    value = line_split[1].strip('{').strip('}')
                    self.circuit_data.Options.options_autoconverge.append([name, value])
                    continue
                ...
                if '.TRAN' in line:
                    ...
                    line_split = line.rstrip('\n').split(' ')
                    self.circuit_data.Analysis.simulation_time.time_start = line_split[1]
                    self.circuit_data.Analysis.simulation_time.time_end = line_split[2]
                    if len(line_split) > 3:
                        self.circuit_data.Analysis.simulation_time.min_time_step = line_split[3]
                    flag_read_autoconverge = False
                    continue
                elif '.AC' in line:
                ...
                # Read time schedule
                if '+ {SCHEDULE(' in line:
                    flag_read_time_schedule = True
                    continue
                # If the line is the end of the time schedule section, skip to the next
                if '+ )}' in line or '+)}' in line:
                    flag_read_time_schedule = False
                    continue
                if flag_read_time_schedule and '+ ' in line:
                    line_split = line.rstrip('\n').split('+ ')
                    line_split = line_split[1].split(',')
                    name  = line_split[0].strip(' ').strip('\t').strip(' ')
                    value = line_split[1].strip(' ').strip('\t').strip(' ')
                    self.circuit_data.Analysis.simulation_time.time_schedule.append([name, value])
                    continue

                # Read probe
                if '.PROBE' in line:
                    if '/CSDF' in line:
                        self.circuit_data.PostProcess.probe.flag_CSDF = True
                    else:
                        self.circuit_data.PostProcess.probe.flag_CSDF = False
                    flag_read_probe = True
                    # TODO: Known issue: If probe variables are defined in this same line, they are ignored
                    continue
                if flag_read_probe and '+ ' in line:
                    line_split = line.rstrip('\n').split(' ')
                    value = line_split[1]
                    self.circuit_data.PostProcess.probe.variables.append(value)
                    continue

                # Read additional files
                if '.INC' in line:
                    continue
                # TODO. EXAMPLE: .INC configurationFileFrequency.cir

                # Read netlist - If this code is read, it means that this line does not define special, and hence it defines a component of the netlist
                line_split = line.rstrip('\n').split('(')
                name = line_split[0].strip(' ')
                print(name)
                ...
                value = line_split[1].strip(' ').strip('{').strip('}')
                if not self.circuit_data.Netlist[0].name == None:
                    self.circuit_data.Netlist.append(Component())
                self.circuit_data.Netlist[-1].name  = name
                self.circuit_data.Netlist[-1].nodes = nodes
                self.circuit_data.Netlist[-1].value = value
                # self.circuit_data.Netlist[-1].type  =  # TODO: Add logic
                # TODO: Parametrized element
    ...
    def write_netlist(self, full_path_file_name: str, verbose: bool = False):
        '''
        ** Writes a PSPICE netlist file **
        ...
        for s, global_parameter in enumerate(self.circuit_data.Global_Parameters.global_parameters):
            # Check input size
        ...
        if not self.circuit_data.Netlist[0].type:
            raise Exception('At least one netlist entry of known type must be added. Supported component types:\n' +
                            '- comment\n' +
                            '- standard_component\n' +
                            '- stimulus_controlled_component\n' +
                            '- parametrized_component\n' +
        ...
        for s, component in enumerate(self.circuit_data.Netlist):
            ...
            elif type == 'standard_component':
                if name == None or nodes == None or value == None:
                    raise Exception('Netlist in position #{} is of type {} and requires name, nodes, and value.'.format(s+1, type))
                rows_netlist.append(add_standard_component(name, nodes, value))
            elif type == 'stimulus_controlled_component':
                if name == None or nodes == None or value == None:
                    raise Exception('Netlist in position #{} is of type {} and requires name, nodes, and value.'.format(s+1, type))
                rows_netlist.append(add_stimulus_controlled_component(name, nodes, value))
            elif type == 'parametrized_component':
                if name == None or nodes == None or value == None or parameters == None or len(parameters[0]) == 0:
                    raise Exception('Netlist {} in position #{} is of type {} and requires name, nodes, value, and parameters.'.format(name, s+1, type))
        ...
        flag_CSDF = self.circuit_data.PostProcess.probe.flag_CSDF
        probe_variables = self.circuit_data.PostProcess.probe.variables
        if not flag_CSDF:
            flag_CSDF = False
        rows_post_processing.append(add_probe(flag_CSDF, probe_variables))

        # Prepare additional files to include
        rows_files_to_include = []
        # TODO: add this logic. EXAMPLE: .INC configurationFileFrequency.cir
...
def add_parametrized_component(name: str, nodes: list, value: str, parameters: list):
    ...
    print('formatted_parameters={}'.format(formatted_parameters))
    print('type(formatted_parameters)={}'.format(type(formatted_parameters)))
    print('len(formatted_parameters)={}'.format(len(formatted_parameters)))
...
def add_probe(flag_CSDF: bool, probe_variables: list):
    ''' Format probe row '''
    formatted_text = '.PROBE'
    if flag_CSDF:
        formatted_text = formatted_text + '/CSDF'
...
def ComparePSPICEParameters(fileA, fileB, max_relative_error=1E-5, show_indices=0):
    '''
    ...
    ## Check Inputs
    for attribute in pp_a.circuit_data.Netlist:
        Diff = compare_two_parameters(pp_a.circuit_data.__getattribute__('Netlist'),
                                      pp_b.circuit_data.__getattribute__('Netlist'),
