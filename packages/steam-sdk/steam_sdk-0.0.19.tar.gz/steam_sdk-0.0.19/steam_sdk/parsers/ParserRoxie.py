...
class ParserRoxie:
                            ...
                            if statement[1][:5] == 'Hyper':
                                ...
                                if lineType == 'Line':
                                    ...
                                    if lineType[6:] == 'Out':
                                        ...
                                        data.key_points[arg] = pd.Coord(x=data.key_points[arg1].x,
                                                                        y=data.key_points[arg0].y)
                                    else:
                                        arg = arg0 + arg1 + 'In'
                                        data.key_points[arg] = pd.Coord(x=data.key_points[arg0].x,
                                                                        y=data.key_points[arg1].y)
    ...
    def findConductorPositions(self, block: pd.Block = None, conductor: pd.CondPar = None, verbose: bool = False):
        ...
        for c in range(self.block.nco):
            ...
            # ax.add_patch(patches.Circle((coil.bore_center.x, coil.bore_center.y), radius=radius, color='b'))
            # self.ax.add_line(lines.Line2D([xCable[0], xCable[3]], [yCable[0], yCable[3]], color='red'))
            # self.ax.add_line(lines.Line2D([xCable[3], xCable[2]], [yCable[3], yCable[2]], color='red'))
            # self.ax.add_line(lines.Line2D([xCable[2], xCable[1]], [yCable[2], yCable[1]], color='red'))
            # self.ax.add_line(lines.Line2D([xCable[1], xCable[0]], [yCable[1], yCable[0]], color='red'))
