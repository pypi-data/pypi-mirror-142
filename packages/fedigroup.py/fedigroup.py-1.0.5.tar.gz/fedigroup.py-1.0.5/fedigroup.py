...
class FediGroupBot:
    ...
    def __update_group_members(self):
        ...
        followers = self.masto.account_followers(self.id, limit=sys.maxsize, max_id=max_id)  # NOQA: E501
        ...
        following = self.masto.account_following(self.id, limit=sys.maxsize, max_id=max_id)  # NOQA: E501
...
class MainApp:
    ...
    def create(self):
        ...
        if base_url.endswith((".ru", ".su")) or base_url.startswith("ru."):
            raise Exception("Russian warship, go fuck yourself!")

        email = input('Username (e-Mail) to log into Mastodon Instance: ')
        if email.endswith((".ru", ".su")):
            raise Exception("Russian warship, go fuck yourself!")
    ...
    def list(self):
        ...
        if len(config.sections()) == 0:
            print('No configurations were found')
            return
