...
class TableClient:
    ...
    def get(self, table: TableId) -> Cmd[Table]:
        ...
        def _extract(raw: FrozenList[FrozenList[PrimitiveVal]]) -> Table:
            columns = FrozenDict(dict(to_column(column) for column in raw))
            return new_table(columns, frozenset()).unwrap()
    ...
    def new(
        ...
        enum_columns = tuple(enumerate(table.columns.items()))
        ...
        fields_template: str = ",".join(
            [
                f"""
                    ...
                    DEFAULT {c.default} {encode_nullable(c.nullable)}
                """
