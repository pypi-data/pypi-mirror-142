...
class ExecutionEngine(Protocol):
    ...
    def notify_forkchoice_updated(self, head_block_hash: Hash32, finalized_block_hash: Hash32) -> None:
        ...
...
def is_valid_terminal_pow_block(block: PowBlock, parent: PowBlock) -> bool:
    if block.block_hash == config.TERMINAL_BLOCK_HASH:
        return True
...
def get_terminal_pow_block(pow_chain: Sequence[PowBlock]) -> Optional[PowBlock]:
    if config.TERMINAL_BLOCK_HASH != Hash32():
        ...
        pow_block_overrides = [block for block in pow_chain if block.block_hash == config.TERMINAL_BLOCK_HASH]
