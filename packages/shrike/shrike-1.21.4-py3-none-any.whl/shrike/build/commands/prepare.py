...
"""
    ...
    def get_additional_includes_contents(self, component)-> Union[List[str], None]:
        ...
        component_additional_includes_path=self.get_theoretical_additional_includes_path(component)
        ...
        if Path(component_additional_includes_path).exists():
            ...
            log.info("No additional_includes file could be found for the component '" + component + "'. If you tried to create such a file, remember it should live next to the component spec file and should be named '{spec_file_name}.additional_includes'. " +
            "For example, if the component spec file is named 'component_spec.yaml', the additional_includes file should be named 'component_spec.additional_includes'. In this specific case, the expected additional_includes file name is: '" + component_additional_includes_path + "'.")
            ...
            if self.check_for_wrongly_named_additional_includes(component):
                raise ValueError("An additional_includes file which does not respect the naming pattern was found. Please rename this file. Remember it should live next to the component spec file and should be named '{spec_file_name}.additional_includes'." +
    ...
    def get_global_dependency_hints_contents(self, component) -> Union[List[str], None]:
        if len(self.config.dependency_hints) > 0:
            ...
            for component_folder_paths, dependency_hints_paths in self.config.dependency_hints.items():
                component_folder_absolute_paths = [
                    str(p.absolute().resolve())
                    for p in Path(self.config.working_directory).glob(component_folder_paths)
                ]
                if str(Path(component).parent.resolve()) in component_folder_absolute_paths:
                    ...
                    for dependency_hints_path in list(dependency_hints_paths):
                        global_dependency_hints_contents += [
                            str(p.absolute().resolve())
                            for p in Path(self.config.working_directory).glob(dependency_hints_path)
                        ]
    ...
    def get_dependency_hints_contents(self, component)-> Union[List[str], None]:
        component_dependency_hints_contents = self.get_global_dependency_hints_contents(component)
        # First we figure out the expected path of the dependency_hints file
        component_dependency_hints_path=self.get_theoretical_dependency_hints_path(component)
        # And we load it if it exists.
        if Path(component_dependency_hints_path).exists():
            with open(
                ...
                if component_dependency_hints_contents is not None:
                    component_dependency_hints_contents += component_dependency_hints.readlines()
                else:
    ...
    def _create_dependencies_files_for_single_component(
        ...
        pip_dependencies, conda_dependencies, _ = self._extract_dependencies_and_channels(component)
