...
export class PrivateRunTrace {
            ...
            for (const segmentKey of this._startSegmentKeys) {
                this._getSegment(segmentKey)
                    .then((segment: GraphDbLinkedSegment) => {
                        try {
                            const vertex =
                                segment.vertexByKey[this._startVertexOrEdgeKey];
                            if (vertex) {
                            ...
                            const edge =
                                segment.edgeByKey[this._startVertexOrEdgeKey];
                            if (edge) {
    ...
    private _traceEdge(
        ...
        if (this._checkAlreadyTraced({ edgeKey: edge.key })) return;

        const fromSrcVertex =
            fromVertex && edge.srcVertex.key == fromVertex.key;
        if (
            !this._matchTraceRules({ edge: edge, fromSrcVertex: fromSrcVertex })
        )
            return;
    ...
    private _traceVertex(
        segment: GraphDbLinkedSegment,
        vertex: GraphDbLinkedVertex
    ): void {
        if (this._checkAlreadyTraced({ vertexKey: vertex.key })) return;

        this._addVertex(vertex);

        if (!this._matchTraceRules({ vertex: vertex })) return;
    ...
    private _traceEdgesInNextSegment(vertexKey: string, segmentKey: string) {
        this._getSegment(segmentKey)
            .then((segment: GraphDbLinkedSegment) => {
                ...
                if (vertex == null)
                    throw new Error(
                        `Vertex '${vertexKey}' is not in segment '${segmentKey}'`
                    );
    ...
    private _checkAlreadyTraced(params: {
        ...
        if (!traced) this._alreadyTracedSet[val] = true;
    ...
    private _matchTraceRules(params: {
        ...
        const isStartVertex =
            isVertex && params.vertex.key == this._startVertexOrEdgeKey;

        const props =
            params.vertex != null ? params.vertex.props : params.edge.props;
        ...
        for (let rule of this._traceRules) {
            ...
            if (rule.action == rule.ACTION_STOP_TRACE) return false;
        }
    }
