...
from quantplay_utils.utils.constant import Order, ExchangeName
...
import copy
import uuid
from quantplay_utils.utils.transaction_utils import TransactionCost
...
class StrategyReport:
    ...
    def get_sharpe_ratio(portfolio):
        ...
        response[
            StrategyMetricsTableColumns.monthly_sharpe_ratio
        ] = monthly_sharpe_ratio
        response[StrategyMetricsTableColumns.sharpe_ratio] = Sharpe_Ratio
        response[StrategyMetricsTableColumns.monthly_pnl] = monthly_data[
            "profit"
        ].mean()
    ...
    def add_more_params(portfolio, cm_slippage=0, fno_slippage=0):
        ...
        portfolio.loc[:, "order_timestamp"] = pd.to_datetime(
            portfolio["order_timestamp"]
        )
        ...
        portfolio.loc[:, Order.profit] = np.where(portfolio.exchange == ExchangeName.nse, 
    ...
    def create_single_report(portfolio):
        ...
        response['drawdown_pnl_ratio'] = float(response['max_drawdown']) / response['monthly_pnl']
