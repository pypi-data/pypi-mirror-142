...
class Constants:
    ...
    def move_to_local(data, stock_name, interval, folder_path=None):
        if folder_path == None:
            folder_path = (
                Constants.base_path + "/tmp/stock_data_local/" + interval + "/"
            )
    ...
    @staticmethod
    def get_alphas(
        day_candle_data, bucket="nifty", suffix="bucket", market_alpha=False
    ):
        alphas = [
            k
            for k in day_candle_data.columns
            if "alpha" in k and "market_alpha" not in k
        ]
...
class MarketTime(NamedTuple):
    ...
    def ticks_until(
        ...
        return int(
            ...
            // TickInterval.get_interval_seconds(tick_interval)
        )
            ...
            if "MetricName" in kwargs_main:
                data = {
                    ...
                    "Dimensions": [
                        ...
                        {"Name": "InstanceType", "Value": Constants.instance_type},
                    ],
                }
                Constants.logger.info("{}".format(json.dumps(data)))
            else:
                Constants.logger.error(
                    "No metric found in {}".format(function.__name__)
                )
            return result
