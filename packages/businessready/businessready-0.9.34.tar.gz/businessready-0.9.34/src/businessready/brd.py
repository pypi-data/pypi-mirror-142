...
def ISE_network_access_dictionaries_authentication(url, username, password):
    try:
        ...
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = networkDictionariesAuthentication_template.render(networkDictionariesAuthentication = networkDictionariesAuthenticationJSON['response'],filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
...
def ISE_network_access_dictionaries_authorization(url, username, password):
    try:
        ...
        if networkDictionariesAuthorizationJSON is not None:
            networkDictionariesAuthorization_template = env.get_template('ISE_network_dictionaries_authorization.j2')
            loop_counter = 0

        # Render Templates
            for filetype in filetype_loop:
                parsed_output = networkDictionariesAuthorization_template.render(networkDictionariesAuthorization = networkDictionariesAuthorizationJSON['response'],filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
...
def ISE_network_access_dictionaries_policy_set(url, username, password):
    try:
        ...
        if networkDictionariesPolicyJSON is not None:
            networkDictionariesPolicy_template = env.get_template('ISE_network_dictionaries_policy_set.j2')
            loop_counter = 0

        # Render Templates
            for filetype in filetype_loop:
                parsed_output = networkDictionariesPolicy_template.render(networkDictionariesPolicy = networkDictionariesPolicyJSON['response'],filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
