...
def proc_slider(
    ...
    image = cv2.imread(image_dir)  # 这个时候还是三个值，需要灰值化
    image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # 灰值化
    height = image_gray.shape[0]  # 高
    weight = image_gray.shape[1]  # 宽
...
def identify_gap_locations(
        background_img_url: str,  # 背景图地址
        slider_img_url: str,  # 滑块图地址
    ...
    background_dir = easydownload.download(
        url=background_img_url,
        filename='background',
        suffix_name='png',
        path=save_path
    )['file_dir']  # 下载背景图
    slider_dir = easydownload.download(
        url=slider_img_url,
        filename='slider',
        suffix_name='png',
        path=save_path
...
def get_track(
    ...
    while current < distance:
        ...
        # 加入轨迹
        if current > distance:
            track.append(move - current + distance)
        else:
            track.append(move)
    return track
