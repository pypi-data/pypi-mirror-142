...
# numpy画像をbase64に変換 (一瞬ファイルを経由)
def img2base64(img):
	import cv2
	filename = "__b64_temp_img__.jpg"
	# 一度ファイルに保存
	cv2.imwrite(filename, img)
	# ファイルからbinaryで読みこんでbase64に変換
	data = fies[filename, "bin"]
	img_b64 = base64.encodebytes(data).decode("ascii")
	# 削除
	os.remove(filename)
	return img_b64
...
def browser_arena(game, game_params):
	...
	def home():
		...
		img_b64 = img2base64(img)	# numpy画像をbase64に変換 (一瞬ファイルを経由)
		# メモに登録
		...
		row_ls.append('<img src = "data:image/jpg;base64,%s">'%img_b64)
