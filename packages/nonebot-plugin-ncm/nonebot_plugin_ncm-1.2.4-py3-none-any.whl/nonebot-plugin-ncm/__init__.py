...
music_regex = on_regex("(song|url)\?id=([0-9]+)(|&)", priority=ncm_config.ncm_priority,
                       block=False)  # 歌曲id识别 (新增json识别)
playlist_regex = on_regex("playlist\?id=([0-9]+)&", priority=ncm_config.ncm_priority, block=False)  # 歌单识别
music_reply = on_message(priority=ncm_config.ncm_priority, block=False)  # 回复下载
search = on_regex("搜(歌|歌单|用户)", priority=ncm_config.ncm_priority, block=False)  # 搜东西
