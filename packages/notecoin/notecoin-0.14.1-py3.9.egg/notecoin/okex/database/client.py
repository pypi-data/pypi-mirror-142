...
class OkexClientAccountBalance(Base, BaseTable):
    ...
    isoEq = Column(String(20), comment='币种逐仓仓位权益')
    availEq = Column(String(20), comment='可用保证金')
    ...
    upl = Column(String(20), comment='未实现盈亏')
    uplLiab = Column(String(20), comment='由于仓位未实现亏损导致的负债')
    crossLiab = Column(String(20), comment='币种全仓负债额')
    isoLiab = Column(String(20), comment='币种逐仓负债额')
    mgnRatio = Column(String(20), comment='保证金率')
    interest = Column(String(20), comment='计息')
    twap = Column(Float(), comment='当前负债币种触发系统自动换币的风险')
    maxLoan = Column(String(20), comment='币种最大可借')
    eqUsd = Column(Float(), comment='币种权益美金价值')
    notionalLever = Column(String(20), comment='币种杠杆倍数')
    stgyEq = Column(Float(), comment='策略权益')
    isoUpl = Column(String(20), comment='逐仓未实现盈亏')
