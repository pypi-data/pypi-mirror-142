    ...
    def __load_config__(self):
        ...
        for set_config in config["sets"]:
            if set_config["name"] == set_name:
                self.name = set_config["name"]
                self.description = set_config["name"]

                if "builds" in set_config:
                    builds = []
                    builds = set_config["builds"]
                    if builds:
                        for build in builds:
                            self.plan.append({"build": build, "tests": []})
...
class PlanConfig:
    ...
    def __load_config__(self):
        ...
        for cfg in config["jobs"]:
            if self.job_name is not None and cfg.get("name") != self.job_name:
                continue
            builds = []
            if "build" in cfg:
                builds = [cfg["build"]]
            ...
            if builds:
                for build in builds:
                    self.plan.append({"build": build, "tests": tests})
            else:
                self.plan.append({"build": None, "tests": tests})
