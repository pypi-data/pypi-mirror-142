...
from eisenradio.lib.eisdb import delete_radio, get_db_connection, get_db_smaller, get_db_path, \
    status_read_status_set, db_get_id_from_name, enum_radios
...
def db_import_ini(station_dict, global_dict=None, path_ini=None):
    ...
    """all radios with url from ini file"""
...
def add_new_radio_btn_dicts(str_radio):
    """add new radio to button dicts"""
    new_radio_db_id = db_get_id_from_name(str_radio)
    ghettoApi.lis_btn_dict[new_radio_db_id] = 0
    ghettoApi.rec_btn_dict[new_radio_db_id] = 0
    radio_list = enum_radios()
    for row in radio_list:
        # api
        ghettoApi.radios_in_view_dict[row['id']] = status_read_status_set(False, 'posts', 'title', row['id'])