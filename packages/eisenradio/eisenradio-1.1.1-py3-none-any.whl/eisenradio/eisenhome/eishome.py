...
def dispatch_record_title(table_id, record, listen):
    ...
    try:
        ...
        thread = threading.Thread(name=str_radio + '_display', target=display_write_db, args=(str_radio, table_id), daemon=True)
        thread.start()
        return True
    else:
...
def display_write_db(radio, table_id):  # title "britney pierced - dump it"; other thread can collect or pull it or else
    title = ''
    while 1:
