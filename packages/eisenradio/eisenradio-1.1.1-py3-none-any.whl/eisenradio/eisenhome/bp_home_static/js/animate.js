...
class flowFieldEffect {
    ...
    animate(timeStamp){     // requestAnimationFrame() feeds timeStamp
...
function rotateVisualiserGradient(){
    ...
    gradient.addColorStop("0.19", paletteArray[rndOmNum][2]);
    gradient.addColorStop("0.26", '#aaa');
    gradient.addColorStop("0.3", '#ccc');
    gradient.addColorStop("0.35", 'rgba(26,26,26,1)');
    gradient.addColorStop("0.65", 'rgba(26,26,26,1)');
    gradient.addColorStop("0.7", paletteArray[rndOmNum][3]);
...
function animatedBars() {
    ...
    for (let i = 0; i < bufferLength; i++) {
        ...
        let darkBody = getBodyColor();
        if (darkBody) {
            canvasMasterCtx.fillStyle = 'red';  //  #d441fc
            canvasMasterCtx.fillRect(x, canvasMaster.height - barHeight - 3, barWidth, 3);
            canvasMasterCtx.fillStyle = 'rgb(219, 111, 52)';  //  #d441fc
            ...
            canvasMasterCtx.fillStyle = 'rgb(219, 111, 52)';   //  #d441fc
            canvasMasterCtx.fillRect(x, canvasMaster.height - barHeight - 6, barWidth, 8);
        }
...
function initFlowFieldEnv(){
    ...
    fieldAnimationRandom = Math.random() * (0.02 - 0.005) + 0.005;    // Math.floor(Math.random() * (max - min + 1)) + min;
...
function initStarFieldEnv(){
    ...
    for (let i = 0; i < STAR_NUM; i++) {
        let speedMultiplier = Math.random() * 2 + 0.5;
...
function visualiseAudio(var_canvas) {
    ...
    try {
        ...
        if (analyserRandom == 3) {
                                    ...
                                    animatedBars(0); }
        if (analyserRandom == 4) {
                                    canvasMasterGlobalAlpha = 1;  // floating less dominant
                                    initFloatingStage();
                                    initFlowFieldEnv();
