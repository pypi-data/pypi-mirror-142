...
class RendevousManager(object):
    ...
    def init_horovod_if_needed(self):
        ...
        logger.info(
            "worker_host:{} local_process_id:{}, get info "
            "from RendezvousServer -> "
            "rank = {}, size = {}, "
            "local_rank = {}, local_size = {}, "
            "cross_rank = {}, cross_size = {}".format(
                self._worker_host,
                self._worker_local_process_id,
                rank_response.rank_id,
                rank_response.world_size,
                rank_response.local_rank,
                rank_response.local_size,
                rank_response.cross_rank,
                rank_response.cross_size,
            )
        )
        ...
        if rank_response.rendezvous_id != self._rendezvous_id:
            logger.info(
                "Initialize Horovod with rank = {}, size = {}, "
                "local_rank = {}, local_size = {}, "
                "cross_rank = {}, cross_size = {}".format(
                    rank_response.rank_id,
                    rank_response.world_size,
    ...
    def _set_horovod_env(self):
        ...
        if master_addr_port:
            ...
            logger.warning("Set HorovodEnv.RENDEZVOUS_ADDR error!")
