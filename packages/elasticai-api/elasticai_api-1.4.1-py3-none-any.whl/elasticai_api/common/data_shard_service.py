...
class DataShardService(object):
    def __init__(
        ...
        self._shuffle = False
        self._shuffle_shards = shuffle_shards or shuffle
    ...
    def get_task(self, task_type=None):
        for _ in range(5):
            if self._dataset_name:
                success, task = self._mc.get_dataset_task(self._dataset_name)
            else:
                success, task = self._mc.get_task(task_type)
            if success:
                break
            time.sleep(5)
...
class RecordIndexService(DataShardService):
    def __init__(
        ...
        # RecordIndexService return indices,don't need shuffle_shards
