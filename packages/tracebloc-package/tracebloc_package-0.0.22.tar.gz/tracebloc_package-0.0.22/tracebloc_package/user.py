        ...
        *******
        ...
        try:
            if self.__token == "" or self.__token == None:
                print("You are not logged in. Please go back to ‘1. Connect to Tracebloc’ and proceed with logging in.")
                return
            self.__weights = weights
            self.__modelName = modelname
            model = Model(self.__modelName,self.__token,weights)
            ...
            text = colored('You are not logged in. Please go back to ‘1. Connect to Tracebloc’ and proceed with logging in.', 'red')
            print(text, "\n")
    ...
    def linkModelDataset(self,datasetId:str):
        ...
        try:
            ...
            if self.__modelId == "" or self.__modelId == None:
                text = colored('Model not uploaded. Please first upload the model.', 'red')
                print(text, "\n")
                return None
            if self.__checkmodel(datasetId):
                return LinkModelDataSet(self.__modelId,datasetId,self.__token,self.__weights)
            else:
                return None
        except:
            text = colored(
    ...
    def __checkmodel(self, datasetId):
        try:
            ...
            if re.status_code == 403:
                text = colored(f'There is no dataset with ID: {datasetId} in your dataset table.\n'
