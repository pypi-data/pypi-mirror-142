...
class ModelBundle:
    """
    The :class:`~ enaptorch.ModelBundle` class provides a straightforward experimentation tool for efficient and entirely
    customizable finetuning of the whole neural network training procedure with PyTorch. The
    :class:`~ enaptorch.ModelBundle` object takes care of the training and testing processes while also managing to keep
    traces of all pertinent information via the automatic logging option.

    Use ``ModelBundle.from_*`` methods to instanciate a :class:`~ enaptorch.ModelBundle`.
    """
    ...
    def from_network(
        ...
        *,
        ...
        """
        Instanciate a :class:`~ enaptorch.ModelBundle` from a PyTorch :class:`~torch.nn.Module` instance.
        ...
        Args:
            ...
            batch_metrics (list): List of functions with the same signature as a loss function or objects with the same
                signature as either :class:`~ enaptorch.Metric` or :class:`torchmetrics.Metric <torchmetrics.Metric>`. It can
        ...
        Examples:
            Using a PyTorch DataLoader, on classification task with SGD optimizer::
                ...
                from  enaptorch import ModelBundle
    ...
    def from_model(
        ...
        *,
        ...
        # pylint: disable=line-too-long
        """
        Instanciate a :class:`~ enaptorch.ModelBundle` from a :class:`~ enaptorch.Model` instance.

        Args:
            directory (str): Path to the model bundle's working directory. Will be used for automatic logging.
            model ( enaptorch.Model): A Model instance..
            logging (bool): Whether or not to log the model bundle's progress. If true, various logging
                callbacks will be inserted to output training and testing stats as well as to save model checkpoints,
                for example, automatically. See :func:`~ModelBundle.train()` and :func:`~ModelBundle.test()` for more
                details. (Default value = True)
        ...
        Examples:
            Using a PyTorch DataLoader, on classification task with SGD optimizer::
                ...
                import torch
                from torch.utils.data import DataLoader, TensorDataset
                from  enaptorch import Model, ModelBundle
    ...
    def train(self, train_generator, valid_generator=None, **kwargs) -> List[Dict]:
        ...
        included. Notably, two :class:`~ enaptorch.ModelCheckpoint` objects will take care of saving the last and every
        new best (according to monitor mode) model weights in appropriate checkpoint files.
        :class:`~ enaptorch.OptimizerCheckpoint` and :class:`~ enaptorch.LRSchedulerCheckpoint` will also respectively
        handle the saving of the optimizer and LR scheduler's respective states for future retrieval. Moreover, a
        :class:`~ enaptorch.AtomicCSVLogger` will save all available epoch statistics in an output .tsv file. Lastly, a
        :class:`~ enaptorch.TensorBoardLogger` handles automatic TensorBoard logging of various neural network
        statistics.
        ...
        Args:
            ...
            valid_generator (optional): Generator-like object for the validation set. See
                ...
                (Default value = None)
            callbacks (List[~ enaptorch.Callback]): List of callbacks that will be called during
    ...
    def test(self, test_generator, **kwargs):
        ...
        included to save the test metrics. Moreover, a :class:`~ enaptorch.AtomicCSVLogger` will save the test
    ...
    def is_better_than(self, another_model_bundle) -> bool:
        ...
        Args:
            another_model_bundle (~ enaptorch.ModelBundle): Another  enaptorch model bundle to compare results with.
