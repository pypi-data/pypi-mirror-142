...
def one_cycle_phases(
    ...
    Returns:
        A list of configured :class:`~ enaptorch.Phase` instances.
...
class OptimizerPolicy(Callback):
    """
    Combine different :class:`~ enaptorch.Phase` instances
    in an :class:`~ enaptorch.OptimizerPolicy` and execute the policies in a
    row.

    Args:
        phases (List[~ enaptorch.Phase]):
            A list of :class:`~ enaptorch.Phase` instances.
    ...
    def plot(self, param_name: str = "lr", ax=None):
        """
        Visualize all  :class:`~ enaptorch.Phase`s of
        this  :class:`~ enaptorch.OptimizerPolicy`.
