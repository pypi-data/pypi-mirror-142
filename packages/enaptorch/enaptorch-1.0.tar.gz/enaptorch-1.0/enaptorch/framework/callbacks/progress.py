...
class ProgressionCallback(Callback):
    """
    Default progression callback used in :class:`~ enaptorch.Model`. You can use the ``progress_options``
    in :class:`~ enaptorch.Model` instead of instantiating this callback. If you choose to use this callback
    anyway, make sure to pass ``verbose=False`` to :func:`~ enaptorch.Model.fit()` or
    :func:`~ enaptorch.Model.fit_generator()`.
