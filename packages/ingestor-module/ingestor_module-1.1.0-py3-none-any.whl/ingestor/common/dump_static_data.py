...
def dump_clustercategory_nodes():
    # to dump cluster-category nodes into GraphDB
    clustercat_data = read_csv(
        STATIC_FILES_PATH +
        "ClusterCategory_Having_Content_20220104.csv"
    )
    clustercat_cls = StaticNodeGenerator(
        data=clustercat_data,
        label="cluster_category",
        connection_uri=LOCAL_CONNECTION_URI
    )
    clustercat_cls.clustercategory_controller()
...
if __name__ == '__main__':
    ...
    print("Dumping Cluster-Category nodes....")
    dump_clustercategory_nodes()
