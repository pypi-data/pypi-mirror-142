...
from ingestor.user_profile.constants import CUSTOMER_CREATED_ON,\
    CUSTOMER_MODIFIED_ON, PAYTVPROVIDER_ID, GENDER, GENDER_VALUES, \
    BIRTHDAY, DEFAULT_DATE, AGE, MEDIAN_AGE, AGE_UPPER_BOUND, \
    DEFAULT_NA, CUSTOMER_ID
...
class PreprocessDemography:
    ...
    def calculate_age(
        ...
        df[AGE].values[df[AGE] > AGE_UPPER_BOUND] = MEDIAN_AGE
    ...
    def preprocess_paytv_provider(
        ...
        return Utils.fillna_and_cast_lower(
            data=df,
            feature=PAYTVPROVIDER_ID,
            default_val=DEFAULT_NA
        )
    ...
    def preprocess_created_modified_on(
        ...
        df[CUSTOMER_CREATED_ON] = to_datetime(df[CUSTOMER_CREATED_ON])
        df[CUSTOMER_MODIFIED_ON] = to_datetime(df[CUSTOMER_MODIFIED_ON])
