...
from ingestor.user_profile.constants import SOLO_FEATURE_LIST, FEATURE_DICT, \
    CUSTOMER_ID, CSV_EXTENSION, USER_DEMOGRAPHY, FINAL_MERGED_DF
...
class MainImplementation:
    ...
    def controller(
        ...
        demography = PreprocessDemography()
        conn = ConnectS3()

        demo = demography.controller(
            df=self.df1
        )
        conn.write_csv_to_s3(
            bucket_name=S3_BUCKET_NAME,
            object_name=S3_OBJECT_NAME + USER_DEMOGRAPHY + CSV_EXTENSION,
            df_to_upload=demo,
            resource=resource)
