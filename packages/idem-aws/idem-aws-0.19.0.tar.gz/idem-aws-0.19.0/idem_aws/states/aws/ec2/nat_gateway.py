    ...
    **Autogenerated function**
    ...
    if before and before["result"] and before["ret"].get("NatGateways"):
        ...
        if (
            ...
            result["comment"] = (f"'{name}' already present. No update in tags",)
    else:
...
async def absent(hub, ctx, name: str, resource_id: str) -> Dict[str, Any]:
    ...
    **Autogenerated function**
    ...
    Args:
        name(Text): Name of nat gateway.
        resource_id(Text): The AWS ID of the nat gateway.
    ...
    if not before["result"]:
        ...
        try:
            if before["ret"]["NatGateways"][0]["State"] == "deleted":
                result["comment"] = (f"'{name}' is in deleted state.",)
            else:
                result[
                    "old_state"
                ] = hub.tool.aws.ec2.conversion_utils.convert_raw_nat_gateway_to_present(
...
def is_pending(hub, ret):
    ...
    if status and isinstance(status, str):
        hub.log.debug(f"Nat gateway {resource_id} is_pending() status {status}")
        if status.casefold() == "available" or status.casefold() == "deleted":
            hub.log.debug(
                f"No need to reconcile new state {new_state} with status {status}"
            )
            return False
        if status.casefold() == "pending" or status.casefold() == "deleting":
            hub.log.debug(f"Reconcile new state {new_state} with status {status}")
            return True
