...
def _has_preserved_ir(mge_hit, ctx):
    ...
    min_hsp_len = ctx.cnf_get_default('getint', 'putative_composite_transposon', 'min_ir_segment_length')
    min_aln_len = ctx.cnf_get_default('getint', 'putative_composite_transposon', 'min_ir_aln_length')
...
def _predict_putative_composite_tn(
    ...
    for raw_mge in raw_mges:
        ...
        if all((_has_preserved_ir(raw_mge, ctx), _is_type(raw_mge, MgeType.INSERTION_SEQUENCE, ctx.mge_db))):
