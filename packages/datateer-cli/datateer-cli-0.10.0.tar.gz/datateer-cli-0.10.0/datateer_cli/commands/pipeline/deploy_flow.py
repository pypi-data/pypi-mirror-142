...
def do_deploy(
    ...
    deploy_key_prefect_lib = deploy_key_prefect_lib or os.getenv("DEPLOY_KEY_PREFECT_LIB")
    ...
    if os.getenv("MELTANO_DATABASE_URI") is None:
        print("MELTANO_DATABASE_URI is None, so this flow will not attempt to connect to a meltano database")
    else:
        print(f'MELTANO_DATABASE_URI is populated with value {os.getenv("MELTANO_DATABASE_URI")[0:12]}***')
    ...
    flow.storage = Docker(
        ...
        image_tag="latest",
        files={full_path: os.path.join("/datateer/modules", module_path) for full_path, module_path in custom_modules},
        env_vars={"DATATEER_ENV": environment, "PYTHONPATH": "$PYTHONPATH:/datateer/modules/"},
        build_kwargs={
            "buildargs": {
    ...
    task_definition = {
        ...
        "volumes": [
            {
                "name": "pipeline-efs",
                "efsVolumeConfiguration": {"fileSystemId": get_efs_id_by_name("datateer-pipeline-efs")},
            }
        ],
    }
    env_vars = {"DATATEER_ENV": environment, "AWS_DEFAULT_REGION": aws_region}
    if prefect_results_bucket is not None:
        env_vars["S3_PREFECT_RESULTS_BUCKET"] = prefect_results_bucket
