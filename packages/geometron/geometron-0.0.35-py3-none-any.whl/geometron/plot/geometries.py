...
def plot_profile(ax=None, obj=None, name=''):
    if isinstance(obj, LineString):
        ...
        plot_shapely_obj(ax=ax, obj=Point(obj.coords[0]), marker=symbols['start'], color='g')  # start
        for i in range(1, len(obj.coords)):
            plot_shapely_obj(ax=ax, obj=Point(obj.coords[i]), marker=symbols['stake'], color='grey')
        plot_shapely_obj(ax=ax, obj=Point(obj.coords[-1]), marker=symbols['end'], color='r')  # end
