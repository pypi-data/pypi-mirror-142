...
def plot_convergence(loss,stdev,isurr=None,ylim=[],xlim=[]):
...
def get_custom_data(results,config=None):
    ...
    for hp_name in results.columns:
        ...
        if hp_name.startswith('hp'):
            ...
            text_coord += (hp_name+' = %{customdata['+hp_idx+']:'+hp_fmt+'}<br>')
            customdata = numpy.vstack((customdata,hp_vals))
    text_coord += '<extra></extra>'
    return text_coord, customdata.T

def hpo_solutions(results,ms=20,edgewidth=2,config=None):
    text_coord, custom_data = get_custom_data(results,config)
    fig = go.Figure(
        data=go.Scatter(
            x=results.loss,
            y=results.stdev,
