...
class Provider(object):
    ...
    def __process_single_group__(self):
        try:
            ...
            group.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group.columns]  # Renaming columns

            cf_type = self.get_chartfactor_type(g.get_name)
            if cf_type != 'ATTRIBUTE' and g.get_name == sort_name:
                group = group.sort_values([re.sub(r'[\.\s]*', '', g.get_name)], ascending=[asc]).reset_index(drop=True)
            if self.__visualization == 'Disk':
                group = group.sort_values([sort_name], ascending=[True]).reset_index(drop=True)
            ...
            for row in group.itertuples():
                ...
                for m in self.__metrics:
                    if m.func != cnts.COUNT:
                        if m.name not in current_metrics:
                            current_metrics[m.name] = {}
                        current_metrics[m.name][m.func.replace('mean', 'avg')] = round(float(row[f'{m.name.replace(".", "").replace(" ", "")}_{m.func}']), 2)

                if str(row[re.sub(r'[\.\s]*', '', g.get_name)]) in ['None', 'nan']:
                    row_value = None
                else:
                    row_value = int(row[re.sub(r'[\.\s]*', '', g.get_name)]) if self.__visualization == 'Disk' else str(row[re.sub(r'[\.\s]*', '', g.get_name)])

                response.get('data').append({
                    'group': [row_value],
                    'current': {
                        'count': int(row[f'{g.get_name.replace(".", "").replace(" ", "")}_count']),
                        'metrics': current_metrics
                    }
                })
    ...
    def __process_two_groups__(self):
        try:
            ...
            sub_query1.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in sub_query1.columns]
            ...
            sub_query2.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in sub_query2.columns]

            metrics_select = ""
            for m in self.__metrics:
                metrics_select += f"sq1.{str(m.name).replace('.', '').replace(' ', '')}_{m.func},\n"

            if sort_name_g1 == g1.get_name:
                first_sort = re.sub(r'[\.\s]*', '', str(sort_name_g1))
            else:
                first_sort = f"{str(sort_name_g1).replace('.', '').replace(' ', '')}_{sort_func_g1}"

            if sort_name_g2 == g2.get_name:
                second_sort = re.sub(r'[\.\s]*', '', str(sort_name_g2))
            else:
                second_sort = f"{str(sort_name_g2).replace('.', '').replace(' ', '')}_{sort_func_g2}"

            final_query = f"""
            SELECT * FROM (
                SELECT sq1.{g1.get_name},
                       sq1.{g2.get_name},
                       {metrics_select}
                       sq1.{g1.get_name}_count as count,
                       ROW_NUMBER() OVER (PARTITION BY sq1.{g1.get_name} ORDER BY sq1.{second_sort} {sort_dir_g2}) as rank
                FROM sub_query1 as sq1 join sub_query2 as sq2
                ON sq1.{g1.get_name} = sq2.{g1.get_name}
            ...
            for row in groups.itertuples():
                ...
                for m in self.__metrics:
                    ...
                    current_metrics[m.name][m.func.replace('mean', 'avg')] = round(
                        float(row[f"{str(m.name).replace('.', '').replace(' ', '')}_{m.func}"]), 2)

                    group1 = None if str(row[g1.get_name]) in ['None', 'nan'] else str(row[g1.get_name])
                    group2 = None if str(row[g2.get_name]) in ['None', 'nan'] else str(row[g2.get_name])
        ...
        try:
            if len(self.__groups) > 0:
                if len(self.__metrics) > 0:
                    ...
                    group1.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group1.columns]

                    group2 = self.__data.groupby([g.get_name]).agg({m.name: [quantile_25]}).reset_index()
                    group2.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group2.columns]

                    group3 = self.__data.groupby([g.get_name]).agg({m.name: [quantile_50]}).reset_index()
                    group3.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group3.columns]

                    group4 = self.__data.groupby([g.get_name]).agg({m.name: [quantile_75]}).reset_index()
                    group4.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group4.columns]

                    group5 = self.__data.groupby([g.get_name]).agg({m.name: [quantile_100]}).reset_index()
                    group5.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group5.columns]

                    merge = pd.merge(group1, group2, on=g.get_name)
                    merge = pd.merge(merge, group3, on=g.get_name)
                    merge = pd.merge(merge, group4, on=g.get_name)
                    merge = pd.merge(merge, group5, on=g.get_name)
                    merge = merge.sort_values([g.get_name], ascending=[asc]).iloc[:limit].reset_index(drop=True)
                    ...
                    for row in merge.itertuples():
                        ...
                        for met in self.__metrics:
                            if met.name == m.name:
                                ...
                                q_0 = round(float(row[f'{m.name.replace(".", "").replace(" ", "")}_quantile_0']), 2)
                                q_25 = round(float(row[f'{m.name.replace(".", "").replace(" ", "")}_quantile_25']), 2)
                                q_50 = round(float(row[f'{m.name.replace(".", "").replace(" ", "")}_quantile_50']), 2)
                                q_75 = round(float(row[f'{m.name.replace(".", "").replace(" ", "")}_quantile_75']), 2)
                                q_100 = round(float(row[f'{m.name.replace(".", "").replace(" ", "")}_quantile_100']), 2)
    ...
    def __process_histogram__(self):
        try:
            if len(self.__metrics) > 0:
                ...
                group.columns = [re.sub(r'[\.\s]*', '', '_'.join(filter(None, x))) for x in group.columns]
                if self.__limit > 0:
                ...
                for row in limited_group.itertuples():
                    ...
                    response.get('data').append({
                        'group': [[g1, g2]],
                        'current': {
                            'count': round(float(row[f"{m.name.replace('.', '').replace(' ', '')}_count"]), 2)
                        }
                    })
