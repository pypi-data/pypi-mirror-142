...
from pydantic import BaseModel, Field, conint, validate_arguments, validator
...
class TimePoint(MixinConfig):
    ...
    def validate_end_time(cls, v, values):
        ...
        assert v >= values['start_time'], f"end_time {v} must be >= start_time {values['start_time']}"
        return v
...
class VideoTask(DeeplabelBase):
    video_task_id: str
    graph_id: str
    graph_node:GraphNodeAnnotation = Field(..., alias='graph_node_id')
    is_shown: bool
    ...
    def insert_detections(
        self, detections: List["deeplabel.infer.videos.detections.Detection"], chunk_size=500
    ):
        ...
        for dets in chunk(detections, chunk_size):
            ...
            count += len(data)
            self.client.post("/detections", {"data": data, "batch":True})
        logger.info(
            f"Completed pushing {len(detections)} detections for videoTaskId: {self.video_task_id}"
        )
    
    @validate_arguments
    def update(self,final_time_points:Optional[List[TimePoint]]=None, progress:Optional[conint(ge=0,le=100)]=None, status:Optional[VideoTaskStatus] = None):
        data = {}
        if final_time_points is not None:
            data['finalTimePoints'] = [tp.dict(by_alias=True) for tp in final_time_points]
