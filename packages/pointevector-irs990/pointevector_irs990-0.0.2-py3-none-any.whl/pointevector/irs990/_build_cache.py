...
def build(config: Config):
    ...
    archive_index = pd.read_csv(config['cache']['archive_directory'].joinpath('index.csv'), dtype=str).set_index('ein')
    companies = pd.read_csv(config['company_map'], dtype=str).set_index('ein')
    reduced_index = pd.merge(companies, archive_index, on='ein')

    archives = {archive: zipfile.ZipFile(archive) for archive in reduced_index.archive.unique()}
