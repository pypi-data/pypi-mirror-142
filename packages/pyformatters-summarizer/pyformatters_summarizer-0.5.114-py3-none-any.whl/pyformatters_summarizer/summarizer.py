...
class SummarizerParameters(FormatterParameters):
    ...
    Specifying this parameter will lead the model to use beam search instead of greedy search, setting num_beams to 4, will allow the model to lookahead for four possible words (1 in the case of greedy search), to keep the most likely 4 of hypotheses at each time step, and choosing the one that has the overall highest probability.""",
    )
