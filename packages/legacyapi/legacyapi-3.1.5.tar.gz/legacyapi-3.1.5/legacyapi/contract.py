...
from legacyapi.common.abi import (
    filter_by_type,
...
from legacyapi.common.contracts import (
    find_matching_fn_abi,
    encode_abi,
    get_function_info,
    FallbackFn
)
from legacyapi.common.datatypes import PropertyCheckingFactory
from legacyapi.common.encoding import to_4byte_hex
from legacyapi.common.normalizers import (
    normalize_abi,
    normalize_bytecode,
...
class ContractFunctions:
    ...
    def __init__(self, abi, legacy, address=None):
        if abi:
            self.abi = abi
...
class Contract:
    ...
    def __init__(self, address=None):
        ...
        if self.legacy is None:
            raise AttributeError(
                ...
                '`legacy.contract` interface to create your contract class.'
            )

        if address:
            self.address = self.legacy.address.to_hex(address)

        if not self.address:
            raise TypeError("The address argument is required to instantiate a contract.")

        self.functions = ContractFunctions(self.abi, self.legacy, self.address)
        self.fallback = Contract.get_fallback_function(self.abi, self.legacy, self.address)

    @classmethod
    def factory(cls, legacy, class_name=None, **kwargs):
        ...
        setattr(contract, 'functions', ContractFunctions(contract.abi, contract.legacy))
        setattr(contract, 'fallback', Contract.get_fallback_function(contract.abi, contract.legacy))
    ...
    def deploy(cls, **kwargs):
        ...
        """
        return cls.legacy.transaction_builder.create_smart_contract(
            **kwargs,
            abi=cls.abi,
    ...
    def constructor(cls):
        ...
        return ContractConstructor(cls.legacy,
                                   cls.abi,
                                   cls.bytecode)

    @combomethod
    def encodeABI(cls, fn_name, args=None, kwargs=None, data=None):
        """Encodes the arguments using the Legacy ABI for the contract function
        ...
        return encode_abi(cls.legacy, fn_abi, fn_arguments, data)

    @staticmethod
    def get_fallback_function(abi, legacy, address=None):
    ...
    def all_functions(self):
        return find_functions_by_identifier(
            self.abi, self.legacy, self.address, lambda _: True
        )
    ...
    def get_function_by_signature(self, signature):
        ...
        fns = find_functions_by_identifier(self.abi, self.legacy, self.address, callable_check)
    ...
    def find_functions_by_name(self, fn_name):
        ...
        return find_functions_by_identifier(
            self.abi, self.legacy, self.address, callable_check
        )
...
class ContractConstructor:
    ...
    def __init__(self, legacy, abi, bytecode):
        self.legacy = legacy
        self.abi = abi
    ...
    def transact(self, **kwargs):
        ...
        return self.legacy.transaction_builder.create_smart_contract(
            **kwargs,
            abi=self.abi,
...
def find_functions_by_identifier(contract_abi, legacy, address, callable_check):
