...
<script>
  ...
  <tr id="board_<%text>${number}</%text>">
    ...
    <td class="center aligned<%text> ${c2_break}</%text>" id="score_<%text>${number}</%text>_2"></td>
     ...
     $('tbody input[name="coins_' + number + '_1"], tbody input[name="coins_' + number + '_2"], tbody input[name="queen_' + number +'"]')
         .keyup(function() {
             compute_scores_and_totals($(this).attr('name'));
         })
         .change(function() {
             compute_scores_and_totals($(this).attr('name'));
         });
 ...
 function compute_scores_and_totals(what_changed) {
     const max_allowed_boards = ${ 100 if match.final else 9 },
           boards = $('form > table > tbody > tr').length - 1;

     if(what_changed && what_changed.startsWith('coins')) {
         var coins = $('input[name="' + what_changed + '"]').val();
         // We where triggered by an user change
         if(what_changed.endsWith('1') && coins !== '') {
             $('input[name="' + what_changed.slice(0, -1) + '2' + '"]').val('0')
         }
        if(what_changed.endsWith('2') && coins !== '') {
             $('input[name="' + what_changed.slice(0, -1) + '1' + '"]').val('0')
         }
     }
     ...
     for(var board = 1; board <= boards; board++) {
         var $coins_1 = $('input[name="coins_' + board + '_1"]'),
             ...
             queen_1 = $('input[name="queen_' + board + '"][value="1"]').prop('checked'),
             queen_2 = $('input[name="queen_' + board + '"][value="2"]').prop('checked');
         ...
         if(coins_1 === '0' && coins_2 === '0') {
             ...
             if(queen_1 && score_1 > score_2 && total_1 < 22)
                 score_1 += 3;
             if(queen_2 && score_1 < score_2 && total_2 < 22)
                 score_2 += 3;
         }
         if(score_1 > score_2) {
     ...
     if(last_ok && boards < max_allowed_boards && total_1 < 25 && total_2 < 25) {
         $('button.ui.center.aligned.primary.button').removeClass('disabled');
     } else {
         $('button.ui.center.aligned.primary.button').addClass('disabled');
     }
 ...
 $(document).ready(function() {
     ...
     $('button.ui.center.aligned.primary.button').click(function() {
         var board = $('form > table > tbody > tr').length,
             coins_1 = $('input[name="coins_' + (board - 1) + '_1"]').val(),
             coins_2 = $('input[name="coins_' + (board - 1) + '_2"]').val(),
             total_1 = $('#total_1').text(),
             total_2 = $('#total_2').text(),
             queen_1 = $('input[name="queen_' + (board - 1) + '"][value="1"]').prop('checked'),
             queen_2 = $('input[name="queen_' + (board - 1) + '"][value="2"]').prop('checked');
         if(board == 1
            || (board < max_allowed_boards
                && (coins_1 || coins_2)
                && parseInt(coins_1) < 10
                && parseInt(coins_2) < 10
                && (queen_1 || queen_2)
     ...
     $("form").submit(function(event) {
         if(!confirm("${_('Do you confirm that the match has ended?')|n}"))
