...
class UdsClient:
    ...
    def request(self, req: bytes, suppress_response: bool = False) -> Optional[dict]:
        ...
        expected_timeout = ts_request_sent + datetime.timedelta(seconds=self.session_parameters.get("p2*_server_max"))
        # note p2*_server_max seems to be the maximum timeout for a response
        while datetime.datetime.now() < expected_timeout:
            try:
