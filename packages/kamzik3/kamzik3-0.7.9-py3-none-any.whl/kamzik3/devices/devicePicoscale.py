...
class DevicePicoscale(Device):
    ...
    def set_frame_aggregation(self, value):
        try:
            si.SetProperty_i32(
                self.handle, si.EPK(si.Property.FRAME_AGGREGATION, 0, 0), int(value)
            )
        except si.Error as e:
            raise DeviceError(self._get_exception(e))
    ...
    def set_frame_rate(self, value: int) -> None:
        try:
            si.SetProperty_i32(
                self.handle, si.EPK(si.Property.FRAME_RATE, 0, 0), int(value)
            )
    ...
    def process_buffer(self, buffer) -> bool:
        """
            ...
            while True:
                try:
                    ...
                    if ev.type == si.EventType.STREAMBUFFER_READY:
                        ...
                        print(
                            f"Received unexpected event type: {ev.type} (parameter: {ev.devEventParameter})"
                        )
                        break
...
class DevicePicoscaleChannel(DeviceChannel):
    ...
    def handle_configuration(self):
        ...
        head_type = self.device.available_head_types.inv[
