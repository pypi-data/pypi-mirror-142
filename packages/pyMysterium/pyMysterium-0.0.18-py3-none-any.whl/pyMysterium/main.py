...
**Note: leave it default"""
    ...
    def agree_terms(self, agreed: bool = True) -> bool:
        """Agree to Mysterium's terms and conditions"""
    ...
    def get_balance(self) -> float:
        """Returns current identitiy's balance"""
        response = requests.put(
            self.urls.balance.format(id=self.current_identity))
        if response.status_code == 200:
            try:
                return json.loads(response.content).get('Balance')/10**18
            except:
                return json.loads(response.content).get('balance_tokens').get('human')
        elif response.status_code == 400:
            raise BadRequestError(json.loads(response.content))
        elif response.status_code == 500:
            raise InternalServerError(json.loads(response.content))
    ...
    def get_int_balance(self) -> int:
        """Returns current identitiy's balance as integer"""
        response = requests.put(
            self.urls.balance.format(id=self.current_identity))
        if response.status_code == 200:
            return json.loads(response.content).get('Balance', json.loads(response.content).get('balance') )
        elif response.status_code == 400:
            raise BadRequestError(json.loads(response.content))
        elif response.status_code == 500:
            raise InternalServerError(json.loads(response.content))
