from .core import q, k, h, c, Stefan_Boltzmann, cosd, sind, arcsind, arccosd, Wien, pi
...
def blackbody_spectrum(wavelength, T=6000):
    """Return the blackbody irradiance (W/m2/nm) at a given wavelength (nm) and temperature, T (K). """
    wavelength = wavelength * 1e-9
    F = 2 * pi * h * c ** 2 / ((wavelength ** 5) * (np.exp(h * c / (wavelength * T * k)) - 1))
    return F * 1e-9  # convert W/m3 to W/m2/nm
...
def sun_position(dayNo, latitude, longitude, GMTOffset, H, M):
    """Return the position of the sun as a elevation(degrees) and azimuth(degrees) given
    latitude(degrees), logitude(degrees) and the GMTOffset(hours), """
