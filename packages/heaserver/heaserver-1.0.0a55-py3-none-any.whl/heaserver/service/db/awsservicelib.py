...
async def get_all_buckets(request: web.Request) -> List[DesktopObjectDict]:
    ...
    try:
        ...
        # mongo_attr = {'name': 'hci-foundation' } #bucket_name_list[0]}
        # user = request.headers.get(SUB)
        # bucket_dict = await request.app[HEA_DB].get(request, MONGODB_BUCKET_COLLECTION, mongoattributes=mongo_attr, sub=user)
        # if not bucket_dict:
        #     return web.HTTPBadRequest()

        buck_list = await asyncio.gather(*async_bucket_list, return_exceptions=True)
    except ClientError as e:
        logging.error(e)
        return response.status_bad_request()
    bucket_dict_list = [buck.to_dict() for buck in buck_list if buck and type(buck) is AWSBucket]

    return await response.get(request, {'Buckets': bucket_dict_list})
...
async def _get_bucket(volume_id: str, s3_resource: boto3.resource, s3_client: boto3.client,
                      ...
                      creation_date: Optional[str] = None) -> Union[AWSBucket, Response]:
    """
    ...
    :return: Returns either the AWSBucket it retrieved successfully, or HTTPErrorResponse
    """
    try:
        ...
        try:
            ...
            if e.response['Error']['Code'] != 'ObjectLockConfigurationNotFoundError':
                logging.error(e)
                return web.HTTPBadRequest(body="Can't determine if bucket object is locked or not")
            b.locked = False
