...
for myins in INS.splitlines():
    ...
    inscode2name[mycode]=myname
    insname2code[myname]=mycode

def importfromIRM():
    #Import des fichiers Excel pour toutes les communes depuis le site de l'IRM
    import requests

    for key,myins in enumerate(inscode2name):
        url="https://www.meteo.be//resources//climatology//climateCity//xls//IDF_table_INS"+myins+".xlsx"
        response=requests.get(url)
        if str(response.content).find("Page not found")==-1 :
            file=open("./irm/"+myins+".xlsx", 'wb')
            file.write(response.content)
            file.close()
            print(myins)


def ins_read_excel(code='',name=''):
    ...
    if code !='':
        ...
        loccode=insname2code[name]

    returnlevel=pd.read_excel("./irm/"+loccode+".xlsx",0,skiprows=range(7),nrows=19,usecols="A:M")
    standarddev=pd.read_excel("./irm/"+loccode+".xlsx",1,skiprows=range(7),nrows=19,usecols="A:M")
    confintlow=pd.read_excel("./irm/"+loccode+".xlsx",2,skiprows=range(7),nrows=19,usecols="A:M")
    confintup=pd.read_excel("./irm/"+loccode+".xlsx",3,skiprows=range(7),nrows=19,usecols="A:M")
    montana=pd.read_excel("./irm/"+loccode+".xlsx",4,skiprows=range(11),nrows=12,usecols="A:G")

    return returnlevel,standarddev,confintlow,confintup,montana

#Import des fichiers Excel IRM
#importfromIRM()

#Exemple de lecture pour la commune de Sambreville
all=ins_read_excel(name="Sambreville")

#Affichage des coefficients de Montana
print(all[4])
