...
class Size:
	...
	def set(self, max_no_peaks: float, window=repcap.Window.Default, marker=repcap.Marker.Default) -> None:
		...
		Snippet: driver.calculate.marker.function.fpeaks.listPy.size.set(max_no_peaks = 1.0, window = repcap.Window.Default, marker = repcap.Marker.Default) \n
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> float:
		"""SCPI: CALCulate<n>:MARKer<m>:FUNCtion:FPEaks:LIST:SIZE \n
		Snippet: value: float = driver.calculate.marker.function.fpeaks.listPy.size.get(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command defines the maximum number of peaks that the R&S FSW looks for during a peak search. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param marker: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Marker')
