...
class Result:
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> float:
		...
		Snippet: value: float = driver.calculate.marker.function.summary.mean.average.result.get(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command queries the average mean time domain power. The query is only possible if averaging has been activated
		...
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:MARKer{marker_cmd_val}:FUNCtion:SUMMary:MEAN:AVERage:RESult?')
