...
class SearchSignal:
	...
	def set(self, event: enums.EventOnce, window=repcap.Window.Default, marker=repcap.Marker.Default) -> None:
		"""SCPI: CALCulate<n>:MARKer<m>:FUNCtion:TOI:SEARchsignal \n
		Snippet: driver.calculate.marker.function.toi.searchSignal.set(event = enums.EventOnce.ONCE, window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:MARKer{marker_cmd_val}:FUNCtion:TOI:SEARchsignal {param}')

	# noinspection PyTypeChecker
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> enums.EventOnce:
		"""SCPI: CALCulate<n>:MARKer<m>:FUNCtion:TOI:SEARchsignal \n
		Snippet: value: enums.EventOnce = driver.calculate.marker.function.toi.searchSignal.get(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command initiates a search for signals in the current trace to determine the third intercept point. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param marker: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Marker')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		marker_cmd_val = self._cmd_group.get_repcap_cmd_value(marker, repcap.Marker)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:MARKer{marker_cmd_val}:FUNCtion:TOI:SEARchsignal?')
