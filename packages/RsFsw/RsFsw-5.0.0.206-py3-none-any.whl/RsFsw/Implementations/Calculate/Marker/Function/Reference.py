...
class Reference:
	...
	def set(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> None:
		...
		Snippet: driver.calculate.marker.function.reference.set(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command matches the reference level to the power level of a marker. If you use the command in combination with a
	...
	def set_with_opc(self, window=repcap.Window.Default, marker=repcap.Marker.Default, opc_timeout_ms: int = -1) -> None:
		...
		marker_cmd_val = self._cmd_group.get_repcap_cmd_value(marker, repcap.Marker)
		"""SCPI: CALCulate<n>:MARKer<m>:FUNCtion:REFerence \n
		Snippet: driver.calculate.marker.function.reference.set_with_opc(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command matches the reference level to the power level of a marker. If you use the command in combination with a
		delta marker, that delta marker is turned into a normal marker. \n
