...
class Result:
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default, trace=repcap.Trace.Default) -> float:
		...
		Snippet: value: float = driver.calculate.marker.function.ademod.freqError.result.get(window = repcap.Window.Default, marker = repcap.Marker.Default, trace = repcap.Trace.Default) \n
		This command queries the carrier offset (= frequency error) for FM and PM demodulation. The carrier offset is determined
		from the current measurement data (CLR/WRITE) . The modulation is removed using low pass filtering. \n
			...
			:return: carr_offset: The deviation of the calculated carrier frequency to the ideal carrier frequency in Hz."""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		marker_cmd_val = self._cmd_group.get_repcap_cmd_value(marker, repcap.Marker)
		trace_cmd_val = self._cmd_group.get_repcap_cmd_value(trace, repcap.Trace)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:MARKer{marker_cmd_val}:FUNCtion:ADEMod:FERRor:RESult{trace_cmd_val}?')
