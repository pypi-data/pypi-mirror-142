...
class Result:
	...
	def get(self, result_type: enums.ResultTypeB, window=repcap.Window.Default, trace=repcap.Trace.Default) -> List[float]:
		"""SCPI: CALCulate<n>:STATistics:RESult<res> \n
		Snippet: value: List[float] = driver.calculate.statistics.result.get(result_type = enums.ResultTypeB.ALL, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'CALCulate{window_cmd_val}:STATistics:RESult{trace_cmd_val}? {param}')
		return response
