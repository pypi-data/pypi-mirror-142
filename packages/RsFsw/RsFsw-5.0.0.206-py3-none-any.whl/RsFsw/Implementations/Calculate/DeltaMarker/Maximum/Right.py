...
class Right:
	...
	def set(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, opc_timeout_ms: int = -1) -> None:
		...
		Snippet: driver.calculate.deltaMarker.maximum.right.set(window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default) \n
		This command moves a delta marker to the next higher value. The search includes only measurement values to the right of
		the current marker position. In the spectrogram, the command moves a marker horizontally to the maximum level in the
