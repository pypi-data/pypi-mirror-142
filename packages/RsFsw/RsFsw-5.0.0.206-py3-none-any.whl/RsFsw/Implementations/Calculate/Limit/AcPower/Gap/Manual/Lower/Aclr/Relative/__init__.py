...
class Relative:
	...
	def set(self, sb_gaps: enums.SubBlockGaps, limit: float, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default, gapChannel=repcap.GapChannel.Default) -> None:
		...
		Snippet: driver.calculate.limit.acPower.gap.manual.lower.aclr.relative.set(sb_gaps = enums.SubBlockGaps.AB, limit = 1.0, window = repcap.Window.Default, limitIx = repcap.LimitIx.Default, gapChannel = repcap.GapChannel.Default) \n
		This command defines the relative limit for the ACLR power in the specified lower gap channel. The reference value for
		...
		gap channels (see [SENSe:]POWer:ACHannel:GAP<gap>:MODE) . \n
			...
			:param limit: Defines the relative limit for the ACLR power in the specified gap channel. Unit: DB
	...
	def get(self, sb_gaps: enums.SubBlockGaps, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default, gapChannel=repcap.GapChannel.Default) -> float:
		"""SCPI: CALCulate<n>:LIMit<li>:ACPower:GAP<gap>:MANual:LOWer:ACLR[:RELative] \n
		Snippet: value: float = driver.calculate.limit.acPower.gap.manual.lower.aclr.relative.get(sb_gaps = enums.SubBlockGaps.AB, window = repcap.Window.Default, limitIx = repcap.LimitIx.Default, gapChannel = repcap.GapChannel.Default) \n
		This command defines the relative limit for the ACLR power in the specified lower gap channel. The reference value for
		the relative limit is the measured channel power. If you define both an absolute limit and a relative limit, the R&S FSW
		uses the lower value for the limit check. This command is only available for for asymmetrical (manual) configuration of
		gap channels (see [SENSe:]POWer:ACHannel:GAP<gap>:MODE) . \n
			...
			:param limitIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Limit')
			:param gapChannel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Gap')
			:return: limit: Defines the relative limit for the ACLR power in the specified gap channel. Unit: DB"""
