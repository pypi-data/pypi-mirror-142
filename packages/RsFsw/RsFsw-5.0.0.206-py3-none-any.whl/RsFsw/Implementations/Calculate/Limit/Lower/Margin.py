...
class Margin:
	...
	def set(self, margin: float, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default) -> None:
		...
		Snippet: driver.calculate.limit.lower.margin.set(margin = 1.0, window = repcap.Window.Default, limitIx = repcap.LimitIx.Default) \n
	...
	def get(self, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default) -> float:
		"""SCPI: CALCulate<n>:LIMit<li>:LOWer:MARGin \n
		Snippet: value: float = driver.calculate.limit.lower.margin.get(window = repcap.Window.Default, limitIx = repcap.LimitIx.Default) \n
		This command defines an area around a lower limit line where limit check violations are still tolerated. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param limitIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Limit')
