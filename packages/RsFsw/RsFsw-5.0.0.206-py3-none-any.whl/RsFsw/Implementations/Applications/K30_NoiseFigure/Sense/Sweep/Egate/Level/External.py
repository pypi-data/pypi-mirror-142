...
class External:
	...
	def set(self, level: float, externalPort=repcap.ExternalPort.Default) -> None:
		"""SCPI: [SENSe]:SWEep:EGATe:LEVel[:EXTernal<1|2|3>] \n
		Snippet: driver.applications.k30NoiseFigure.sense.sweep.egate.level.external.set(level = 1.0, externalPort = repcap.ExternalPort.Default) \n
		Defines the gate level for which the gate is open. This command is only available for triggered gated measurements
		([SENSe:]SWEep:EGATe:AUTOMAN) . \n
		...
		self._core.io.write(f'SENSe:SWEep:EGATe:LEVel:EXTernal{externalPort_cmd_val} {param}')

	def get(self, externalPort=repcap.ExternalPort.Default) -> float:
		"""SCPI: [SENSe]:SWEep:EGATe:LEVel[:EXTernal<1|2|3>] \n
		Snippet: value: float = driver.applications.k30NoiseFigure.sense.sweep.egate.level.external.get(externalPort = repcap.ExternalPort.Default) \n
		Defines the gate level for which the gate is open. This command is only available for triggered gated measurements
		([SENSe:]SWEep:EGATe:AUTOMAN) . \n
			:param externalPort: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
			:return: level: No help available"""
		externalPort_cmd_val = self._cmd_group.get_repcap_cmd_value(externalPort, repcap.ExternalPort)
		response = self._core.io.query_str(f'SENSe:SWEep:EGATe:LEVel:EXTernal{externalPort_cmd_val}?')
