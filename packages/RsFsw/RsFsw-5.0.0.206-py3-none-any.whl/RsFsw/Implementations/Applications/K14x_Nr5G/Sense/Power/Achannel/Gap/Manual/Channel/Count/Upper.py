...
class Upper:
	...
	def set(self, sb_gaps: enums.SubBlockGaps, count: float, gapChannel=repcap.GapChannel.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.power.achannel.gap.manual.channel.count.upper.set(sb_gaps = enums.SubBlockGaps.AB, count = 1.0, gapChannel = repcap.GapChannel.Default) \n
		...
		self._core.io.write(f'SENSe:POWer:ACHannel:GAP{gapChannel_cmd_val}:MANual:CHANnel:COUNt:UPPer {param}'.rstrip())

	def get(self, sb_gaps: enums.SubBlockGaps, gapChannel=repcap.GapChannel.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:GAP<gap>:MANual:CHANnel:COUNt:UPPer \n
		Snippet: value: float = driver.applications.k14Xnr5G.sense.power.achannel.gap.manual.channel.count.upper.get(sb_gaps = enums.SubBlockGaps.AB, gapChannel = repcap.GapChannel.Default) \n
		No command help available \n
			:param sb_gaps: No help available
			:param gapChannel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Gap')
		...
		gapChannel_cmd_val = self._cmd_group.get_repcap_cmd_value(gapChannel, repcap.GapChannel)
		response = self._core.io.query_str(f'SENSe:POWer:ACHannel:GAP{gapChannel_cmd_val}:MANual:CHANnel:COUNt:UPPer? {param}')
