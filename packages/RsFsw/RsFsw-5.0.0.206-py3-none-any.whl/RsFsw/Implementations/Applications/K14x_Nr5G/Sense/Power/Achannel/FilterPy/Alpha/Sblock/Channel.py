...
class Channel:
	...
	def set(self, alpha: float, subBlock=repcap.SubBlock.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SENSe]:POWer:ACHannel:FILTer:ALPHa:SBLock<sb>:CHANnel<ch> \n
		Snippet: driver.applications.k14Xnr5G.sense.power.achannel.filterPy.alpha.sblock.channel.set(alpha = 1.0, subBlock = repcap.SubBlock.Default, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SENSe:POWer:ACHannel:FILTer:ALPHa:SBLock{subBlock_cmd_val}:CHANnel{channel_cmd_val} {param}')

	def get(self, subBlock=repcap.SubBlock.Default, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:FILTer:ALPHa:SBLock<sb>:CHANnel<ch> \n
		Snippet: value: float = driver.applications.k14Xnr5G.sense.power.achannel.filterPy.alpha.sblock.channel.get(subBlock = repcap.SubBlock.Default, channel = repcap.Channel.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sblock')
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Channel')
		...
		subBlock_cmd_val = self._cmd_group.get_repcap_cmd_value(subBlock, repcap.SubBlock)
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SENSe:POWer:ACHannel:FILTer:ALPHa:SBLock{subBlock_cmd_val}:CHANnel{channel_cmd_val}?')
