...
class Gap:
	...
	def set(self, bandwidth: float, gapChannel=repcap.GapChannel.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.power.achannel.bandwidth.gap.set(bandwidth = 1.0, gapChannel = repcap.GapChannel.Default) \n
		...
		self._core.io.write(f'SENSe:POWer:ACHannel:BWIDth:GAP{gapChannel_cmd_val} {param}')

	def get(self, gapChannel=repcap.GapChannel.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:BWIDth:GAP<gap> \n
		Snippet: value: float = driver.applications.k14Xnr5G.sense.power.achannel.bandwidth.gap.get(gapChannel = repcap.GapChannel.Default) \n
		No command help available \n
			:param gapChannel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Gap')
			:return: bandwidth: No help available"""
		gapChannel_cmd_val = self._cmd_group.get_repcap_cmd_value(gapChannel, repcap.GapChannel)
		response = self._core.io.query_str(f'SENSe:POWer:ACHannel:BWIDth:GAP{gapChannel_cmd_val}?')
