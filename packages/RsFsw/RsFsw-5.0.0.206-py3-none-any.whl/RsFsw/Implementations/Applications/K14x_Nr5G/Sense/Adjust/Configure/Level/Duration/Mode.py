...
class Mode:
	...
	def set(self, mode: enums.AutoManualMode) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.adjust.configure.level.duration.mode.set(mode = enums.AutoManualMode.AUTO) \n
	...
	def get(self) -> enums.AutoManualMode:
		"""SCPI: [SENSe]:ADJust:CONFigure[:LEVel]:DURation:MODE \n
		Snippet: value: enums.AutoManualMode = driver.applications.k14Xnr5G.sense.adjust.configure.level.duration.mode.get() \n
		In order to determine the ideal reference level, the R&S FSW performs a measurement on the current input data.
		This command selects the way the R&S FSW determines the length of the measurement . \n
