...
class Bitmap:
	...
	def set(self, value: float, carrierComponent=repcap.CarrierComponent.Default, frame=repcap.Frame.Default, bwPart=repcap.BwPart.Default, csiRs=repcap.CsiRs.Default) -> None:
		...
		This command defines the bitmap setting for the CSI-RS. Note that you cannot change the bitmap value for selected
		location settings (method RsFsw.Applications.K14x_Nr5G.Configure.Nr5G.Downlink.Cc.Frame.BwPart.Csi.Row.set) . \n
