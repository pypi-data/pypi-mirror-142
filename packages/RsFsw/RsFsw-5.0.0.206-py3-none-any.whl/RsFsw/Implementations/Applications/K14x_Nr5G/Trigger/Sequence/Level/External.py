...
class External:
	...
	def set(self, level: float, externalPort=repcap.ExternalPort.Default) -> None:
		"""SCPI: TRIGger[:SEQuence]:LEVel[:EXTernal<1..3>] \n
		Snippet: driver.applications.k14Xnr5G.trigger.sequence.level.external.set(level = 1.0, externalPort = repcap.ExternalPort.Default) \n
		This command defines the level for an external trigger. \n
			:param level: Range: 0.5 V to 3.5 V, Unit: V
	...
	def get(self, externalPort=repcap.ExternalPort.Default) -> float:
		"""SCPI: TRIGger[:SEQuence]:LEVel[:EXTernal<1..3>] \n
		Snippet: value: float = driver.applications.k14Xnr5G.trigger.sequence.level.external.get(externalPort = repcap.ExternalPort.Default) \n
		This command defines the level for an external trigger. \n
			:param externalPort: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
			:return: level: Range: 0.5 V to 3.5 V, Unit: V"""
		externalPort_cmd_val = self._cmd_group.get_repcap_cmd_value(externalPort, repcap.ExternalPort)
