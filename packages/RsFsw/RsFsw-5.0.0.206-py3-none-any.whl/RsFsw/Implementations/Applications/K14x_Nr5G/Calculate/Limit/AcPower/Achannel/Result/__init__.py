...
class Result:
	...
	def get(self, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default) -> Results:
		...
		Snippet: value: Results = driver.applications.k14Xnr5G.calculate.limit.acPower.achannel.result.get(window = repcap.Window.Default, limitIx = repcap.LimitIx.Default) \n
		This command queries the limit check results for the adjacent channels during ACLR measurements. \n
		...
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:LIMit{limitIx_cmd_val}:ACPower:ACHannel:RESult?', self.__class__.Results())
