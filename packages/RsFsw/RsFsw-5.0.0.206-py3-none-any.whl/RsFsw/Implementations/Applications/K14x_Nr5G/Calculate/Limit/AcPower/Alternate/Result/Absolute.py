...
class Absolute:
	...
	def get(self, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default, channel=repcap.Channel.Default) -> Results:
		...
		Snippet: value: Results = driver.applications.k14Xnr5G.calculate.limit.acPower.alternate.result.absolute.get(window = repcap.Window.Default, limitIx = repcap.LimitIx.Default, channel = repcap.Channel.Default) \n
		This command queries the absolute limit check results for the alternate channels (MC ACLR measurements) .
		...
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:LIMit{limitIx_cmd_val}:ACPower:ALTernate{channel_cmd_val}:RESult:ABSolute?', self.__class__.Results())
