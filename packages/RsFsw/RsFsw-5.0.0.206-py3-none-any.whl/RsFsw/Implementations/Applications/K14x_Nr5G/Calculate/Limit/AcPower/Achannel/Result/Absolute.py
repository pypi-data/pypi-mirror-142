...
class Absolute:
	...
	def get(self, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default) -> Results:
		...
		Snippet: value: Results = driver.applications.k14Xnr5G.calculate.limit.acPower.achannel.result.absolute.get(window = repcap.Window.Default, limitIx = repcap.LimitIx.Default) \n
		This command queries the absolute limit check results for adjacent channels (ACLR measurements) .
		...
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:LIMit{limitIx_cmd_val}:ACPower:ACHannel:RESult:ABSolute?', self.__class__.Results())
