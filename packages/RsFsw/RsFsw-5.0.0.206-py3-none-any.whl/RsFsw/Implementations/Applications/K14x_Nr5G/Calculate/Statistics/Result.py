...
class Result:
	...
	def get(self, result_type: enums.ResultTypeB, window=repcap.Window.Default, trace=repcap.Trace.Default) -> float:
		"""SCPI: CALCulate<n>:STATistics:RESult<t> \n
		Snippet: value: float = driver.applications.k14Xnr5G.calculate.statistics.result.get(result_type = enums.ResultTypeB.ALL, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		...
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:STATistics:RESult{trace_cmd_val}? {param}')
