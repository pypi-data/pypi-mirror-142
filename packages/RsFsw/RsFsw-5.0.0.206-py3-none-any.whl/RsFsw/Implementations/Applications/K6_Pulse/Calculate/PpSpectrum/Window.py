...
class Window:
	...
	def set(self, window_type: enums.SpectrumWindowType, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:PPSPectrum:WINDow \n
		Snippet: driver.applications.k6Pulse.calculate.ppSpectrum.window.set(window_type = enums.SpectrumWindowType.BARTlett, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:PPSPectrum:WINDow {param}')

	# noinspection PyTypeChecker
	def get(self, window=repcap.Window.Default) -> enums.SpectrumWindowType:
		"""SCPI: CALCulate<n>:PPSPectrum:WINDow \n
		Snippet: value: enums.SpectrumWindowType = driver.applications.k6Pulse.calculate.ppSpectrum.window.get(window = repcap.Window.Default) \n
		Defines the used FFT window type for pulse-to-pulse spectrum displays. For more information see 'Parameter spectrum
		calculation'. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: window_type: RECTangle | BARTlett | HAMMing | HANNing | BLACkman"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:PPSPectrum:WINDow?')
