...
class Limit:
	...
	def set(self, lower_limit: float, upper_limit: float, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TABLe:EMODel:FLPLevel:LIMit \n
		Snippet: driver.applications.k6Pulse.calculate.table.emodel.fallLowPoint.level.limit.set(lower_limit = 1.0, upper_limit = 1.0, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:TABLe:EMODel:FLPLevel:LIMit {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> LimitStruct:
		"""SCPI: CALCulate<n>:TABLe:EMODel:FLPLevel:LIMit \n
		Snippet: value: LimitStruct = driver.applications.k6Pulse.calculate.table.emodel.fallLowPoint.level.limit.get(window = repcap.Window.Default) \n
		Defines the valid value range for the limit check for the selected parameter if limit check is active
		(CALCulate<n>:TABLe:<ParameterGroup>:<Parameter>:LIMit:STATeON) . Commands for the parameter group <TSIDelobe> are only
		available if the additional option R&S FSW-K6S is installed. For details on the individual parameters see 'Pulse
		parameters'. \n
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:TABLe:EMODel:FLPLevel:LIMit?', self.__class__.LimitStruct())
