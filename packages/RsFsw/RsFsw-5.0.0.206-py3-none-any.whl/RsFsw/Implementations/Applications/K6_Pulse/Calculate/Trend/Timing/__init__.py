...
class Timing:
	...
	def set(self, yaxis: enums.PulseTimingItems, xaxis: enums.PulseAxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:TIMing \n
		Snippet: driver.applications.k6Pulse.calculate.trend.timing.set(yaxis = enums.PulseTimingItems.DCYCle, xaxis = enums.PulseAxisItems.DCYCle, window = repcap.Window.Default) \n
		...
		Applications.K6_Pulse.Calculate.Trend.Timing.X.set) CALCulate<n>:TRENd:TIMing:Y <YAxis> (see method RsFsw.Applications.
		K6_Pulse.Calculate.Trend.Timing.Y.set) \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.PulseTimingItems), ArgSingle('xaxis', xaxis, DataType.Enum, enums.PulseAxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:TIMing {param}'.rstrip())
