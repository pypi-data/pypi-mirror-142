...
class Qcomponent:
	"""Qcomponent commands group definition. 6 total commands, 5 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> List[float]:
		"""SCPI: [SENSe]:PULSe:POWer:AMPLitude:Q \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.power.amplitude.qcomponent.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:POWer:AMPLitude:Q? {param}')
		return response
