...
class Count:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:OVERshoot[:PERCent]:COUNt \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.overshoot.percent.count.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:OVERshoot:PERCent:COUNt? {param}')
