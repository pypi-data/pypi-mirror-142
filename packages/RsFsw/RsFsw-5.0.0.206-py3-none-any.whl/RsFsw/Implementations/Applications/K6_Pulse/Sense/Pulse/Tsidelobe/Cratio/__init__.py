...
class Cratio:
	"""Cratio commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:CRATio \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.tsidelobe.cratio.get(query_range = enums.SelectionRange.ALL) \n
		Returns the compression ratio within the time sidelobe range for the specified pulse(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:CRATio? {param}')
