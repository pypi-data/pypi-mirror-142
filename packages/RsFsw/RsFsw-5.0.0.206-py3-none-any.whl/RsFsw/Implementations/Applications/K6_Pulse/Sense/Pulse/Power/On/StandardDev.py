...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:ON:SDEViation \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.on.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:ON:SDEViation? {param}')
