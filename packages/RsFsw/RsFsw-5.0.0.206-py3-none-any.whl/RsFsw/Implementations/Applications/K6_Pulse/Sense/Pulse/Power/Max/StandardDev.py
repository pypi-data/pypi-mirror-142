...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:MAX:SDEViation \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.max.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the maximum transmission power over the specified pulses. \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:MAX:SDEViation? {param}')
