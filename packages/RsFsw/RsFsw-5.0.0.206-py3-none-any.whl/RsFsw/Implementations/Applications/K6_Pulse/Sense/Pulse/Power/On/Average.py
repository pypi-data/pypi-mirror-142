...
class Average:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:ON:AVERage \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.on.average.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:ON:AVERage? {param}')
