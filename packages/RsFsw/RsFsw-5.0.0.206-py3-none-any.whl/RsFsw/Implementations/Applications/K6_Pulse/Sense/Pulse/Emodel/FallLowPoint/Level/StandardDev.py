...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:FLPLevel:SDEViation \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.fallLowPoint.level.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:FLPLevel:SDEViation? {param}')
		return response
