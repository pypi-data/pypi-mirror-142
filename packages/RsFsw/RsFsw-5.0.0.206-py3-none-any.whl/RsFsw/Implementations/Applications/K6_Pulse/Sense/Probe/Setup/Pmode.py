...
class Pmode:
	...
	def set(self, mode: enums.ProbeMode, probe=repcap.Probe.Default) -> None:
		...
		Snippet: driver.applications.k6Pulse.sense.probe.setup.pmode.set(mode = enums.ProbeMode.CM, probe = repcap.Probe.Default) \n
	...
	def get(self, probe=repcap.Probe.Default) -> enums.ProbeMode:
		"""SCPI: [SENSe]:PROBe<pb>:SETup:PMODe \n
		Snippet: value: enums.ProbeMode = driver.applications.k6Pulse.sense.probe.setup.pmode.get(probe = repcap.Probe.Default) \n
		No command help available \n
			:param probe: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Probe')
