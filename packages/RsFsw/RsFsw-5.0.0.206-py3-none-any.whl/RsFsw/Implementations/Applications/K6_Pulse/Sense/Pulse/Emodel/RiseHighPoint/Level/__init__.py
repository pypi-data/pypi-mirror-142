...
class Level:
	...
	def get(self, query_range: enums.SelectionRange) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:RHPLevel \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.riseHighPoint.level.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:RHPLevel? {param}')
		return response
