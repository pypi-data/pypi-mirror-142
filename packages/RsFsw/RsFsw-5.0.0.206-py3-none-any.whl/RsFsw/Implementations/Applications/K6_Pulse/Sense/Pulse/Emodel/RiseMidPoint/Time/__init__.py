...
class Time:
	...
	def get(self, query_range: enums.SelectionRange) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:RMPTime \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.riseMidPoint.time.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:RMPTime? {param}')
		return response
