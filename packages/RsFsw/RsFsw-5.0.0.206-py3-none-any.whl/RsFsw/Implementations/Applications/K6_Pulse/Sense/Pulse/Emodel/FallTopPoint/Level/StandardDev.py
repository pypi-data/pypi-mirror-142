...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:FTPLevel:SDEViation \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.fallTopPoint.level.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:FTPLevel:SDEViation? {param}')
		return response
