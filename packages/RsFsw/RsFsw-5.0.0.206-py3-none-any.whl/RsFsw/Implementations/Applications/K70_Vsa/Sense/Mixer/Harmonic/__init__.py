...
class Harmonic:
	"""Harmonic commands group definition. 5 total commands, 4 Subgroups, 0 group commands"""
	...
	def low(self):
		...
		if not hasattr(self, '_low'):
			from .Low import Low
			self._low = Low(self._core, self._cmd_group)
		return self._low
	...
	def clone(self) -> 'Harmonic':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Harmonic(self._core, self._cmd_group.parent)
