...
class External:
	...
	def set(self, level: float, externalPort=repcap.ExternalPort.Default) -> None:
		"""SCPI: TRIGger[:SEQuence]:LEVel[:EXTernal<1|2|3>] \n
		Snippet: driver.applications.k149Uwb.trigger.sequence.level.external.set(level = 1.0, externalPort = repcap.ExternalPort.Default) \n
	...
	def get(self, externalPort=repcap.ExternalPort.Default) -> float:
		"""SCPI: TRIGger[:SEQuence]:LEVel[:EXTernal<1|2|3>] \n
		Snippet: value: float = driver.applications.k149Uwb.trigger.sequence.level.external.get(externalPort = repcap.ExternalPort.Default) \n
		This command defines the level the external signal must exceed to cause a trigger event. \n
			:param externalPort: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
