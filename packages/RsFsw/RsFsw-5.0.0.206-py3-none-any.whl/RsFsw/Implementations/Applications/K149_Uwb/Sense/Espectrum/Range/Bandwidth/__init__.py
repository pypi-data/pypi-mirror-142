...
class Bandwidth:
	...
	def set(self, max_py: float, subBlock=repcap.SubBlock.Default, rangePy=repcap.RangePy.Default) -> None:
		...
		Snippet: driver.applications.k149Uwb.sense.espectrum.range.bandwidth.set(max_py = 1.0, subBlock = repcap.SubBlock.Default, rangePy = repcap.RangePy.Default) \n
	...
	def get(self, subBlock=repcap.SubBlock.Default, rangePy=repcap.RangePy.Default) -> float:
		"""SCPI: [SENSe]:ESPectrum<sb>:RANGe<range>:BANDwidth \n
		Snippet: value: float = driver.applications.k149Uwb.sense.espectrum.range.bandwidth.get(subBlock = repcap.SubBlock.Default, rangePy = repcap.RangePy.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Espectrum')
			:param rangePy: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Range')
