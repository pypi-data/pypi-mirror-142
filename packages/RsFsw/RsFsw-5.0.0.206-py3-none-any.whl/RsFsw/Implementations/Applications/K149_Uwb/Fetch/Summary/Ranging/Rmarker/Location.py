...
class Location:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:RANGing:RMARker:LOCation \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.ranging.rmarker.location.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:RANGing:RMARker:LOCation?')
