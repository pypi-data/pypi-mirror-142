...
class Peak:
	"""Peak commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:XCORr:MLOBe:PEAK \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.xcorr.mlobe.peak.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:XCORr:MLOBe:PEAK?')
