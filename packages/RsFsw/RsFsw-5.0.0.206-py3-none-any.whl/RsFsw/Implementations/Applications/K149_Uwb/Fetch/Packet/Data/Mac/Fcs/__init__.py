...
class Fcs:
	"""Fcs commands group definition. 2 total commands, 1 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:PACKet:DATA:MAC:FCS \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.packet.data.mac.fcs.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:MAC:FCS?')
