...
class Nrmse:
	"""Nrmse commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:EVM:PSDU:NRMSe \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.evm.psdu.nrmse.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:EVM:PSDU:NRMSe?')
		return Conversions.str_to_float(response)

	def clone(self) -> 'Nrmse':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Nrmse(self._core, self._cmd_group.parent)
