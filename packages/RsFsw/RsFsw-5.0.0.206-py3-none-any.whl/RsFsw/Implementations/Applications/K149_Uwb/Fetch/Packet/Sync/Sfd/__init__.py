...
class Sfd:
	"""Sfd commands group definition. 2 total commands, 1 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:PACKet:SYNC:SFD \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.packet.sync.sfd.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:SYNC:SFD?')
	...
	def clone(self) -> 'Sfd':
		...
		new_group = Sfd(self._core, self._cmd_group.parent)
