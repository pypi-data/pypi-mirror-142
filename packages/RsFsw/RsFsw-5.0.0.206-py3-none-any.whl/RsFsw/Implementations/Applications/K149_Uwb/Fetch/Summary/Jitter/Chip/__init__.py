...
class Chip:
	"""Chip commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:JITTer:CHIP \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.jitter.chip.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:JITTer:CHIP?')
	...
	def clone(self) -> 'Chip':
		...
		new_group = Chip(self._core, self._cmd_group.parent)
