...
class Monotonic:
	"""Monotonic commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("monotonic", core, parent)
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:SUMMary:PULSe:RISE:MONotonic \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.summary.pulse.rise.monotonic.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:PULSe:RISE:MONotonic?')
		return trim_str_response(response)

	def clone(self) -> 'Monotonic':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Monotonic(self._core, self._cmd_group.parent)
