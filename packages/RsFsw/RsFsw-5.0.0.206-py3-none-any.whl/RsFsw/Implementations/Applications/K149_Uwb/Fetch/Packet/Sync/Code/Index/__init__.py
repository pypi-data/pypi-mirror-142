...
class Index:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:PACKet:SYNC:CODE:INDex \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.packet.sync.code.index.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:SYNC:CODE:INDex?')
