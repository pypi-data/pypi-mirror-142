...
class Bit:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:PACKet:DATA:RANGing:BIT \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.packet.data.ranging.bit.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:RANGing:BIT?')
