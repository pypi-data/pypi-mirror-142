...
class StaFrequency:
	"""StaFrequency commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:STATe:STAFrequency \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.state.staFrequency.get(query_range = enums.SelectionRange.ALL) \n
		Returns the nominal hop state frequency from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:STATe:STAFrequency? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'StaFrequency':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = StaFrequency(self._core, self._cmd_group.parent)
