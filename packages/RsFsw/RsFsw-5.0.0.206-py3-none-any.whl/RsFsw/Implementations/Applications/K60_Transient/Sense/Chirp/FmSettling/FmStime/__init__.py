...
class FmStime:
	"""FmStime commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:FMSettling:FMSTime \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.fmSettling.fmStime.get(query_range = enums.SelectionRange.ALL) \n
		Returns the FM settling time from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:FMSettling:FMSTime? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'FmStime':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = FmStime(self._core, self._cmd_group.parent)
