...
class RelFrequency:
	"""RelFrequency commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:FREQuency:RELFrequency \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.relFrequency.get(query_range = enums.SelectionRange.ALL) \n
		Returns the relative hop-to-hop frequency from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:RELFrequency? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'RelFrequency':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = RelFrequency(self._core, self._cmd_group.parent)
