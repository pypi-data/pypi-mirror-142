...
class FmStime:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:FMSettling:FMSTime \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.fmSettling.fmStime.get(query_range = enums.SelectionRange.ALL) \n
		Returns the FM settling time from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FMSettling:FMSTime? {param}')
