...
class Minimum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:PMSettling:PMSTime:MINimum \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.pmSettling.pmStime.minimum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:PMSettling:PMSTime:MINimum? {param}')
