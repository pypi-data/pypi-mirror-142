...
class PmStime:
	"""PmStime commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("pmStime", core, parent)
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:PMSettling:PMSTime \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.pmSettling.pmStime.get(query_range = enums.SelectionRange.ALL) \n
		Returns the PM settling time from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:PMSettling:PMSTime? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'PmStime':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = PmStime(self._core, self._cmd_group.parent)
