...
class Maximum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:POWer:MAXPower:MAXimum \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.maxPower.maximum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:MAXPower:MAXimum? {param}')
