...
class Frequency:
	"""Frequency commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:FREQuency:FREQuency \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.frequency.frequency.get(query_range = enums.SelectionRange.ALL) \n
		Returns the average frequency from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:FREQuency:FREQuency? {param}')
