...
class PmStime:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:PMSettling:PMSTime \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.pmSettling.pmStime.get(query_range = enums.SelectionRange.ALL) \n
		Returns the PM settling time from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:PMSettling:PMSTime? {param}')
