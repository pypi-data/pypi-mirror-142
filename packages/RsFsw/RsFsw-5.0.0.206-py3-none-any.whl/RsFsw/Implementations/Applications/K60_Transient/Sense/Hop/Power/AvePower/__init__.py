...
class AvePower:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:POWer:AVEPower \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.avePower.get(query_range = enums.SelectionRange.ALL) \n
		Returns the average power from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:AVEPower? {param}')
