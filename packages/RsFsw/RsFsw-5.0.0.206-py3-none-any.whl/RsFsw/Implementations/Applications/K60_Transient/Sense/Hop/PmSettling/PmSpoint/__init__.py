...
class PmSpoint:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:PMSettling:PMSPoint \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.pmSettling.pmSpoint.get(query_range = enums.SelectionRange.ALL) \n
		Returns the PM settling point from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:PMSettling:PMSPoint? {param}')
