...
class MaxFm:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:FREQuency:MAXFm \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.maxFm.get(query_range = enums.SelectionRange.ALL) \n
		Returns the maximum Frequency Deviation from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:MAXFm? {param}')
