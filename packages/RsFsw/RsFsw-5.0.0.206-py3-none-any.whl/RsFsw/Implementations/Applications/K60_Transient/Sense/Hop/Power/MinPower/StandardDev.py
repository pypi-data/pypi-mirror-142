...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:POWer:MINPower:SDEViation \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.minPower.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:MINPower:SDEViation? {param}')
