...
class End:
	...
	def set(self, time: float, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:HOPDetection:PDEViation:OFFSet:END \n
		...
		Defines the end of the measurement range for phase deviation results as an offset in seconds from the hopend .
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:PDEViation:OFFSet:END {param}')
	...
	def get(self, window=repcap.Window.Default) -> float:
		...
		HopDetection.Pdeviation.Reference.set) . For details on the measurement range parameters see 'Measurement range'. \n
			...
			:return: time: Unit: S"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:HOPDetection:PDEViation:OFFSet:END?')
