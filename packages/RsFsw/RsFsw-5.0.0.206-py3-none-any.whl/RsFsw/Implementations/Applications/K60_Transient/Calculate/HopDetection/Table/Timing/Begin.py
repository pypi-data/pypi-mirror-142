...
class Begin:
	...
	def set(self, state: bool, scaling: enums.TimeScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:TIMing:BEGin \n
		Snippet: driver.applications.k60Transient.calculate.hopDetection.table.timing.begin.set(state = False, scaling = enums.TimeScaling.MS, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:TIMing:BEGin {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> BeginStruct:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:TIMing:BEGin \n
		Snippet: value: BeginStruct = driver.applications.k60Transient.calculate.hopDetection.table.timing.begin.get(window = repcap.Window.Default) \n
		If enabled, the specified time parameter is included in the result tables (see 'Timing parameters') . Note that only the
		enabled columns are returned for the method RsFsw.Applications.K60_Transient.Calculate.HopDetection.Table.Results.
		get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:TIMing:BEGin?', self.__class__.BeginStruct())
