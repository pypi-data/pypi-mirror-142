...
class Ival:
	...
	def get(self, window=repcap.Window.Default) -> GetStruct:
		...
		Snippet: value: GetStruct = driver.applications.k60Transient.calculate.rtms.window.ival.get(window = repcap.Window.Default) \n
		...
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:RTMS:WINDow:IVAL?', self.__class__.GetStruct())
