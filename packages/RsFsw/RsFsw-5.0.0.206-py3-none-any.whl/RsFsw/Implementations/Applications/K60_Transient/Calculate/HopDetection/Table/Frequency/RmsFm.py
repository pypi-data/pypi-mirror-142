...
class RmsFm:
	...
	def set(self, state: bool, scaling: enums.FrequencyScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:FREQuency:RMSFm \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:FREQuency:RMSFm {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> RmsFmStruct:
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:FREQuency:RMSFm?', self.__class__.RmsFmStruct())
