...
class Timing:
	...
	def set(self, yaxis: enums.AxisTimingItems, xaxis: enums.ChirpXaxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:CHIRp:TIMing \n
		Snippet: driver.applications.k60Transient.calculate.trend.chirp.timing.set(yaxis = enums.AxisTimingItems.BEGin, xaxis = enums.ChirpXaxisItems.AVGFm, window = repcap.Window.Default) \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.AxisTimingItems), ArgSingle('xaxis', xaxis, DataType.Enum, enums.ChirpXaxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:CHIRp:TIMing {param}'.rstrip())
