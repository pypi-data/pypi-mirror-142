...
class PmSettling:
	...
	def set(self, yaxis: enums.XaXisPmSettling, xaxis: enums.HopXaxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:HOP:PMSettling \n
		Snippet: driver.applications.k60Transient.calculate.trend.hop.pmSettling.set(yaxis = enums.XaXisPmSettling.PMSLength, xaxis = enums.HopXaxisItems.AVGFm, window = repcap.Window.Default) \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.XaXisPmSettling), ArgSingle('xaxis', xaxis, DataType.Enum, enums.HopXaxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:HOP:PMSettling {param}'.rstrip())

	def clone(self) -> 'PmSettling':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = PmSettling(self._core, self._cmd_group.parent)
