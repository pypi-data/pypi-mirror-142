...
class CmOffset:
	...
	def set(self, cm_offset: float, probe=repcap.Probe.Default) -> None:
		...
		Snippet: driver.applications.k91Wlan.sense.probe.setup.cmOffset.set(cm_offset = 1.0, probe = repcap.Probe.Default) \n
	...
	def get(self, probe=repcap.Probe.Default) -> float:
		"""SCPI: [SENSe]:PROBe<pb>:SETup:CMOFfset \n
		Snippet: value: float = driver.applications.k91Wlan.sense.probe.setup.cmOffset.get(probe = repcap.Probe.Default) \n
		No command help available \n
			:param probe: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Probe')
