...
class Dpath:
	...
	def set(self, state: enums.AutoOrOff, inputIx=repcap.InputIx.Default) -> None:
		...
		Snippet: driver.applications.k91Wlan.inputPy.dpath.set(state = enums.AutoOrOff.AUTO, inputIx = repcap.InputIx.Default) \n
	...
	def get(self, inputIx=repcap.InputIx.Default) -> enums.AutoOrOff:
		"""SCPI: INPut<Undef>:DPATh \n
		Snippet: value: enums.AutoOrOff = driver.applications.k91Wlan.inputPy.dpath.get(inputIx = repcap.InputIx.Default) \n
		Enables or disables the use of the direct path for frequencies close to 0 Hz. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
