...
class Text:
	...
	def set(self, text: str, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		Snippet: driver.applications.k91Wlan.display.window.trace.label.text.set(text = '1', window = repcap.Window.Default, trace = repcap.Trace.Default) \n
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> str:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:LABel:TEXT \n
		Snippet: value: str = driver.applications.k91Wlan.display.window.trace.label.text.get(window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
