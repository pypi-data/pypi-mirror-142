...
class Value:
	...
	def set(self, position: float, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k91Wlan.calculate.msra.aline.value.set(position = 1.0, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: CALCulate<n>:MSRA:ALINe[:VALue] \n
		Snippet: value: float = driver.applications.k91Wlan.calculate.msra.aline.value.get(window = repcap.Window.Default) \n
		This command defines the position of the analysis line for all time-based windows in all MSRA secondary applications and
		the MSRA primary application. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
