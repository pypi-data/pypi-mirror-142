...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:P2DB:MINimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p2Db.minimum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P2DB:MINimum:RESult?')
