...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:GAIN:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.gain.minimum.y.result.get() \n
		These commands query the result values for the Gain result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:GAIN:MINimum:Y:RESult?')
