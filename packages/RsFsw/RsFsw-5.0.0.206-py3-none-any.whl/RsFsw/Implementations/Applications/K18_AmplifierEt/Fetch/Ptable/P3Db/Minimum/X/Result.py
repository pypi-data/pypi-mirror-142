...
class Result:
	...
	def get(self) -> float:
		...
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p3Db.minimum.x.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P3DB:MINimum:X:RESult?')
