...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:ICC:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.icc.minimum.y.result.get() \n
		These commands query the result values for the I_cc result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:ICC:MINimum:Y:RESult?')
