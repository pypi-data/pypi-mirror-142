...
class StDeviation:
	...
	def get(self, voltage: enums.SelectionRangeB) -> float:
		"""SCPI: FETCh:STABle:VCC:MIN:STDeviation \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.stable.vcc.min.stDeviation.get(voltage = enums.SelectionRangeB.ALL) \n
		No command help available \n
			:param voltage: No help available
			:return: result: No help available"""
		param = Conversions.enum_scalar_to_str(voltage, enums.SelectionRangeB)
		response = self._core.io.query_str(f'FETCh:STABle:VCC:MIN:STDeviation? {param}')
