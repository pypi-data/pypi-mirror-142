...
class Result:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh:PTABle:ACP:ACHannel<ch>:BALanced:MAXimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.acp.achannel.balanced.maximum.x.result.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:ACP:ACHannel{window_cmd_val}:BALanced:MAXimum:X:RESult?')
