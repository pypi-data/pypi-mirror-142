...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:BBPower:MINimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.bbPower.minimum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:BBPower:MINimum:RESult?')
