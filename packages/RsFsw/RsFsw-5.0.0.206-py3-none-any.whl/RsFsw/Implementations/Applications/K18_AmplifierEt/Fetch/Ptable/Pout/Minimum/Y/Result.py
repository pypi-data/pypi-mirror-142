...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:POUT:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.pout.minimum.y.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:POUT:MINimum:Y:RESult?')
