...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:CFACtor:MINimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.cfactor.minimum.x.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:CFACtor:MINimum:X:RESult?')
