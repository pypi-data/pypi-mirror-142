...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:AMPM:CWIDth:MAXimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.amPm.cwidth.maximum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:AMPM:CWIDth:MAXimum:RESult?')
