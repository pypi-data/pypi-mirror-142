...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:AMAM:CWIDth:MAXimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.amam.cwidth.maximum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:AMAM:CWIDth:MAXimum:RESult?')
