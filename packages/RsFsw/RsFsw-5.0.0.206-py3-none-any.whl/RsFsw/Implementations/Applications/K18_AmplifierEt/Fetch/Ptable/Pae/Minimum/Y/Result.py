...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:PAE:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.pae.minimum.y.result.get() \n
		These commands query the result values for the PAE result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:PAE:MINimum:Y:RESult?')
