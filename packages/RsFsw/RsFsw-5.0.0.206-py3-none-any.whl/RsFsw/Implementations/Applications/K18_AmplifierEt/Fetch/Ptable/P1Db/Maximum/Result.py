...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:P1DB:MAXimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p1Db.maximum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P1DB:MAXimum:RESult?')
