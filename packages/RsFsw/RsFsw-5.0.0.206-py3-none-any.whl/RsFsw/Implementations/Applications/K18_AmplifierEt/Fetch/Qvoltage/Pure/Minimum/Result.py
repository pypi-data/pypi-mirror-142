...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:QVOLtage:PURE:MINimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.qvoltage.pure.minimum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:QVOLtage:PURE:MINimum:RESult?')
