...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:P2DB:MAXimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p2Db.maximum.x.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P2DB:MAXimum:X:RESult?')
