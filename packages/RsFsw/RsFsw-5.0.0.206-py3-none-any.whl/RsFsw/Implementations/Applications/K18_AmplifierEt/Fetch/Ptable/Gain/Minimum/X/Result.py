...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:GAIN:MINimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.gain.minimum.x.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:GAIN:MINimum:X:RESult?')
