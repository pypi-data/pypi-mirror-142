...
class Result:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh:PTABle:ACP:ACHannel<ch>:LOWer:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.acp.achannel.lower.minimum.y.result.get(window = repcap.Window.Default) \n
		These commands query the result values for the lower adjacent channel power as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:ACP:ACHannel{window_cmd_val}:LOWer:MINimum:Y:RESult?')
