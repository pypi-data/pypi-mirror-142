...
class Minimum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: FETCh:STABle:P2DB:IN:MINimum \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.stable.p2Db.inputPy.minimum.get(query_range = enums.SelectionRangeB.ALL) \n
		No command help available \n
			:param query_range: No help available
			:return: result: No help available"""
		param = Conversions.enum_scalar_to_str(query_range, enums.SelectionRangeB)
		response = self._core.io.query_str(f'FETCh:STABle:P2DB:IN:MINimum? {param}')
