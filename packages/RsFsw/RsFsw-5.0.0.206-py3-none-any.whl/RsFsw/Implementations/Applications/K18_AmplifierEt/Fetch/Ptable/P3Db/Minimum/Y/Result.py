...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:P3DB:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p3Db.minimum.y.result.get() \n
		These commands query the result values for the 3 dB Compression Point result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P3DB:MINimum:Y:RESult?')
