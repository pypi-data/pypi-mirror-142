...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:BBPower:CURRent[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.bbPower.current.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:BBPower:CURRent:RESult?')
