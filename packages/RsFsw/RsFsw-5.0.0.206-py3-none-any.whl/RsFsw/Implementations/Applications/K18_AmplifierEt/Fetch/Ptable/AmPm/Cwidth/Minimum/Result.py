...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:AMPM:CWIDth:MINimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.amPm.cwidth.minimum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:AMPM:CWIDth:MINimum:RESult?')
