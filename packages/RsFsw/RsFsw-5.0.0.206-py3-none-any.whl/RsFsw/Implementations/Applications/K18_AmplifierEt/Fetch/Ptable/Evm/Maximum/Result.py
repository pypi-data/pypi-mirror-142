...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:EVM:MAXimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.evm.maximum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:EVM:MAXimum:RESult?')
