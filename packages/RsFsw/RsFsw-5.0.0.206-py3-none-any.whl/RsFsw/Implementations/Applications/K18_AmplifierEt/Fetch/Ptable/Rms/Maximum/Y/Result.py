...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:RMS:MAXimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.rms.maximum.y.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:RMS:MAXimum:Y:RESult?')
