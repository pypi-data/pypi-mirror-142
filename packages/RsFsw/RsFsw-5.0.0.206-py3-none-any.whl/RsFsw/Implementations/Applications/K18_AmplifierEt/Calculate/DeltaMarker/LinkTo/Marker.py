...
class Marker:
	...
	def set(self, state: bool, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, markerDestination=repcap.MarkerDestination.Default) -> None:
		"""SCPI: CALCulate<n>:DELTamarker<ms>:LINK:TO:MARKer<mt> \n
		Snippet: driver.applications.k18AmplifierEt.calculate.deltaMarker.linkTo.marker.set(state = False, window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default, markerDestination = repcap.MarkerDestination.Default) \n
		This command links delta marker <m1> to any active normal marker <m2>. If you change the horizontal position of marker
		<m2>, delta marker <m1> changes its horizontal position to the same value. \n
		...
		self._core.io.write_with_opc(f'CALCulate{window_cmd_val}:DELTamarker{deltaMarker_cmd_val}:LINK:TO:MARKer{markerDestination_cmd_val} {param}')

	def get(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, markerDestination=repcap.MarkerDestination.Default) -> bool:
		"""SCPI: CALCulate<n>:DELTamarker<ms>:LINK:TO:MARKer<mt> \n
		Snippet: value: bool = driver.applications.k18AmplifierEt.calculate.deltaMarker.linkTo.marker.get(window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default, markerDestination = repcap.MarkerDestination.Default) \n
		This command links delta marker <m1> to any active normal marker <m2>. If you change the horizontal position of marker
		<m2>, delta marker <m1> changes its horizontal position to the same value. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param deltaMarker: optional repeated capability selector. Default value: Nr1 (settable in the interface 'DeltaMarker')
		...
		deltaMarker_cmd_val = self._cmd_group.get_repcap_cmd_value(deltaMarker, repcap.DeltaMarker)
		markerDestination_cmd_val = self._cmd_group.get_repcap_cmd_value(markerDestination, repcap.MarkerDestination)
		response = self._core.io.query_str_with_opc(f'CALCulate{window_cmd_val}:DELTamarker{deltaMarker_cmd_val}:LINK:TO:MARKer{markerDestination_cmd_val}?')
