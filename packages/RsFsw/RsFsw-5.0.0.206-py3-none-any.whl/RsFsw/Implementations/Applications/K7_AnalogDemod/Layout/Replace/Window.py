...
class Window:
	...
	def set(self, window_name: str, window_type: enums.WindowTypeK7) -> None:
		"""SCPI: LAYout:REPLace[:WINDow] \n
		Snippet: driver.applications.k7AnalogDemod.layout.replace.window.set(window_name = '1', window_type = enums.WindowTypeK7.AmSpectrum='XTIM:AM:RELative:AFSPectrum') \n
		This command replaces the window type (for example from 'Diagram' to 'Result Summary') of an already existing window in
		the active channel while keeping its position, index and window name. To add a new window, use the method RsFsw.Layout.
		Add.Window.get_ command. \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('window_name', window_name, DataType.String), ArgSingle('window_type', window_type, DataType.EnumExt, enums.WindowTypeK7))
