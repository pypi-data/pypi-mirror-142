...
class External:
	...
	def set(self, level: float, externalPort=repcap.ExternalPort.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.trigger.sequence.level.external.set(level = 1.0, externalPort = repcap.ExternalPort.Default) \n
	...
	def get(self, externalPort=repcap.ExternalPort.Default) -> float:
		"""SCPI: TRIGger[:SEQuence]:LEVel[:EXTernal<tp>] \n
		Snippet: value: float = driver.applications.k10Xlte.trigger.sequence.level.external.get(externalPort = repcap.ExternalPort.Default) \n
		This command defines the level for an external trigger. \n
			:param externalPort: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
