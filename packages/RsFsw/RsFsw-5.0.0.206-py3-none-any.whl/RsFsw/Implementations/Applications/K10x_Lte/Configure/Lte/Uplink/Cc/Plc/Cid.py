...
class Cid:
	...
	def set(self, cell_id: float, carrierComponent=repcap.CarrierComponent.Default) -> None:
		"""SCPI: CONFigure[:LTE]:UL[:CC<cc>]:PLC:CID \n
		Snippet: driver.applications.k10Xlte.configure.lte.uplink.cc.plc.cid.set(cell_id = 1.0, carrierComponent = repcap.CarrierComponent.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:UL:CC{carrierComponent_cmd_val}:PLC:CID {param}')

	def get(self, carrierComponent=repcap.CarrierComponent.Default) -> float:
		"""SCPI: CONFigure[:LTE]:UL[:CC<cc>]:PLC:CID \n
		Snippet: value: float = driver.applications.k10Xlte.configure.lte.uplink.cc.plc.cid.get(carrierComponent = repcap.CarrierComponent.Default) \n
		This command defines the cell ID. \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
			:return: cell_id: AUTO Automatically determines the cell ID. numeric value (integer only) Number of the cell ID. Range: 0 to 503"""
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		response = self._core.io.query_str(f'CONFigure:LTE:UL:CC{carrierComponent_cmd_val}:PLC:CID?')
