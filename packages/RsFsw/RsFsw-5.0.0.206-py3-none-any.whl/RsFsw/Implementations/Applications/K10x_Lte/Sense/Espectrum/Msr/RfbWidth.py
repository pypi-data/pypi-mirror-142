...
class RfbWidth:
	...
	def set(self, bandwidth: float, subBlock=repcap.SubBlock.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.sense.espectrum.msr.rfbWidth.set(bandwidth = 1.0, subBlock = repcap.SubBlock.Default) \n
	...
	def get(self, subBlock=repcap.SubBlock.Default) -> float:
		"""SCPI: [SENSe]:ESPectrum<sb>:MSR:RFBWidth \n
		Snippet: value: float = driver.applications.k10Xlte.sense.espectrum.msr.rfbWidth.get(subBlock = repcap.SubBlock.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Espectrum')
