...
class PmOffset:
	...
	def set(self, pm_offset: float, probe=repcap.Probe.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.sense.probe.setup.pmOffset.set(pm_offset = 1.0, probe = repcap.Probe.Default) \n
	...
	def get(self, probe=repcap.Probe.Default) -> float:
		"""SCPI: [SENSe]:PROBe<pb>:SETup:PMOFfset \n
		Snippet: value: float = driver.applications.k10Xlte.sense.probe.setup.pmOffset.get(probe = repcap.Probe.Default) \n
		No command help available \n
			:param probe: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Probe')
