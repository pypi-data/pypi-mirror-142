...
class DmOffset:
	...
	def set(self, dm_offset: float, probe=repcap.Probe.Default) -> None:
		...
		Snippet: driver.applications.k40PhaseNoise.sense.probe.setup.dmOffset.set(dm_offset = 1.0, probe = repcap.Probe.Default) \n
	...
	def get(self, probe=repcap.Probe.Default) -> float:
		"""SCPI: [SENSe]:PROBe<pb>:SETup:DMOFfset \n
		Snippet: value: float = driver.applications.k40PhaseNoise.sense.probe.setup.dmOffset.get(probe = repcap.Probe.Default) \n
		No command help available \n
			:param probe: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Probe')
