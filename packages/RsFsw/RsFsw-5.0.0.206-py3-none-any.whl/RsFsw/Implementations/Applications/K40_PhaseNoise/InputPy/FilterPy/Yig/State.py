...
class State:
	...
	def set(self, state: bool, inputIx=repcap.InputIx.Default) -> None:
		...
		Snippet: driver.applications.k40PhaseNoise.inputPy.filterPy.yig.state.set(state = False, inputIx = repcap.InputIx.Default) \n
	...
	def get(self, inputIx=repcap.InputIx.Default) -> bool:
		"""SCPI: INPut<ip>:FILTer:YIG[:STATe] \n
		Snippet: value: bool = driver.applications.k40PhaseNoise.inputPy.filterPy.yig.state.get(inputIx = repcap.InputIx.Default) \n
		Enables or disables the YIG filter. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
