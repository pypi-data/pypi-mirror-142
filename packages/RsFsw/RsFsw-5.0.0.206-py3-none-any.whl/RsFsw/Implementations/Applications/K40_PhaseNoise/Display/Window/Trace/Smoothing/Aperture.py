...
class Aperture:
	...
	def set(self, aperture: float, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:SMOothing:APERture \n
		Snippet: driver.applications.k40PhaseNoise.display.window.trace.smoothing.aperture.set(aperture = 1.0, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> float:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:SMOothing:APERture \n
		Snippet: value: float = driver.applications.k40PhaseNoise.display.window.trace.smoothing.aperture.get(window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
