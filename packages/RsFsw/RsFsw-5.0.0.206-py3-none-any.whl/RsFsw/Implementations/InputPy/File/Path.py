...
class Path:
	...
	def set(self, filename: str, analysis_bw: float = None, inputIx=repcap.InputIx.Default) -> None:
		"""SCPI: INPut<ip>:FILE:PATH \n
		Snippet: driver.inputPy.file.path.set(filename = '1', analysis_bw = 1.0, inputIx = repcap.InputIx.Default) \n
	...
	def get(self, inputIx=repcap.InputIx.Default) -> PathStruct:
		"""SCPI: INPut<ip>:FILE:PATH \n
		Snippet: value: PathStruct = driver.inputPy.file.path.get(inputIx = repcap.InputIx.Default) \n
		This command selects the I/Q data file to be used as input for further measurements.
			INTRO_CMD_HELP: The I/Q data file must be in one of the following supported formats: \n
			- .iq.tar
			- .iqw
