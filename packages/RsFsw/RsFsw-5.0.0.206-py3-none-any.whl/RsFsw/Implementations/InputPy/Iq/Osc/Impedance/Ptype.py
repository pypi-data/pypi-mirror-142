...
class Ptype:
	...
	def set(self, pad_type: enums.PadType, inputIx=repcap.InputIx.Default) -> None:
		"""SCPI: INPut<ip>:IQ:OSC:IMPedance:PTYPe \n
		Snippet: driver.inputPy.iq.osc.impedance.ptype.set(pad_type = enums.PadType.MLPad, inputIx = repcap.InputIx.Default) \n
		Defines the type of matching pad used for impedance conversion for analog baseband input. For RF input, use the method
		RsFsw.InputPy.Impedance.Ptype.set command. For analog baseband input without an oscilloscope, use the method RsFsw.
		InputPy.Iq.Impedance.Ptype.set command. \n
		...
		self._core.io.write(f'INPut{inputIx_cmd_val}:IQ:OSC:IMPedance:PTYPe {param}')

	# noinspection PyTypeChecker
	def get(self, inputIx=repcap.InputIx.Default) -> enums.PadType:
		"""SCPI: INPut<ip>:IQ:OSC:IMPedance:PTYPe \n
		Snippet: value: enums.PadType = driver.inputPy.iq.osc.impedance.ptype.get(inputIx = repcap.InputIx.Default) \n
		Defines the type of matching pad used for impedance conversion for analog baseband input. For RF input, use the method
		RsFsw.InputPy.Impedance.Ptype.set command. For analog baseband input without an oscilloscope, use the method RsFsw.
		InputPy.Iq.Impedance.Ptype.set command. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
		...
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		response = self._core.io.query_str(f'INPut{inputIx_cmd_val}:IQ:OSC:IMPedance:PTYPe?')
