...
class Limit:
	...
	def set(self, filename: str, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.massMemory.load.limit.set(filename = '1', window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: MMEMory:LOAD<n>:LIMit \n
		Snippet: value: str = driver.massMemory.load.limit.get(window = repcap.Window.Default) \n
		Loads the limit line from the selected file in .CSV format. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Load')
