...
class Event:
	...
	def get(self) -> str:
		...
		Snippet: value: str = driver.status.questionable.sync.event.get() \n
		This command reads out the EVENt section of the status register. The command also deletes the contents of the EVENt
		section. \n
		...
		response = self._core.io.query_str(f'STATus:QUEStionable:SYNC:EVENt?')
