...
class Enable:
	...
	def set(self, bit_definition: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.diq.enable.set(bit_definition = 1, channel_name = '1') \n
		This command controls the ENABle part of a register. The ENABle part allows true conditions in the EVENt part of the
		status register to be reported in the summary bit. If a bit is 1 in the enable register and its associated event bit
		...
		self._core.io.write(f'STATus:QUEStionable:DIQ:ENABle {param}'.rstrip())
	...
	def get(self) -> EnableStruct:
		"""SCPI: STATus:QUEStionable:DIQ:ENABle \n
		Snippet: value: EnableStruct = driver.status.questionable.diq.enable.get() \n
		This command controls the ENABle part of a register. The ENABle part allows true conditions in the EVENt part of the
		status register to be reported in the summary bit. If a bit is 1 in the enable register and its associated event bit
		transitions to true, a positive transition will occur in the summary bit reported to the next higher level. \n
			:return: structure: for return value, see the help for EnableStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:DIQ:ENABle?', self.__class__.EnableStruct())
