...
class Resolution:
	...
	def ratio(self):
		...
		if not hasattr(self, '_ratio'):
			...
			self._ratio = Ratio(self._core, self._cmd_group)
		return self._ratio
	...
	def set(self, bandwidth: float) -> None:
		...
		Snippet: driver.sense.bandwidth.resolution.set(bandwidth = 1.0) \n
		This command defines the resolution bandwidth and decouples the resolution bandwidth from the span. In the Real-Time
		application, the resolution bandwidth is always coupled to the span. For statistics measurements, this command defines
		the demodulation bandwidth. \n
			:param bandwidth: refer to data sheet Unit: Hz
		"""
	...
	def get(self) -> float:
		"""SCPI: [SENSe]:BWIDth[:RESolution] \n
		Snippet: value: float = driver.sense.bandwidth.resolution.get() \n
		This command defines the resolution bandwidth and decouples the resolution bandwidth from the span. In the Real-Time
		application, the resolution bandwidth is always coupled to the span. For statistics measurements, this command defines
		the demodulation bandwidth. \n
			:return: bandwidth: refer to data sheet Unit: Hz"""
	...
	def clone(self) -> 'Resolution':
		...
		new_group = Resolution(self._core, self._cmd_group.parent)
