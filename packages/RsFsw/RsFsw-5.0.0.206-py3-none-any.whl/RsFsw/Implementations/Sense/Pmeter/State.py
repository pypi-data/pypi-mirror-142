...
class State:
	...
	def set(self, state: bool, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.sense.pmeter.state.set(state = False, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: [SENSe]:PMETer<p>[:STATe] \n
		Snippet: value: bool = driver.sense.pmeter.state.get(powerMeter = repcap.PowerMeter.Default) \n
		This command turns a power sensor on and off. \n
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
