...
class State:
	...
	def set(self, state: bool, inputIx=repcap.InputIx.Default, fileList=repcap.FileList.Default) -> None:
		...
		Snippet: driver.sense.correction.fresponse.inputPy.user.flist.phase.state.set(state = False, inputIx = repcap.InputIx.Default, fileList = repcap.FileList.Default) \n
		...
		self._core.io.write(f'SENSe:CORRection:FRESponse:INPut{inputIx_cmd_val}:USER:FLISt{fileList_cmd_val}:PHASe:STATe {param}')

	def get(self, inputIx=repcap.InputIx.Default, fileList=repcap.FileList.Default) -> bool:
		"""SCPI: [SENSe]:CORRection:FRESponse:INPut<ip>:USER:FLISt<fli>:PHASe[:STATe] \n
		Snippet: value: bool = driver.sense.correction.fresponse.inputPy.user.flist.phase.state.get(inputIx = repcap.InputIx.Default, fileList = repcap.FileList.Default) \n
		Activates or deactivates the use of the correction data in the selected file for magnitude results. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
			:param fileList: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Flist')
		...
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		fileList_cmd_val = self._cmd_group.get_repcap_cmd_value(fileList, repcap.FileList)
		response = self._core.io.query_str(f'SENSe:CORRection:FRESponse:INPut{inputIx_cmd_val}:USER:FLISt{fileList_cmd_val}:PHASe:STATe?')
