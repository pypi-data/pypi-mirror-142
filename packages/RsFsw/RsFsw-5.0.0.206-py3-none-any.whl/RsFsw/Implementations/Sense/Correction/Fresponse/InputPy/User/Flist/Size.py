...
class Size:
	...
	def get(self, inputIx=repcap.InputIx.Default, fileList=repcap.FileList.Default) -> int:
		"""SCPI: [SENSe]:CORRection:FRESponse:INPut<ip>:USER:FLISt<fli>:SIZE \n
		Snippet: value: int = driver.sense.correction.fresponse.inputPy.user.flist.size.get(inputIx = repcap.InputIx.Default, fileList = repcap.FileList.Default) \n
		...
		response = self._core.io.query_str(f'SENSe:CORRection:FRESponse:INPut{inputIx_cmd_val}:USER:FLISt{fileList_cmd_val}:SIZE?')
