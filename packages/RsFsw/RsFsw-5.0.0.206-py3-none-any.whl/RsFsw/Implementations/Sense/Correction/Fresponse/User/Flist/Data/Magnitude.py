...
class Magnitude:
	...
	def get(self, fileList=repcap.FileList.Default) -> float:
		...
		Snippet: value: float = driver.sense.correction.fresponse.user.flist.data.magnitude.get(fileList = repcap.FileList.Default) \n
		...
		response = self._core.io.query_str(f'SENSe:CORRection:FRESponse:USER:FLISt{fileList_cmd_val}:DATA:MAGNitude?')
