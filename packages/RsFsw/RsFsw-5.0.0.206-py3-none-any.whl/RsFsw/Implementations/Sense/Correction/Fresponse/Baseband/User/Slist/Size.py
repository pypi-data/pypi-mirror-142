...
class Size:
	...
	def get(self, touchStone=repcap.TouchStone.Default) -> int:
		...
		Snippet: value: int = driver.sense.correction.fresponse.baseband.user.slist.size.get(touchStone = repcap.TouchStone.Default) \n
		...
		response = self._core.io.query_str(f'SENSe:CORRection:FRESponse:BASeband:USER:SLISt{touchStone_cmd_val}:SIZE?')
