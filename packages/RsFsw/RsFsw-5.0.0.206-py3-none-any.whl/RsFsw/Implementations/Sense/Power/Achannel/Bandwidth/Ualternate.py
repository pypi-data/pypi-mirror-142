...
class Ualternate:
	...
	def set(self, bandwidth: float, upperAltChannel=repcap.UpperAltChannel.Default) -> None:
		...
		Snippet: driver.sense.power.achannel.bandwidth.ualternate.set(bandwidth = 1.0, upperAltChannel = repcap.UpperAltChannel.Default) \n
		This command defines the channel bandwidth of the upper alternate channels in asymmetrical configurations. To configure
		the bandwidth for the lower alternate channel, use the [SENSe:]POWer:ACHannel:BANDwidth:ALTernate<ch> command.
	...
	def get(self, upperAltChannel=repcap.UpperAltChannel.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:BWIDth:UALTernate<ch> \n
		Snippet: value: float = driver.sense.power.achannel.bandwidth.ualternate.get(upperAltChannel = repcap.UpperAltChannel.Default) \n
		This command defines the channel bandwidth of the upper alternate channels in asymmetrical configurations. To configure
		the bandwidth for the lower alternate channel, use the [SENSe:]POWer:ACHannel:BANDwidth:ALTernate<ch> command.
		Steep-edged channel filters are available for fast ACLR measurements. \n
			:param upperAltChannel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ualternate')
