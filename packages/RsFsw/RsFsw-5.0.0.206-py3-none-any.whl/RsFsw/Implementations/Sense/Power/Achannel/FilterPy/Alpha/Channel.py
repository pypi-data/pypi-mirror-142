...
class Channel:
	...
	def set(self, alpha: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.sense.power.achannel.filterPy.alpha.channel.set(alpha = 1.0, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:FILTer:ALPHa:CHANnel<ch> \n
		Snippet: value: float = driver.sense.power.achannel.filterPy.alpha.channel.get(channel = repcap.Channel.Default) \n
		This command defines the roll-off factor for the transmission channel weighting filter. \n
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Channel')
