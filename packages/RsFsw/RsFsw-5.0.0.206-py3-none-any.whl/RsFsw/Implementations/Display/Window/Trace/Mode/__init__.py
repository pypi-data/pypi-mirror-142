...
class Mode:
	...
	def set(self, mode: enums.TraceModeC, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		the end of the measurement is possible only in single sweep mode. In the R&S FSW AM/FM/PM Modulation Analysis application
		when you configure the traces for a window with a specific evaluation (e.g. AM time domain) , the traces in all windows
		with the same evaluation are configured identically. For more information see 'Analyzing Several Traces - Trace Mode'. \n
