...
class State:
	...
	def set(self, state: bool, status=repcap.Status.Default) -> None:
		"""SCPI: HCOPy:TDSTamp:STATe<1|2> \n
		Snippet: driver.hardCopy.tdDtamp.state.set(state = False, status = repcap.Status.Default) \n
		...
		self._core.io.write(f'HCOPy:TDSTamp:STATe{status_cmd_val} {param}')

	def get(self, status=repcap.Status.Default) -> bool:
		"""SCPI: HCOPy:TDSTamp:STATe<1|2> \n
		Snippet: value: bool = driver.hardCopy.tdDtamp.state.get(status = repcap.Status.Default) \n
		This command includes or excludes the time and date in the printout. \n
			:param status: optional repeated capability selector. Default value: Nr1 (settable in the interface 'State')
			:return: state: 1 | 0 | ON | OFF 1 | ON The time and date are printed. 0 | OFF The time and date are not printed."""
		status_cmd_val = self._cmd_group.get_repcap_cmd_value(status, repcap.Status)
		response = self._core.io.query_str(f'HCOPy:TDSTamp:STATe{status_cmd_val}?')
