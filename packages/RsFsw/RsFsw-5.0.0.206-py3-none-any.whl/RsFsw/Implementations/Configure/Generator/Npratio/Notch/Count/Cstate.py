...
class Cstate:
	...
	def get(self, notch=repcap.Notch.Default) -> enums.ControlState:
		"""SCPI: CONFigure:GENerator:NPRatio:NOTCh<notch>:COUNt:CSTate \n
		Snippet: value: enums.ControlState = driver.configure.generator.npratio.notch.count.cstate.get(notch = repcap.Notch.Default) \n
		Queries the state of the number of notches. \n
		...
		response = self._core.io.query_str(f'CONFigure:GENerator:NPRatio:NOTCh{notch_cmd_val}:COUNt:CSTate?')
