...
class Cstate:
	...
	def get(self, notch=repcap.Notch.Default) -> enums.ControlState:
		"""SCPI: CONFigure:GENerator:NPRatio:NOTCh<notch>:FREQuency:OFFSet:CSTate \n
		Snippet: value: enums.ControlState = driver.configure.generator.npratio.notch.frequency.offset.cstate.get(notch = repcap.Notch.Default) \n
		...
		response = self._core.io.query_str(f'CONFigure:GENerator:NPRatio:NOTCh{notch_cmd_val}:FREQuency:OFFSet:CSTate?')
