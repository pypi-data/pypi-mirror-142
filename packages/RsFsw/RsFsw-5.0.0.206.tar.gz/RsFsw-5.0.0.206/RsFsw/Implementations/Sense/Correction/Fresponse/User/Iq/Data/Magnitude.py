...
class Magnitude:
	...
	def get(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.sense.correction.fresponse.user.iq.data.magnitude.get() \n
		Queries the trace values for the combined user correction files (.snp+.fres) in IQ mode. \n
			:return: trace_data: No help available"""
		response = self._core.io.query_bin_or_ascii_float_list_with_opc(f'FORMAT REAL,32;SENSe:CORRection:FRESponse:USER:IQ:DATA:MAGNitude?')
		return response
