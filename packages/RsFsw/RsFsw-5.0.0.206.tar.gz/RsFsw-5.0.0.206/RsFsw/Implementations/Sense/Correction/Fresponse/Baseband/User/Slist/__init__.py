...
class Slist:
	...
	def clear(self, touchStone=repcap.TouchStone.Default) -> None:
		...
		Snippet: driver.sense.correction.fresponse.baseband.user.slist.clear(touchStone = repcap.TouchStone.Default) \n
		...
		self._core.io.write(f'SENSe:CORRection:FRESponse:BASeband:USER:SLISt{touchStone_cmd_val}:CLEar')

	def clear_with_opc(self, touchStone=repcap.TouchStone.Default, opc_timeout_ms: int = -1) -> None:
		touchStone_cmd_val = self._cmd_group.get_repcap_cmd_value(touchStone, repcap.TouchStone)
		"""SCPI: [SENSe]:CORRection:FRESponse:BASeband:USER:SLISt<sli>:CLEar \n
		Snippet: driver.sense.correction.fresponse.baseband.user.slist.clear_with_opc(touchStone = repcap.TouchStone.Default) \n
		Removes all touchstone files from the current configuration for the selected or all input types. \n
		...
		self._core.io.write_with_opc(f'SENSe:CORRection:FRESponse:BASeband:USER:SLISt{touchStone_cmd_val}:CLEar', opc_timeout_ms)
	...
	def move(self, direction: enums.UpDownDirection, touchStone=repcap.TouchStone.Default) -> None:
		...
		Snippet: driver.sense.correction.fresponse.baseband.user.slist.move(direction = enums.UpDownDirection.DOWN, touchStone = repcap.TouchStone.Default) \n
		...
		touchStone_cmd_val = self._cmd_group.get_repcap_cmd_value(touchStone, repcap.TouchStone)
		self._core.io.write(f'SENSe:CORRection:FRESponse:BASeband:USER:SLISt{touchStone_cmd_val}:MOVE {param}')
