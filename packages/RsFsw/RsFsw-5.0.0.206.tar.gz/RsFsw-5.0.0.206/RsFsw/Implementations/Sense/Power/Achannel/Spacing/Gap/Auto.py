...
class Auto:
	...
	def set(self, spacing: float, gapChannel=repcap.GapChannel.Default) -> None:
		...
		This command defines the distance from sub block to the specified gap channel. In 'Auto' gap mode, channels in the upper
		gap are identical to those in the lower gap. Thus, only 2 gap channels are configured. The spacing for gap channels is
		defined in relation to the outer edges of the surrounding sub blocks, i.e. Spacing = [CF of the gap channel] - [left sub
		block center] + ([RF bandwidth of left sub block] /2) (See also Figure 'MSR signal structure' and Figure 'Sub block
		definition'.) \n
