...
class Manual:
	...
	def set(self, channel_number: float) -> None:
		...
		Snippet: driver.sense.power.achannel.reference.txChannel.manual.set(channel_number = 1.0) \n
		This command defines a reference channel for relative ACLR measurements. You need at least one channel for the command to
		work. Note that this command is not available for MSR ACLR measurements (see method RsFsw.Calculate.Marker.Function.Power.
		preset) . \n
	...
	def get(self) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:REFerence:TXCHannel:MANual \n
		Snippet: value: float = driver.sense.power.achannel.reference.txChannel.manual.get() \n
		This command defines a reference channel for relative ACLR measurements. You need at least one channel for the command to
		work. Note that this command is not available for MSR ACLR measurements (see method RsFsw.Calculate.Marker.Function.Power.
		preset) . \n
