...
class Channel:
	...
	def set(self, state: bool, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.sense.power.achannel.filterPy.state.channel.set(state = False, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> bool:
		"""SCPI: [SENSe]:POWer:ACHannel:FILTer[:STATe]:CHANnel<ch> \n
		Snippet: value: bool = driver.sense.power.achannel.filterPy.state.channel.get(channel = repcap.Channel.Default) \n
		This command turns the weighting filter for a transmission channel on and off. \n
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Channel')
