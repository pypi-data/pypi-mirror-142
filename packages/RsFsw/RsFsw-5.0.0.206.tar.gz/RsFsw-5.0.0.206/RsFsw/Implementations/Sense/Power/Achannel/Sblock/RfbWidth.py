...
class RfbWidth:
	...
	def set(self, bandwidth: float, subBlock=repcap.SubBlock.Default) -> None:
		...
		Snippet: driver.sense.power.achannel.sblock.rfbWidth.set(bandwidth = 1.0, subBlock = repcap.SubBlock.Default) \n
		This command defines the bandwidth of the individual MSR sub block. Note that sub block ranges also affect the position
		of the adjacent gap channels (CACLR) . This command is for MSR signals only (see method RsFsw.Calculate.Marker.Function.
		Power.preset) . For details on MSR signals see 'Measurement on multi-standard radio (MSR) signals'. \n
	...
	def get(self, subBlock=repcap.SubBlock.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:SBLock<sb>:RFBWidth \n
		Snippet: value: float = driver.sense.power.achannel.sblock.rfbWidth.get(subBlock = repcap.SubBlock.Default) \n
		This command defines the bandwidth of the individual MSR sub block. Note that sub block ranges also affect the position
		of the adjacent gap channels (CACLR) . This command is for MSR signals only (see method RsFsw.Calculate.Marker.Function.
		Power.preset) . For details on MSR signals see 'Measurement on multi-standard radio (MSR) signals'. \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sblock')
