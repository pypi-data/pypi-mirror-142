...
class Channel:
	...
	def set(self, name: str, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.sense.power.achannel.name.channel.set(name = '1', channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> str:
		"""SCPI: [SENSe]:POWer:ACHannel:NAME:CHANnel<ch> \n
		Snippet: value: str = driver.sense.power.achannel.name.channel.get(channel = repcap.Channel.Default) \n
		This command defines a name for a transmission channel. \n
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Channel')
