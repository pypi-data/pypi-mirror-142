...
class State:
	...
	def set(self, state: bool, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.sense.pmeter.update.state.set(state = False, powerMeter = repcap.PowerMeter.Default) \n
		This command turns continuous update of power sensor measurements on and off. If on, the results are update even if a
		single sweep is complete. \n
	...
	def get(self, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: [SENSe]:PMETer<p>:UPDate[:STATe] \n
		Snippet: value: bool = driver.sense.pmeter.update.state.get(powerMeter = repcap.PowerMeter.Default) \n
		This command turns continuous update of power sensor measurements on and off. If on, the results are update even if a
		single sweep is complete. \n
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
