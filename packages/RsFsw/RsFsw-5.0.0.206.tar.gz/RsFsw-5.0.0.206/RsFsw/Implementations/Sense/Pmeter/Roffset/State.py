...
class State:
	...
	def set(self, state: bool, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.sense.pmeter.roffset.state.set(state = False, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: [SENSe]:PMETer<p>:ROFFset[:STATe] \n
		Snippet: value: bool = driver.sense.pmeter.roffset.state.get(powerMeter = repcap.PowerMeter.Default) \n
		This command includes or excludes the reference level offset of the analyzer for power sensor measurements. \n
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
