...
class Result:
	...
	def get(self, trace_mode: enums.TraceModeB) -> float:
		"""SCPI: [SENSe]:ADEMod:ACV[:TDOMain]:RESult \n
		Snippet: value: float = driver.sense.ademod.acv.tdomain.result.get(trace_mode = enums.TraceModeB.AVERage) \n
		...
		response = self._core.io.query_str(f'SENSe:ADEMod:ACV:TDOMain:RESult? {param}')
