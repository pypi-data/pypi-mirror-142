...
class Enable:
	...
	def set(self, bit_definition: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.sync.enable.set(bit_definition = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:SYNC:ENABle {param}'.rstrip())
	...
	def get(self) -> EnableStruct:
		...
		return self._core.io.query_struct(f'STATus:QUEStionable:SYNC:ENABle?', self.__class__.EnableStruct())
