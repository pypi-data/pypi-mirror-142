...
class Ntransition:
	...
	def set(self, summary_bit: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.pnoise.ntransition.set(summary_bit = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:PNOise:NTRansition {param}'.rstrip())
	...
	def get(self) -> NtransitionStruct:
		"""SCPI: STATus:QUEStionable:PNOise:NTRansition \n
		Snippet: value: NtransitionStruct = driver.status.questionable.pnoise.ntransition.get() \n
		No command help available \n
			:return: structure: for return value, see the help for NtransitionStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:PNOise:NTRansition?', self.__class__.NtransitionStruct())
