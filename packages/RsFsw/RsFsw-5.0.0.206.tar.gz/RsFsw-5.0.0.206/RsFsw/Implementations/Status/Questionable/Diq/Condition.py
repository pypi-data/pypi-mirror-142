...
class Condition:
	...
	def get(self) -> str:
		...
		Snippet: value: str = driver.status.questionable.diq.condition.get() \n
		This command reads out the CONDition section of the method RsFsw.Status.Questionable.Diq.Condition.get_ status register.
		...
		response = self._core.io.query_str(f'STATus:QUEStionable:DIQ:CONDition?')
