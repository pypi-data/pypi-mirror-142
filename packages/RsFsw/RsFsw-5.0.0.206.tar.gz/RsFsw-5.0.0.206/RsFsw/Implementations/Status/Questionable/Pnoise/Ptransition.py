...
class Ptransition:
	...
	def set(self, summary_bit: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.pnoise.ptransition.set(summary_bit = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:PNOise:PTRansition {param}'.rstrip())
	...
	def get(self) -> PtransitionStruct:
		"""SCPI: STATus:QUEStionable:PNOise:PTRansition \n
		Snippet: value: PtransitionStruct = driver.status.questionable.pnoise.ptransition.get() \n
		No command help available \n
			:return: structure: for return value, see the help for PtransitionStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:PNOise:PTRansition?', self.__class__.PtransitionStruct())
