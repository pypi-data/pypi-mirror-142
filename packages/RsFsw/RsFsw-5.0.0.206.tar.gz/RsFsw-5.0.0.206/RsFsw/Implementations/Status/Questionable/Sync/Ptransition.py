...
class Ptransition:
	...
	def set(self, bit_definition: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.sync.ptransition.set(bit_definition = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:SYNC:PTRansition {param}'.rstrip())
	...
	def get(self) -> PtransitionStruct:
		"""SCPI: STATus:QUEStionable:SYNC:PTRansition \n
		Snippet: value: PtransitionStruct = driver.status.questionable.sync.ptransition.get() \n
		These commands control the Positive TRansition part of a register. Setting a bit causes a 0 to 1 transition in the
		corresponding bit of the associated register. The transition also writes a 1 into the associated bit of the corresponding
		EVENt register. \n
			:return: structure: for return value, see the help for PtransitionStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:SYNC:PTRansition?', self.__class__.PtransitionStruct())
