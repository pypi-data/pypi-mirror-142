...
class Enable:
	...
	def set(self, summary_bit: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.pnoise.enable.set(summary_bit = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:PNOise:ENABle {param}'.rstrip())
	...
	def get(self) -> EnableStruct:
		"""SCPI: STATus:QUEStionable:PNOise:ENABle \n
		Snippet: value: EnableStruct = driver.status.questionable.pnoise.enable.get() \n
		No command help available \n
			:return: structure: for return value, see the help for EnableStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:PNOise:ENABle?', self.__class__.EnableStruct())
