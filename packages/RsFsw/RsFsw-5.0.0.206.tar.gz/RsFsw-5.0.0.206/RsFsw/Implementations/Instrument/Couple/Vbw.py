...
class Vbw:
	...
	def set(self, state: enums.Synchronization) -> None:
		...
		Snippet: driver.instrument.couple.vbw.set(state = enums.Synchronization.ALL) \n
		...
		self._core.io.write(f'INSTrument:COUPle:VBW {param}')

	# noinspection PyTypeChecker
	def get(self) -> enums.Synchronization:
		"""SCPI: INSTrument:COUPle:VBW \n
		Snippet: value: enums.Synchronization = driver.instrument.couple.vbw.get() \n
		This command turns synchronization of the video bandwidth between measurement channels on and off. \n
			:return: state: ALL | NONE ALL Turns on synchronization. NONE Turns off synchronization."""
		response = self._core.io.query_str(f'INSTrument:COUPle:VBW?')
