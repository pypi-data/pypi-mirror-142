...
class Right:
	...
	def set(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, opc_timeout_ms: int = -1) -> None:
		...
		Snippet: driver.calculate.deltaMarker.minimum.right.set(window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default) \n
		This command moves a delta marker to the next higher minimum value. The search includes only measurement values to the
		right of the current marker position. In the spectrogram, the command moves a marker horizontally to the minimum level in
		...
		self._core.io.write_with_opc(f'CALCulate{window_cmd_val}:DELTamarker{deltaMarker_cmd_val}:MINimum:RIGHt', opc_timeout_ms)
