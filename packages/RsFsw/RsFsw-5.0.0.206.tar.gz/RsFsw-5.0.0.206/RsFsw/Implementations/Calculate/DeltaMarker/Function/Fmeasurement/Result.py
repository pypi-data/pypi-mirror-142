...
class Result:
	...
	def get(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default) -> float:
		...
		Snippet: value: float = driver.calculate.deltaMarker.function.fmeasurement.result.get(window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default) \n
		...
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:DELTamarker{deltaMarker_cmd_val}:FUNCtion:FMEasurement:RESult?')
