...
class Next:
	...
	def set(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, opc_timeout_ms: int = -1) -> None:
		...
		Snippet: driver.calculate.deltaMarker.minimum.next.set(window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default) \n
		This command moves a marker to the next higher minimum value. In the spectrogram, the command moves a marker horizontally
		to the minimum level in the currently selected frame. The vertical marker position remains the same. \n
