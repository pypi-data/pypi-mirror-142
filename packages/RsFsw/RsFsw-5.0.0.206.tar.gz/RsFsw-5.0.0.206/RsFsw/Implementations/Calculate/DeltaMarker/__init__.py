...
class DeltaMarker:
	"""DeltaMarker commands group definition. 51 total commands, 14 Subgroups, 0 group commands
	...
	@property
	def mref(self):
		"""mref commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_mref'):
			from .Mref import Mref
			self._mref = Mref(self._core, self._cmd_group)
		return self._mref
	...
	def mreference(self):
		...
		if not hasattr(self, '_mreference'):
			from .Mreference import Mreference
			self._mreference = Mreference(self._core, self._cmd_group)
		return self._mreference

	@property
	def z(self):
		"""z commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_z'):
			from .Z import Z
			self._z = Z(self._core, self._cmd_group)
		return self._z
	...
	def spectrogram(self):
		"""spectrogram commands group. 4 Sub-classes, 0 commands."""
	...
	@property
	def x(self):
		"""x commands group. 1 Sub-classes, 1 commands."""
	...
	def clone(self) -> 'DeltaMarker':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = DeltaMarker(self._core, self._cmd_group.parent)
