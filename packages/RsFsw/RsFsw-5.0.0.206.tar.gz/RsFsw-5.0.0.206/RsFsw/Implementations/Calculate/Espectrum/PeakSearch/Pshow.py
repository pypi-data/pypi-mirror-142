...
class Pshow:
	...
	def set(self, state: bool, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.calculate.espectrum.peakSearch.pshow.set(state = False, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> bool:
		"""SCPI: CALCulate<n>:ESPectrum:PSEarch:PSHow \n
		Snippet: value: bool = driver.calculate.espectrum.peakSearch.pshow.get(window = repcap.Window.Default) \n
		This command turns the peak labels in the diagram on and off. Peak labels are blue squares. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
