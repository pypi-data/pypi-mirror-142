...
class LoExclude:
	...
	def set(self, state: bool, window=repcap.Window.Default, marker=repcap.Marker.Default) -> None:
		...
		Snippet: driver.calculate.marker.loExclude.set(state = False, window = repcap.Window.Default, marker = repcap.Marker.Default) \n
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> bool:
		"""SCPI: CALCulate<n>:MARKer<m>:LOEXclude \n
		Snippet: value: bool = driver.calculate.marker.loExclude.get(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command turns the suppression of the local oscillator during automatic marker positioning on and off (for all
		markers in all windows) . \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param marker: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Marker')
