...
class Result:
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> float:
		...
		This command queries the average standard deviation of the time domain power. The query is only possible if averaging has
		been activated previously using method RsFsw.Calculate.Marker.Function.Summary.Average.set. To get a valid result, you
		...
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:MARKer{marker_cmd_val}:FUNCtion:SUMMary:SDEViation:AVERage:RESult?')
