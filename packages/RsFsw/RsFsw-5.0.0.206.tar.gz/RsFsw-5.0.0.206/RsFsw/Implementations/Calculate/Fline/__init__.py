...
class Fline:
	...
	def set(self, frequency: float, window=repcap.Window.Default, freqLine=repcap.FreqLine.Default) -> None:
		"""SCPI: CALCulate<n>:FLINe<dl> \n
		Snippet: driver.calculate.fline.set(frequency = 1.0, window = repcap.Window.Default, freqLine = repcap.FreqLine.Default) \n
		This command defines the position of a frequency line. \n
			:param frequency: Note that you can not set a frequency line to a position that is outside the current span. Range: 0 Hz to Fmax, Unit: HZ
	...
	def get(self, window=repcap.Window.Default, freqLine=repcap.FreqLine.Default) -> float:
		"""SCPI: CALCulate<n>:FLINe<dl> \n
		Snippet: value: float = driver.calculate.fline.get(window = repcap.Window.Default, freqLine = repcap.FreqLine.Default) \n
		This command defines the position of a frequency line. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param freqLine: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fline')
			:return: frequency: Note that you can not set a frequency line to a position that is outside the current span. Range: 0 Hz to Fmax, Unit: HZ"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		freqLine_cmd_val = self._cmd_group.get_repcap_cmd_value(freqLine, repcap.FreqLine)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:FLINe{freqLine_cmd_val}?')
		return Conversions.str_to_float(response)

	def clone(self) -> 'Fline':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Fline(self._core, self._cmd_group.parent)
