...
class Ssize:
	...
	def set(self, stepsize: enums.Stepsize, window=repcap.Window.Default, marker=repcap.Marker.Default) -> None:
		...
		Snippet: driver.applications.k91Wlan.calculate.marker.x.ssize.set(stepsize = enums.Stepsize.POINts, window = repcap.Window.Default, marker = repcap.Marker.Default) \n
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> enums.Stepsize:
		"""SCPI: CALCulate<n>:MARKer<m>:X:SSIZe \n
		Snippet: value: enums.Stepsize = driver.applications.k91Wlan.calculate.marker.x.ssize.get(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param marker: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Marker')
