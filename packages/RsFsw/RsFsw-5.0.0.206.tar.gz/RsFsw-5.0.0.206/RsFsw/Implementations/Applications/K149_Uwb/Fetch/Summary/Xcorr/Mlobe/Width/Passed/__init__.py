...
class Passed:
	"""Passed commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:SUMMary:XCORr:MLOBe:WIDTh:PASSed \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.summary.xcorr.mlobe.width.passed.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:XCORr:MLOBe:WIDTh:PASSed?')
		return trim_str_response(response)

	def clone(self) -> 'Passed':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Passed(self._core, self._cmd_group.parent)
