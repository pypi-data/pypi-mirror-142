...
class Width:
	"""Width commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:XCORr:MLOBe:MINimum:WIDTh \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.xcorr.mlobe.minimum.width.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:XCORr:MLOBe:MINimum:WIDTh?')
		return Conversions.str_to_float(response)

	def clone(self) -> 'Width':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Width(self._core, self._cmd_group.parent)
