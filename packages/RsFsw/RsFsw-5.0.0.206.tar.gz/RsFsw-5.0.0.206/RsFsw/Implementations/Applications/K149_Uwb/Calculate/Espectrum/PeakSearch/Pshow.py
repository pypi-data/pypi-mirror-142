...
class Pshow:
	...
	def set(self, state: bool, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k149Uwb.calculate.espectrum.peakSearch.pshow.set(state = False, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> bool:
		"""SCPI: CALCulate<n>:ESPectrum:PSEarch:PSHow \n
		Snippet: value: bool = driver.applications.k149Uwb.calculate.espectrum.peakSearch.pshow.get(window = repcap.Window.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
