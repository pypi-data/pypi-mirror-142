...
class Minimum:
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:SUMMary:PULSe:RISE:TIME:MINimum \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.summary.pulse.rise.time.minimum.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:PULSe:RISE:TIME:MINimum?')
