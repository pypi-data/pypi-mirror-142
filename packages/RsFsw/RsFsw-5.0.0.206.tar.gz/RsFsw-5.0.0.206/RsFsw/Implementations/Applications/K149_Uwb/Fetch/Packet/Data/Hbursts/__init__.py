...
class Hbursts:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("hbursts", core, parent)
	...
	def get(self, window=repcap.Window.Default) -> str:
		...
		Snippet: value: str = driver.applications.k149Uwb.fetch.packet.data.hbursts.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:HBURsts?')
		return trim_str_response(response)

	def clone(self) -> 'Hbursts':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Hbursts(self._core, self._cmd_group.parent)
