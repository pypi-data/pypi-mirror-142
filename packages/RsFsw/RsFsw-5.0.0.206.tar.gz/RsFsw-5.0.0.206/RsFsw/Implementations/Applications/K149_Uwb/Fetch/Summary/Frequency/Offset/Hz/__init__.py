...
class Hz:
	"""Hz commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("hz", core, parent)
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:FREQuency:OFFSet:HZ \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.frequency.offset.hz.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:FREQuency:OFFSet:HZ?')
	...
	def clone(self) -> 'Hz':
		...
		new_group = Hz(self._core, self._cmd_group.parent)
