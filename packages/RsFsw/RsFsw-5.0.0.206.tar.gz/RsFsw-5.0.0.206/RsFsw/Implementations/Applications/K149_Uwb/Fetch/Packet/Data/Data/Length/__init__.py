...
class Length:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:PACKet:DATA:DATA:LENGth \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.packet.data.data.length.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:DATA:LENGth?')
