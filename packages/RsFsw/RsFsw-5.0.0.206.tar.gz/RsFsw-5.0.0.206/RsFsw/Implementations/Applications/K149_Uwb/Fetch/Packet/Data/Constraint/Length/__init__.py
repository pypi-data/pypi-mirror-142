...
class Length:
	...
	def get(self, window=repcap.Window.Default) -> float:
		...
		Snippet: value: float = driver.applications.k149Uwb.fetch.packet.data.constraint.length.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:CONStraint:LENGth?')
