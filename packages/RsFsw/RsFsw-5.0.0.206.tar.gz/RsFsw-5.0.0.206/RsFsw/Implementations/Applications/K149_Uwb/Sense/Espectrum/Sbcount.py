...
class Sbcount:
	...
	def set(self, count: float, subBlock=repcap.SubBlock.Default) -> None:
		...
		Snippet: driver.applications.k149Uwb.sense.espectrum.sbcount.set(count = 1.0, subBlock = repcap.SubBlock.Default) \n
	...
	def get(self, subBlock=repcap.SubBlock.Default) -> float:
		"""SCPI: [SENSe]:ESPectrum<sb>:SBCount \n
		Snippet: value: float = driver.applications.k149Uwb.sense.espectrum.sbcount.get(subBlock = repcap.SubBlock.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Espectrum')
