...
class State:
	...
	def set(self, state: bool, inputIx=repcap.InputIx.Default) -> None:
		...
		Snippet: driver.applications.k70Vsa.inputPy.eatt.state.set(state = False, inputIx = repcap.InputIx.Default) \n
	...
	def get(self, inputIx=repcap.InputIx.Default) -> bool:
		"""SCPI: INPut<ip>:EATT:STATe \n
		Snippet: value: bool = driver.applications.k70Vsa.inputPy.eatt.state.get(inputIx = repcap.InputIx.Default) \n
		This command turns the electronic attenuator on and off. This command requires the electronic attenuation hardware option.
		It is not available if the optional Digital Baseband Interface is active. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
