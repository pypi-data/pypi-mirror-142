...
class Result:
	...
	def get(self, result: enums.AcpResult = None, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default) -> str:
		...
		Snippet: value: str = driver.applications.k10Xlte.calculate.limit.acPower.achannel.result.get(result = enums.AcpResult.ABSolute, window = repcap.Window.Default, limitIx = repcap.LimitIx.Default) \n
		...
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:LIMit{limitIx_cmd_val}:ACPower:ACHannel:RESult? {param}'.rstrip())
