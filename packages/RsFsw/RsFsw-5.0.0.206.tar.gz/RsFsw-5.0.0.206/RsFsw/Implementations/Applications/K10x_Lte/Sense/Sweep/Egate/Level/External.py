...
class External:
	...
	def set(self, level: float, instrument=repcap.Instrument.Default, externalPort=repcap.ExternalPort.Default) -> None:
		"""SCPI: [SENSe]:SWEep:EGATe:LEVel<ant>[:EXTernal<tp>] \n
		Snippet: driver.applications.k10Xlte.sense.sweep.egate.level.external.set(level = 1.0, instrument = repcap.Instrument.Default, externalPort = repcap.ExternalPort.Default) \n
		...
		self._core.io.write(f'SENSe:SWEep:EGATe:LEVel{instrument_cmd_val}:EXTernal{externalPort_cmd_val} {param}')

	def get(self, instrument=repcap.Instrument.Default, externalPort=repcap.ExternalPort.Default) -> float:
		"""SCPI: [SENSe]:SWEep:EGATe:LEVel<ant>[:EXTernal<tp>] \n
		Snippet: value: float = driver.applications.k10Xlte.sense.sweep.egate.level.external.get(instrument = repcap.Instrument.Default, externalPort = repcap.ExternalPort.Default) \n
		No command help available \n
			:param instrument: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Level')
			:param externalPort: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
		...
		instrument_cmd_val = self._cmd_group.get_repcap_cmd_value(instrument, repcap.Instrument)
		externalPort_cmd_val = self._cmd_group.get_repcap_cmd_value(externalPort, repcap.ExternalPort)
		response = self._core.io.query_str(f'SENSe:SWEep:EGATe:LEVel{instrument_cmd_val}:EXTernal{externalPort_cmd_val}?')
