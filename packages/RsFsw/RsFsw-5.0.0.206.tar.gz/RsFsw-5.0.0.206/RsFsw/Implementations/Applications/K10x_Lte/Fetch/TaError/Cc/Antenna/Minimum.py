...
class Minimum:
	...
	def get(self, carrierComponent=repcap.CarrierComponent.Default, antenna=repcap.Antenna.Default) -> float:
		"""SCPI: FETCh:TAERror[:CC<cc>]:ANTenna<ant>:MINimum \n
		Snippet: value: float = driver.applications.k10Xlte.fetch.taError.cc.antenna.minimum.get(carrierComponent = repcap.CarrierComponent.Default, antenna = repcap.Antenna.Default) \n
		This command queries the time alignment error. \n
		...
		response = self._core.io.query_str(f'FETCh:TAERror:CC{carrierComponent_cmd_val}:ANTenna{antenna_cmd_val}:MINimum?')
