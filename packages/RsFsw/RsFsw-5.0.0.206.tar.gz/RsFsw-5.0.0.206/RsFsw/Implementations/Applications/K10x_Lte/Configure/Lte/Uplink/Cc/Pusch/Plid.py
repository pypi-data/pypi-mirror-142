...
class Plid:
	...
	def set(self, identity: float, carrierComponent=repcap.CarrierComponent.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.configure.lte.uplink.cc.pusch.plid.set(identity = 1.0, carrierComponent = repcap.CarrierComponent.Default) \n
		This command defines the (cell) identity of the PUSCH. \n
		...
		self._core.io.write(f'CONFigure:LTE:UL:CC{carrierComponent_cmd_val}:PUSCh:PLID {param}')

	def get(self, carrierComponent=repcap.CarrierComponent.Default) -> float:
		"""SCPI: CONFigure[:LTE]:UL[:CC<cc>]:PUSCh:PLID \n
		Snippet: value: float = driver.applications.k10Xlte.configure.lte.uplink.cc.pusch.plid.get(carrierComponent = repcap.CarrierComponent.Default) \n
		This command defines the (cell) identity of the PUSCH. \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
		...
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		response = self._core.io.query_str(f'CONFigure:LTE:UL:CC{carrierComponent_cmd_val}:PUSCh:PLID?')
