...
class State:
	...
	def set(self, state: bool, window=repcap.Window.Default, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.display.window.pmeter.state.set(state = False, window = repcap.Window.Default, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, window=repcap.Window.Default, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: DISPlay[:WINDow<n>]:PMETer<p>:STATe \n
		Snippet: value: bool = driver.applications.k10Xlte.display.window.pmeter.state.get(window = repcap.Window.Default, powerMeter = repcap.PowerMeter.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
