...
class Alpha:
	...
	def set(self, alpha: float, subBlock=repcap.SubBlock.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.sense.espectrum.filterPy.rrc.alpha.set(alpha = 1.0, subBlock = repcap.SubBlock.Default) \n
	...
	def get(self, subBlock=repcap.SubBlock.Default) -> float:
		"""SCPI: [SENSe]:ESPectrum<sb>:FILTer[:RRC]:ALPHa \n
		Snippet: value: float = driver.applications.k10Xlte.sense.espectrum.filterPy.rrc.alpha.get(subBlock = repcap.SubBlock.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Espectrum')
