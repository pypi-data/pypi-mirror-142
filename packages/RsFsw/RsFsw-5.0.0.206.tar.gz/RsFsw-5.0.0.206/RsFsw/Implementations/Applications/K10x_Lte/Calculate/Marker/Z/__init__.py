...
class Z:
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> float:
		...
		Snippet: value: float = driver.applications.k10Xlte.calculate.marker.z.get(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command queries the marker position on the z-axis of three-dimensional result displays. This command returns the
		type of value displayed in the selected result display (EVM, Power or Allocation ID) . \n
	...
	def clone(self) -> 'Z':
		...
		new_group = Z(self._core, self._cmd_group.parent)
