...
class Path:
	...
	def set(self, filename: str, demod_bw: float = None, inputIx=repcap.InputIx.Default) -> None:
		"""SCPI: INPut<ip>:FILE:PATH \n
		Snippet: driver.applications.k10Xlte.inputPy.file.path.set(filename = '1', demod_bw = 1.0, inputIx = repcap.InputIx.Default) \n
		...
		I/Q Data Files'. \n
			...
			:param demod_bw: No help available
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('filename', filename, DataType.String), ArgSingle('demod_bw', demod_bw, DataType.Float, None, is_optional=True))
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		self._core.io.write(f'INPut{inputIx_cmd_val}:FILE:PATH {param}'.rstrip())

	# noinspection PyTypeChecker
	class PathStruct(StructBase):
		"""Response structure. Fields: \n
			- Filename: str: String containing the path and name of the source file. The file extension is *.iq.tar.
			- Demod_Bw: float: No parameter help available"""
		__meta_args_list = [
			ArgStruct.scalar_str('Filename'),
			ArgStruct.scalar_float('Demod_Bw')]

		def __init__(self):
			StructBase.__init__(self, self)
			self.Filename: str = None
			self.Demod_Bw: float = None

	def get(self, inputIx=repcap.InputIx.Default) -> PathStruct:
		"""SCPI: INPut<ip>:FILE:PATH \n
		Snippet: value: PathStruct = driver.applications.k10Xlte.inputPy.file.path.get(inputIx = repcap.InputIx.Default) \n
		This command selects the I/Q data file to be used as input for further measurements.
			INTRO_CMD_HELP: The I/Q data file must be in one of the following supported formats: \n
			- .iq.tar
			- .iqw
		...
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		return self._core.io.query_struct(f'INPut{inputIx_cmd_val}:FILE:PATH?', self.__class__.PathStruct())
