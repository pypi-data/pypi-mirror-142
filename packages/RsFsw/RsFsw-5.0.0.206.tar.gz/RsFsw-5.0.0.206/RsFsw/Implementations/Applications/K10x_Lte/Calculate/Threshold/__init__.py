...
class Threshold:
	...
	def set(self, level: float, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.calculate.threshold.set(level = 1.0, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: CALCulate<n>:THReshold \n
		Snippet: value: float = driver.applications.k10Xlte.calculate.threshold.get(window = repcap.Window.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
