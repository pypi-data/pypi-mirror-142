...
class Bcategory:
	...
	def set(self, category: float, subBlock=repcap.SubBlock.Default) -> None:
		...
		Snippet: driver.applications.k10Xlte.sense.espectrum.msr.bcategory.set(category = 1.0, subBlock = repcap.SubBlock.Default) \n
	...
	def get(self, subBlock=repcap.SubBlock.Default) -> float:
		"""SCPI: [SENSe]:ESPectrum<sb>:MSR:BCATegory \n
		Snippet: value: float = driver.applications.k10Xlte.sense.espectrum.msr.bcategory.get(subBlock = repcap.SubBlock.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Espectrum')
