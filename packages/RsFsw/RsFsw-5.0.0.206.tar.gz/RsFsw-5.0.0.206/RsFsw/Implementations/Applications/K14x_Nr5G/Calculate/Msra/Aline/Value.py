...
class Value:
	...
	def set(self, position: float, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.calculate.msra.aline.value.set(position = 1.0, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: CALCulate<n>:MSRA:ALINe[:VALue] \n
		Snippet: value: float = driver.applications.k14Xnr5G.calculate.msra.aline.value.get(window = repcap.Window.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
