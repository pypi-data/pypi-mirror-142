...
class Ualternate:
	...
	def set(self, state: bool, upperAltChannel=repcap.UpperAltChannel.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.power.achannel.filterPy.state.ualternate.set(state = False, upperAltChannel = repcap.UpperAltChannel.Default) \n
	...
	def get(self, upperAltChannel=repcap.UpperAltChannel.Default) -> bool:
		"""SCPI: [SENSe]:POWer:ACHannel:FILTer[:STATe]:UALTernate<ch> \n
		Snippet: value: bool = driver.applications.k14Xnr5G.sense.power.achannel.filterPy.state.ualternate.get(upperAltChannel = repcap.UpperAltChannel.Default) \n
		No command help available \n
			:param upperAltChannel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ualternate')
