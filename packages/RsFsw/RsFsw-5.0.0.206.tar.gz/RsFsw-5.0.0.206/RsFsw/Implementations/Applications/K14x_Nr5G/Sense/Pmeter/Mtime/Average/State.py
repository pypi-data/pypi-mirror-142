...
class State:
	...
	def set(self, state: bool, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.pmeter.mtime.average.state.set(state = False, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: [SENSe]:PMETer<p>:MTIMe:AVERage[:STATe] \n
		Snippet: value: bool = driver.applications.k14Xnr5G.sense.pmeter.mtime.average.state.get(powerMeter = repcap.PowerMeter.Default) \n
		No command help available \n
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
