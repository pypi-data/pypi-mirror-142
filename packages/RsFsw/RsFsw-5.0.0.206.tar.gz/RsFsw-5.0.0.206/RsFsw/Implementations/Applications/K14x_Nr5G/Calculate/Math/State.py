...
class State:
	...
	def set(self, state: bool, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.calculate.math.state.set(state = False, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> bool:
		"""SCPI: CALCulate<n>:MATH:STATe \n
		Snippet: value: bool = driver.applications.k14Xnr5G.calculate.math.state.get(window = repcap.Window.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
