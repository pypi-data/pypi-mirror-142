...
class Channel:
	...
	def set(self, name: str, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.power.achannel.name.channel.set(name = '1', channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SENSe:POWer:ACHannel:NAME:CHANnel{channel_cmd_val} {param}')

	def get(self, channel=repcap.Channel.Default) -> str:
		"""SCPI: [SENSe]:POWer:ACHannel:NAME:CHANnel<ch> \n
		Snippet: value: str = driver.applications.k14Xnr5G.sense.power.achannel.name.channel.get(channel = repcap.Channel.Default) \n
		No command help available \n
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Channel')
			:return: name: No help available"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SENSe:POWer:ACHannel:NAME:CHANnel{channel_cmd_val}?')
