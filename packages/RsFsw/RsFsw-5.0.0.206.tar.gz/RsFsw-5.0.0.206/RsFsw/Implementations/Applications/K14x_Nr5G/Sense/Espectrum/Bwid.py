...
class Bwid:
	...
	def set(self, bandwidth: float, subBlock=repcap.SubBlock.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.espectrum.bwid.set(bandwidth = 1.0, subBlock = repcap.SubBlock.Default) \n
	...
	def get(self, subBlock=repcap.SubBlock.Default) -> float:
		"""SCPI: [SENSe]:ESPectrum<sb>:BWID \n
		Snippet: value: float = driver.applications.k14Xnr5G.sense.espectrum.bwid.get(subBlock = repcap.SubBlock.Default) \n
		No command help available \n
			:param subBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Espectrum')
