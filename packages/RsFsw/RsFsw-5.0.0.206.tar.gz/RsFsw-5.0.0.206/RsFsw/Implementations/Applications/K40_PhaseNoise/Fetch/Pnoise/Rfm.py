...
class Rfm:
	...
	def get(self, trace=repcap.Trace.Default) -> float:
		...
		Snippet: value: float = driver.applications.k40PhaseNoise.fetch.pnoise.rfm.get(trace = repcap.Trace.Default) \n
		...
		response = self._core.io.query_str(f'FETCh:PNOise{trace_cmd_val}:RFM?')
