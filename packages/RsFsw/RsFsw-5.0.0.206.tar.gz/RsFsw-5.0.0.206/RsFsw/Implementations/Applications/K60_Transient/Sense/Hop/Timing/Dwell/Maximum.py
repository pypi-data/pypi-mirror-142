...
class Maximum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:TIMing:DWELl:MAXimum \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.timing.dwell.maximum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:TIMing:DWELl:MAXimum? {param}')
