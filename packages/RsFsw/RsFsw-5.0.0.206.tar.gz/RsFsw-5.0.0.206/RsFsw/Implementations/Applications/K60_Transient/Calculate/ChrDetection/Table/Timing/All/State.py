...
class State:
	...
	def set(self, state: bool, scaling: enums.TimeScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:TIMing:ALL[:STATe] \n
		Snippet: driver.applications.k60Transient.calculate.chrDetection.table.timing.all.state.set(state = False, scaling = enums.TimeScaling.MS, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:TIMing:ALL:STATe {param}'.rstrip())
