...
class Bandwidth:
	"""Bandwidth commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:FREQuency:BWIDth \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.frequency.bandwidth.get(query_range = enums.SelectionRange.ALL) \n
		Returns the chirp bandwidth from the Results table for the specified chirp(s) . \n
			...
			:return: result: Unit: kHz"""
		param = Conversions.enum_scalar_to_str(query_range, enums.SelectionRange)
		response = self._core.io.query_str(f'SENSe:CHIRp:FREQuency:BWIDth? {param}')
