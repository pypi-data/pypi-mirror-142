...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:CHIRp:TIMing:RATE:SDEViation \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.timing.rate.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the chirp rate from the statistics table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:TIMing:RATE:SDEViation? {param}')
