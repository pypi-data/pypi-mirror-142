...
class Reference:
	...
	def set(self, reference: enums.ResultDevReference, window=repcap.Window.Default) -> None:
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:PDEViation:REFerence {param}')
	...
	def get(self, window=repcap.Window.Default) -> enums.ResultDevReference:
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:HOPDetection:PDEViation:REFerence?')
