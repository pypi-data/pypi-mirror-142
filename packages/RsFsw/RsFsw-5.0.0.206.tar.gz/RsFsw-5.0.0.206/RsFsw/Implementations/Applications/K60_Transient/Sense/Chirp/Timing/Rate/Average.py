...
class Average:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:CHIRp:TIMing:RATE:AVERage \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.timing.rate.average.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:TIMing:RATE:AVERage? {param}')
