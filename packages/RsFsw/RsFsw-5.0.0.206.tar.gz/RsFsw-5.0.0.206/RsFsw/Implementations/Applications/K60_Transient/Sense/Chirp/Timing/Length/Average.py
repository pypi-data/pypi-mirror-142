...
class Average:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:CHIRp:TIMing:LENGth:AVERage \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.timing.length.average.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:TIMing:LENGth:AVERage? {param}')
