...
class State:
	...
	def set(self, yaxis: enums.AxisHopState, xaxis: enums.HopXaxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:HOP:STATe \n
		Snippet: driver.applications.k60Transient.calculate.trend.hop.state.set(yaxis = enums.AxisHopState.INDex, xaxis = enums.HopXaxisItems.AVGFm, window = repcap.Window.Default) \n
		Configures the x-axis and y-axis of the Parameter Trend result display for hop trends over time. \n
			:param yaxis: INDex | STAFrequency INDex Hop index STAFrequency State frequency (nominal)
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.AxisHopState), ArgSingle('xaxis', xaxis, DataType.Enum, enums.HopXaxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:HOP:STATe {param}'.rstrip())
