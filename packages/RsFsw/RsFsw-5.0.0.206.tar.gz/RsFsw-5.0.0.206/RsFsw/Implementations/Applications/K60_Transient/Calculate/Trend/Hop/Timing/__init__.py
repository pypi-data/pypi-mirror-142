...
class Timing:
	...
	def set(self, yaxis: enums.AxisHopTiming, xaxis: enums.HopXaxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:HOP:TIMing \n
		Snippet: driver.applications.k60Transient.calculate.trend.hop.timing.set(yaxis = enums.AxisHopTiming.BEGin, xaxis = enums.HopXaxisItems.AVGFm, window = repcap.Window.Default) \n
		Configures the x-axis and y-axis of the Parameter Trend result display for hop trends over time. \n
			:param yaxis: BEGin | DWELl | SWITching BEGin Hop Begin DWELl Hop dwell time SWITching Switching time
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.AxisHopTiming), ArgSingle('xaxis', xaxis, DataType.Enum, enums.HopXaxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:HOP:TIMing {param}'.rstrip())
