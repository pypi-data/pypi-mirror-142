...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:CHIRp:FREQuency:CHERror:SDEViation \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.frequency.chError.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the chirp rate deviation from the statistics table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:FREQuency:CHERror:SDEViation? {param}')
