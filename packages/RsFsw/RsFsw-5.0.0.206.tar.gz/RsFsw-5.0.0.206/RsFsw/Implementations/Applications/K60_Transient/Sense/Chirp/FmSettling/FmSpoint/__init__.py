...
class FmSpoint:
	"""FmSpoint commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:FMSettling:FMSPoint \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.fmSettling.fmSpoint.get(query_range = enums.SelectionRange.ALL) \n
		Returns the FM settling point from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:FMSettling:FMSPoint? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'FmSpoint':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = FmSpoint(self._core, self._cmd_group.parent)
