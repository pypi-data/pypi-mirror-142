...
class AvgPm:
	"""AvgPm commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:PHASe:AVGPm \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.phase.avgPm.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:PHASe:AVGPm? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'AvgPm':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = AvgPm(self._core, self._cmd_group.parent)
