...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:FREQuency:FMERror:SDEViation \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.fmError.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:FMERror:SDEViation? {param}')
