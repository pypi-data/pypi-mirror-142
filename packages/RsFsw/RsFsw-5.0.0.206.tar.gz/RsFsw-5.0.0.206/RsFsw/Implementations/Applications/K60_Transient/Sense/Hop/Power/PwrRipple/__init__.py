...
class PwrRipple:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:POWer:PWRRipple \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.pwrRipple.get(query_range = enums.SelectionRange.ALL) \n
		Returns the ripple power from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:PWRRipple? {param}')
