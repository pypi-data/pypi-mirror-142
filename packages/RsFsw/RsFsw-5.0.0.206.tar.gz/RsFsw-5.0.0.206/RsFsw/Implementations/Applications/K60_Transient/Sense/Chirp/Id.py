...
class Id:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:CHIRp:ID \n
		Snippet: value: str = driver.applications.k60Transient.sense.chirp.id.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the chirp IDs from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:ID? {param}')
