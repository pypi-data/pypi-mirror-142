...
class MaxPower:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:POWer:MAXPower \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.maxPower.get(query_range = enums.SelectionRange.ALL) \n
		Returns the maximum hop power from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:MAXPower? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'MaxPower':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = MaxPower(self._core, self._cmd_group.parent)
