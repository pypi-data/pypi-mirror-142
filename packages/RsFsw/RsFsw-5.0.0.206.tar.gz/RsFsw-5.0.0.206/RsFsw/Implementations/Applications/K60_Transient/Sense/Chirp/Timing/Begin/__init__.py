...
class Begin:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:TIMing:BEGin \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.timing.begin.get(query_range = enums.SelectionRange.ALL) \n
		Returns the chirp begin time from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:TIMing:BEGin? {param}')
