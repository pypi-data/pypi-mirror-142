...
class PmStime:
	"""PmStime commands group definition. 1 total commands, 0 Subgroups, 1 group commands"""
	...
	def set(self, state: bool, scaling: enums.TimeScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:PMSettling:PMSTime \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:PMSettling:PMSTime {param}'.rstrip())

	# noinspection PyTypeChecker
	class PmStimeStruct(StructBase):
	...
	def get(self, window=repcap.Window.Default) -> PmStimeStruct:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:PMSettling:PMSTime \n
		...
		Table.Results.get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: structure: for return value, see the help for PmStimeStruct structure arguments."""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:PMSettling:PMSTime?', self.__class__.PmStimeStruct())
