...
class RmsFm:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:FREQuency:RMSFm \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.rmsFm.get(query_range = enums.SelectionRange.ALL) \n
		Returns the RMS Frequency Deviation from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:RMSFm? {param}')
