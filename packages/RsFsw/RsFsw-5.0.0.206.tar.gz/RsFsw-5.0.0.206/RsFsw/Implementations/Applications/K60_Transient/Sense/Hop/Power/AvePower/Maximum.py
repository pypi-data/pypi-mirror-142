...
class Maximum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:POWer:AVEPower:MAXimum \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.avePower.maximum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:AVEPower:MAXimum? {param}')
