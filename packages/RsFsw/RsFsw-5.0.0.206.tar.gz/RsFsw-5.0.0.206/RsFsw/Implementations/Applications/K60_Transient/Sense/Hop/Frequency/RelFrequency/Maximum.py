...
class Maximum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:FREQuency:RELFrequency:MAXimum \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.relFrequency.maximum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:RELFrequency:MAXimum? {param}')
