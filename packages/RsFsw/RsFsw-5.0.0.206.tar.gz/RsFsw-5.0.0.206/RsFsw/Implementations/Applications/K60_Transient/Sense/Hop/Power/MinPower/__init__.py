...
class MinPower:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:POWer:MINPower \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.power.minPower.get(query_range = enums.SelectionRange.ALL) \n
		Returns the minimum hop power from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:POWer:MINPower? {param}')
