...
class Pdivision:
	...
	def set(self, value: float, window=repcap.Window.Default) -> None:
		...
		example, this command is not available. \n
			:param value: numeric value WITHOUT UNIT (unit according to the result display) Defines the range per division (total range = 10*Value) Unit: DBM
	...
	def get(self, window=repcap.Window.Default) -> float:
		...
		example, this command is not available. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:return: value: numeric value WITHOUT UNIT (unit according to the result display) Defines the range per division (total range = 10*Value) Unit: DBM"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
