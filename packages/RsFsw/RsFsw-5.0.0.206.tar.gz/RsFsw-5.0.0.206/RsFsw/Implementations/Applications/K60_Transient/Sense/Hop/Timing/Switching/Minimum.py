...
class Minimum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:TIMing:SWITching:MINimum \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.timing.switching.minimum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:TIMing:SWITching:MINimum? {param}')
