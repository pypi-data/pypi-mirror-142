...
class Angle:
	...
	def set(self, unit: enums.AngleUnit, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k60Transient.calculate.unit.angle.set(unit = enums.AngleUnit.DEG, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> enums.AngleUnit:
		"""SCPI: CALCulate<n>:UNIT:ANGLe \n
		Snippet: value: enums.AngleUnit = driver.applications.k60Transient.calculate.unit.angle.get(window = repcap.Window.Default) \n
		This command selects the global unit for phase results. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: unit: DEG | RAD"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
