...
class Dwell:
	...
	def set(self, state: bool, scaling: enums.TimeScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:TIMing:DWELl \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:TIMing:DWELl {param}'.rstrip())

	# noinspection PyTypeChecker
	class DwellStruct(StructBase):
	...
	def get(self, window=repcap.Window.Default) -> DwellStruct:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:TIMing:DWELl \n
		...
		get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: structure: for return value, see the help for DwellStruct structure arguments."""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:TIMing:DWELl?', self.__class__.DwellStruct())
