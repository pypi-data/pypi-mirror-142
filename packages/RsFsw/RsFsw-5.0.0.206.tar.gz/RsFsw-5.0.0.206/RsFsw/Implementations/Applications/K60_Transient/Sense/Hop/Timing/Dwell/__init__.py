...
class Dwell:
	"""Dwell commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:TIMing:DWELl \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.timing.dwell.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:TIMing:DWELl? {param}')
