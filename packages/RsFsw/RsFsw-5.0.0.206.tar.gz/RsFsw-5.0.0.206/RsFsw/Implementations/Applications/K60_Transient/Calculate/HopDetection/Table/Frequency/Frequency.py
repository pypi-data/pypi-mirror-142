...
class Frequency:
	...
	def set(self, state: bool, scaling: enums.FrequencyScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:FREQuency:FREQuency \n
		Snippet: driver.applications.k60Transient.calculate.hopDetection.table.frequency.frequency.set(state = False, scaling = enums.FrequencyScaling.GHZ, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:FREQuency:FREQuency {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> FrequencyStruct:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:FREQuency:FREQuency \n
		Snippet: value: FrequencyStruct = driver.applications.k60Transient.calculate.hopDetection.table.frequency.frequency.get(window = repcap.Window.Default) \n
		If enabled, the specified frequency parameter is included in the result tables (see 'Frequency parameters') . Note that
		only the enabled columns are returned for the method RsFsw.Applications.K60_Transient.Calculate.HopDetection.Table.
		Results.get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:FREQuency:FREQuency?', self.__class__.FrequencyStruct())
