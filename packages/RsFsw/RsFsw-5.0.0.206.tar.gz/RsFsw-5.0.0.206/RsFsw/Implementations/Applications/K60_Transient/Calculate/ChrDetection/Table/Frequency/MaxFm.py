...
class MaxFm:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("maxFm", core, parent)

	def set(self, state: bool, scaling: enums.FrequencyScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:FREQuency:MAXFm \n
		Snippet: driver.applications.k60Transient.calculate.chrDetection.table.frequency.maxFm.set(state = False, scaling = enums.FrequencyScaling.GHZ, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:FREQuency:MAXFm {param}'.rstrip())

	# noinspection PyTypeChecker
	class MaxFmStruct(StructBase):
	...
	def get(self, window=repcap.Window.Default) -> MaxFmStruct:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:FREQuency:MAXFm \n
		Snippet: value: MaxFmStruct = driver.applications.k60Transient.calculate.chrDetection.table.frequency.maxFm.get(window = repcap.Window.Default) \n
		If enabled, the specified frequency parameter is included in the result tables (see 'Frequency parameters') . Note that
		only the enabled columns are returned for the method RsFsw.Applications.K60_Transient.Calculate.ChrDetection.Table.
		Results.get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: structure: for return value, see the help for MaxFmStruct structure arguments."""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:FREQuency:MAXFm?', self.__class__.MaxFmStruct())
