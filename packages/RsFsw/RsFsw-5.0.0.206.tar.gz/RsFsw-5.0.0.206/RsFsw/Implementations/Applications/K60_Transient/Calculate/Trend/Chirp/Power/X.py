...
class X:
	...
	def set(self, xaxis: enums.AxisPower, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:CHIRp:POWer:X \n
		Snippet: driver.applications.k60Transient.calculate.trend.chirp.power.x.set(xaxis = enums.AxisPower.AVGPower, window = repcap.Window.Default) \n
		Configures the x-axis of the Parameter Trend result display for chirp power parameters. \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:CHIRp:POWer:X {param}')
