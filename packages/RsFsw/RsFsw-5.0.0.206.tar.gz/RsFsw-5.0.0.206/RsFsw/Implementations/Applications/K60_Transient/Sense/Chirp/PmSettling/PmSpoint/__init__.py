...
class PmSpoint:
	"""PmSpoint commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:PMSettling:PMSPoint \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.pmSettling.pmSpoint.get(query_range = enums.SelectionRange.ALL) \n
		Returns the PM settling point from the Results table for the specified chirp(s) . \n
			...
			:return: result: SELected | CURRent | ALL SELected Currently selected pulse CURRent Detected pulses in the current capture buffer ALL All detected pulses in the entire measurement."""
		param = Conversions.enum_scalar_to_str(query_range, enums.SelectionRange)
		response = self._core.io.query_str(f'SENSe:CHIRp:PMSettling:PMSPoint? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'PmSpoint':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = PmSpoint(self._core, self._cmd_group.parent)
