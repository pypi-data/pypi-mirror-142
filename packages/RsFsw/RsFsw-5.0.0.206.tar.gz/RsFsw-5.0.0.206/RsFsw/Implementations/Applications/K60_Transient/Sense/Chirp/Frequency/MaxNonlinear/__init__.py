...
class MaxNonlinear:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:FREQuency:MAXNonlinear \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.frequency.maxNonlinear.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:FREQuency:MAXNonlinear? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'MaxNonlinear':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = MaxNonlinear(self._core, self._cmd_group.parent)
