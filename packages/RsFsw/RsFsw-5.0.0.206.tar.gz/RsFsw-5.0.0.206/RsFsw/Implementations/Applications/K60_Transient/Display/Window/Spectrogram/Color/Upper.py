...
class Upper:
	...
	def set(self, percentage: float, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k60Transient.display.window.spectrogram.color.upper.set(percentage = 1.0, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'DISPlay:WINDow{window_cmd_val}:SPECtrogram:COLor:UPPer {param}')

	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: DISPlay[:WINDow<n>]:SPECtrogram:COLor:UPPer \n
		Snippet: value: float = driver.applications.k60Transient.display.window.spectrogram.color.upper.get(window = repcap.Window.Default) \n
		This command defines the end point of the color map. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:return: percentage: Statistical frequency percentage. Range: 0 to 66, Unit: %"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		response = self._core.io.query_str(f'DISPlay:WINDow{window_cmd_val}:SPECtrogram:COLor:UPPer?')
