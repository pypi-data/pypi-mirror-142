...
class Number:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:HOP:NUMBer \n
		Snippet: value: str = driver.applications.k60Transient.sense.hop.number.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the hop numbers from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:NUMBer? {param}')
