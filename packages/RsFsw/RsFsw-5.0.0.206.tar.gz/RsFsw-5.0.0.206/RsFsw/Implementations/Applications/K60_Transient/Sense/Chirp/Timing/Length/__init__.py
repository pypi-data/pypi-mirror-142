...
class Length:
	"""Length commands group definition. 5 total commands, 4 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:TIMing:LENGth \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.timing.length.get(query_range = enums.SelectionRange.ALL) \n
		Returns the chirp length from the Results table for the specified chirp(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:TIMing:LENGth? {param}')
