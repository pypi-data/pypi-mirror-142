...
class Minimum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		...
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.fmError.minimum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:FMERror:MINimum? {param}')
