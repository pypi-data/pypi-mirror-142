...
class MaxFm:
	...
	def set(self, state: bool, scaling: enums.FrequencyScaling = None, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k60Transient.calculate.hopDetection.table.frequency.maxFm.set(state = False, scaling = enums.FrequencyScaling.GHZ, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:FREQuency:MAXFm {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> MaxFmStruct:
		"""SCPI: CALCulate<n>:HOPDetection:TABLe:FREQuency:MAXFm \n
		Snippet: value: MaxFmStruct = driver.applications.k60Transient.calculate.hopDetection.table.frequency.maxFm.get(window = repcap.Window.Default) \n
		If enabled, the specified frequency parameter is included in the result tables (see 'Frequency parameters') . Note that
		only the enabled columns are returned for the method RsFsw.Applications.K60_Transient.Calculate.HopDetection.Table.
		Results.get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:HOPDetection:TABLe:FREQuency:MAXFm?', self.__class__.MaxFmStruct())
