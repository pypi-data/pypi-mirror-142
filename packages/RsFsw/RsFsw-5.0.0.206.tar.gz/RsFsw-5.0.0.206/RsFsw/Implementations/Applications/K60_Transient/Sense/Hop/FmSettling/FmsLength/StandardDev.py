...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:FMSettling:FMSLength:SDEViation \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.fmSettling.fmsLength.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the FM settled length from the statistics table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FMSettling:FMSLength:SDEViation? {param}')
