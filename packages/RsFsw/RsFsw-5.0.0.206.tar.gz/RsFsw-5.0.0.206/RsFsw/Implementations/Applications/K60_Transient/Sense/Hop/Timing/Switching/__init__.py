...
class Switching:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:TIMing:SWITching \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.timing.switching.get(query_range = enums.SelectionRange.ALL) \n
		Returns the switching time from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:TIMing:SWITching? {param}')
		return Conversions.str_to_float(response)

	def clone(self) -> 'Switching':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Switching(self._core, self._cmd_group.parent)
