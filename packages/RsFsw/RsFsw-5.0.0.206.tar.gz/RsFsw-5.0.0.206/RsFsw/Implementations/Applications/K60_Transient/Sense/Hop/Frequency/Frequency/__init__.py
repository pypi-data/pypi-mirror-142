...
class Frequency:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:HOP:FREQuency:FREQuency \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.frequency.frequency.get(query_range = enums.SelectionRange.ALL) \n
		Returns the average frequency from the Results table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:FREQuency:FREQuency? {param}')
