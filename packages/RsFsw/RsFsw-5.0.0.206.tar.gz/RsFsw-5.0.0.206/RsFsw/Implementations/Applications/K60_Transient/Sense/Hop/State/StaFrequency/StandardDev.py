...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: [SENSe]:HOP:STATe:STAFrequency:SDEViation \n
		Snippet: value: float = driver.applications.k60Transient.sense.hop.state.staFrequency.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the nominal hop state frequency from the statistics table for the specified hop(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:HOP:STATe:STAFrequency:SDEViation? {param}')
