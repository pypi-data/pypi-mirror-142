...
class FmSettling:
	"""FmSettling commands group definition. 3 total commands, 2 Subgroups, 1 group commands"""
	...
	def set(self, yaxis: enums.AxisFmSettling, xaxis: enums.ChirpXaxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:CHIRp:FMSettling \n
		Snippet: driver.applications.k60Transient.calculate.trend.chirp.fmSettling.set(yaxis = enums.AxisFmSettling.FMSLength, xaxis = enums.ChirpXaxisItems.AVGFm, window = repcap.Window.Default) \n
		Configures the x-axis and y-axis of the Parameter Trend result display for chirp trends over time. \n
			:param yaxis: FMSLength | FMSPoint | FMSTime FMSPoint FM settling point FMSTime FM settling time FMSLength FM settled length
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.AxisFmSettling), ArgSingle('xaxis', xaxis, DataType.Enum, enums.ChirpXaxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:CHIRp:FMSettling {param}'.rstrip())
