...
class PmSettling:
	...
	def set(self, yaxis: enums.XaXisPmSettling, xaxis: enums.ChirpXaxisItems, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:TRENd:CHIRp:PMSettling \n
		Snippet: driver.applications.k60Transient.calculate.trend.chirp.pmSettling.set(yaxis = enums.XaXisPmSettling.PMSLength, xaxis = enums.ChirpXaxisItems.AVGFm, window = repcap.Window.Default) \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('yaxis', yaxis, DataType.Enum, enums.XaXisPmSettling), ArgSingle('xaxis', xaxis, DataType.Enum, enums.ChirpXaxisItems))
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		self._core.io.write(f'CALCulate{window_cmd_val}:TRENd:CHIRp:PMSettling {param}'.rstrip())
