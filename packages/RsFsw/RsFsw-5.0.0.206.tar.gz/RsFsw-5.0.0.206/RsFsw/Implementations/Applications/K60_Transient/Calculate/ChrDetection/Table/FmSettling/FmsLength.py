...
class FmsLength:
	...
	def set(self, state: bool, scaling: enums.TimeScaling = None, window=repcap.Window.Default) -> None:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:FMSettling:FMSLength \n
		Snippet: driver.applications.k60Transient.calculate.chrDetection.table.fmSettling.fmsLength.set(state = False, scaling = enums.TimeScaling.MS, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:FMSettling:FMSLength {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> FmsLengthStruct:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:FMSettling:FMSLength \n
		Snippet: value: FmsLengthStruct = driver.applications.k60Transient.calculate.chrDetection.table.fmSettling.fmsLength.get(window = repcap.Window.Default) \n
		If enabled, the FM settling time parameter is included in the result tables (see 'FM settling parameters') . Note that
		only the enabled columns are returned for the method RsFsw.Applications.K60_Transient.Calculate.ChrDetection.Table.
		Results.get_ query. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:FMSettling:FMSLength?', self.__class__.FmsLengthStruct())
