...
class Maximum:
	...
	def set(self, max_py: float, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		Snippet: driver.applications.k9X11Ad.display.window.trace.y.scale.maximum.set(max_py = 1.0, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> float:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:Y[:SCALe]:MAXimum \n
		Snippet: value: float = driver.applications.k9X11Ad.display.window.trace.y.scale.maximum.get(window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
