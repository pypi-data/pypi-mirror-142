...
class Relative:
	...
	def get(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default) -> float:
		...
		Snippet: value: float = driver.applications.k9X11Ad.calculate.deltaMarker.x.relative.get(window = repcap.Window.Default, deltaMarker = repcap.DeltaMarker.Default) \n
		This command queries the relative position of a delta marker on the x-axis. If necessary, the command activates the delta
		marker first. \n
			...
			:return: xvalue_relative: No help available"""
