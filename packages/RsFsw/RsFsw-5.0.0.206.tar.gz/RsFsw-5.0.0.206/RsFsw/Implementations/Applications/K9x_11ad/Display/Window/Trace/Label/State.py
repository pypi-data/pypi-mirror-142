...
class State:
	...
	def set(self, state: bool, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		Snippet: driver.applications.k9X11Ad.display.window.trace.label.state.set(state = False, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> bool:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:LABel[:STATe] \n
		Snippet: value: bool = driver.applications.k9X11Ad.display.window.trace.label.state.get(window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
