...
class Value:
	...
	def set(self, freq_high: float) -> None:
		...
		Snippet: driver.applications.k9X11Ad.sense.mixer.harmonic.high.value.set(freq_high = 1.0) \n
		This command specifies the harmonic order to be used for the high (second) range. \n
			:param freq_high: 1..n irrelevant
		"""
	...
	def get(self) -> float:
		"""SCPI: [SENSe]:MIXer:HARMonic:HIGH[:VALue] \n
		Snippet: value: float = driver.applications.k9X11Ad.sense.mixer.harmonic.high.value.get() \n
		This command specifies the harmonic order to be used for the high (second) range. \n
