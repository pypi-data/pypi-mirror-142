...
class Amplitude:
	"""Amplitude commands group definition. 19 total commands, 8 Subgroups, 1 group commands"""
	...
	@property
	def icomponent(self):
		"""icomponent commands group. 5 Sub-classes, 1 commands."""
	...
	@property
	def qcomponent(self):
		"""qcomponent commands group. 5 Sub-classes, 1 commands."""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:AMPLitude \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.amplitude.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:AMPLitude? {param}')
		return trim_str_response(response)

	def clone(self) -> 'Amplitude':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Amplitude(self._core, self._cmd_group.parent)
