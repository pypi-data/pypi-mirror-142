...
class Settling:
	"""Settling commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:TIMing:SETTling \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.timing.settling.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TIMing:SETTling? {param}')
		return trim_str_response(response)

	def clone(self) -> 'Settling':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Settling(self._core, self._cmd_group.parent)
