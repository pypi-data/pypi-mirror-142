...
class Rbw:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: CALCulate<n>:PPSPectrum:RBW \n
		Snippet: value: float = driver.applications.k6Pulse.calculate.ppSpectrum.rbw.get(window = repcap.Window.Default) \n
		...
		K6_Pulse.Calculate.Pspectrum.BlockSize.set) . \n
			...
			:return: rbw: 1..n Window"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:PPSPectrum:RBW?')
