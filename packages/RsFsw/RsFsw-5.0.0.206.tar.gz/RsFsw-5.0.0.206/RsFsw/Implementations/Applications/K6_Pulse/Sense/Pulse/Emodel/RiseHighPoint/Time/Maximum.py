...
class Maximum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:RHPTime:MAXimum \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.riseHighPoint.time.maximum.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:RHPTime:MAXimum? {param}')
		return response
