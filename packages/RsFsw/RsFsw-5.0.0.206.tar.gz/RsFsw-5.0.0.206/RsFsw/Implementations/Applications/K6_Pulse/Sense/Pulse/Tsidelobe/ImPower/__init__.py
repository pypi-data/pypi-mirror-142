...
class ImPower:
	"""ImPower commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:IMPower \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.tsidelobe.imPower.get(query_range = enums.SelectionRange.ALL) \n
		Returns the integrated mainlobe level for the specified pulse(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:IMPower? {param}')
