...
class Comment:
	...
	def set(self, comment: str, store=repcap.Store.Default) -> None:
		...
		Snippet: driver.applications.k6Pulse.massMemory.store.iq.comment.set(comment = '1', store = repcap.Store.Default) \n
	...
	def get(self, store=repcap.Store.Default) -> str:
		"""SCPI: MMEMory:STORe<n>:IQ:COMMent \n
		Snippet: value: str = driver.applications.k6Pulse.massMemory.store.iq.comment.get(store = repcap.Store.Default) \n
		This command adds a comment to a file that contains I/Q data. \n
			:param store: optional repeated capability selector. Default value: Pos1 (settable in the interface 'Store')
