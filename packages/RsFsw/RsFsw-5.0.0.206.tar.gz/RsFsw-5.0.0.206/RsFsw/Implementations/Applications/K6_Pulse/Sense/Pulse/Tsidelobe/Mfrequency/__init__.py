...
class Mfrequency:
	"""Mfrequency commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:MFRequency \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.tsidelobe.mfrequency.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:MFRequency? {param}')
		return trim_str_response(response)

	def clone(self) -> 'Mfrequency':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Mfrequency(self._core, self._cmd_group.parent)
