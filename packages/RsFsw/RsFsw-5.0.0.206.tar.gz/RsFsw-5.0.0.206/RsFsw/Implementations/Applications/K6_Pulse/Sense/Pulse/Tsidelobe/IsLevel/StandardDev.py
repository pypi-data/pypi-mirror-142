...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:ISLevel:SDEViation \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.tsidelobe.isLevel.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the integrated sidelobe level within the time sidelobe range. \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:ISLevel:SDEViation? {param}')
