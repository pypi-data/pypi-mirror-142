...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:IMPower:SDEViation \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.tsidelobe.imPower.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the integrated mainlobe power within the time sidelobe range. \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:IMPower:SDEViation? {param}')
