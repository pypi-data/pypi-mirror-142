...
class Average:
	...
	def get(self, query_range: enums.SelectionRangeB) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:FMPLevel:AVERage \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.fallMidPoint.level.average.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:FMPLevel:AVERage? {param}')
		return response
