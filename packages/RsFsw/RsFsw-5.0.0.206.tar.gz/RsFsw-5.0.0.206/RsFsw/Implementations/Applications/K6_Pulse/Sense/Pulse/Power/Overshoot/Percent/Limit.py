...
class Limit:
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:OVERshoot[:PERCent]:LIMit \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.overshoot.percent.limit.get(query_range = enums.SelectionRange.ALL) \n
		...
		are only available if the additional option R&S FSW-K6S is installed. \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:OVERshoot:PERCent:LIMit? {param}')
