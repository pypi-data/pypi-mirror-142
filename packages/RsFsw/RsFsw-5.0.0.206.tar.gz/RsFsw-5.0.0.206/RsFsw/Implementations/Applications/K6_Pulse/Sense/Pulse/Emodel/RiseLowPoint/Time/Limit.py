...
class Limit:
	...
	def get(self, query_range: enums.SelectionRange) -> List[str]:
		"""SCPI: [SENSe]:PULSe:EMODel:RLPTime:LIMit \n
		Snippet: value: List[str] = driver.applications.k6Pulse.sense.pulse.emodel.riseLowPoint.time.limit.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:EMODel:RLPTime:LIMit? {param}')
