...
class Count:
	...
	def set(self, count: float) -> None:
		...
		This command defines the number of measurements that the application uses to average traces. (For Pulse measurements, see
		also 'Trace statistics') . In continuous measurement mode, the application calculates the moving average over the average
		count. In single measurement mode, the application stops the measurement and calculates the average after the average
