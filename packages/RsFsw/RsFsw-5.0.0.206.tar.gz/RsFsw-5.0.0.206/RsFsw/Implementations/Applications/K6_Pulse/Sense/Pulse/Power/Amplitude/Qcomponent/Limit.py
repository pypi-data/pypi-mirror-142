...
class Limit:
	...
	def get(self, query_range: enums.SelectionRange) -> List[str]:
		"""SCPI: [SENSe]:PULSe:POWer:AMPLitude:Q:LIMit \n
		Snippet: value: List[str] = driver.applications.k6Pulse.sense.pulse.power.amplitude.qcomponent.limit.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:AMPLitude:Q:LIMit? {param}')
