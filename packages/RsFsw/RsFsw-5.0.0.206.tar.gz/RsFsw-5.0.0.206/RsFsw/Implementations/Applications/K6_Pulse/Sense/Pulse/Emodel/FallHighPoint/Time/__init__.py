...
class Time:
	"""Time commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:FHPTime \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.fallHighPoint.time.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:FHPTime? {param}')
		return response
