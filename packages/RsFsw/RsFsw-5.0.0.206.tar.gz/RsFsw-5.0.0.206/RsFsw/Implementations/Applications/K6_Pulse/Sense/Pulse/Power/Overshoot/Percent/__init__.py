...
class Percent:
	"""Percent commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:OVERshoot[:PERCent] \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.overshoot.percent.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:OVERshoot:PERCent? {param}')
