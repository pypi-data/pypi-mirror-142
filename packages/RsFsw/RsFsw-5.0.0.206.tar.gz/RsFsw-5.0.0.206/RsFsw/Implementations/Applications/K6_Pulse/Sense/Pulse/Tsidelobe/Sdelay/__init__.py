...
class Sdelay:
	"""Sdelay commands group definition. 7 total commands, 6 Subgroups, 1 group commands"""
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:SDELay \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.tsidelobe.sdelay.get(query_range = enums.SelectionRange.ALL) \n
		Returns the sidelobe delay for the time sidelobe range for the specified pulse(s) . \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:SDELay? {param}')
