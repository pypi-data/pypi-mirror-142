...
class Limit:
	...
	def get(self, query_range: enums.SelectionRange) -> List[str]:
		"""SCPI: [SENSe]:PULSe:TSIDelobe:AMPower:LIMit \n
		Snippet: value: List[str] = driver.applications.k6Pulse.sense.pulse.tsidelobe.amPower.limit.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TSIDelobe:AMPower:LIMit? {param}')
