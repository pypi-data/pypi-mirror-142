...
class Count:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:RIPPle:DB:COUNt \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.ripple.db.count.get(query_range = enums.SelectionRangeB.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:RIPPle:DB:COUNt? {param}')
