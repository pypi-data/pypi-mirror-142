...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:TIMing:PRF:SDEViation \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.timing.prf.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the Pulse Repetition Frequency (Hz) over the specified pulses. \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:TIMing:PRF:SDEViation? {param}')
