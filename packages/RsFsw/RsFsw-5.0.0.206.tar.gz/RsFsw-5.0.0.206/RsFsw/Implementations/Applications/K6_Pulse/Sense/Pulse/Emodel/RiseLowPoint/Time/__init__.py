...
class Time:
	...
	def get(self, query_range: enums.SelectionRange) -> List[float]:
		"""SCPI: [SENSe]:PULSe:EMODel:RLPTime \n
		Snippet: value: List[float] = driver.applications.k6Pulse.sense.pulse.emodel.riseLowPoint.time.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SENSe:PULSe:EMODel:RLPTime? {param}')
		return response
