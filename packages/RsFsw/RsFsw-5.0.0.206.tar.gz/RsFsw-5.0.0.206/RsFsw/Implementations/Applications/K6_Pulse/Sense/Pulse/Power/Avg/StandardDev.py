...
class StandardDev:
	...
	def get(self, query_range: enums.SelectionRangeB) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:AVG:SDEViation \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.avg.standardDev.get(query_range = enums.SelectionRangeB.ALL) \n
		Returns the statistical value for the average transmission power over the specified pulses. \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:AVG:SDEViation? {param}')
