...
class Limit:
	...
	def get(self, query_range: enums.SelectionRange) -> str:
		"""SCPI: [SENSe]:PULSe:POWer:RIPPle[:PERCent]:LIMit \n
		Snippet: value: str = driver.applications.k6Pulse.sense.pulse.power.ripple.percent.limit.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:PULSe:POWer:RIPPle:PERCent:LIMit? {param}')
