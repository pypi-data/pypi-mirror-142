...
class Low:
	...
	def set(self, bias_setting: float) -> None:
		...
		Snippet: driver.applications.k30NoiseFigure.sense.mixer.bias.low.set(bias_setting = 1.0) \n
		This command defines the bias current for the low (first) range. . This command is only available if the external mixer
	...
	def get(self) -> float:
		"""SCPI: [SENSe]:MIXer:BIAS[:LOW] \n
		Snippet: value: float = driver.applications.k30NoiseFigure.sense.mixer.bias.low.get() \n
		This command defines the bias current for the low (first) range. . This command is only available if the external mixer
		is active (see [SENSe:]MIXer<x>[:STATe]) . \n
