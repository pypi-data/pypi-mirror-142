...
class Auto:
	...
	def set(self, state: bool, inputIx=repcap.InputIx.Default) -> None:
		...
		Snippet: driver.applications.k18AmplifierEt.inputPy.eatt.auto.set(state = False, inputIx = repcap.InputIx.Default) \n
	...
	def get(self, inputIx=repcap.InputIx.Default) -> bool:
		"""SCPI: INPut<ip>:EATT:AUTO \n
		Snippet: value: bool = driver.applications.k18AmplifierEt.inputPy.eatt.auto.get(inputIx = repcap.InputIx.Default) \n
		This command turns automatic selection of the electronic attenuation on and off. If on, electronic attenuation reduces
		the mechanical attenuation whenever possible. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
