...
class State:
	...
	def set(self, state: bool, inputIx=repcap.InputIx.Default) -> None:
		"""SCPI: INPut<ip>:EATT:STATe \n
		Snippet: driver.applications.k18AmplifierEt.inputPy.eatt.state.set(state = False, inputIx = repcap.InputIx.Default) \n
	...
	def get(self, inputIx=repcap.InputIx.Default) -> bool:
		"""SCPI: INPut<ip>:EATT:STATe \n
		Snippet: value: bool = driver.applications.k18AmplifierEt.inputPy.eatt.state.get(inputIx = repcap.InputIx.Default) \n
		This command turns the electronic attenuator on and off. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
