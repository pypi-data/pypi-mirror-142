...
class Next:
	...
	def set(self, window=repcap.Window.Default, marker=repcap.Marker.Default, opc_timeout_ms: int = -1) -> None:
		...
		Snippet: driver.applications.k18AmplifierEt.calculate.marker.maximum.next.set(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		...
		self._core.io.write_with_opc(f'CALCulate{window_cmd_val}:MARKer{marker_cmd_val}:MAXimum:NEXT', opc_timeout_ms)
