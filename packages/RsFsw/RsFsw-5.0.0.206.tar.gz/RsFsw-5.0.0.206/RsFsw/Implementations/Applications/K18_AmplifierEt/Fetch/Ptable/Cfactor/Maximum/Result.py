...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:CFACtor:MAXimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.cfactor.maximum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:CFACtor:MAXimum:RESult?')
