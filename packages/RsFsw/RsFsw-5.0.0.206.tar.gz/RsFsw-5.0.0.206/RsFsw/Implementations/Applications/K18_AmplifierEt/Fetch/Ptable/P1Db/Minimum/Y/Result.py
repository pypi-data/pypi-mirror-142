...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:P1DB:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p1Db.minimum.y.result.get() \n
		These commands query the result values for the 1 dB Compression Point result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P1DB:MINimum:Y:RESult?')
