...
class Right:
	...
	def set(self, window=repcap.Window.Default, marker=repcap.Marker.Default, opc_timeout_ms: int = -1) -> None:
		...
		Snippet: driver.applications.k18AmplifierEt.calculate.marker.maximum.right.set(window = repcap.Window.Default, marker = repcap.Marker.Default) \n
		This command moves a marker to the next lower peak. The search includes only measurement values to the right of the
