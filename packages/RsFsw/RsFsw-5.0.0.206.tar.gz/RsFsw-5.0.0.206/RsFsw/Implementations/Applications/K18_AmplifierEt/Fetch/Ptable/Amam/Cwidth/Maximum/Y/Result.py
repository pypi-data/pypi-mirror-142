...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:AMAM:CWIDth:MAXimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.amam.cwidth.maximum.y.result.get() \n
		These commands query the result values for the AM/AM Curve Width result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:AMAM:CWIDth:MAXimum:Y:RESult?')
