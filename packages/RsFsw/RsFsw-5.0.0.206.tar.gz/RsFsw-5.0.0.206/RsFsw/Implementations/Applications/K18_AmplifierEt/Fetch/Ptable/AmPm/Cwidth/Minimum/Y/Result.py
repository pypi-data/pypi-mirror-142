...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:AMPM:CWIDth:MINimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.amPm.cwidth.minimum.y.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:AMPM:CWIDth:MINimum:Y:RESult?')
