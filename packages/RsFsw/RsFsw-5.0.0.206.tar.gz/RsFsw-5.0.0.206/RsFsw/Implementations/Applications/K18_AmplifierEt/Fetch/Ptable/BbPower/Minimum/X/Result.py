...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:BBPower:MINimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.bbPower.minimum.x.result.get() \n
		These commands query the result values for the Baseband Power (I_cc * V_cc) result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:BBPower:MINimum:X:RESult?')
