...
class Alternate:
	...
	def set(self, value: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.applications.k18AmplifierEt.sense.power.achannel.filterPy.alpha.alternate.set(value = 1.0, channel = repcap.Channel.Default) \n
		This command defines the roll-off factor for the alternate channel weighting filter. \n
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SENSe]:POWer:ACHannel:FILTer:ALPHa:ALTernate<n> \n
		Snippet: value: float = driver.applications.k18AmplifierEt.sense.power.achannel.filterPy.alpha.alternate.get(channel = repcap.Channel.Default) \n
		This command defines the roll-off factor for the alternate channel weighting filter. \n
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Alternate')
