...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:EVM:MAXimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.evm.maximum.y.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:EVM:MAXimum:Y:RESult?')
