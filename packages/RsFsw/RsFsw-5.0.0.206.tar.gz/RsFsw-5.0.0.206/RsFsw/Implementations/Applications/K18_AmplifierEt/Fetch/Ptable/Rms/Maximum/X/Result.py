...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:RMS:MAXimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.rms.maximum.x.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:RMS:MAXimum:X:RESult?')
