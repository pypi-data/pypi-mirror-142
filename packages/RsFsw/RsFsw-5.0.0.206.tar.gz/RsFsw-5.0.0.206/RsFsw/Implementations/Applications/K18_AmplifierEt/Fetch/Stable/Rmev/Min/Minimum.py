...
class Minimum:
	...
	def get(self, query_range: enums.SelectionRangeB) -> float:
		"""SCPI: FETCh:STABle:RMEV:MIN:MINimum \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.stable.rmev.min.minimum.get(query_range = enums.SelectionRangeB.ALL) \n
		No command help available \n
			:param query_range: No help available
			:return: result: No help available"""
		param = Conversions.enum_scalar_to_str(query_range, enums.SelectionRangeB)
		response = self._core.io.query_str(f'FETCh:STABle:RMEV:MIN:MINimum? {param}')
