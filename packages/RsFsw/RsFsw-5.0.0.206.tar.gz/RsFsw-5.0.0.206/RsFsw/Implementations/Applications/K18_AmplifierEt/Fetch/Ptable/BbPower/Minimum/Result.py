...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:BBPower:MINimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.bbPower.minimum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:BBPower:MINimum:RESult?')
