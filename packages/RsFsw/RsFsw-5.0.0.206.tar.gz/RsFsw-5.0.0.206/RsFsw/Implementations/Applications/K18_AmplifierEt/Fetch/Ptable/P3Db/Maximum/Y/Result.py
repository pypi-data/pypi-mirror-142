...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:P3DB:MAXimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.p3Db.maximum.y.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:P3DB:MAXimum:Y:RESult?')
