...
class StDeviation:
	...
	def get(self, power: enums.SelectionRangeB) -> float:
		"""SCPI: FETCh:STABle:BBPower:MIN:STDeviation \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.stable.bbPower.min.stDeviation.get(power = enums.SelectionRangeB.ALL) \n
		No command help available \n
			:param power: No help available
			:return: result: No help available"""
		param = Conversions.enum_scalar_to_str(power, enums.SelectionRangeB)
		response = self._core.io.query_str(f'FETCh:STABle:BBPower:MIN:STDeviation? {param}')
