...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:RMS:MINimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.rms.minimum.x.result.get() \n
		These commands query the result values for the RMS Power result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:RMS:MINimum:X:RESult?')
