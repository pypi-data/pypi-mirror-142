...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:BBPower:MAXimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.bbPower.maximum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:BBPower:MAXimum:RESult?')
