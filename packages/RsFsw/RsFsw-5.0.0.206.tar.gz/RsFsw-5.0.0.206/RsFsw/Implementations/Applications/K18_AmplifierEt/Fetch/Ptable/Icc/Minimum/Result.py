...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:ICC:MINimum[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.icc.minimum.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:ICC:MINimum:RESult?')
