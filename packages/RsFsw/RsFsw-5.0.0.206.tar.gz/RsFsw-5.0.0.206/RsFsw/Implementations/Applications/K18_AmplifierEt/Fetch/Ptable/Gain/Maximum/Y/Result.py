...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:GAIN:MAXimum:Y[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.gain.maximum.y.result.get() \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:GAIN:MAXimum:Y:RESult?')
