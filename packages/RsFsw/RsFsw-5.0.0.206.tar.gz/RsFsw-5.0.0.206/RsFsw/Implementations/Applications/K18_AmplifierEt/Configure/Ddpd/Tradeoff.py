...
class Tradeoff:
	...
	def set(self, power_linearity_tradeoff: float) -> None:
		"""SCPI: CONFigure:DDPD:TRADeoff \n
		Snippet: driver.applications.k18AmplifierEt.configure.ddpd.tradeoff.set(power_linearity_tradeoff = 1.0) \n
		...
		self._core.io.write(f'CONFigure:DDPD:TRADeoff {param}')

	def get(self) -> float:
		"""SCPI: CONFigure:DDPD:TRADeoff \n
		Snippet: value: float = driver.applications.k18AmplifierEt.configure.ddpd.tradeoff.get() \n
		This command defines the power / linearity tradeoff for direct DPD calculation.
			INTRO_CMD_HELP: Prerequisites for this command \n
			- Turn on direct DPD (method RsFsw.Applications.K18_AmplifierEt.Configure.Ddpd.State.set) . \n
			:return: power_linearity_tradeoff: numeric value Unit: PCT"""
		response = self._core.io.query_str(f'CONFigure:DDPD:TRADeoff?')
