...
class Result:
	...
	def get(self) -> float:
		"""SCPI: FETCh:PTABle:ACP:MINimum:X[:RESult] \n
		Snippet: value: float = driver.applications.k18AmplifierEt.fetch.ptable.acp.minimum.x.result.get() \n
		These commands query the result values for the ACP result as shown in the Parameter Sweep Table. \n
		...
		response = self._core.io.query_str(f'FETCh:PTABle:ACP:MINimum:X:RESult?')
