...
class Time:
	...
	def set(self, time: float) -> None:
		...
		Snippet: driver.applications.k17Mcgd.trigger.sequence.holdoff.time.set(time = 1.0) \n
	...
	def get(self) -> float:
		"""SCPI: TRIGger[:SEQuence]:HOLDoff[:TIME] \n
		Snippet: value: float = driver.applications.k17Mcgd.trigger.sequence.holdoff.time.get() \n
		Defines the time offset between the trigger event and the start of the sweep . \n
