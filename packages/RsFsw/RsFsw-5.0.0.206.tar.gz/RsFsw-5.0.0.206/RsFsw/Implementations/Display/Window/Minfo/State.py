...
class State:
	...
	def set(self, state: bool, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.display.window.minfo.state.set(state = False, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> bool:
		"""SCPI: DISPlay[:WINDow<n>]:MINFo[:STATe] \n
		Snippet: value: bool = driver.display.window.minfo.state.get(window = repcap.Window.Default) \n
		This command turns the marker information in all diagrams on and off. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
