...
class Utilities:
	...
	Access snippet: ``utils = RsFsw.utilities``"""
	...
	def logger(self) -> ScpiLogger:
		...
		Access snippet: ``logger = RsFsw.utilities.logger``"""
	...
	def get_lock(self) -> threading.RLock:
		...
		By default:
			- If you create standard new RsFsw instance with new VISA session, the session gets a new thread lock. You can assign it to other RsFsw sessions in order to share one physical instrument with a multi-thread access.
			- If you create new RsFsw from an existing session, the thread lock is shared automatically making both instances multi-thread safe.
