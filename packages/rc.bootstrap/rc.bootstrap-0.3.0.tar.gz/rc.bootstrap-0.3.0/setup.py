...
__email__     = 'andre.merzky@radical-consulting.com'
...
def get_version(_mod_root):
    ...
    try:
        ...
        # store version in `bin/rc_bootstrap.sh` before install
        with open('bin/rc_bootstrap.sh.in', 'r') as fin:
            with open('bin/rc_bootstrap.sh', 'w') as fout:
                for line in fin.readlines():
                    line = line.replace('###RC_BOOTSTRAP_VERSION###',
                                        _version_base)
                    fout.write(line)

        os.chmod('bin/rc_bootstrap.sh', 0o755)
...
setup_args = {
    ...
    'version'            : version,
    'description'        : 'RC bootstrapper',
    'author'             : 'Andre Merzky',
    'author_email'       : 'andre.merzky@radical-consulting.com',
    'maintainer'         : 'RADICAL-Consulting UG',
    'maintainer_email'   : 'devel@radical-consulting.com',
    'url'                : 'https://www.github.com/radical-consulting/rc.bootstrap/',
    'license'            : 'AGPL',
    'keywords'           : 'utilities',
    'python_requires'    : '>=3.6',
    'classifiers'        : [
    ...
    'packages'           : find_namespace_packages('src', include=['rc.*']),
    'package_dir'        : {'': 'src'},
    'scripts'            : [
                            'bin/rc_bootstrap.sh'],
...
if sdist_level == 0:
    ...
    os.system('rm -vf  bin/rc_bootstrap.sh')
