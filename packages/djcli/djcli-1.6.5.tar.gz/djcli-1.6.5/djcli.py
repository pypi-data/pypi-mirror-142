...
def settings():
    ...
    def find_manage(current):
        dirs = []
        for item in pathlib.Path(current).iterdir():
            if item.name == 'manage.py':
                return item
            elif item.is_dir():
                dirs.append(item)
        for dirpath in dirs:
            result = find_manage(dirpath)
            if result:
                return result

    found = find_manage(os.getcwd())

    if '.' not in sys.path:
        sys.path.insert(0, os.path.dirname(found.absolute()))
...
def save(modelname, **kwargs):
    ...
    Use field=value to set values.
    Use +field=value to set WHERE filter.

    Example to create user or update their email by username:
