...
class ProcesarEncuestas : 
    ...
    def registroEjecucion(self) :
        ...
        registro["total_tablas"] = self.importarDatos.estadisticas.obtenerTotalTablas() if self.importarDatos else 0
        registro["total_encuestas"] = self.importarDatos.estadisticas.obtenerNumeroEstadisticas() if self.importarDatos else 0
        registro["total_creados"] = self.importarDatos.estadisticas.obtenerTotalEstadisticas("C") if self.importarDatos else 0
        registro["total_actualizados"] = self.importarDatos.estadisticas.obtenerTotalEstadisticas("U") if self.importarDatos else 0
        registro["total_no_actualizados"] = self.importarDatos.estadisticas.obtenerTotalEstadisticas("NU") if self.importarDatos else 0
        registro["total_errores"] = self.importarDatos.estadisticas.obtenerTotalEstadisticas("E") if self.importarDatos else 0
        registro["total_registros"] = self.importarDatos.estadisticas.obtenerTotalRegistros() if self.importarDatos else 0
        registro["total_operaciones"] = self.importarDatos.estadisticas.obtenerTotalOperaciones() if self.importarDatos else 0
        registro["inicio"] = "{}".format(self.importarDatos.estadisticas.timer.startTime) if self.importarDatos else ""
        registro["fin"] = "{}".format(self.importarDatos.estadisticas.timer.endTime) if self.importarDatos else ""
        registro["duracion_total"] = "{}".format(self.importarDatos.estadisticas.timer.timeSpan) if self.importarDatos else ""

        if self.importarDatos and self.importarDatos.estadisticas.obtenerTotalRegistros() > 0:
