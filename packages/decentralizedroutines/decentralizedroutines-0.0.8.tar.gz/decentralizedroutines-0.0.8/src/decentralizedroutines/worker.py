...
if 'GIT_TOKEN' in os.environ:
    ...
    raise ValueError('GIT_TOKEN not found')

if 'GIT_ACRONYM' in os.environ:
    GIT_ACRONYM=os.environ['GIT_ACRONYM']
else:
    raise ValueError('GIT_ACRONYM not found')


GIT_URL='https://'+GIT_TOKEN+'@github.com/'+GIT_ACRONYM+'/'
            ...
            if ((command['target']==username) | (command['target']=='ALL')):
                if command['job'] == 'command':                    
                    ...
                    if repo_exists:                 
                        ...
                        # pull existing repo   
                        cmd = ['git','-C',str(repo_path.parents[0]),'pull',GIT_URL+command['repo']]
