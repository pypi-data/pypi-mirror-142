...
def recvsize(sock, size: int):
    print(f"Receiving {size} bytes...")
    data = b""
    size = size - 24 # Account for the 24 bytes of overhead
    counter = 1
    while True:
        part = sock.recv(4096)
        data += part
...
def first_time_config():
    ...
    print(
        f"Your secret key can be found in {appdata}{SEPARATOR}server.ini. Only share it with people you trust!\n"
    )
        ...
        if command[0] == "pwd":
            ...
            if os.path.exists(directory):
                if os.path.isdir(directory):
                    conn.sendall(
                        user_safe.encrypt_cmd_resp(f"Directory changed to {directory}")
                    )
                else:
                    conn.sendall(
                        user_safe.encrypt_cmd_resp(f"{directory} is not a directory")
                    )
                    directory = old_dir
            else:
            ...
            size = int(command[1]) - 24 # Account for the 24 bytes of overhead
            command = recvsize(conn, size)
            command = user_safe.decrypt_cmd(command)
            data = bytes.fromhex(command[2])
            with open(f"{directory}{SEPARATOR}{command[1]}", "wb") as f:
                f.write(data)
            conn.sendall(
                user_safe.encrypt_cmd_resp(f"{command[1]} uploaded successfully")
            )
        elif command[0] == "get":
            if os.path.exists(f"{directory}{SEPARATOR}{command[1]}"):
                if not os.path.isdir(f"{directory}{SEPARATOR}{command[1]}"):
                    with open(f"{directory}{SEPARATOR}{command[1]}", "rb") as f:
                        data = f.read()
                    data = user_safe.encrypt_cmd_resp(f"{command[1]}, {data.hex()}")
                    size = sys.getsizeof(data)
                    logger.info(f"Sending size of file packet to {addr[0]}")
                    conn.sendall(user_safe.encrypt_cmd_resp(f"{size}"))
                    logger.info(f"Sending file packets to {addr[0]}")
                    conn.sendall(data)
            else:
