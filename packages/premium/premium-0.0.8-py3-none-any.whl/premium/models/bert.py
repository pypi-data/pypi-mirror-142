...
class BertClassifier(object):
    def __init__(self,
                 ...
                 layer_number: int = 3,
                 bert_name: str = 'bert-large-uncased',
    ...
    def get_tokenizer(self):
        if self.bert_name in ('bert-large-uncased', 'bert-base-uncased'):
            ...
            return DistilBertTokenizer.from_pretrained(self.bert_name)
        else:
    ...
    def get_pretrained_model(self):
        ...
        if self.bert_name in ('bert-large-uncased', 'bert-base-uncased'):
            return TFBertModel.from_pretrained(self.bert_name, config=config)
        elif self.bert_name in ('roberta-large', 'roberta-base'):
    ...
    def fit(self,
        ...
        if batch_size == -1:
            ...
            def _run_model_once(__batch_size):
                model.fit([ids, masks],
                          y,
            ...
            while low < high:
                try:
                    ...
                    _run_model_once(batch_size)
                except timeout_decorator.timeout_decorator.TimeoutError:
                    low = batch_size + 1
                    batch_size = (low + high) // 2
                    cf.info(
                        f'\nbatch size {low} is SOLID, try increase it to {batch_size}'
                    )
                except Exception as e:
                    high = batch_size - 1
                    batch_size = (low + high) // 2
                    cf.warning(
                        f'\nbatch size {high} is too LARGE, reduce it to {batch_size}'
                    )
                    cf.warning('Error message:', e)
                else:
                    break
