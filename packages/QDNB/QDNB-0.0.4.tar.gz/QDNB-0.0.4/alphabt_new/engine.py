...
class BackTestingEngine:
    ...
    def multidim_backtest(self, shift=None, benchmark=None, groups=None, plotit=False,
                          perf_cal_return="gross_return", name=None, save_to: str=None, fig_size=(12, 12)):
    ...
    def combo_backtest(self, shift=None, benchmark=None, plotit=False, perf_cal_return="gross_return",
                       name=None, save_to=None, fig_size=(12, 12)):
        if shift is None:
            shift = self.ctx.shift
        if benchmark is None:
            benchmark = self.ctx.benchmark
        ...
        if plotit:
            self.plot_comb_pnl(l_pnl, lsidx_pnl, name=name, size=fig_size, save_to=save_to)
    ...
    @staticmethod
    def plot_pnl(ls, lsidx, gprtr, name=None, size=(12, 12), save_to=None):
        ...
        ax3.set_title("group")
        if name:
            plt.title(f"{name} Perf")
        if save_to:
            plt.savefig(str(Path(save_to) / "fig.png"))
        plt.show()

    @staticmethod
    def plot_comb_pnl(l_pnl, lsidx_pnl, name=None, size=(12, 12), save_to=None):
        dateFmt = mdate.DateFormatter('%Y%m%d')
        plt.figure(figsize=size)
