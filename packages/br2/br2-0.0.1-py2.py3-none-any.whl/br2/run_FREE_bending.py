...
def main():
    ...
    # Simulation
    while True:
        time, systems, _, done, steady_state = env.step(action, time)

        if steady_state:
            action += 5
            env.save_data(os.path.join(PATH, 'bending_{}.npz'.format(int(action))))
            print('done: {}'.format(action))
        
        if time > next_save_time:
            next_save_time += save_interval
            env.post_processing(
                filename_video="bending_quasi_static",
                save_folder=PATH,
    ...
    if SLACK:
        ...
        response = slack_client.chat_postMessage(
            ...
            text='\n'.join([
                'Simulation Finished :',
                '  Program name - single FREE quasi-static bending',
