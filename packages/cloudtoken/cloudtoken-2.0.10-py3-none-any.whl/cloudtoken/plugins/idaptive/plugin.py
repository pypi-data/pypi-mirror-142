...
class Plugin(CloudtokenPlugin):
    ...
    def _perform_appclick(self):
        ...
        saml_response = self._get_saml_response_from_html(response)
        return saml_response

    def _get_saml_response_from_html(self, response):
        # https://docs.python-guide.org/scenarios/scrape/
        tree = html.fromstring(response.content)
        try:
            saml_response = tree.xpath("//input[@name='SAMLResponse']")[0].value
            # self._logger.debug(f"Retrieved SAMLResponse {base64.b64decode(saml_response)}")
        except IndexError:
            self._logger.debug(f"Expected page containing SAMLResponse, instead received {response.text}")
