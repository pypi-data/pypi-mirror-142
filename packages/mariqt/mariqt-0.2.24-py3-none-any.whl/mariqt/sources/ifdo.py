...
class iFDO_Reader:
    ...
    def writeWorldFilesForPhotos(self,destDir:str,imageSourceDir:str):
        ...
        for photo in positionsLatLon:
            try:
                ...
                focalLenghPixels, msg = self.__tryGetFocalLengthInPixels(photo)
                ...
                iFDOexceptions.append([photo,e.args])
    ...
    def checkItemHash(self,item:str,fileDir:str):
        ...
        if not itemEntry['image-hash-sha256'] == miqtc.sha256HashFile(file):
            print(miqtc.sha256HashFile(file),file)
            raise miqtc.IfdoException(item, "iFDO entry is not up to date, image hash does not match.")


    def __tryGetFocalLengthInPixels(self,item:str,domePort=True):
        """ 
        ...
        if domePort = False, falt port is assumed and a correction factor of 1.33 is applied
        ...
        try:
            ...
            return [e*correctionFkt for e in focalLengthXY], "Parsed from 'image-camera-calibration-model'['calibration-focal-length-xy-pixel']"
    ...
    def __tryDetermineFocalLenghtInPixelsFromExif_2(self,exifDict:dict):
        ...
        print("focalLengthEq35mmFloat",focalLengthEq35mmFloat)
        print("scaleFkt",scaleFkt)
