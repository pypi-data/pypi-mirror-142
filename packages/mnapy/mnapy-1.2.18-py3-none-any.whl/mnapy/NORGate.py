...
class NORGate:
    def __init__(
        ...
        self.option_limits = NORGateLimits.NORGateLimits(
    ...
    def vout_nor(self, ui: List[float]) -> float:
        sum = 0
        N = 2
        size = len(ui)
    ...
    def partial_nor(self, terminal: float, ui: List[float], ui_prime: List[float]) -> float:
        sum = 0
        N = 2
        size = len(ui)
    ...
    def update(self):
        if self.context.Params.SystemFlags.FlagSimulating and self.context.solutions_ready:
            ...
            self.V_out = self.vout_nor([self.V_1, self.V_2])
            self.V_partial1 = Utils.Utils.limit(
                self.partial_nor(0, [self.V_1, self.V_2], [self.V_1_prime, self.V_2_prime]),
                0.0,
                1.0
            )
            self.V_partial2 = Utils.Utils.limit(
                self.partial_nor(1, [self.V_1, self.V_2], [self.V_1_prime, self.V_2_prime]),
                0.0,
                1.0
            )
    ...
    def stamp(self):
        self.context.stamp_gate2(self.Nodes[2], self.V_partial1, self.V_partial2, self.V_eq, self.context.ELEMENT_NOR_OFFSET + self.SimulationId)
