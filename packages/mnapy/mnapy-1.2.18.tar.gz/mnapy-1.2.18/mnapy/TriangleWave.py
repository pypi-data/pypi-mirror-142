...
class TriangleWave:
    def __init__(
        ...
        self.option_limits = TriangleWaveLimits.TriangleWaveLimits(
    ...
    def stamp(self) -> None:
        ...
        self.context.stamp_voltage(
            ...
            self.Nodes[1],
            self.Offset
            + ((2 * self.Voltage) / math.pi)
            * math.asin(
                math.sin(
                    2 * self.context.simulation_time * math.pi * self.Frequency
                    + math.radians(self.Phase)
                )
            ),
            self.context.ELEMENT_TRI_OFFSET + self.SimulationId,
        )
