...
class Zoom(Craft):
    def __init__(self, scale: tuple, interp=LINEAR, aligned=True, scope='ZOOM'):
    ...
    def run(self, x):
        y = nn.functional.interpolate(x, scale_factor=self.scale, mode=self.mode, align_corners=self.aligned)
        return y
