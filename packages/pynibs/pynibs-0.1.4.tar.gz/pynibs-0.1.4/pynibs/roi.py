...
def get_sphere_in_nii(center, radius, nii=None, out_fn=None,
    ...
    if outside_radius != np.inf:
        # searach space is whole nifti space
        x_min, y_min, z_min = nib.affines.apply_affine(nii.affine, [0, 0, 0]).astype(int)
        x_max, y_max, z_max = nib.affines.apply_affine(nii.affine, list(nii.shape[:3])).astype(int)
    else:
        # if outside_val should not be set we only need to search within center-radius
        x_min, y_min, z_min = center - radius
        x_max, y_max, z_max = center + radius

    # speed up the long loop below
    affine_inv = np.linalg.inv(nii.affine)
    apply_affine = nib.affines.apply_affine
    norm = np.linalg.norm
    ...
    for xyz in tqdm.tqdm(product(range(x_min, x_max), range(y_min, y_max), range(z_min, z_max)),
        ...
        dist = norm(center - xyz)
        if dist < radius:
            vox_x, vox_y, vox_z = apply_affine(affine_inv, xyz).astype(int)
            roi_data[vox_x, vox_y, vox_z] = val_in
        elif dist > outside_radius:
            # Set values outside of outside_radius to outside_val
            vox_x, vox_y, vox_z = apply_affine(affine_inv, xyz).astype(int)
            roi_data[vox_x, vox_y, vox_z] = outside_val
