...
# added eeg_raw to the spec which would include the raw eeg timeseries at 100Hz
    ...
    if np.sum(data_exists[8:11]) != 0:
        if eeg_exist:
            ...
            # alpha 
            eeg_bands[:, 0] = np.sum(abs(Sxx[20:32, 1:])* np.sum(window_256), axis=0) 
            # theta 
            eeg_bands[:, 1] = np.sum(abs(Sxx[16:21, 1:]) * np.sum(window_256), axis=0) 
            # spindle 
            eeg_bands[:, 2] = np.sum(abs(Sxx[31:39, 1:]) * np.sum(window_256), axis=0) 
            # other 
            eeg_bands[:, 3] = np.sum(abs(Sxx[41:103, 1:]) * np.sum(window_256), axis=0) 
    ...
    header = '{"version":"PSG", "revision": "3", "duration": '+ str(int(totalEpochs*30)) + ', "url":"' + urlSafeHash + '", "spec":' + json.dumps(list(spec.keys())) + '}'
    header = '{:<256}'.format(header[:256])
