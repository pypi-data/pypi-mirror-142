    ...
    def get_by_id(self, org_id=None):
        ...
        if org_id is not None:
            response = requests.get(
                url=self.get_by_id_endpoint.format(base_url=self.get_fhir_url(), id=org_id),
...
class LocationFHIR(BaseFHIR):
    ...
    def get_by_id(self, location_id=None):
        ...
        if location_id is not None:
            response = requests.get(
                url=self.get_by_id_endpoint.format(base_url=self.get_fhir_url(), id=location_id),
                headers=self.headers,
                verify=self.validate_certs
            )
            if response.status_code == 200:
...
class PractitionerFHIR(BaseFHIR):
    ...
    get_by_id_endpoint = practitioner_endpoint + "/{id}"
