...
class ModMailManager(DatabaseChecker):
    def __init__(self, bot: Union[commands.Bot, commands.AutoShardedBot], trigger: str, embed_color = discord.Color.blurple):
        self.bot = bot
    ...
    async def get_message(self, ctx: commands.Context) -> str:
        ...
        embed = discord.Embed(
            title="ModMail", description="Please type your message to the Mods.", color=self.embed_color
        )
        await ctx.send(embed=embed)
