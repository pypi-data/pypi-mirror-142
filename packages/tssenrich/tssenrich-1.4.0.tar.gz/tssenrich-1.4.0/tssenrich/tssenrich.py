...
def samtools_bedcov(bed_file_path, bam_file_path, memory_gb: float = 1.0,
    ...
    with tempfile.TemporaryDirectory(dir=temp_dir) as temp_dir_name:
        ...
        with open(sorted_path, 'wb') as temp_sorted:
            subprocess.run((samtools_path, 'sort',
                '-m', '{}M'.format(int(1024 / threads * memory_gb)),
                '-@', str(threads),
                '-T', str(temp_dir or tempfile.gettempdir()),
                bam_file_path), stdout=temp_sorted, stderr=log_file)
            subprocess.run((samtools_path, 'index', sorted_path))
        with subprocess.Popen(
            (samtools_path, 'bedcov', '-Q', str(mapping_quality),
                bed_file_path, sorted_path),
            stdout=subprocess.PIPE, stderr=log_file) as bedcov:
...
def tss_enrichment(bam_file_path, genome='hg38', memory_gb: float = 1.0,
    ...
    return calculate_enrichment(generate_coverage_values(samtools_bedcov(
        tss_flanks.fn, bam_file_path, memory_gb=memory_gb,
        threads=threads, mapping_quality=mapping_quality,
        samtools_path=samtools_path, log_file_path=log_file_path,
        temp_dir=temp_dir)))
...
def parse_arguments():
    ...
    parser.add_argument('--genome', metavar='{hg38, hg19, mm10, </path/to/file.bed>}',
