...
def fast_local_function(data,mesh,terminal,edge,gamma,nu,Qterm,Pperm,Pterm):
    ...
    alt_scale           = []
    ####################
    total_flow          = data[edge,22]+Qterm*2
    upstream            = data[edge,0:3]
    downstream          = data[edge,3:6]
    upstream_length     = np.linalg.norm(mesh-upstream,      axis=1)
    downstream_length   = np.linalg.norm(mesh-downstream,    axis=1)
    terminal_length     = np.linalg.norm(mesh-terminal,      axis=1)
