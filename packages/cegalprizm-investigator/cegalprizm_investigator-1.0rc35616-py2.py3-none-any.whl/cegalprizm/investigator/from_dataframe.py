...
_WELL_KNOWN_DATASET_COLUMN_NAME = "Dataset"
    ...
    for col in dataframe.columns.values:
        ...
        if dataframe.dtypes[col] == np.object and col not in _WELL_KNOWN_SAMPLE_INDEX_COLUMN_NAMES:
            if dataset_column_name is None:
                if col == _WELL_KNOWN_DATASET_COLUMN_NAME:
                    guess_dataset_column_name = _WELL_KNOWN_DATASET_COLUMN_NAME
            elif col != dataset_column_name:
                guess_discrete_names.append(col)
        if dataframe.dtypes[col] == np.int32 or dataframe.dtypes[col] == np.int64:
