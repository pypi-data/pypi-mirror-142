...
class RabbitMQConsumer(object):
    ...
    def base64_to_json(self, body):
        """
        base64编码的字节 解码成字符串后, 再json格式化成object
        """
    ...
    @staticmethod
    def _get_jstr_from_cls(result, cls):
        """
        按照自定义PythonObjectEncoder进行json格式化, 若错误则同时返回异常, 若正常则同时返回空字符串
        """
        try:
            json_result = json.dumps(result, cls=cls)
            _t = ''
        except:
            json_result = str(result) if result else ''
            _t = traceback.format_exc()
        return json_result, _t
    ...
    def do_work(self, connection, channel, method_frame, header_frame, body):
        ...
        try:
            ...
            # json格式化result
            json_result, _traceback_info = self._get_jstr_from_cls(result, cls=PythonObjectEncoder)
            if json_result == 'false':
                kwargs = {
                    "status": "FAILURE",
                    "result": json_result,
                    "traceback": _traceback_info,
                }
            else:
                kwargs = {
                    "status": "SUCCESS",
    ...
    def update_task_result(self, task_id, consumer, **kwargs):
        ...
        for _ in range(self._retry_times):
            try:
                ...
                # 成功不发送消息执行结果?? status=False发送 暂不改
                rest_client.post(self._message_backend_svc, api_path=self._message_backend_api, json=data)
                logger.info(
