...
from quo.types import File, Path
from pymysql import OperationalError
from cli_helpers.tabular_output import TabularOutputFormatter
from cli_helpers.tabular_output import preprocessors
from cli_helpers.utils import strip_ansi
import sqlparse
from piedpy.packages.parseutils import is_dropping_database, is_destructive
from quo.console import app, arg, command
from quo.completion import DynamicCompleter
from quo.enums import DEFAULT_BUFFER, EditingMode
from quo.i_o.termui import scrollable as echo_via_pager
from quo.keys.key_binding.bindings.named_commands import register as prompt_register
from quo.prompt import Prompt, CompleteStyle
from quo.document import Document
from quo.filters import HasFocus, IsDone
from quo.text import ANSI
from quo.layout.processors import (HighlightMatchingBracketProcessor, ConditionalProcessor)
...
try:
    ...
    from piedpy.packages.paramiko_stub import paramiko
...
class MyCli():
    ...
    system_config_files = [
        '/etc/piedpyrc',
        os.path.join(os.path.expanduser(xdg_config_home), "piedpy", "piedpyrc")
    ]

    pwd_config_file = os.path.join(os.getcwd(), ".piedpyrc")
    ...
    def __init__(self, sqlexecute=None, prompt=None,
        ...
        if c.filename not in self.system_config_files and not os.path.exists(piedpyrc):
            write_default_config(piedpyrc)
    ...
    def initialize_logging(self):
        ...
        root_logger = logging.getLogger('piedpy')
        root_logger.addHandler(handler)
        root_logger.setLevel(level_map[log_level.upper()])

        logging.captureWarnings(True)

        root_logger.debug('Initializing piedpy logging.')
    ...
    def run_cli(self):
        ...
        history_file = os.path.expanduser(
            os.environ.get('MYCLI_HISTFILE', '~/.piedpy-history'))
            ...
            if buf:
                if output_via_pager:
                    def newlinewrapper(text):
                        for line in text:
                            yield line + "\n"
                    echo_via_pager(newlinewrapper(buf))
                else:
                    for line in buf:
                        echo(line)
...
@command()
@app('-h', '--host', envvar='MYSQL_HOST', help='Host address of the database.')
@app('-P', '--port', envvar='MYSQL_TCP_PORT', type=int, help='Port number to use for connection. Honors ' '$MYSQL_TCP_PORT.')
@app('-u', '--user', help='User name to connect to the database.')
@app('-S', '--socket', envvar='MYSQL_UNIX_PORT', help='The socket file to use for connection.')
@app('-p', '--password', 'password', envvar='MYSQL_PWD', type=str, help='Password to connect to the database.')
@app('--pass', 'password', envvar='MYSQL_PWD', type=str, help='Password to connect to the database.')
@app('--ssh-user', help='User name to connect to ssh server.')
@app('--ssh-host', help='Host name to connect to ssh server.')
@app('--ssh-port', default=22, help='Port to connect to ssh server.')
@app('--ssh-password', help='Password to connect to ssh server.')
@app('--ssh-key-filename', help='Private key filename (identify file) for the ssh connection.')
@app('--ssh-config-path', help='Path to ssh configuration.',
              default=os.path.expanduser('~') + '/.ssh/config')
@app('--ssh-config-host', help='Host to connect to ssh server reading from ssh configuration.')
@app('--ssl-ca', help='CA file in PEM format.', type=Path(exists=True))
@app('--ssl-capath', help='CA directory.')
@app('--ssl-cert', help='X509 cert in PEM format.', type=Path(exists=True))
@app('--ssl-key', help='X509 key in PEM format.', type=Path(exists=True))
@app('--ssl-cipher', help='SSL cipher to use.')
@app('--ssl-verify-server-cert', is_flag=True, help=('Verify server\'s "Common Name" in its cert against '
...
@app('-V', '--version', is_flag=True, help='Output piedpy\'s version.')
@app('-v', '--verbose', is_flag=True, help='Verbose output.')
@app('-D', '--database', 'dbname', help='Database to use.')
@app('-d', '--dsn', default='', envvar='DSN', help='Use DSN configured into the [alias_dsn] section of myclirc file.')
@app('--list-dsn', 'list_dsn', is_flag=True, help='list of DSN configured into the [alias_dsn] section of piedpyrc file.')
@app('--list-ssh-config', 'list_ssh_config', is_flag=True, help='list ssh configurations in the ssh config (requires paramiko).')
@app('-R', '--prompt', 'prompt',
              help='Prompt format (Default: "{0}").'.format(
                  MyCli.default_prompt))
@app('-l', '--logfile', type=File(mode='a', encoding='utf-8'),
              help='Log every query and its results to a file.')
@app('--defaults-group-suffix', type=str,
              help='Read MySQL config groups with the specified suffix.')
@app('--defaults-file', type=Path(),
              help='Only read MySQL options from the given file.')
@app('--piedpyrc', type=Path(), default="~/.piedpyrc",
              help='Location of piedpyrc file.')
@app('--auto-vertical-output', is_flag=True,
              help='Automatically switch to vertical output mode if the result is wider than the terminal width.')
@app('-t', '--table', is_flag=True,
              help='Display batch output in table format.')
@app('--csv', is_flag=True,
              help='Display batch output in CSV format.')
@app('--warn/--no-warn', default=None,
              help='Warn before running a destructive query.')
@app('--local-infile', type=bool,
              help='Enable/disable LOAD DATA LOCAL INFILE.')
@app('-g', '--login-path', type=str,
              help='Read this path from the login file.')
@app('-e', '--execute',  type=str,
              help='Execute command and quit.')
@app('--init-command', type=str,
              help='SQL statement to execute after connecting.')
@app('--charset', type=str,
              help='Character set for MySQL session.')
@app('--password-file', type=Path(), help='File or FIFO path containing the password to connect to the db if not specified otherwise.')
@arg('database', default='', nargs=1)

def cli(database, user, host, port, socket, password, dbname, version, verbose, prompt, logfile, defaults_group_suffix, defaults_file, login_path, auto_vertical_output, local_infile, ssl_ca, ssl_capath, ssl_cert, ssl_key, ssl_cipher,  ssl_verify_server_cert, table, csv, warn, execute, piedpyrc, dsn, list_dsn, ssh_user, ssh_host, ssh_port, ssh_password, ssh_key_filename, list_ssh_config, ssh_config_path, ssh_config_host, init_command, charset, password_file):
    ...
    piedpy = MyCli(prompt=prompt, logfile=logfile,
    ...
    if list_dsn:
        try:
            alias_dsn = piedpy.config['alias_dsn']
        except KeyError as err:
            echo('Invalid DSNs found in the config file. '\
                'Please check the "[alias_dsn]" section in piedpyrc.',
                 err=True, fg='vred')
            exit(1)
        except Exception as e:
            echo(str(e), err=True, fg='red')
            exit(1)
        for alias, value in alias_dsn.items():
            if verbose:
                echo("{} : {}".format(alias, value))
            else:
                echo(alias)
        sys.exit(0)
    if list_ssh_config:
    ...
    if (piedpy.config['alias_dsn'] and database and '://' not in database
    ...
    if dsn:
        try:
            dsn_uri = piedpy.config['alias_dsn'][dsn]
        except KeyError:
            echo('Could not find the specified DSN in the config file. '
                        'Please check the "[alias_dsn]" section in your '
                        'piedpyrc.', err=True, fg='red')
            exit(1)
        else:
            piedpy.dsn_alias = dsn
    ...
    piedpy.logger.debug('Launch Params: \n'
            '\tdatabase: %r'
            '\tuser: %r'
            '\thost: %r'
    ...
    if execute:
        try:
            if csv:
                piedpy.formatter.format_name = 'csv'
            elif not table:
                piedpy.formatter.format_name = 'tsv'

            piedpy.run_query(execute)
            exit(0)
        except Exception as e:
            echo(str(e), err=True, fg='red')
            exit(1)
    ...
    if sys.stdin.isatty():
        ...
        stdin = textstream('stdin')
        try:
            stdin_text = stdin.read()
        except MemoryError:
            echo('Failed! Ran out of memory.', err=True, fg='red')
            echo('You might want to try the official mysql client.', err=True, fg='red')
            echo('Sorry... :(', err=True, fg='red')
            exit(1)

        if piedpy.destructive_warning and is_destructive(stdin_text):
            try:
                sys.stdin = open('/dev/tty')
                warn_confirmed = confirm_destructive_query(stdin_text)
            except (IOError, OSError):
                piedpy.logger.warning('Unable to open TTY as stdin.')
        ...
        try:
            ...
            piedpy.run_query(stdin_text, new_line=new_line)
            exit(0)
        except Exception as e:
            echo(str(e), err=True, fg='red')
            exit(1)
...
def thanks_picker():
    import piedpy
    lines = (
        resources.read_text(piedpy, 'AUTHORS') +
        resources.read_text(piedpy, 'SPONSORS')
    ).split('\n')
