...
from piedpy.packages.prompt_utils import confirm_destructive_query
...
def write_tee(output):
    global tee_file
    if tee_file:
        echo(output, file=tee_file, nl=False)
...
def write_once(output):
    ...
    if output and once_file:
        echo(output, file=once_file, nl=False)
...
def write_pipe_once(output):
    ...
    if output and pipe_once_process:
        try:
            echo(output, file=pipe_once_process.stdin, nl=False)
