...
class ConstraintAttr:
    def __init__(
        self, name: str = None, type=None, inheritable: bool = True, choices=None
    ):
        """
    ...
    def check(self, plugin):
        ...
        # Check choices
        if self.choices:
            if plugin_attr not in self.choices:
                raise ValueError(
                    f"class attribute {self.name} must be in {self.choices!r}"
                )
...
class PluginManager(KoalakContainer):
    ...
    def attr(self, type=None, inheritable=True, choices=None):
        return ConstraintAttr(type=type, inheritable=inheritable, choices=choices)
