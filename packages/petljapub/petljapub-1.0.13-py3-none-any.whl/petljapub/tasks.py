...
def run(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1):
    """
    ...
    task = Task(os.getcwd())
    if example >= 1:
        task.run(solution(sol), lang, testcase=task.testcase_path("example", example), output="stdout")
    elif generated >= 1:
        task.run(solution(sol), lang, testcase=task.testcase_path("generated", generated), output="stdout")
    elif crafted >= 1:
        task.run(solution(sol), lang, testcase=task.testcase_path("crafted", crafted), output="stdout")
    else:
        logger.info("Running interactively, enter input:")
        task.run_interactive(solution(sol), lang)
    
@task
def r(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1):
    """
    Run a solution (shortcut for 'petljapub run')
    """
    run(ctx, lang, sol, testcase, example, generated, crafted)
...
def test(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, recompile=True, timeout=1.0, verbosity=3):
    """
...
@task
def t(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, recompile=True, timeout=1.0, verbosity=3):
    """
...
@task(help={'lang': "Test just this language",
            ...
            'verbosity': "Level of messages printed to the user (from 0/quiet to 5/full)"})
def test_all(ctx, lang=None, recompile=True, timeout=1.0, verbosity=3):
    """
...
def yaml_tex(ctx, yaml, dst, tasks_dir=None, pub_dir=None, header='header.md', tex_template='default.latex', standalone=True, lat=False, tex=True):
    """
