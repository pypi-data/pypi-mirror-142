...
class ValueCache:
    ...
    def _get_or_value_from_cache(
        ...
        """Load data"""
        if (global_value := self._client.central.device_data.get_device_data(interface=self._client.name, channel_address=channel_address, parameter=parameter)) is not None:
            return global_value
