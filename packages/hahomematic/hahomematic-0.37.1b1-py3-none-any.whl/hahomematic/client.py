...
from hahomematic.const import (
    ...
    PROXY_INIT_SUCCESS,
    HmEventType,
    HmInterfaceEventType, REGA_SCRIPT_PATH, REGA_SCRIPT_DATA_LOAD
)
...
class ClientCCU(Client):
    ...
    async def fetch_all_device_data(self) -> None:
        ...
        if not self._has_credentials:
            _LOGGER.warning(
                "fetch_all_device_data: No username set. Not getting paramset description via JSON-RPC."
            )
            return {}
        ...
        try:
            ...
            script_file = os.path.join(source_path.parent, REGA_SCRIPT_PATH, REGA_SCRIPT_DATA_LOAD)
            ...
            if response[ATTR_ERROR] is None and response[ATTR_RESULT]:
                _LOGGER.debug(
                    "fetch_all_device_data: Fetching all device data."
                )
                json_data = json.loads(response[ATTR_RESULT])
                self.central.device_data.add_device_data(json_data)
                return
