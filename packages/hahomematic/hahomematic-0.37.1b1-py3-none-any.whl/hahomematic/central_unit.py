    ...
    async def delete_devices(self, interface_id: str, addresses: list[str]) -> None:
        ...
        for address in addresses:
            try:
                ...
                self.device_data.remove(address=address)
...
class DeviceDataCache:
    """Cache for device/channel initila data."""
    ...
    def add_device_data(self, device_data: dict[str, Any]) -> None:
        ...
        for device_adr, value in device_data.items():
            ...
            if interface not in self._value_cache:
                self._value_cache[interface] = {}
            channel_address = device_adrs[1]
            if channel_address not in self._value_cache[interface]:
                self._value_cache[interface][channel_address] = {}
            parameter = device_adrs[2]
            if parameter not in self._value_cache[interface][channel_address]:
                self._value_cache[interface][channel_address][parameter] = {}
            self._value_cache[interface][channel_address][parameter] = value

    def get_device_data(self, interface: str, channel_address: str, parameter: str) -> Any | None:
        """Get value from cache."""
        return self._value_cache.get(interface, {}).get(channel_address, {}).get(parameter)

    def remove(self, address: str) -> None:
        """Remove name from cache."""
        if address in self._device_data:
            del self._device_data[address]
