...
class HmDevice:
    ...
    def __init__(
        ...
        self._hm_interface = self._central.device_details.get_interface(device_address)
    ...
    @property
    def hm_interface(self) -> str:
        """Return the hm_interface."""
        return self._hm_interface
...
class ValueCache:
    ...
    async def _get_or_load_value(
        ...
        if cache_entry := self._value_cache[paramset_key].get(
        ...
        try:
            if values := await self._client.get_paramset(
                hm_interface=self._device.hm_interface,
                channel_address=channel_address,
                paramset_key=paramset_key,
            ):
