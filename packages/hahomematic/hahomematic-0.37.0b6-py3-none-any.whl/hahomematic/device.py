...
class ValueCache:
    ...
    def __init__(self, device: HmDevice):
        ...
        # { parparamset_key, {channel_address, CacheEntry}
        self._value_cache: dict[str, dict[str, CacheEntry]] = {}
    ...
    async def _get_or_load_value(
        ...
        if cache_entry := self._value_cache[paramset_key].get(channel_address):
            if updated_within_seconds(
                last_update=cache_entry.last_update, age_seconds=age_seconds
            ):
                if cache_value := cache_entry.paramset.get(parameter):
                    return cache_value
        ...
        try:
            ...
            paramset = (
                await self._client.get_cached_paramset(
        ...
        self._value_cache[paramset_key][channel_address] = CacheEntry(
            paramset=paramset, last_update=datetime.now()
        )
        if parameter in paramset:
            return paramset[parameter]

        # try a direct device access to load missing data
        if (
            self._device.central.force_init_data_load
            and self._device.client.supports_cached_values
        ):
            _LOGGER.warning(
                "_get_or_load_value: No data in cache. Trying a load from device for %s.",
                parameter,
            )
            value: Any = None

            try:
                value = await self._client.get_value_by_paramset_key(
                    channel_address=channel_address,
                    paramset_key=paramset_key,
                    parameter=parameter,
                )
                ...
                _LOGGER.debug(
                    "_get_or_load_value: Failed to get get_value_by_paramset_key for %s, %s, %s: %s",
                    self._device.device_type,
                    channel_address,
                    parameter,
                    bhe,
            ...
            if channel_address not in self._value_cache[paramset_key]:
                self._value_cache[paramset_key][channel_address] = CacheEntry(
                    paramset={parameter: value}, last_update=datetime.now()
                )
            else:
                cache_entry = self._value_cache[paramset_key][channel_address]
                cache_entry.paramset[parameter] = value
