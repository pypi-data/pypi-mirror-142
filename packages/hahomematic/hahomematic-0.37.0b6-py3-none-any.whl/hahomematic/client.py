...
class Client(ABC):
    ...
    @property
    def supports_cached_values(self) -> bool:
        """Return if the backend supports cached values."""
        return False
    ...
    async def get_cached_value(self, channel_address: str, parameter: str) -> Any:
        """
        Return a cached value from CCU.
        Return the non-cached version, if cached version is not avalable.
        """
    ...
    async def get_cached_paramset(self, channel_address: str, paramset_key: str) -> Any:
        """
        Return a cached paramset from CCU.
        Return the non-cached version, if cached version is not avalable.
        """
        return self.get_paramset(
            channel_address=channel_address, paramset_key=paramset_key
        )
...
class ClientCCU(Client):
    ...
    async def get_cached_paramset(self, channel_address: str, paramset_key: str) -> Any:
        ...
        if not self._has_credentials:
            _LOGGER.warning(
                "get_value: You have to set username ans password to get a paramset via JSON-RPC"
            )
            return value
...
class _ClientConfig:
    ...
    async def _get_hm_interface(self, port: int) -> str:
        """Get the hm interface."""
        value = None
        # if not self._has_credentials:
        #     _LOGGER.warning(
        #         "get_value: You have to set username ans password to get a value via JSON-RPC"
        ...
        try:
            ...
            response = await self.central.json_rpc_session.post(
    ...
    async def get_client(self) -> Client:
        ...
        hm_interface = await self._get_hm_interface(2001)
        try:
