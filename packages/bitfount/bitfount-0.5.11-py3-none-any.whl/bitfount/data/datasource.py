...
class _BaseSource(ABC):
    ...
    @abstractmethod
    def get_values(self, col_name: str, **kwargs: Any) -> Iterable[Any]:
        """Implement this method to get distinct values from column."""
...
class CSVSource(_BaseSource):
    ...
    def get_values(self, col_name: str, **kwargs: Any) -> Iterable[Any]:
        """Get distinct values from column in CSV dataset.

        Args:
            col_name: The name of the column whose distinct values should be
                returned.

        Returns:
            The distinct values of the requested column as a series.
        """
...
class DataFrameSource(_BaseSource):
    ...
    def get_values(self, col_name: str, **kwargs: Any) -> Iterable[Any]:
        """Get distinct values from column in DataFrame dataset.
...
class DatabaseSource(_BaseSource):
    ...
    def get_values(
        ...
        """Get distinct values from column in Database dataset.
        ...
        if self.query is not None:
            with Session(self.con) as session:
                results = session.execute(self.query).columns(col_name).unique()
        else:
            ...
            with Session(self.con) as session:
                results = session.query(table.columns[col_name]).distinct()
        values = np.array([v for v, in results])
        return values
...
class DataSource:
    ...
    Attributes:
        ...
        seed: Random number seed. Used for setting random seed for all libraries.
        auto_tidy: Whether to tidy the data automatically (cleaning and
            normalizing).
    ...
    def _get_data(self, sql_query: Optional[str] = None) -> Optional[_DataFrameType]:
        ...
        Returns:
            A DataFrame-type object which contains the data.
        """
        data = self.loader.get_data(sql_query=sql_query)
        return data
    ...
    def get_values(
        ...
        """Get distinct values from column in data.
        ...
        Returns:
            The distinct values of the requested column as a series.
        """
        return self.loader.get_values(col_name=col_name, table_name=table_name)
    ...
    def load_data(self, sql_query: Optional[str] = None) -> None:
