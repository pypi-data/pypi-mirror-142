...
class TestHelperFunctions:
    ...
    @pytest.mark.parametrize("secure_aggregation", [True, False])
    def test_create_federated_averaging_protocol_factory_no_iterations_model_epochs(
        self, schema: BitfountSchema, secure_aggregation: bool
    ) -> None:
        """Tests create_fed_avg protocol factory with no iteration provided.

        Tests that iterations is set to a default value if no value is provided.
        """
        model = Mock(epochs=1, steps=None)
        algorithm = FederatedModelTraining(model=model)
        aggregator = _create_aggregator(
            model=algorithm.model, secure_aggregation=secure_aggregation
        )
    ...
    @pytest.mark.parametrize("secure_aggregation", [True, False])
    def test_create_federated_averaging_protocol_factory_no_iterations_model_steps(
        self, schema: BitfountSchema, secure_aggregation: bool
    ) -> None:
        """Tests create_federated_averaging_protocol_factory with no iteration provided.

        Tests that iterations is set to a default value based on the model
        if no value is provided.
        """
        model = Mock(steps=1, epochs=None)
        algorithm = FederatedModelTraining(model=model)
        aggregator = _create_aggregator(
            model=algorithm.model, secure_aggregation=secure_aggregation
        )
    ...
    @pytest.mark.parametrize("secure_aggregation", [True, False])
    def test_create_federated_averaging_protocol_factory_no_iter_bfmodel_epochs(
        self, schema: BitfountSchema, secure_aggregation: bool
    ) -> None:
        """Tests create_fed_avg protocol factory with no iteration provided.

        Tests that iterations is set to a default value if no value
        is provided when the model is a `BitfountModelReference`.
        """
    ...
    @pytest.mark.parametrize("secure_aggregation", [True, False])
    def test_create_federated_averaging_protocol_factory_no_iter_bfmodel_steps(
        self, schema: BitfountSchema, secure_aggregation: bool
    ) -> None:
        """Tests create_fed_avg protocol factory with no iteration provided.
    ...
    def test__check_and_update_pod_ids(self, mocker: MockerFixture) -> None:
        ...
        mock_hub.username = "MyUsername"
        pod_id_1 = "MyPod"
        pod_id_2 = "YourUsername/YourPod"
        pod_ids = _check_and_update_pod_ids([pod_id_1, pod_id_2], mock_hub)
        assert pod_ids == ["MyUsername/MyPod", "YourUsername/YourPod"]
    ...
    def test_combine_pod_schemas_raises_error_if_there_are_duplicated_table_names(
        self, schema: BitfountSchema, mocker: MockerFixture
    ) -> None:
