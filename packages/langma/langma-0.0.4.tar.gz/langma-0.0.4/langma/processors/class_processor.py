...
class ClassificationProcessor(ABCProcessor):
    ...
    def __init__(self, multi_label: bool = False, **kwargs: Any) -> None:
        from langma.utils import MultiLabelBinarizer
    ...
    def build_vocab_generator(self, generators: List[CorpusGenerator]) -> None:
        ...
        for generator in generators:
            if self.multi_label:
                for _, label in tqdm.tqdm(
                        generator,
                        desc="Preparing classification label vocab dict"):
                    for token in label:
    ...
    def inverse_transform(
        ...
        if self.multi_label:
            return self.multi_label_binarizer.inverse_transform(
