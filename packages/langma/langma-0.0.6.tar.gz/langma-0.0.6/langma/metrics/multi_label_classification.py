...
from langma.types import MultiLabelClassificationLabelVar
...
def multi_label_classification_report(y_true: MultiLabelClassificationLabelVar,
    ...
    for c_index, c in enumerate(binarizer.classes):
        precision = metrics.precision_score(y_true_b[:, c_index],
                                            y_pred_b[:, c_index])
        recall = metrics.recall_score(y_true_b[:, c_index], y_pred_b[:,
                                                                     c_index])
