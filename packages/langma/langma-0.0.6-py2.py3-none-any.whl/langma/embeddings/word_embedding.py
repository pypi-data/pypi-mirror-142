...
class WordEmbedding(ABCEmbedding):
    ...
    def load_embed_vocab(self) -> Optional[Dict[str, int]]:
        try:
            ...
            w2v = KeyedVectors.load(self.w2v_path, **self.w2v_kwargs)
        ...
        try:
            ...
            for token in w2v.wv.index2word:
                token2idx[token] = len(token2idx)
        ...
        try:
            ...
            vector_matrix[4:] = w2v.wv.vectors

        try:
            self.embedding_size = w2v.vector_size
        except:
            self.embedding_size = w2v.wv.vector_size
        self.w2v_matrix = vector_matrix
        try:
            w2v_top_words = w2v.index2entity[:50]
        except:
            w2v_top_words = w2v.wv.index2entity[:50]
