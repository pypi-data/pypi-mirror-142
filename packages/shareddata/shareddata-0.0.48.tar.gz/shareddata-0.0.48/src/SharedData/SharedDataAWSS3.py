...
def S3SyncDownloadDataFrame(path,shm_name):
    ...
    process = subprocess.Popen(['aws','s3','sync',awsfolder,dbfolder,\
        '--profile',os.environ['AWS_PROFILENAME'],\
        '--exclude','*',\
    ...
    success= rc==0
    if success:
        Logger.log.debug('AWS S3 Sync DataFrame %s,%s DONE!' % (os.environ['AWS_PROFILENAME'],shm_name))
    else:
        Logger.log.error('AWS S3 Sync DataFrame %s,%s ERROR!' % (os.environ['AWS_PROFILENAME'],shm_name))
...
def S3SyncDownloadTimeSeries(path,shm_name):    
    ...
    success= rc==0
    if success:
        Logger.log.debug('AWS S3 Sync timeseries %s,%s DONE!' % (os.environ['AWS_PROFILENAME'],shm_name))
    else:
        Logger.log.error('AWS S3 Sync timeseries %s,%s ERROR!' % (os.environ['AWS_PROFILENAME'],shm_name))
...
def S3SyncDownloadMetadata(pathpkl,name):
    ...
    success= rc==0
    if success:
        Logger.log.debug('AWS S3 Sync download metadata %s,%s DONE!' % (os.environ['AWS_PROFILENAME'],name))
    else:
        Logger.log.error('AWS S3 Sync download metadata %s,%s ERROR!' % (os.environ['AWS_PROFILENAME'],name))
...
def S3Upload(localfilepath):
    ...
    try:                
        ...
        Logger.log.debug(os.environ['AWS_PROFILENAME']+' Uploading to S3 '+str(localfilepath)+' DONE!')
    except Exception as e:
        Logger.log.error(os.environ['AWS_PROFILENAME']+' Uploading to S3 '+localfilepath+' ERROR! %s' % str(e))               