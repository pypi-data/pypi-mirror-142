...
class RTDBPointTable:
    ...
    def __init__(self, rtdb: List[str] = None, project: List[str] = None):
        ...
        """
        self.rtdb = rtdb if rtdb is not None else ['240:jet_rtdb_v10_prod_4_hongbo', '234:jet_rtdb_v10_prod_4_hongbo']
    ...
    def _one_rtdb_table(host, name):
        ...
        # 合并
        df = reduce(lambda x, y: pd.merge(x, y, how='outer'), [point, meter, project, tenant])
        return df
    ...
    def _one_project_table(host, name):
        ...
        # 项目表选择列
        tb_equip_col = [
            'group_id', 'site_id', 'equip_sys_type_id', 'equip_sys_id', 'equip_id', 'equip_name', 'equip_type',
        ...
        df = reduce(lambda x, y: pd.merge(x, y, how='outer'), [equip, sys_type, equip_type])
        return df
    ...
    def pt(self):
        ...
        df['rtdb_id'] = df[id_col].apply(lambda x: '.'.join([str(i) for i in x]), axis=1)
        name_col = ['tenant_name', 'project_name', 'meter_name', 'point_name']
        df['rtdb_name'] = df[name_col].apply(lambda x: '.'.join(str(i) for i in x), axis=1)
        # return
        return df
