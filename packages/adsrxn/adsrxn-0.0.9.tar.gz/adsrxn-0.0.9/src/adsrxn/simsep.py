...
## mass, momentum, and energy balance
    def run_mamo(self, t_max, n_sec = 5, CPUtime_print = False):
        ...
        '''
        2022 TO DO: 
        1) replace internal routine of def massmoebal(y,t): with other methods.
        2) replace isotherm fitting with pyiast functions
        '''
        def massmomebal(y,t):
            C = []
            q = []
            ...
            # initialize array for temperature dat
            Tg = y[2*n_comp*N : 2*n_comp*N + N]         # array for gas phase temperature
            Ts = y[2*n_comp*N + N : 2*n_comp*N + 2*N]   # array for solid phase temperature
 
            # Array for concentration derivatives
            dC = []
            ddC = []

            # concentration/pressure array initialization
            C_ov = np.zeros(N)
            P_ov = np.zeros(N)
            P_part = []
            Mu = np.zeros(N)
            ...
            # initialize concentration/pressure/viscosity for each species (n_comp)
            for ii in range(n_comp):
                dC.append(self._d@C[ii])
                ddC.append(self._dd@C[ii])
                P_part.append(C[ii]*R_gas*T/1E5) # in bar 0.0.8
                C_ov = C_ov + C[ii]
            ...
            # Ergun equation
            v, dv = Ergun(C,Tg,self._M_m,Mu,self._D_p,epsi,self._d,self._dd,self._d_fo, self._N)
            ...
            if self._order_MTC == 1:
                for ii in range(n_comp):
                    dqdt_tmp = self._k_mtc[ii]*(qsta[ii] - q[ii])*self._a_surf      # linear driving force (LDF) model
                    dqdt.append(dqdt_tmp)
            elif self._order_MTC == 2:      # second order mass transfer definition. need reference for the form of equation below
                for ii in range(n_comp):
            ...
            # interstitial velocity in z-direction
            if self._const_v:
            ...
            D_dis = self._D_disp
            h = self._h
            # Gas phase mass balance: dc_i/dt = d/dz (cD_L*dy/dz + c_i*v) - (1-epsion)/epsilon * dq_i/dt
            dCdt = []
            for ii in range(n_comp):
            ...
            '''
            Gas phase energy (temperature) balance: 
            
            (1-epsilon)/epsilon * (rho_s*Cp_solid+Cp_adsorbed*sum(q_ii))*dTdt = K_z/epsilon*ddTdz - Cp_gas/R*1/dz*(nu*P)-Cp_gas/R*dPdt ...
            - (1 - epsilon)/epsilon * Cp_adsorbed * T * sum(dq_i/dt) + (1 - epsilon)/epsilon*sum(-dH_i*dq_idt) - 2*h_in/(epsilon*r_in)*(T - T_w)

            rho_s: density of adsorbent
            Cp_solid: specific heat capacity of the adsorbent
            Cp_adsorbed: specific heat capacity of the adsorbed phase
            Cp_gas: specific heat capacity of the gas phase
            K_z: the effective gas thermal conductivity
            dH_i: the heat of adsorption of component i
            h_in: the overall insdie heat-transfer coefficient
            r_in: the inner radius of the column
            T_w: wall temperature

            Equation (9) from Indistrual & Engineering Chemistry Research 2016, 55, 5993 - 6005
            
            '''
            Cov_Cpg = np.zeros(N)   # heat capacity (overasll) J/K/m^3
            for ii in range(n_comp):
            ...
            # boundary conditions for energy balance
        ...
        '''
        Now solve the systems of ODEs using odeint
        '''        
        #RUN
        ...
        #self._Tg_res = y_result[:,n_comp*2*N : n_comp*2*N+N] # check if this line can replace the line immediately above
        if CPUtime_print:
    ...
    def Q_valve(self, draw_graph = False, y = None):
        ...
        return Q_0, Q_L
    '''
    breakthrough function is broken. It only works when the adsorber has finite pressure and mole component of gas mixture that is being fed.
    Fix is needed so that the initial concentration/pressure is based on helium gas.
    '''
    def breakthrough(self, draw_graph = False):
        N = self._N
        n_comp = self._n_comp
        C_end =[]                                   # initialize an empty array for concentration at z = L
        C_ov = np.zeros(len(self._t))               # initialize all zero-ed array for concentration throughout all time
        for ii in range(n_comp):                    # goes through each component, here ii = 0 is CO and ii = 1 is CO2
            Cend_tmp = self._y[:,(ii+1)*N-1]        # self._y is a 2-dimensional array where [concentration, species]
            C_end.append(Cend_tmp)                  # check this suspicious operation
            C_ov = C_ov + Cend_tmp
        y = []                                      # initialize an empty array for concentration
        fn_y = []                                   # initialize an empty array for ???
        for ii in range(n_comp):
            y_tmp = C_end[ii]/C_ov                  # convert to mole fraction
            y.append(y_tmp)
            fn_tmp = interp1d(self._t, y_tmp, kind = 'cubic' ) 
            fn_y.append(fn_tmp)
        if draw_graph:
            t_dom = np.linspace(self._t[0], self._t[-1],1001) # get run time for the simulation
            print(t_dom)
            y_again = []
