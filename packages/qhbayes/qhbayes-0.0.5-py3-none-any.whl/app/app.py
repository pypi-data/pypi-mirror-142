...
from ..utilities.number_tools import (
    float_string,
    lin_levels,
    lin_steps,
    log_levels,
...
def make_dashboard():
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                ...
                                children=[
                                    html.Label(
                                        "Select observable (explanatory) variable:"
                                    ),
                                    dcc.RadioItems(
                                        id="x-var",
                                        options=[
                                ...
                                children=[
                                    ...
                                    html.Div(
                                        ...
                                        children=[
                                            ...
                                            Download(id="prediction_download"),
                                        ],
                                    ),
                                    html.Div(
                                        className="formfield",
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Span("Volcanic eruption Q\u2013H data analysis")
                        ],
                    ),
                ],
            ),
    ...
    def download_samples(n_clicks, prediction_samples, fmt):
        ...
        if "prediction-download-button" in changed_id:
            ...
            if fmt == "xlsx":
                return send_data_frame(df.to_excel, "samples.xlsx")
            elif fmt == "csv":
                return send_data_frame(df.to_csv, "samples.csv")
    ...
    def update_explore_plot(
        ...
        if color_column is None:
            ...
            if color_column == "Climate zone":
                ...
                data = data.sort_values(by="Climate zone")
                color_discrete_map = {
                    "Tropical": "IndianRed",
                    "Subtropics": "Peru",
                ...
                data = data.sort_values(by=color_column)
                color_discrete_map = {
            ...
            print(
                f"cmax - cmin = {cmax - cmin}, floor(np.log10(cmax - cmin)) = {floor(np.log10(cmax - cmin))}, step = {step}"
            )

            tickvals = lin_steps(cmin, cmax, step=step)
            print(
                f"linear: min = {np.nanmin(colors)}, cmin = {cmin}, max = {np.nanmax(colors)}, cmax = {cmax}"
            )
            print(tickvals)
            ticktext = tickvals
        ...
        fig = px.scatter(
            ...
            labels={
                ...
                "Plume height": "Column height (km a.v.l.)",
            },
            log_x=True,
            opacity=0.5,
            color=colors,
            ...
            hover_data=["Volcano", "color_value", "size_value"],
        )

        fig.layout.coloraxis.colorbar.title = color_column
        hvtmp = "<b>%{hovertext}</b><br><br>Mass eruption rate = %{x} kg/s<br>Column height = %{y} km a.v.l.<br>"
        if (
            ...
            hvtmp += size_column + " = %{customdata[2]} " + "<br>"
        if (
    ...
    def make_regression_fig(xvar, dataset):
        ...
        data.dropna(
            axis=0,
            how="any",
            subset=[IVESPA.height_column, IVESPA.mer_column],
            inplace=True,
        )
        ...
        hvtmp = "<b>%{hovertext}</b><br><br>Mass eruption rate (kg/s) = %{x}<br>Column height (km a.v.l.) = %{y}<br>"
        ...
        cntr_fig = go.Figure(
            data=go.Contour(
                ...
                colorscale="BuPu",
                contours=dict(
                    start=-5 if xvar == "H" else -7,
                    end=1 if xvar == "H" else 2,
                    size=1,
                ),
                colorbar=dict(
