...
def submit_run(
    ...
    attach_ebs: bool = False,
    priority_class: Optional[str] = None,
):
    """Submit a run through the k8s submission pipeline.

    Args:
        config (RunConfig): The ``RunConfig`` for the run to be submitted.
        attach_ebs (bool, optional): True if the run should attempt to attach EBS. Defaults to False.
        priority_class (str, optional): The priority at which the run should be scheduled. Not all platforms
                                        support priority values. If you attempt to set a priority value that
                                        is not supported, a ``ValueError`` will be thrown. Defaults to None.
