...
def compress_by_path(function, path, opt_print = True, date_auto = True, category_auto = True, json_path="dtypes.json"):
...
def compress(data, date_auto = True, category_auto = True, json_path="dtypes.json", opt_print = True):
...
def read_data(function, data_path, json_path, nrows=""):
  ...
  with open(json_path, "r") as a_file:
    dtypes = eval(json.load(a_file).replace('\\', ''))
  dtypes_ = dtypes
  print(dtypes_)
  ...
  #nrows는 여기 추가할 것
  if nrows=="":
    df_result = function(data_path, dtype=dtypes_)
  else:
    df_result = function(data_path, dtype=dtypes_, nrows=nrows)
