...
def read_perf_data(device):
    ...
    try:
        ...
        for line in data_lines:
            colunms = ['time','app_cpu','total_cpu','memory','native_mem','dalvik_mem','total_traffic','traffic_up',
                       'traffic_down','gpu','threads','virtual_mem','activity','fps','mediasvrd_cpu','wakeups']
            if line.startswith("TimeStamp") or len(line.split(','))!=len(colunms):
                continue
            result.append(dict(zip(colunms,line.split(','))))
