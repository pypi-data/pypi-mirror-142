...
class Session(UserAuthored, Started, Ended, Workable, InvoiceGenerator):
    ...
    def get_invoiceable_partner(self):
        if self.ticket_id and self.ticket.site_id:
            return self.ticket.site.get_invoiceable_partner()
    ...
    def get_generators_for_plan(cls, plan, partner=None):
        ...
        qs = super(Session, cls).get_generators_for_plan(plan, partner)
        ...
        if plan.order is not None:
            ...
            if dd.plugins.invoicing.order_model is rt.models.orders.Order:
                qs = qs.filter(ticket__site__order=plan.order)
        ...
        if partner is None:
            # only with a partner (because only these get invoiced
            # per course).
            qs = qs.filter(ticket__site__company__isnull=False)
        else:
            fldname = rt.models.tickets.Site.get_partner_filter_field(partner)
            qs = cls.filter_by_invoice_recipient(qs, partner, 'ticket__site__'+fldname)
            # q1 = models.Q(
