...
class AnalysisContext:
    ...
    def download_resource(self, path, destination_file_path, res_type="tool_resource"):
        """
        ...
        data_to_send = {"analysis_id": self.analysis_id, "type": res_type, "path": path}
        try:
    ...
    def get_parent_file_selection(self, input_id):
        ...
        for setting_value in parent_data_settings.values():
            ...
            if (setting_container_id == container_id) and setting_passed:
                for setting_filter in setting_value.get("filters", dict()).values():
                    if not setting_filter["passed"]:
                        continue
                    selected_files.update([ff["name"] for ff in setting_filter.get("files", [])])
...
class Container:
    ...
    def get_metadata(self, patient_secret_name=None, ssid=None):
        return self.__context._get_metadata_value(None, patient_secret_name, ssid)
    ...
    def _set_metadata_value(self, key, value, patient_secret_name=None, ssid=None):
        return self.__context._set_metadata_value(key, value, patient_secret_name, ssid)
