    ...
    def _post_fit(self, optimization_res, coeff_names, sample_size, hess_inv=None, verbose=1):
        ...
        if is_latent_class:
            ...
            for i in range(self.num_classes):
                X_class_idx = np.concatenate([np.where(np.char.find(self.Xnames, param) != -1)[0]
                for param in self.class_params_spec[i]])
    ...
    def summary(self):
        ...
        if hasattr(self, 'is_latent_class'):
            ...
            for ii, member_class in enumerate(self.member_params_spec):
                ...
                member_class_names_idx = [np.where(np.char.find(self.varnames, param) != -1)[0] for param in member_class]
