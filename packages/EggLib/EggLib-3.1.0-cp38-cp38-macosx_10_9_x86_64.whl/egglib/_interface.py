...
def struct_from_iterable(iterable, fmt=None, data=None, missing=None,
    ...
    their order of apparent (the first label is applied to the first
    ...
    or :class:`Container` passed as *aln*.
    ...
    :param fmt: specifies the meaning of items contained by each row. If
        ...
        optional and must also appear also once. If present, *aln* must
        also be specified and the passed instance must not have
        duplicate names. If a name is missing in *aln*, it will be
    ...
    :param missing: value used to specify that a sample should not be
        ...
        as read from a file) is not interpreted as ``None`` .
    ...
    :param function: a function or lambda to be applied to each
...
class StructureShuffler(object):
    ...
    def __enter__(self):
        self._struct._obj.shuffle_init(self._mode, random._static_random)
