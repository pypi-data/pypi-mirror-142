...
class RollingOriginValidator(BaseCrossValidator):
    ...
    .. _sklearn.BaseCrossValidator: https://scikit-learn.org/stable/modules/cross_validation.html
    ...
    def n_step(self, val):
        if isinstance(val, int):
            ...
            raise AzureMLError.create(
                InvalidArgumentType, target="n_step", argument="n_step", actual_type=type(val), expected_types="int"
            )
        ...
        if val is None:
            ...
            if val < 1:
                raise AzureMLError.create(
                    ArgumentOutOfRange,
                    target="max_horizon",
                    argument_name="max_horizon ({})".format(val),
                    min=1,
                    max="inf",
                )
    ...
    def split(self, X, y=None, groups=None):
        ...
        if not isinstance(X, TimeSeriesDataFrame):
            raise AzureMLError.create(
                ...
                argument="X",
                actual_type=type(X),
                expected_types="TimeSeriesDataFrame",
            )
        ...
        fold_origins = pd.date_range(end=fcast_index_sort[-1], periods=n_splits, freq=index_delta)

        for fcast_time in fold_origins:
            origin_valid = fcast_index == fcast_time if has_origin else time_value > fcast_time
            if self.max_horizon is not None:
                try:
            ...
            yield (
                np.array(np.where(time_value <= fcast_time)).flatten(),
