...
class SbobetApi(requests.Session):
    _token = None
    _base_domain = None
    ...
    def __init__(self, credentials, base_domain=None):
        super(SbobetApi, self).__init__()
        self.credentials = credentials
        self.base_domain = base_domain or settings.SBOBET_DOMAIN
    ...
    def random_ip(self, base='103.149.172.{}'):
        self.headers.update({
            'X-Forwarded-For': base.format(randrange(200), randrange(200))
        })
    ...
    def base_domain(self, value):
        self._base_domain = urlparse(value)
    ...
    def refresh(self):
        if self.refresh_token:
            requests.post(self.refresh_url, {
