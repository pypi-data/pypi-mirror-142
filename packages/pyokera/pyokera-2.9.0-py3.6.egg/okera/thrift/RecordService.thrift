...
struct TRequestContext {
  ...
  // Optional full user request, e.g. full sql query
  4: optional string request_query
...
struct TGetPartitionNamesParams {
  // Context metadata for the request.
  100: optional TRequestContext ctx

  1: required TDatabaseName database
  2: required string table
  3: required i16 max_parts
  4: optional string requesting_user
}

struct TGetPartitionNamesResult {
...
// Enum for type of expression in an expression operator tree
enum TExprType {
  LITERAL
  OPERATOR,
  FUNCTION,
...
struct TExprNode {
  ...
  4: optional TExprNodeType node_type
}
...
struct TTransformExpr {
  // The expression to match transforms
  1: required TExprTree expr

  // The function to transform with
  2: required TExprTree fn
}
...
struct TPrivilege {
  ...
  /**
   * Base64 encoded privilege_name useful for using http interaction.
   */
  24: optional string privilege_id
   ...
   * Can optionally return the expression in a structured tree
   */
  12: optional string attribute_expression
  21: optional TExprTree attribute_expression_tree
  ...
  22: optional list<TTransformExpr> transform_expressions
  ...
  23: optional TExprTree filter_expression_tree
...
service RecordServicePlanner {
  ...
  // Returns the partition names in this table.
  TGetPartitionNamesResult GetPartitionNames(1:TGetPartitionNamesParams params)
      throws(1:TRecordServiceException ex);
