...
struct TAuthorizeQueryParams {
  ...
  // DEPRECRATED: use default_namespace instead
  18: optional string default_db

  // If true, this view definition is for a temporary view create statement.
  19: optional bool is_create_tmp_view

  // Used for proxies. If set, the proxied host. For example, if we are proxing
  // snowflake, this is the SF warehouse host this request is for.
  20: optional string connection_proxy_host

  // The default namespace this query is running in, if any: this is used
  // to fully qualify any under-qualified catalog objects.
  // For example, if the default namespace is a database and schema,
  // this option would be set to ["my_db","my_schema"].
  // This should be used instead of default_db.
  21: optional list<string> default_namespace

  // If true, run a validation query for a rewritten query (by issuing the query
  // to the underlying RDBMS) and failing the request if it fails.
  22: optional bool validate_rewrite
}
...
struct TAuthorizeQueryFilters {
  ...
  // If set, the list of all columns referenced across all filters
  3: optional list<TAuthorizeQueryColumn> columns
}

struct TColumnAccess {
  // If true, the user has access to this column, false means no access
  1: bool accessible
  2: optional TAuthorizeQueryColumn column

  // If set, the SQL expression for this column.
  3: optional string transform
}

struct TAuthorizeQueryJdbcRefTable {
  1: optional string orig_tbl_name_in_query
  2: optional string jdbc_fq_tbl_name
  3: optional string jdbc_tbl_type
}
...
struct TAuthorizeQueryResult {
  ...
  // If set, contains the map of okera tables (which need to be authorized or which are
  // referenced for cte_rewrite) to corresponding JDBC tables.
  // This is passed to the caller to create temporary roles, grants, row level and/or
  // column level policies on these jdbc tables in the native JDBC source.
  15: optional map<string, TAuthorizeQueryJdbcRefTable> jdbc_referenced_tables
}
...
struct TDataRegConnectionParams {
  ...
  /**
   * A jdbc driver (string) for which matching data registration connection
   * records are returned.
   */
  6: optional string jdbc_driver
  /**
   * If set, resolve the credentials for input connection and return the connection object
   * with plain text credentials. It can be used only with TObjectOpType = GET
   */
  7: optional bool resolve_creds = false
  /**
   * If set, it indicates that user has entered credentials in plain text. Okera would
   * generate secret key_ids for these credentials and store them in a secret store.
   */
  8: optional bool direct_credentials = false
}
...
struct TDataRegConnectionResult {
   ...
   * <i>ManageDataRegConnection::LIST</i> (by `jdbc_driver`):
   * List of the matching data registration connections.
   */
...
struct TListDatabasesParams {
  ...
  /**
    * If true, only return databases that are crawlers.
    * Note: because crawler databases are internal databases,
    * you must also ensure that ignore_internal_dbs is set to `false`
    */
  11: optional bool is_crawler = false
  /**
   * If true, only return the names of the databases, with no other metadata
   */
  12: optional bool names_only = false
}
...
struct TListRolesParams {
  ...
  // If true, return any policy (ABAC) expressions in structured format as well
  12: optional bool return_structured_exprs

  // List of exact group names to match against granted group names
  13: optional list<string> group_names
}
...
struct TGetRoleParams {
  ...
  // If true, return any policy (ABAC) expressions in structured format as well
  5: optional bool return_structured_exprs
}
...
service OkeraRecordServicePlanner extends RecordService.RecordServicePlanner {
  ...
  /**
   * API to evaluate expressions on the planner.
   */
