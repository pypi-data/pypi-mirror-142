...
def run(repl:str, run:bool=True, stop:bool=False, restart:bool=False):
	...
	id = get_json(user, replname, key)
	if run:
		requests.post("https://replops.coolcodersj.repl.co/run", data=json.dumps({
			"UUID": id,
			"sid": key
		}),
		headers = {'Content-type': 'application/json', 'Accept': 'text/plain'})
		...
		requests.post("https://replops.coolcodersj.repl.co/stop", data=json.dumps({
			"UUID": id,
			"sid": key
		}),
		headers = {'Content-type': 'application/json', 'Accept': 'text/plain'})
		...
		requests.post("https://replops.coolcodersj.repl.co/restart", data=json.dumps({
			"UUID": id,
			"sid": key
		}),
		headers = {'Content-type': 'application/json', 'Accept': 'text/plain'})
