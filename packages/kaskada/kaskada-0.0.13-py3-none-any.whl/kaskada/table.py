...
def create_table(table_name: str, time_column_name: str, entity_key_column_name: str, subsort_column_name: str = None, grouping_id: str = None, client: Client = None) -> table_pb.CreateTableResponse:
    """
    Creates a table
    ...
    try:
        ...
        if subsort_column_name:
            table_args['subsort_column_name'] = wrappers.StringValue(value = subsort_column_name)
...
def upload_from_s3(table: Union[table_pb.Table, table_pb.CreateTableResponse, table_pb.GetTableResponse, str], access_key: str = None, secret: str = None, virtual_hosted: str = None, region: str = None, bucket: str = None, key: str = None, client: Client = None):
    """
    Uploads a file from an external S3 source to a table. Credentials are required and are not persisted in any way.

    Args:
        table (Union[table_pb.Table, table_pb.CreateTableResponse, table_pb.GetTableResponse, str]): The target table object
        access_key (str, optional): Amazon Web Services Access Key. Used only for authentication and is not persisted.
        secret (str, optional): Amazon Web Services Secret Key. Used only for authentication and is not persisted.
        virtual_hosted (str, optional): The Amazon Virtual Hosted URI to the file. See: https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html.
                                        If not provided, the region + bucket + key will be used.
        region (str, optional): The target region of the S3 bucket. Only used if virtual_hosted is not provided.
        bucket (str, optional): The target S3 bucket. Only used if virtual_hosted is not provided.
        key (str, optional): The target key to the object in the S3 bucket. Only used if virtual_hosted is not provided.
        client (Client, optional): The Kaskada Client. Defaults to kaskada.KASKADA_DEFAULT_CLIENT.
    ...
    try:
        ...
        req = table_pb.LoadDataRequest(
            table_name = table_name,
            external_s3_file = external_s3_file
        )
        return client.tableStub.LoadData(req, metadata=client.get_metadata())
    except grpc.RpcError as e:
        kaskada.handleGrpcError(e)
