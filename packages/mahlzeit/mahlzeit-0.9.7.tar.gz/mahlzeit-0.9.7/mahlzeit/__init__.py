...
class Esser():
    def __init__(self, name, gewichtung):
        ...
        if re.search(r' {2,}', name):
            raise Exception('account names cannot have more than two spaces "{}"'.format(name))
        self.name = name
    ...
    def calc(self):
        ...
        for e in self.einkaeufe:
            for name, betrag in e.iterate('bezahler'):
                ausgleich[name] = Decimal(ausgleich.get(name, Decimal(0))) + betrag
            for name, betrag in e.iterate('esser'):
                ausgleich[name] = Decimal(ausgleich.get(name, Decimal(0))) - betrag
        for b in self.bezahlungen:
            for bzahler, bzahlbetrag in b.iterate('bezahler'):
                ausgleich[bzahler] = Decimal(ausgleich.get(bzahler, Decimal(0))) + bzahlbetrag
            for bzahlter, bzahlterbetrag in b.iterate('bezahlter'):
                ausgleich[bzahlter] = Decimal(ausgleich.get(bzahlter, Decimal(0))) - bzahlterbetrag

        # sanity check makes sure that we produce at most 0.01 error per einkauf
        assert sum([ Decimal(b) for _, b in ausgleich.items() ]) <= (Decimal(0.01) * len(self.einkaeufe)), sum([ Decimal(b) for _, b in ausgleich.items() ])
