...
from reportlab.graphics.barcode import code39, code93, code128, common, eanbc, qr, usps, usps4s
from reportlab.graphics.shapes import Drawing

from pyametista.engine.commonutilities import add_attr2attributes, replace_text
...
def draw_basic_barcode(report, barcode_type, element, attributes):
    ...
    if code_expression is not None and code_expression != "":
        ...
        barcode = barcode_type(
            data_value,
            barHeight=attributes.get("barHeight", attributes["height"]),
            stop=1,
            checksum=check_sum,
        )

        # scale barcode to specified width
        report["canvas"].translate(
            attributes["x"], report["cur_y"] - attributes["y"] - attributes["height"]
        )
        report["canvas"].scale(
            attributes.get("barWidth", attributes["width"]) / barcode.width,
...
def draw_advanced_barcode(report, barcode_type, element, attributes):
    ...
    if code_expression is not None and code_expression != "":
        ...
        d = Drawing(
            ...
            attributes["height"],
            transform=[
                attributes.get("barWidth", attributes["width"]) / width,
                0,
                0,
                attributes.get("barHeight", attributes["height"]) / height,
                0,
                0,
            ],
        )
