...
class DefaultDatasetBuilderConfig(tfds.core.BuilderConfig):
  ...
  # 参考：Positional argument v.s. keyword argument  https://stackoverflow.com/questions/9450656/positional-argument-v-s-keyword-argument
...
class SuperDatasetBuilder(tfds.core.GeneratorBasedBuilder):
    ...
    def load_batched_datasets(dataset_builder_cls_name=None,
        ...
        for dataset in datasets:
            if map_fn_feature_label:
                dataset = dataset.map(map_fn_feature_label, num_parallel_calls=tf.data.experimental.AUTOTUNE)
            if shuffle < 1:
    ...
    def split_full_datasets(full_datasets: tf.data.Dataset, num_examples=0, train_size=0.7, validation_size=0, test_size=0.3):
        ...
        ```
        """
        train_shard_num = int(EnhancedFloat(train_size)*10)
        train_size = int(EnhancedFloat(train_size) * num_examples)
        validation_shard_num = int(EnhancedFloat(validation_size)*10)
        validation_size = int(EnhancedFloat(validation_size) * num_examples)
        test_shard_num = int(EnhancedFloat(test_size)*10)
        test_size = int(EnhancedFloat(test_size) * num_examples)
...
if __name__ == '__main__':
    ...
    batched_datasets = SuperDatasetBuilder.load_batched_datasets("mnist",
                                                                 split=[tfds.Split.TRAIN],
                                                                 map_fn_feature_label=normalize_img)
