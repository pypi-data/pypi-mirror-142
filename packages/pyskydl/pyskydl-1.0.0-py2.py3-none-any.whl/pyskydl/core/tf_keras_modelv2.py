# -*- coding: utf-8 -*-
# 1.解决启动Warning：This TensorFlow binary is optimized with oneAPI Deep Neural Network Library (oneDNN) to use the following CPU instructions in performance-critical operations:  AVX2 FMA
import os
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'
# 2.当你和其他人共用同台机器上的多卡gpu时，防止和别人争用资源，每个人都最好指定使用哪张GPU卡
# os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"
import numpy as np
import tensorflow as tf
import tensorflow_datasets as tfds
from pyskydl.core.modelv2 import ModelV2
from pyskydl.core.tf_utils import TFUtils
...
from pyskydl.core.tf_keras_netv2 import TfKerasNetV2
...
class TfKerasModelV2(ModelV2):
    ...
    def compile(self,
        ...
        self.net.get_proxy().compile(optimizer=self.optimizer, loss=self.loss, metrics=self.metrics)
        # self.log.info(self.net.get_proxy().summary())  # 训练mnist会碰到个issue: This model has not yet been built.
        ...
        if self.parser_args.init_from_saver and latest_checkpoint:
            ...
            self.net.get_proxy().load_weights(latest_checkpoint)
    ...
    def fit(self, train_dataloaders, val_dataloaders):
        ...
        older: def fit(self, *args, **kwargs): xxx
        """
    ...
    def evaluate(self, test_dataloaders):
        """
        评估模型
        older: def evaluate(self, *args, **kwargs): xxx
        """
    ...
    def predict(self, predict_dataloaders):
        """
        预测模型
        older: predict(self, *args, **kwargs): xxx
        """
    ...
    def serving(self, *args, **kwargs):
        ...
        predictions = self.net.get_proxy().predict(test_batched_features)
        print("serving predict, real:", np.argmax(predictions[0]), test_batched_labels[0])

        # save model for tf serving
        if not self.net.get_proxy().inputs:
            for batched_examples in tfds.as_numpy(evaluate_dataset.take(1)):
                test_batched_features, test_batched_labels = batched_examples
                self.net.get_proxy()._set_inputs(test_batched_features)
        tf.saved_model.save(self.net.get_proxy(), saved_serving_path)
        return self
    ...
    def do_parse_args(self):
        ...
        if self.num_gpus > 0:
            if not self.parser_args.use_cuda:
                self.log.info(f"WARNING: You have {self.num_gpus} CUDA device, so you should probably run with --cuda")
                # 不使用GPU加速
                self._num_gpu = 0
        else:
    ...
    def _fit_callbacks(self):
        ...
        class PrintLR(tf.keras.callbacks.Callback):
            def __init__(self, optimizer, model_checkpoint_filename):
                self.optimizer = optimizer
                self.net_checkpoint_filename = model_checkpoint_filename

            def on_epoch_end(self, epoch, logs=None):
                # TODO 保存模型的权重和偏置
                # #self.net.save_weights(self.net_checkpoint_filename)
                # this self is inner class self, so no need set self.net.get_proxy()
                print(f'\ntf.keras callbacks-PrintLR.on_epoch_end(): Learning rate for epoch {epoch+1} is {self.optimizer.lr.numpy()}')
        ...
        class EvaluateCallback(tf.keras.callbacks.Callback):
            ...
            def on_epoch_end(self, epoch, logs={}):
                ...
                loss, acc = self.net.evaluate(x, y, verbose=1, batch_size=self.batch_size)
        ...
        callbacks = [
            ...
            PrintLR(self.net.get_proxy().optimizer, self.get_model_checkpoint_dir() + "/" + self.latest_model_filename),
    ...
    def _model_to_estimator(self, keras_model_path=None,
        ...
        # estimator = tf.keras.estimator.model_to_estimator(self.net, config=config)
        return keras.estimator.model_to_estimator(keras_model=self.net,
