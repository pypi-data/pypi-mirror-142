...
import argparse
import numpy as np
from pyskydl.core.v1.super_model import SuperModel
from pyskydl.core.v1.super_torch_net import SuperTorchNet
    ...
    def do_parse_args(self):
        ...
        if self.num_gpu > 0:
            if not self.parser_args.use_cuda:
                self.log.info("WARNING: You have {self._num_gpus} CUDA device, so you should probably run with --cuda")
                # 不使用GPU加速
                self._num_gpu = 0
        else:
