...
from pyskydl.core.super_tf_dataset_builder import SuperDatasetBuilder, DefaultDatasetBuilderConfig
from pyskydl.zoo.bert.official.nlp.bert.tokenization import FullTokenizer, printable_text, convert_to_unicode
...
class ChineseBertNerDatasetBuilder(SuperDatasetBuilder):
    ...
    人民日报语料处理工具集 | Tools for Corpus of People's Daily https://github.com/howl-anderson/tools_for_corpus_of_people_daily
    达观数据如何打造一个中文NER系统 https://zhuanlan.zhihu.com/p/27597790
    Coding Chunkers as Taggers: IO, BIO, BMEWO, and BMEWO+ https://lingpipe-blog.com/2009/10/14/coding-chunkers-as-taggers-io-bio-bmewo-and-bmewo/
    中文自然语言处理数据集，平时做做实验的材料。欢迎补充提交合并。 https://github.com/InsaneLife/ChineseNLPCorpus
    中文命名实体识别，实体抽取，tensorflow，pytorch，BiLSTM+CRF https://github.com/buppt/ChineseNER
    """
    BUILDER_CONFIGS = [
    ...
    def __init__(self, data_dir=None, config=None):
        ...
        self._data_path = "/Users/tony/deep_learning_can_not_delete"
        self._train_data_file = self._data_path + "/data/ner/train.txt"
        self._test_data_file = self._data_path + "/data/ner/test.txt"
        self._validation_data_file = self._data_path + "/data/ner/dev.txt"
    ...
    def _generate_examples(self, **kwargs):
        tokenizer = FullTokenizer(vocab_file=self._data_path + "/tf_hub/bert_zh_L-12_H-768_A-12_4/assets/vocab.txt", do_lower_case=True)
        ...
        for (i, line) in enumerate(lines):
            ...
            rows.append(row)
        for index, curr_row in enumerate(rows):
