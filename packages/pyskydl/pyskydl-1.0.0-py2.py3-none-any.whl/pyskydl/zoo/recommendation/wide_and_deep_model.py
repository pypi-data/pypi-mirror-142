...
from pyskydl.datasets.recommend_dataset_builder import RecommendDatasetBuilder


class WideAndDeepModel(TfKerasModelV2):
    """
    资讯智能推荐系统中用于精排序模型实现
    """
    ...
    def fit(self, *args, **kwargs):
        ...
        self.parser_args.train_phase = TrainPhaseEnum.Predict.value
        return self

    def evaluate(self, *args, **kwargs):
        data = args[0]
        if data:
            self.net.get_proxy().evaluate(data)
            self.net.get_proxy().summary()
            # change to evaluate phase
            self.parser_args.train_phase = TrainPhaseEnum.Test.value
        return self
    ...
    def predict_from_load_weights(self, *args, **kwargs):
        ...
        predictions = self.net.get_proxy().predict(inputs)
        predictions = np.array([a[0] for a in predictions])
        recommended_content_scores = (-predictions).argsort()
