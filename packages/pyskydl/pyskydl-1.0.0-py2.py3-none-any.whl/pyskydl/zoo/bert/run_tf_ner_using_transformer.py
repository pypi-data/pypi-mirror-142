...
来自：https://github.com/huggingface/transformers/blob/master/examples/run_tf_ner.py
```
(建议采用pytorch版本的ner实现)
main方法启动参数：
...
--output_dir=/tony/can_delete/tensorflow/ner
--model_type=bert
    ...
    def train_step(train_features, train_labels):
        def step_fn(train_features, train_labels):
            ...
            with tf.GradientTape() as tape:
                ...
                # logits = tf.reshape(logits, (-1, len(labels) + 1)) # +1 is no need
                logits = tf.reshape(logits, (-1, len(labels)))  # +1 is no need
...
def evaluate(args, strategy, model, tokenizer, labels, pad_token_label_id, mode):
    ...
    for eval_features, eval_labels in eval_iterator:
        ...
        with strategy.scope():
            ...
            # tmp_logits = tf.reshape(logits, (-1, len(labels) + 1))  # +1 is no need
            tmp_logits = tf.reshape(logits, (-1, len(labels)))  # +1 is no need
    ...
    for i in range(label_ids.shape[0]):
        for j in range(label_ids.shape[1]):
            if label_ids[i, j] != pad_token_label_id:
                # y_pred[i].append(labels[preds[i, j] - 1]) # -1 is no need
                # y_true[i].append(labels[label_ids[i, j] - 1]) # -1 is no need
                y_pred[i].append(labels[preds[i, j]])  # -1 is no need
                y_true[i].append(labels[label_ids[i, j]])  # -1 is no need
...
def main(_):
    ...
    # num_labels = len(labels) + 1 # +1 is no need
