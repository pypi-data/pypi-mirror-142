...
cdef floating prox_gl1_scad(floating x, floating nn1, floating gam, floating l1_reg, floating l2_reg, floating norm_X_ii) nogil:
    ...
    if abs_x <= b1:
        ...
        return ((gam - 1.0) / (gam - 2.0)) * prox_l1(x, b2 / (gam - 1.0)) / (l2_reg + norm_X_ii)
...
cdef floating prox_l2(floating x, floating nn, floating l1_reg, floating l2_reg, floating norm_X_ii) nogil:
    ...
    return x * fmax(1. - l1_reg/nn, 0.) / (norm_X_ii + l2_reg)
#---------------------
...
def scad_coordinate_descent_multi_task(
    ...
    if scad_reg == 0:
        warnings.warn("Coordinate descent with a SCAD penalty amount equal to 0 "
    ...
    with nogil:
        ...
        for n_iter in range(max_iter):
            ...
            for f_iter in range(n_features):  # Loop over coordinates
                ...
                for jj in range(n_tasks):
                    # if W[jj, ii] is already active
                    #   or, even with the previous 'inactive set looping',
                    #   feature ii has not entered active set for ALL the tasks
                    # then, 
                    #   no need to update feature ii over ALL the tasks => 0. will be update to 0.
                    if W[jj, ii] != 0.0 or nn == 0.0:
                        continue
            ...
            # for/else, runs if for doesn't end with a `break`
            with gil:
                warnings.warn("SCAD-MTL objective did not converge. You might want to "
