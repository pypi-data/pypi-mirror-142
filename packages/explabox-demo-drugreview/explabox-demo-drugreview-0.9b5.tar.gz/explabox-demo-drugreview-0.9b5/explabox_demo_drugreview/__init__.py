...
...                model=model,
...                splits={'train': 'drugsComTrain_raw.tsv', 'test': 'drugsComTesttsv'})
...
class Inner:
    def __init__(self, path: str = './assets'):
        ...
        if not (path / 'model.onnx').is_file():
            download_file('https://git.science.uu.nl/m.j.robeer/explabox-demo-drugreview/-/raw/main/assets/model.onnx',
                          str(path / 'model.onnx'))
