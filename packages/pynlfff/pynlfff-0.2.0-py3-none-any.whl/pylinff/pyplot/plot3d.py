...
"""
Purpose:   [1] plot the linff product (hdf format) to picture
...
class LinffPlotD3CutCake():
    ...
    def __init__(self, *args, **kwargs):
        ...
        eg: d3_drawer=LinffPlotD3CutCake(figsize=(6,9))
        :param args:
        :param kwargs:
        """
        ...
        self.colormap_max = 0  # colormap will show range [colormap_max,colormap_min]
        self.colormap_min = 0
        self.colormap_value_auto = True  # auto change colormap_max,colormap_min by real value
        self.colormap_zip = 1  # range (0 - 1]  for zip [colormap_max,colormap_min] , and 1 is not zip
        self.colormap_alpha = None # eg 0.8  set for global transparency
        self.colormap_mirror = True # If set True, and the maximum minimum value is positive and negative, respectively, then the final value becomes about 0 symmetry
    ...
    def set_default_cmap(self):
        self.color_map=plt.get_cmap('coolwarm',lut=8)
        a = np.append(np.linspace(0, 1, int(8 / 2)), np.linspace(1, 0, int(8 / 2)))
    ...
    def run_cut(self):
        ...
        for cut in self.__cut_list:
            ...
            if cut["N"] == "Nx":
                ...
                if self.colormap_value_auto:  # set auto for computer max and min
                    self.colormap_max = max(self.colormap_max, data1_x.max())
                    self.colormap_min = min(self.colormap_min, data1_x.min())
                ...
                if self.colormap_value_auto:
                    self.colormap_max = max(self.colormap_max, data2_y.max())
                    self.colormap_min = min(self.colormap_min, data2_y.min())
                ...
                if self.colormap_value_auto:
                    self.colormap_max = max(self.colormap_max, data3_z.max())
                    self.colormap_min = min(self.colormap_min, data3_z.min())
                draw_job_dict = dict(X=data1_x, Y=data2_y, Z=data3_z, zdir="z", offset=z_value)
                contourf_list.append(draw_job_dict)
                z_value_list.append(z_value)
        if 1 > self.colormap_zip > 0:  # for zip color map
            c_min = self.colormap_min * self.colormap_zip
            c_max = self.colormap_max * self.colormap_zip
        else:
            c_min = self.colormap_min
            c_max = self.colormap_max

        if self.colormap_mirror and c_min*c_max < 0: # set True, and the maximum minimum value is positive and negative
        ...
        if self.colormap_alpha is not None:
...
if __name__ == "__main__":
    ...
    # d3_drawer = LinffPlotD3CutCake(**di)
    d3_drawer = LinffPlotD3CutCake(figsize=(6, 6))
    data_hdf_path =r"C:\Users\Zander\PycharmProjects\pylinff\test_data\product\product1\Bxyz.h5"
    picture_path=r"C:\Users\Zander\PycharmProjects\pylinff\test_data\product\product1\LinffPlotD3CutCake.png"
