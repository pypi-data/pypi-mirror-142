...
class ClusterBeta(Cluster, Generic[IsAsynchronous]):
    ...
    def _ipython_display_(self):
        ...
        elif EXECUTION_CONTEXT == "ipython_terminal" and HAS_RICH:
    ...
    def _repr_mimebundle_(
        ...
        cloud = self.cloud
        if EXECUTION_CONTEXT == "ipython_terminal" and HAS_RICH and self.cluster_id:
            from .widgets.rich import RichClusterWidget
    ...
    async def _wait_until_ready(self) -> None:
        ...
        if self.show_widget:
            if EXECUTION_CONTEXT == "ipython_terminal" and HAS_RICH:
                from .widgets.rich import RichClusterWidget

                widget = RichClusterWidget(n_workers=self._start_n_workers)
                ctx = widget
            elif EXECUTION_CONTEXT == "notebook":
                from .widgets.notebook import HTMLClusterWidget
        ...
        with ctx:
            while True:
                ...
                state = ClusterStateEnum(cluster_details["current_state"]["state"])
                reason = cluster_details["current_state"]["reason"]

                if state == ClusterStateEnum.ready:
                    return
                elif state.awaiting_readiness():
                    if timeout_at is not None and datetime.datetime.now() > timeout_at:
                    ...
                    raise ClusterCreationError(
                        f"Cluster status is {state.value} (reason: {reason})",
                        cluster_id=self.cluster_id,
                    )
