...
def get_instance_types(cluster_details: Mapping[str, Any]) -> tuple[str, Counter[str]]:
    ...
    """
    scheduler_instance_type = SCHEDULER_SEARCH.search(cluster_details) or "Unknown"
    worker_instance_types = Counter(
        w.get("instance_type_id", "Unknown")
        for w in WORKER_SEARCH.search(cluster_details)
    )
