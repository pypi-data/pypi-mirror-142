...
def up_get_table(url):
  ...
  try:
    ...
    assert False, f'Puddles says: url is not a legal web site for a table'
  return df
...
def up_get_column(table, column_name):
  ...
  assert column_name in table.columns.to_list(), f'Puddles says: column_name is unrecognized. Check spelling and case. Here are legal column names: {'
