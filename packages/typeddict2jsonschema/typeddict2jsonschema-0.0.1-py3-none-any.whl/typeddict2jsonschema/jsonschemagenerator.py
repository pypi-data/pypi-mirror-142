    ...
    def visit_Subscript(self, node):
        ...
        if node.value.id == "List":
            return {
                "type": "array",
                "items": {
                    "type": typing_to_jsonschema[slices]
                }
            }
            ...
            return { "enum": slices }
        elif node.value.id == "Option":
            slice = self.visit(slices)
            if isinstance(slice, str):
                return { "anyOf": [ {"type": "null"}, {"type": typing_to_jsonschema[slice] }]}
            else:
                raise NotImplementedError("Nested Option not yet supported")
        else: 
