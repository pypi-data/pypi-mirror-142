...
class TestFileService:
    ...
    def test_skip_hidden_folders_and_files_full_load(
        ...
        file_service.stream_handlers['s3'].get_s3_list_filter.assert_called_with(s3_resource,
                 absolute_path=False,
        ...
        assert(len(filtered)==1 and filtered[0]["Key"] == 'prefix/as_of_date=2021-03-01/a.parquet')
    ...
    def test_skip_latest_folders_and_files_incremental_load(
        ...
        assert(len(filtered) == 5) #todo specify a,b,c and nulls [from object_summaries_on_s3 magic weirdness]
