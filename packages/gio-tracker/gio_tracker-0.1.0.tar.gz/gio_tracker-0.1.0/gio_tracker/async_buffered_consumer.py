...
class AsyncBufferedConsumer(DefaultConsumer):
    ...
    def _should_flush(self):
        full = len(self._async_events) >= self._max_size
    ...
    def transfer_buffers(self):
        buf = self._async_events
        while buf:
            self._async_buffers['cstm'].append(buf.pop(0))

    def _sync_flush(self):
        if len(self._async_buffers['cstm']) > 0:
            request_url = self.endpoints['cstm']
            buf = self._async_buffers['cstm']
            while buf:
                batch = buf[:self._max_size]
                self.post_data(request_url, batch)
                buf = buf[self._max_size:]
            self._async_buffers['cstm'] = buf
