    ...
    for tpl_prj in cast(List[RegisteredTemplateProject], pdv_val(pdv, 'tpl_projects')):
        ...
        for tpl_file_path in [file for file in glb(join(tpl_path, "**/.*")) + glb(join(tpl_path, "**/*")) if isf(file)]:
            ...
            if not any(rel_path.startswith(folder_prefix) for folder_prefix in exclude_folders):
                if rel_path == tpl_folder:
                    rel_path = os.path.join(pdv_str(pdv, 'package_path'), rel_path)
                deploy_template(cast(str, tpl_file_path), rel_path, patcher, pdv,
...
def _check_types_linting_tests(pdv: PdvType):
    ...
    with in_wd(project_path):
        ...
        _cl(62, "pylint", extra_args=pylint_args, exit_on_err=False, lines_output=out)  # exit_on_err~=pylint--exit-zero
...
def _children_package_names(ini_pdv: PdvType, names: Iterable[str], chi_vars: 'OrderedDict[str, PdvType]') -> List[str]:
...
def _init_act_args_check(ini_pdv: PdvType, act_spec: Any, act_name: str, act_args: ActionArgs):
    ...
    project_type = pdv_str(ini_pdv, 'project_type')
    cae.dpo(f"    - detected project type '{project_type}' for project in {pdv_str(ini_pdv, 'project_path')}")
...
def _init_children_pdv_args(ini_pdv: PdvType, act_args: ActionArgs) -> List[PdvType]:
    ...
    chi_vars: OrderedDict[str, PdvType] = pdv_val(ini_pdv, 'children_project_vars')
