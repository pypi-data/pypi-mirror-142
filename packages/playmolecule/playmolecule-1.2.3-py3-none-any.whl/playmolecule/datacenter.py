...
class Dataset:
    def __init__(self, datasetid, filename="", _session=None, _props=None) -> None:
        self._session = _session
        self.datasetid = datasetid
        self.filename = filename
        self._session._require_log_in()

        # Handle input case where user gives datasetid="dc://952874/index.csv"
        if isinstance(datasetid, str) and datasetid.startswith("dc://"):
            pieces = datasetid.split("/")
            self.datasetid = int(pieces[2])
            if len(pieces) > 3:
                self.filename = "/".join(pieces[3:])
    ...
    def __getitem__(self, key):
        return Dataset(
            self.datasetid, filename=key, _session=self._session, _props=self.props
        )
        ...
        while not downloaded and attempts > 0:
            try:
                result = _request_url(
                    ...
                    headers={"token": self._session._token, "file": self.filename},
                    _logger=_logger,
                )
    ...
    def remove(self, _logger=True):
        if self.filename != "":
            raise RuntimeError(
                "Cannot delete single file from dataset. Please run the remove command from the complete dataset object."
            )
        ...
        uri = f"{self.datasetid}"
        if len(self.filename):
            uri += f"/{self.filename}"

        r = _request_url(
            "delete",
            f"{self._session._server_endpoint}/datacenter/{uri}",
            headers=headers,
            _logger=_logger,
        )
        ...
        if _logger:
            logger.info(f"Dataset {uri} was deleted successfully.")
