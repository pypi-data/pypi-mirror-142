...
from .ffi import (
    ...
    LOCAL_TRANSACTION_EXECUTE_CALLBACK, _CTraceModel, _CLogLevel
)
...
class Producer(object):
    def __init__(self, group_id, orderly=False, timeout=None, compress_level=None, max_message_size=None):
        ...
        self.set_producer_language('PYTHON')
    ...
    def enable_producer_message_trace(self, enable_trace):
        if enable_trace:
            ffi_check(dll.SetProducerMessageTrace(self._handle, _CTraceModel.OPEN))
...
class PushConsumer(object):
    def __init__(self, group_id, orderly=False, message_model=MessageModel.CLUSTERING):
        ...
        self.set_consumer_language('PYTHON')
        if self._handle is None:
    ...
    def enable_consumer_message_trace(self, enable_trace):
        if enable_trace:
            ffi_check(dll.SetPushConsumerMessageTrace(self._handle, _CTraceModel.OPEN))
    ...
    def set_consumer_log_level(self, level):
        ffi_check(dll.SetPushConsumerLogLevel(self._handle, _CLogLevel(level)))
