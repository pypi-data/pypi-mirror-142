...
# --------
    ...
    def transform(
        ...
        op_deps, ds_deps, parent_ops, arguments = self._get_op_deps_parents_and_args(
            transform, arguments
        )
...
# -----------------------------------------------------
    ...
    def refresh_table(
        ...
        By Default we run the query to re-materializing the table in the background,
        meaning when the function finishes running it this doesn't necessarily mean
        the table recreation has finished.

        To avoid this behavior, set `the param refresh_in_background` to False.

        Args:
            refresh_in_background: If True will refresh the table as a background job, instead
                                   of waiting for the query to finish. If False function will
                                   finish executing after the query to refresh table does
...
# ---------------------------------
    ...
    def _get_op_deps_parents_and_args(
        ...
        This function will also raise important errors to users if they
        entered input args incorrectly. Error will be raised if
          - Someone supplied a arg/kwargs which doesn't exist as a arg on the transform
          - If the arg type was a 'table' or 'table_list' type will raise error if any
            are not type dataset

        Args:
            transform: The Transform which is being applied
            arguments: This of arguments a user supplied to the transform. This includes
        ...
        def update_op_deps__ds_deps__and_parent_ops(
                ds_arg: 'Dataset',
        ) -> None:
            """
        ...
        for arg_name, arg_val in arguments.items():
            arg_type = _get_transform_arg_type(arg_name, transform)

            # If arg type is a 'table', handle accordingly
            # Raise Value Error if not of type Dataset
            if arg_type == TABLE_ARG_TYPE:
                input_ds: Union['Dataset', Any] = arg_val
                if not isinstance(input_ds, self.__class__):
                    raise TypeError(f"The {transform.name}() transform's parameter "
                                    f"`{arg_name}` requires the value to be a single "
                                    f"Dataset obj. Got {type(arg_val)} instead")
                # Update deps and convert arg to fqtn if proper type
                update_op_deps__ds_deps__and_parent_ops(input_ds)
                arguments[arg_name] = input_ds.fqtn

            # If arg type is a 'table_list', handle accordingly
            # Raise Value Error if not of type List['Dataset']
            elif arg_type == TABLE_LIST_ARG_TYPE:
                input_datasets: List[Union['Dataset', Any]] = arg_val
                if not isinstance(input_datasets, list) or any(
                        [not isinstance(x, self.__class__) for x in input_datasets]
                ):
                    raise TypeError(f"The {transform.name}() transform's parameter "
                                    f"`{arg_name}` requires the value to be a non "
                                    f"empty list of Dataset objs.")
                # Update deps and convert arg to fqtns if proper type
                for input_ds in input_datasets:
                    update_op_deps__ds_deps__and_parent_ops(input_ds)
                arguments[arg_name] = [ds.fqtn for ds in input_datasets]
...
def _get_transform_arg_type(
        arg_name: str,
        transform: schemas.Transform
) -> Optional[str]:
    """
    Get transform argument type based on arg name

    If it's the 'apply' transform return None

    Raise TypeError if user entered a wrong kwarg/arg_name
    """
    # If it's the source table arg we set it is of table type
    if arg_name == SOURCE_TABLE_ARG_NAME:
        return TABLE_ARG_TYPE

    # If it's the apply transform don't check for arg types since don't have any
    if transform.name != 'apply':
        # If someone supplied a Kwarg to the transform function
        # which isn't a set param for that transform, raise an error
        matched_arg = [x for x in transform.arguments if x.name == arg_name]
        if not matched_arg:
            raise TypeError(f"The {transform.name}() transform got unexpected argument {arg_name!r}")
        # Return arg type of transform if found
        return matched_arg[0].type
