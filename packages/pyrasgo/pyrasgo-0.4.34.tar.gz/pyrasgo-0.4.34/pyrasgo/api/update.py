...
class Update():
    ...
    def dataset_table(
        ...
        A dataset needs to be published first in order call this function
        ...
        if verbose:
            print(f"Refreshing table {'in background ' if refresh_in_background else ''}"
                  f"for dataset with id '{dataset.id}' at fqtn: '{dataset.fqtn}'")


        # Call endpoint to kick off Query to re-create table in background
        self.api._put(
            f"/datasets/{dataset._api_dataset.id}/table-refresh",
            _json={'refreshInBackground': refresh_in_background},
            api_version=2
        ).json()

        if verbose:
            if refresh_in_background:
                print(f"Job to Refresh table with dataset id '{dataset.id}' at fqtn: '{dataset.fqtn}' kicked off in background")
            else:
