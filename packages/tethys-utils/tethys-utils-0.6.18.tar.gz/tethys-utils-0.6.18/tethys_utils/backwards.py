...
def copy_old_dataset_to_new(source_remote, dest_remote, dataset, temp_path, remove_station_id=True, max_workers=4):
    """
    ...
    titan = Titan(temp_path=temp_path)
    ...
    results_list = []
    for s in stns:
        r1 = tethys.get_results(dataset_id, s['station_id'])
        if remove_station_id:
            r1 = r1.drop('station_id', errors='ignore')

        r_list = titan.save_preprocessed_results(dataset_id, r1)
        results_list.extend(r_list)

    print('-- Process old results into new')
    new_paths11, stn_paths1 = titan.save_final_results(results_list, correct_times=False, max_workers=max_workers)

    print('-- Upload results')
    results_keys, stn_keys = titan.upload_final_results(new_paths11, threads=80)

    print('-- Update aggregates')
    ds_all = titan.update_aggregates(120)
