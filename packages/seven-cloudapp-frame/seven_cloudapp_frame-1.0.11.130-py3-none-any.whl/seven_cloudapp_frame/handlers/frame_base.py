...
def filter_check_high_power(key_names):
    """
    :description: 中台高级权限校验装饰器
    :param key_names: 权限代码 多个逗号分隔
    :last_editors: HuangJianYi
    """
    def check_high_power(handler):
        def wrapper(self, **args):
            if self.request_params:
                app_id = self.get_app_id()
                if not app_id:
                ...
                if app_info_dict:
                    ...
                    is_power = mp_base_model.check_high_power(store_user_nick, app_info_dict["project_code"], key_names)
                if is_power == False:
                    return self.response_json_error("no_power", "没有权限操作")

        return wrapper

    return check_high_power
