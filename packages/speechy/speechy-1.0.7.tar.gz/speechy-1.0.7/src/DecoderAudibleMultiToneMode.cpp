...
DecoderAudibleMultiTone::DecoderAudibleMultiTone(float samplingRate, int buffSize, int windowSize) : Decoder(samplingRate, buffSize, windowSize, Globals::numTokensAudible, Globals::numTonesAudibleMultiTone)
{
#ifdef _IOS_LOG_
  ...
  for (int i = 0; i<mNumTones; i++)
    mFreqsBins[i] = (int)(Globals::getToneFromIdxAudibleMultiTone(i, mSampleRate, mWindowSize) * mFreq2Bin + .5);

  //Optimize size of block spectrogram (only needed bins in token space range)
  mBeginBin = (int)(Globals::getToneFromIdxAudibleMultiTone(0, mSampleRate, mWindowSize) * mFreq2Bin + .5);
  mEndBin = (int)(Globals::getToneFromIdxAudibleMultiTone(mNumTones - 1, mSampleRate, mWindowSize) * mFreq2Bin + .5);
  ...
  Globals::getIdxsFromIdxAudibleMultiTone(idxFrontDoorToken1, &idxTonesFrontDoorToken1);
  idxTonesFrontDoorToken2 = new int[2];
  Globals::getIdxsFromIdxAudibleMultiTone(idxFrontDoorToken2, &idxTonesFrontDoorToken2);

  mDecodingMode = Globals::/*DECODING_MODE::*/DECODING_MODE_AUDIBLE; //0=AUDIBLE, 1=NONAUDIBLE, 2=COMPRESSION, 3=CUSTOM
}
...
int DecoderAudibleMultiTone::getSizeFilledFrameCircularBuffer()
{
  return Decoder::getSizeFilledFrameCircularBuffer();
}

int DecoderAudibleMultiTone::getSizeFilledBlockCircularBuffer()
{
...
int DecoderAudibleMultiTone::AnalyzeStartTokens(float *audioBuffer)
...
int DecoderAudibleMultiTone::AnalyzeToken(float *audioBuffer)
{
  ...
  { //we have a new filled block of (durToken size)
    ...
    for (int i = 0; i<(mSizeBlockCircularBuffer / 2); i++)
    {
	  ...
	  //Statistics
	  mBlockTokenStatistics[s].idxToken = Globals::getIdxTokenFromIdxsTonesAudibleMultiTone(mBlockTokenStatistics[s].idxToneMax, mBlockTokenStatistics[s].idxToneSecond);
    ...
    int max = Globals::getIdxTokenFromIdxsTonesAudibleMultiTone(max1, max2);
...
int DecoderAudibleMultiTone::ComputeStats()
{
  ...
  for (int t=0;t<(mSizeBlockCircularBuffer/2);t++)
  {
    for (int i=0;i<mNumTones;i++)
    {
      ...
      /*Globals::getIdxsFromIdxAudibleMultiTone(i,(int**)&mIdxs);
      //CURRENT TOKEN
      int evalTokenBin1 = (int)(Globals::getToneFromIdxAudibleMultiTone(mIdxs[0], mSampleRate, mWindowSize) * mFreq2Bin + .5);
      int evalTokenBin2 = (int)(Globals::getToneFromIdxAudibleMultiTone(mIdxs[1], mSampleRate, mWindowSize) * mFreq2Bin + .5);
      */
...
/*  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++)
  {
    ...
    int energyRatioTokenIdx = Globals::getIdxTokenFromIdxsTonesAudibleMultiTone(mBlockEnergyRatiosMaxToneIdx[s], mBlockEnergyRatiosSecondToneIdx[s]);
...
float DecoderAudibleMultiTone::ComputeBlockMagSpecSumsLastToken(int midFreqBin, int width, int nbins, std::vector<float> &sumPerFrame)
{
  float mean = 0;
  float sumFr = 0;
...
float DecoderAudibleMultiTone::ComputeBlockMagSpecSumsCurrentToken(int midFreqBin, int width, int nbins, std::vector<float> &sumPerFrame)
{
  float mean = 0;
  float sumFr = 0;
