...
EncoderCompressionMultiTone::EncoderCompressionMultiTone(float samplingRate, int buffsize, int windowSize) : Encoder(samplingRate, buffsize, windowSize, Globals::numTokensCompression, Globals::numTonesCompressionMultiTone)
{
//__android_log_print(ANDROID_LOG_INFO, "OxyCoreLibInfo", "EncoderCompressionMultiTone init" );
  mCurrentFreqs = new float[2];
  mCurrentFreqsLoudness = new float[2];
}

EncoderCompressionMultiTone::~EncoderCompressionMultiTone(void)
{
  delete [] mCurrentFreqs;
  delete [] mCurrentFreqsLoudness;
}

int EncoderCompressionMultiTone::EncodeDataToAudioBuffer(const char *stringToEncode, int type, int size, const char *melodyString, int melodySize)
{
...
//  __android_log_print(ANDROID_LOG_INFO, "OxyCoreLibInfo", "Added digits");
  ...
  for (int i = 0; i < digits.size(); i++)
  {
    ...
    Globals::getFreqsFromIdxCompressionMultiTone(digits[i], mSampleRate, mWindowSize, (float**)&mCurrentFreqs);
    Globals::getLoudnessCompressionMultiToneFromIdx(digits[i], (float**)&mCurrentFreqsLoudness);
    ...
    if (i > 1) //after the start tokens
    {
      ...
      {
        mCurrentFreqs[0] += Globals::freqOffsetForCompressionMultiTone;
        mCurrentFreqs[1] += Globals::freqOffsetForCompressionMultiTone;
      }
      else if ((n % 3) == 2) //Take care, encoder and decoder do not share same odd/even order if you compare mDecoding with digits index
      {
        mCurrentFreqs[0] += Globals::freqOffsetForCompressionMultiTone * 2;
        mCurrentFreqs[1] += Globals::freqOffsetForCompressionMultiTone * 2;
      }
    }
...
int EncoderCompressionMultiTone::GetEncodedAudioBuffer(float *audioBuffer)
{
  return Encoder::GetEncodedAudioBuffer(audioBuffer);
}

int EncoderCompressionMultiTone::ResetEncodedAudioBuffer()
{
