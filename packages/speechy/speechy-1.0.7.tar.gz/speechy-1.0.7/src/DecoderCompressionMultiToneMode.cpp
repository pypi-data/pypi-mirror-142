...
DecoderCompressionMultiTone::DecoderCompressionMultiTone(float samplingRate, int buffSize, int windowSize) : Decoder(samplingRate, buffSize, windowSize, Globals::numTokensCompression, Globals::numTonesCompressionMultiTone)
{
#ifdef _IOS_LOG_
  ios_log("C++ DecoderCompressionMultiTone");
#endif //_IOS_LOG_
  mFreq2Bin = mSpectralAnalysis->mFftSize / mSampleRate;
  mFreqsBins = new int[mNumTones];
  for (int i = 0; i<mNumTones; i++)
    mFreqsBins[i] = (int)(Globals::getToneFromIdxCompressionMultiTone(i, mSampleRate, mWindowSize) * mFreq2Bin + .5);

  //Optimize size of block spectrogram (only needed bins in token space range)
  mBeginBin = (int)(Globals::getToneFromIdxCompressionMultiTone(0, mSampleRate, mWindowSize) * mFreq2Bin + .5);
  mEndBin = (int)(Globals::getToneFromIdxCompressionMultiTone(mNumTones - 1, mSampleRate, mWindowSize) * mFreq2Bin + .5);
  ...
  Globals::getIdxsFromIdxCompressionMultiTone(idxFrontDoorToken1, &idxTonesFrontDoorToken1);
  idxTonesFrontDoorToken2 = new int[2];
  Globals::getIdxsFromIdxCompressionMultiTone(idxFrontDoorToken2, &idxTonesFrontDoorToken2);

  mDecodingMode = Globals::DECODING_MODE_COMPRESSION; //0=AUDIBLE, 1=NONAUDIBLE, 2=COMPRESSION, 3=CUSTOM
}

DecoderCompressionMultiTone::~DecoderCompressionMultiTone(void)
{
  delete[] mFreqsBins;
  delete[] mIdxs;
...
int DecoderCompressionMultiTone::getSizeFilledFrameCircularBuffer()
{
  return Decoder::getSizeFilledFrameCircularBuffer();
}

int DecoderCompressionMultiTone::getSizeFilledBlockCircularBuffer()
{
...
int DecoderCompressionMultiTone::DecodeAudioBuffer(float *audioBuffer, int size)
{
  ...
  {  	//copy from circularBufferFloat to sendBuffer
    ...
    else if (mDecoding > Globals::numMessageTokens) //we have finished decoding a complete word
    {
      ...
      mReceivedOxysVolume = mReceivedOxysVolume / Globals::numMessageTokens;
...
int DecoderCompressionMultiTone::GetDecodedData(char *stringDecoded)
{
  int messageOk = 1;
...
int DecoderCompressionMultiTone::GetSpectrum(float *spectrumBuffer)
{
  return Decoder::GetSpectrum(spectrumBuffer);
}


int DecoderCompressionMultiTone::AnalyzeStartTokens(float *audioBuffer)
{ //float *magSpectrum, float* realSpectrum, float* imagSpectrum
  ...
  __android_log_write(ANDROID_LOG_INFO, "OxyCoreLibInfo", text);//Or ANDROID_LOG_INFO, ...
  //__android_log_write(ANDROID_LOG_INFO, "OxyCoreLibInfo", pStringDecoded);//Or ANDROID_LOG_INFO, ...
#endif
  ...
  __android_log_write(ANDROID_LOG_INFO, "OxyCoreLibInfo", text2);//Or ANDROID_LOG_INFO, ...
  //__android_log_write(ANDROID_LOG_INFO, "OxyCoreLibInfo", pStringDecoded);//Or ANDROID_LOG_INFO, ...
#endif
...
int DecoderCompressionMultiTone::AnalyzeToken(float *audioBuffer)
{
  //float *magSpectrum, float* realSpectrum, float* imagSpectrum
  mSpectralAnalysis->doFFT(audioBuffer, mSpectralAnalysis->mSpecMag, mSpectralAnalysis->mSpecPhase);
  ...
  { //we have a new filled block of (durToken size)
    ...
    for (int i = 0; i<(mSizeBlockCircularBuffer / 2); i++)
    {
	  ...
	  //Statistics
	  mBlockTokenStatistics[s].idxToken = Globals::getIdxTokenFromIdxsTonesCompressionMultiTone(mBlockTokenStatistics[s].idxToneMax, mBlockTokenStatistics[s].idxToneSecond);
    ...
    int max = Globals::getIdxTokenFromIdxsTonesCompressionMultiTone(max1, max2);
    ...
    mReceivedOxysVolume += energy / (float)(mSizeBlockCircularBuffer / 2);
...
int DecoderCompressionMultiTone::ComputeStatsStartTokens()
{
...
int DecoderCompressionMultiTone::ComputeStats()
{
  //energy mean in the alphabet frequency region
  ...
  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++)
  {
    for (int i = 0; i < mNumTones; i++)
    {
      ...
      */
      ...
      if ((mDecoding % 3) == 1)
      {
        evalToneBin += Globals::nBinsOffsetForCompressionMultiTone;
      }
      else if ((mDecoding % 3) == 2)
      {
        evalToneBin += Globals::nBinsOffsetForCompressionMultiTone * 2;
      }
...
/*  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++)
  {
    ...
    int energyRatioTokenIdx = Globals::getIdxTokenFromIdxsTonesCompressionMultiTone(mBlockEnergyRatiosMaxToneIdx[s], mBlockEnergyRatiosSecondToneIdx[s]);
...
float DecoderCompressionMultiTone::ComputeBlockMagSpecSumsLastToken(int midFreqBin, int width, int nbins, std::vector<float> &sumPerFrame)
{
  float mean = 0;
  float sumFr = 0;
...
float DecoderCompressionMultiTone::ComputeBlockMagSpecSumsCurrentToken(int midFreqBin, int width, int nbins, std::vector<float> &sumPerFrame)
{
  float mean = 0;
  float sumFr = 0;
