...
EncoderCustomMultiTone::EncoderCustomMultiTone(float samplingRate, int buffsize, int windowSize) : Encoder(samplingRate, buffsize, windowSize, Globals::numTokensCustom, Globals::numTonesCustomMultiTone)
{
//__android_log_print(ANDROID_LOG_INFO, "OxyCoreLibInfo", "EncoderNonAudibleMultiTone init" );
  mCurrentFreqs = new float[2];
  mCurrentFreqsLoudness = new float[2];
}

EncoderCustomMultiTone::~EncoderCustomMultiTone(void)
{
  delete [] mCurrentFreqs;
  delete [] mCurrentFreqsLoudness;
}

int EncoderCustomMultiTone::EncodeDataToAudioBuffer(const char *stringToEncode, int type, int size, const char *melodyString, int melodySize)
{
...
//  __android_log_print(ANDROID_LOG_INFO, "OxyCoreLibInfo", "Added digits");
  ...
  for (int i = 0; i < digits.size(); i++)
  {
    ...
    Globals::getFreqsFromIdxCustomMultiTone(digits[i], mSampleRate, mWindowSize, (float**)&mCurrentFreqs);
    Globals::getLoudnessCustomMultiToneFromIdx(digits[i], (float**)&mCurrentFreqsLoudness);
  ...
  if ((mnAudioSignatureSamples > 0) && (mAudioSignature)) //Add Audio Signature (mAudioSignature) to mAudioBufferEncodedString of size mNumSamplesEncodedString
  {
...
int EncoderCustomMultiTone::GetEncodedAudioBuffer(float *audioBuffer)
{
  return Encoder::GetEncodedAudioBuffer(audioBuffer);
}

int EncoderCustomMultiTone::ResetEncodedAudioBuffer()
{
