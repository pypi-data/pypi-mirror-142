...
class C50Tree(object):
    ...
    def fit(
        ...
        # executable_name = 'c50_mac' if sys.platform == 'darwin' else 'c50_linux'
        ...
        # bash_execute_command = f'time ./bin/c5.0 -f {self.working_dir}/c5.0-{self.id}'
    ...
    def _parse_output(self, c50_output, rule_based=False):
        ...
        rendered_estimator = self._parse_tree(c50_output)
        with open(f'{self.working_dir}/estimator_{self.id}.py', 'w') as f:
            f.write(rendered_estimator)

    def _parse_tree(self, c50_output):
