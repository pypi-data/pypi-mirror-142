...
class BatchEnquiry(widgets.Frame):
    def __init__(self, screen: Screen, pod: PodTui):
        ...
        self._pod = pod
        self._key_ords = tui_util.get_ords('qbr')
        self._btn_reset()
        ...
        self._btn_bi_edit = widgets.Button("Edit", self._btn_bi_edit_click, disabled=True)
        self._btn_bi_edit_items = widgets.Button("Edit Job Items", self._btn_bi_edit_items_click, disabled=True)
        self._btn_bi_create = widgets.Button("Create", self._btn_bi_create_click, disabled=True)
...
class BatchView(widgets.Frame):
    def __init__(self, screen: Screen, pod: PodTui):
        ...
        layout_btns.add_widget(widgets.Button("OK", self._ok), 0)
        layout_btns.add_widget(widgets.Button("Cancel", self._cancel), 1)
        self.fix()
    ...
    def reset(self):
        ...
        self.data = {
            ...
            'ext_data': str(self._rec.ext_data) if self._rec.ext_data else '',
    ...
    def _validate_record(self) -> bool:
        ...
        if ri < 1 or ri > 60:
            pop = widgets.PopUpDialog(self._screen, "Run Intveral must be between 1 and 60.", ["OK"], has_shadow=True)
