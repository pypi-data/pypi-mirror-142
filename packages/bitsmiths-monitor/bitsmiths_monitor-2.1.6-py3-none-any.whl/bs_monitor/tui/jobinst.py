...
class JobInstEnquiry(widgets.Frame):
    def __init__(self, screen: Screen, pod: PodTui):
        ...
        layout_crit_btns.add_widget(widgets.Button("Refresh", self._btn_refresh_click), 0)
        layout_crit_btns.add_widget(widgets.Button("Reset", self._btn_reset), 1)
        layout_crit_btns.add_widget(widgets.Button("Back", self._btn_back), 2)
        layout_crit_btns.add_widget(widgets.Button("Quit", self._btn_quit), 3)

        self._grid = widgets.MultiColumnListBox(
            widgets.Widget.FILL_FRAME,
            [">10", ">10", "<32", ">10", "<20", "<12", ">10", "<20", "<20", "100%" ],
            [],
            titles=[
    ...
    def _btn_ji_rerun_click(self) -> None:
        self._scene.add_effect(widgets.PopUpDialog(self._screen, "Are you sure you want Rerun?", tui_uti.NO_YES_OPTIONS, has_shadow=True, on_close=self._confirm_rerun))  # noqa

    def _btn_ji_forceok_click(self) -> None:
        self._scene.add_effect(widgets.PopUpDialog(self._screen, "Are you sure you want Force OK?", tui_uti.NO_YES_OPTIONS, has_shadow=True, on_close=self._confirm_force_ok))  # noqa

    def _btn_ji_cancel_click(self) -> None:
        self._scene.add_effect(widgets.PopUpDialog(self._screen, "Are you sure you want Stop/Cancel this job?", tui_uti.NO_YES_OPTIONS, has_shadow=True, on_close=self._confirm_cancel))  # noqa
