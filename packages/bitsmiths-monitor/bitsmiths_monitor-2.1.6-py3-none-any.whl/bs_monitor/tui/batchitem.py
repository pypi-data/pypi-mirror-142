...
class BatchItemEnquiry(widgets.Frame):
    def __init__(self, screen: Screen, pod: PodTui):
        super(BatchItemEnquiry, self).__init__(
            screen, int(screen.height * 2 // 3), 100, on_load=self._btn_refresh_click, has_shadow=True, hover_focus=True,
            can_scroll=True, reduce_cpu=True, title="Batch Item Enquiry"
        )
        ...
        layout_btns.add_widget(widgets.Button("Refresh", self._btn_refresh_click), 0)
        layout_btns.add_widget(widgets.Button("Back", self._btn_back), 1)
        layout_btns.add_widget(widgets.Button("Quit", self._btn_quit), 2)
        ...
        self._btn_bi_create = widgets.Button("Create", self._btn_bi_create_click, disabled=False)
        self._btn_bi_delete = widgets.Button("Delete", self._btn_bi_delete_click, disabled=True)
