...
class GlueKernel(IPythonKernel):
    ...
    def configure(self, configs_json):
        kernel_managed_params = {'profile', 'endpoint', 'region', 'iam_role', 'session_id', 'max_capacity',
                                 'number_of_workers', 'worker_type', 'connections', 'glue_version',
                                 'security_config'}
        try:
            ...
            return
        if not configs:
            self.ipython_display.send_error("No configuration values were provided.")
        else:
    ...
    def _refresh(self):
        ...
        try:
            ...
            self._log_to_terminal(f'Exception encountered when refreshing credentials: {e}')
    ...
    async def _print_startup_text(self):
        ...
        if latest_kernel_version and current_kernel_version and latest_kernel_version != current_kernel_version:
            self._send_output(
