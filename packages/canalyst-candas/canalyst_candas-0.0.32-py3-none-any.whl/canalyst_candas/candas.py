...
class Search:
    ...
    def get_ticker_list(self, ticker=""):
        ...
        if self.ticker_list is None:
            ...
            df = self.ticker_list
        if ticker != "":
            df = df.loc[df["Ticker Alternate 1"] == ticker]
        return df
...
class Model:
    ...
    def forward_data_frame(self):
        ...
        if self.csin == "":
            self.csin, self.latest_version = get_company_info_from_ticker(
                self.ticker, self.api_headers, self.log, self.config.mds_host
            )

        url = get_forecast_url(self.csin, self.latest_version, self.config.mds_host)

        res = get_request(url, self.api_headers, self.log)

        list_out = []

        for res_dict in res.json()["results"]:
            df = get_forecast_url_data(
                res_dict, self.ticker, self.api_headers, self.log
            )
            list_out.append(df)
    ...
    def model_fit(self, time_series_name=""):
        ...
        if self.scenario_url_map.get(self.uuid) is None:
            scenario_response = get_request(
                scenario_url, headers=self.api_headers, logger=self.log
            )
        ...
        if time_series_name != "":
            ...
            res_loop = get_request(url, self.api_headers, self.log)
        ...
        scenario_response = get_request(
            scenario_id_url, headers=self.api_headers, logger=self.log
        )
        all_df = []
