...
if typing.TYPE_CHECKING:
  ...
  from .ffi.wrappers import EnumTraits
...
class KinematicsParserTLS(local):
  ...
  def __init__(self):
    ...
    self.parsed_joint_strs: 'dict[str, Any]' = dict()
    self.parsed_frame_type_strs: 'dict[str, Any]' = dict()
    self.parsed_actuator_strs: 'dict[str, Any]' = dict()
    self.parsed_link_strs: 'dict[str, Any]' = dict()
    self.parsed_bracket_strs: 'dict[str, Any]' = dict()
  ...
  def set_frame_type(self, frame_type: str, value: "Any"):
...
def actuator_str_to_enum(actuator: str):
  ...
  if actuator_ci not in __actuator_to_enum:
    raise ValueError('Unknown actuator type {}'.format(actuator_ci))
...
def link_str_to_enum(link: str):
  ...
  if link_ci not in __actuator_links_to_enum:
    raise ValueError('Unknown link type {}'.format(link_ci))
...
def _add_pure_ints(d: 'dict[Any, str]'):
  to_add = [(int(key), val) for (key, val) in d.items() if type(key) is not int]
  for k, v in to_add:
    d[k] = v
