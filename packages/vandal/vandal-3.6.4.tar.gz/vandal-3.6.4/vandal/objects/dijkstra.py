...
class Dijkstra:
    ...
    def execute(
        ...
        if optimal_distance[self.destination] != float('inf'):
            ...
            return {'Optimal path' : output_path, 'Minimal distance ' : optimal_distance[self.destination]}

        else:

            if filtered == False:
                optimal_distance[self.destination] = None
                output_path = None
                print('Optimal path: ' + str(output_path), '\nMinimal distance: ' + str(optimal_distance[self.destination]))

            return {'Optimal path' : None, 'Minimal distance' : None}
