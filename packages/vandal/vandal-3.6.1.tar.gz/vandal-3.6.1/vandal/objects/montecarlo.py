...
class MonteCarlo:
    ...
    def get_stats(
        ...
        return {'Mean' : mean_value, 'Standard deviation' : standard_deviation, 'Maximum value: ' : maximum_value, 'Minimum value' : minimum_value}
...
def MCapp():
    ...
    while True:
        ...
        if action == 'stats' or action == 'statistics':
            MC.get_stats(filtered = False)

        if action == 'risk':
            sample = int(input('Number of iterations to measure risk on: ') or 5000)

            MC.get_risk(risk_sims = sample, filtered = False)
