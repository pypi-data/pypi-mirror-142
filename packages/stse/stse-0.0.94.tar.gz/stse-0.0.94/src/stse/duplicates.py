...
class Duplicates:
    def filter(self, df, subsets, return_duplicates=False, how='all'):  # *
    ...
    def remove(self, df, subsets):  # *
        ...
        """
        return self.filter(df, subsets=subsets)

    def indices(self, df, column):  # *
        return list(df[pd.DataFrame.duplicated(df, subset=[column])].index)

    def average(self, df, subsets, average_by):  # *
        ...
        out = self.filter(df_copy, subsets)
