...
class DeepTagger(SupervisedNLPModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.output_format == "bio":
            self.inputs.append(
                tf.keras.Input(shape=(None,), dtype=tf.int32, name="tag_ids")
            )
    ...
    def build_output_layer(self, inputs: list[tf.Tensor]) -> list[tf.Tensor]:
        boolean_mask_layer = tf.keras.layers.Lambda(
            lambda inputs: tf.math.logical_and(inputs[0], tf.equal(inputs[1], 0)),
            name="boolean_mask_layer",
        )
        mask_layer = tf.keras.layers.Lambda(
            lambda inputs: tf.ragged.boolean_mask(inputs[0], inputs[1]).to_tensor(),
            name="mask_layer",
        )
        if self.output_format == "bio":
            encodings, type_ids, mask, tag_ids = inputs
            boolean_mask = boolean_mask_layer([mask, type_ids])
            encodings = tf.ragged.boolean_mask(encodings, boolean_mask).to_tensor()
            emissions = MLPLayer(
                self.num_fc_layers,
            ...
            new_mask = mask_layer([mask, boolean_mask])
            ...
            return crf_layer([emissions, tag_ids], new_mask)
        else:
            encodings, type_ids, mask = inputs
            boolean_mask = boolean_mask_layer([mask, type_ids])
            encodings = tf.ragged.boolean_mask(encodings, boolean_mask).to_tensor()
            new_mask = mask_layer([mask, boolean_mask])
    ...
    def predict(
        ...
        *,
        ...
        if isinstance(inputs[0], list):
            texts = [i[0] for i in inputs]
        else:
            texts = inputs
        tokens_list = dataset.tokenize(texts)
