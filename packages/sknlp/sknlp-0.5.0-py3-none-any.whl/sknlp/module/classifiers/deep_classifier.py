...
from sknlp.layers import MLPLayer, PreprocessingLayer
from sknlp.losses import MultiLabelCategoricalCrossentropy
from sknlp.metrics import (
...
class DeepClassifier(SupervisedNLPModel):
    ...
    def thresholds(self, thresholds: Union[list[float], dict[str, float]]) -> None:
        if isinstance(thresholds, list) and len(thresholds) != self.num_classes:
            raise ValueError(
                f"类别数为{self.num_classes}, " f"但`thresholds`长度为{len(thresholds)}."
            )
        elif isinstance(thresholds, dict):
            _thresholds = self.thresholds
            for c, t in thresholds.items():
                _thresholds[self.class2idx(c)] = t
        self._inference_kwargs["thresholds"] = dict(zip(self.classes, thresholds))
    ...
    def get_metrics(self, *args, **kwargs) -> list[tf.keras.metrics.Metric]:
        ...
        if self.num_classes == 2:
            class_id = 1
        return [
            SparsePrecisionWithLogits(activation=activation, class_id=class_id),
            SparseRecallWithLogits(activation=activation, class_id=class_id),
            SparseFBetaScoreWithLogits(
