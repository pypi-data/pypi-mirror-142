...
class Bert2vec(Text2vec):
    def __init__(
        self,
        vocab: Vocab,
        segmenter: str | None = "bert",
        hidden_size: int = 768,
        embedding_size: int | None = 768,
        num_layers: int = 12,
    ...
    def __call__(
        self, inputs: list[tf.Tensor], logits_mask: tf.Tensor | None = None
    ) -> list[tf.Tensor]:
    ...
    def update_dropout(
        self, dropout: float, attention_dropout: float | None = None
    ) -> None:
