...
def np_struct_from_patient(patient, struct_name, case_sensitive):
    ...
    structure_matrix = np.zeros(matrix_shape, dtype=int)
    for imagenum in range(0, len(patient['images'])): 
        ...
        structure_matrix[imagenum] = structure_matrix[imagenum] + struct_slice 
...
def struct_slice_np(name, im_slice, structurepixlut, structure, position, prone, feetfirst):
    ...
    mask = np.zeros(np.rot90(im_slice).shape, dtype=int)
    ...
    if (zmin < 0.5):
        ...
        for contour in structure['planes'][list(structure['zkeys'])[index]]:
            ...
            mask += polygon2mask(np.rot90(im_slice).shape, verts).astype(int)
        ...
        mask[mask > 1] = 1
        return mask
    return np.zeros(im_slice.shape, dtype=int)
