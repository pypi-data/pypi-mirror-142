...
class pure_fall():
    def __init__(
        ...
        #沪深300和中证500成分股
        self.hs300=pd.read_excel(self.homeplace.daily_data_file+'300和500成分股.xlsx',
                                 sheet_name='沪深300').stack().reset_index(level=1)
        self.zz500=pd.read_excel(self.homeplace.daily_data_file+'300和500成分股.xlsx',
                                 sheet_name='中证500').stack().reset_index(level=1)
    ...
    def run(self, whole=False,daily_func=None, monthly_func=None,
            neutralize=False,boxcox=False,hs300=False,zz500=False):
        '''执必要的函数，将分钟数据变成月度因子'''
