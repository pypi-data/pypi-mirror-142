...
function _toRASPJson(context) {
    ...
    const contentTypeHeader = context && context['requestHeaders'] && context['requestHeaders']['content-type'];
    if (_.isValidJsonRequest(contentTypeHeader)) {
        if (!_.isBuffer(context['requestBody'])) {
            return {};
        }

        const parsedBody = _.parseIfJson(context['requestBody'].toString());;
    ...
    if (_.isValidEncodedFormDataRequest(contentTypeHeader)) {
        ...
        return qs.parse(context['requestBody'].toString());
    }
