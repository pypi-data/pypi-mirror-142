    ...
    scanHttpBody(requestData) {
        ...
        try {
            if (
                ...
                !_.isString(requestData.data.requestBody)
            ) {
                return result;
            }
            ...
            if (_.isValidJsonRequest(contentTypeHeader)) {
                requestToCheck.body.raw = requestData.data.requestBody;
            }
            if (_.isValidEncodedFormDataRequest(contentTypeHeader)) {
                requestToCheck.body.raw = JSON.stringify(qs.parse(inputData.requestBody));
            }
