...
def _get_value_dm(product: dict, dm_percent: float):
    ...
    slope = safe_parse_float(get_crop_lookup_value(MODEL, term_id, 'crop_residue_slope'))
    intercept = safe_parse_float(get_crop_lookup_value(MODEL, term_id, COLUMN_NAME))
    ab_bg_ratio = safe_parse_float(
        get_crop_lookup_value(MODEL, term_id, 'ratio_abv_to_below_grou_crop_residue')
    )
