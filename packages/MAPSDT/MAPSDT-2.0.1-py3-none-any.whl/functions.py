...
def processContinuousFeatures(data, attribute, entropy, depth, max_depth):
    if len(set(map(lambda x: x.__getattribute__(attribute.name), data))) <= 20:
        ...
        unique_values = []
        data_min = np.array(list(map(lambda x: x.__getattribute__(attribute.name), data)), dtype=np.float64).min()
        data_max = np.array(list(map(lambda x: x.__getattribute__(attribute.name), data)), dtype=np.float64).max()
        scales = list(range(7))
        for scale in scales:
