...
def getColorMappable( vmin, vmax, cmap = "viridis", return_cbar = False ):
    ...
    if return_cbar :
        c = np.linspace(vmin,vmax , 15)
        _ , ax_ = plt.subplots()
        dummie_cax = ax_.scatter(c, c, c=c, cmap=cmap, norm = cNorm)
        plt.close(_)
        return scalarMap, dummie_cax
    else :
        return scalarMap
...
def set_major_format( ax, format = ".2%" , axis = "x" ) :
    import matplotlib.ticker as tick
    def fun( x , p ):
        return f"{x:{format:}}"
    if axis == "x" :
        ax.xaxis.set_major_formatter(tick.FuncFormatter(fun))
    elif axis == "y" :
        ax.yaxis.set_major_formatter(tick.FuncFormatter(fun))
    else :
        raise(Exception())
