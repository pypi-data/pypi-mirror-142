...
def dfSurface(df, ax=None, nbColors=200, interpolate=True, polar=False, polarConvention="seakeeping",
    ...
    **kwargs:
    ...
    if (vmin is not None) and (vmax is not None): lvls = np.linspace(vmin,vmax,nbColors)
    else: lvls = nbColors
    if clip and ((vmin is not None) or (vmax is not None)): df = df.clip(vmin,vmax)
    ...
    if interpolate:
        cax = ax.contourf(df.columns.astype(float), df.index, val, cmap=cmap, levels=lvls, norm=norm, **kwargs)
    else:
        try:
