...
def load(usermodel_name, load_type, config=None, *args, **kwargs):
    ...
    elif load_type == "PytorchModel" and hasattr(usermodel.Config, "PytorchModel"):
        pytorch_model = PytorchModel().load_from(usermodel)

    elif load_type == "ONNXModel" and hasattr(usermodel.Config, "ONNXModel"):
        onnx_model = ONNXModel().load_from(usermodel)

    elif load_type == "TensorRTModel" and hasattr(usermodel.Config, "TensorRTModel"):
        trt_model = TensorRTModel().load_from(usermodel)

    else:
        raise Exception(
            f"no model can be imported because of load_type {load_type} and config did not match"
        )
    return usermodel
