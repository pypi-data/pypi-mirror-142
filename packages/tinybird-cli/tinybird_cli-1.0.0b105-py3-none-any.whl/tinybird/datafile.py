...
async def new_pipe(p, tb_client: TinyB, replace=False, check=True, populate=False, sample=None, wait_populate=False, skip_tokens=False, tag='', skip_table_checks=False, ignore_sql_errors=False, only_response_times=False, timeout=None, run_tests=False, as_standard=False):  # noqa: C901
    exists = False
    materialized = False
    ...
    for node in p['nodes']:
        ...
        if pipe_type == 'materialized':
            ...
            if populate:
                ...
                if sample:
                    click.echo(FeedbackManager.info_sample_populate_job_url(url=job_url, sample=sample))
                else:
...
async def exec_file(r, tb_client, override, check, debug, populate, sample, wait_populate, tag, skip_table_checks=False, ignore_sql_errors=False, skip_confirmation=False, only_response_times=False, timeout=None, run_tests=False, as_standard=False):
    if debug:
...
async def folder_push(tb_client: TinyB, tag='', filenames=None, dry_run=False, check=False, push_deps=False,  # noqa: C901
                      debug=False, force=False, folder='.', populate=False, sample=None, upload_fixtures=False, wait=False,
            ...
            if name in to_run:
                if not dry_run:
                    if name not in remote_resource_names or resource_versions.get(name.replace(".", "_"), '') != latest_datasource_versions.get(name, '') or force or run_tests:
                        ...
                        try:
                            await exec_file(to_run[name], tb_client, force, check, debug, populate, sample, wait, tag,
