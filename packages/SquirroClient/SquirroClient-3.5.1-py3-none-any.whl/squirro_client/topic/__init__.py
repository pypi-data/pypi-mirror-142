...
class TopicApiBaseMixin:
    def get_projects(self):
        ...
        url = "{ep}/v0/{tenant}/projects".format(
            ep=self.topic_api_url, tenant=self.tenant,
        )
    ...
    def get_pipelet(self, name):
        ...
        url = "{ep}/v0/{tenant}/pipelets/{name}".format(
            ep=self.topic_api_url, tenant=self.tenant, name=name,
        )
        res = self._perform_request("get", url)
        return self._process_response(res)
    ...
    def get_typeahead_suggestions(
        ...
        Example::
            ...
            # - Highlighted tokens are returned as <b> html tags (highlight_cursor_start/end is not used)
            ...
            {"suggestions":[
                  {
                     ...
                     "highlight_cursor_start":0,
                     "highlight_cursor_end":0
                  },
                  {
                     "type":"text",
                     "key":" \"plant proteins\"",
