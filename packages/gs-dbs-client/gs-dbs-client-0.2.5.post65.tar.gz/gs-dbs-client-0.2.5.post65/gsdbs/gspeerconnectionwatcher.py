...
class GSPeerConnectionWatcher:
    class VideoTransformTrack(MediaStreamTrack):
        ...
        def __init__(self, track, source, onframe):
            super().__init__()
            self.track = track
        ...
        async def recv(self):
            ...
            self.onframe(self.source, frame)
            return frame
    ...
    async def create(cls, gsdbs, target, onframe, onmessage):
        ...
        async def offer(id, description):
            ...
            def on_track(track):
                if track.kind == "video":
                    local_video = self.VideoTransformTrack(track, source=self.target, onframe=self.onframe)
