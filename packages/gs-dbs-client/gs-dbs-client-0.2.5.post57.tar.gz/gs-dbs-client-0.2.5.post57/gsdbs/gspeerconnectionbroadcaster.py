...
class GSPeerConnectionBroadcaster:
    ...
    def create_local_tracks(self, device, transcode=True, options=None):
        global relay, webcam
        ...
        if relay is None:
            if platform.system() == "Darwin":
                ...
                webcam = MediaPlayer(device, format="dshow", options=options)
            else:
    ...
    async def create(cls, gsdbs):
        ...
        async def watcher(id):
            ...
            options = {
                "video_size": f"{self.gsdbs.credentials['hres']}x{self.gsdbs.credentials['vres']}",
                "preset": "veryfast",
                "framerate": self.gsdbs.credentials["framerate"],
                "c:v": "h264_v4l2m2m",
                "input_format": "h264",
                "pixelformat": "H264"
            }
            video = self.create_local_tracks(self.gsdbs.credentials["device"], options=options)
            pc.addTrack(video)
