...
class Chromosome:
    ...
    def __str__(self):
        Str = "%s-" * self.length
        Str = Str[:-1]
        return Str % tuple(self.gene_list)
...
class GA:
    ...
    def __str__(self):
        Str = "%s | " * len(self.chromosome_result)
        Str = Str[:-3]
        return Str % tuple(self.chromosome_result)
    ...
    def eliminate(self):
        ...
        new_chromosome = list()
        for choose_i in choose:
            ch_t = self.chromosome_result[randint_choose(chromosome_estimate, choose_i)]
            # if self.maxThreshold >= ch_t.estimate() >= self.minThreshold:
            new_chromosome.append(ch_t)
        self.chromosome_result = new_chromosome
...
if __name__ == '__main__':
    ...
    GA_result = GA(
        *Chromosome_group,
        ...
        group_times=1,
        mutation_pro=0.1,
        static_value=Gene_input_set.set(6)
    )
    GA_result.run()
