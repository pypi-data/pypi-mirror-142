...
@LastEditTime: 2022-03-01 17:02:49
...
class GetCrmIntegralHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        invoke_result_data = self.get_crm_point_available(user_nick, access_token)
...
class SyncPayOrderHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        #请求太频繁限制
        if self.check_post(f"SyncPayOrderHandler_Post_{str(act_id)}_{str(open_id)}", expire=3) == False:
