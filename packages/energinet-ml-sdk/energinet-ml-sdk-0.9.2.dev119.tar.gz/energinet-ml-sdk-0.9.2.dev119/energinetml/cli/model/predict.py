...
@click.command()
@click.option(
    "--json", "-j", "json_str", required=False, type=str, help="Input JSON string"
)
@click.option(
    "--json-file", "-f", "json_file", type=click.File("r"), help="Input JSON filepath"
)
@discover_model()
@discover_trained_model()
def predict(
    json_str: str, json_file: TextIO, model: Model, trained_model: TrainedModel
):
