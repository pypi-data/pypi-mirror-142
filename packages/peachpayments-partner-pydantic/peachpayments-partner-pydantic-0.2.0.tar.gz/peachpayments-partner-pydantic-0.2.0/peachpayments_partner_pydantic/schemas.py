...
from .helpers import Customer, Data, Timestamp, UTCDatetime, validate_currency, str_type_check
...
from .inbound_schemas import PaymentBrand as InboundPaymentBrand
...
from .outbound_schemas import PaymentBrand as OutboundPaymentBrand
...
class DebitRequest(OutboundDebitRequest):
    ...
    @classmethod
    def currency_type_check(cls, currency: OutboundCurrency) -> OutboundCurrency:
        """Validates that currency is a string."""
        return str_type_check(currency)

    @validator("paymentBrand", pre=True)
    @classmethod
    def payment_brand_type_check(cls, payment_brand: OutboundPaymentBrand) -> OutboundPaymentBrand:
        """Validates that paymentBrand is a string."""
        return str_type_check(payment_brand)
...
class WebhookRequest(InboundWebhookRequest):
    ...
    @validator("currency", pre=True)
    @classmethod
    def currency_type_check(cls, currency: InboundPaymentBrand) -> InboundPaymentBrand:
        """Validates that currency is a string."""
        return str_type_check(currency)

    @validator("paymentBrand", pre=True)
    @classmethod
    def payment_brand_type_check(cls, payment_brand: InboundPaymentBrand) -> InboundPaymentBrand:
        """Validates that paymentBrand is a string."""
        return str_type_check(payment_brand)
