...
class GAV3Reader:
    ...
    def _query_handler(self, view_id, service, start_index, date):
        """
    ...
    def run_query(self):
        ...
        for view_id in self.config.get("view_ids"):
            ...
            for date in date_range(self.config.get("start_date"), self.config.get("end_date")):
                print(f"Gathering data for {view_id} at date: {date}.")
                page_token = 1  # handle paging of response
                while True:
                    response = self._query_handler(
                        service=service, view_id=view_id, start_index=page_token, date=date
                    )
