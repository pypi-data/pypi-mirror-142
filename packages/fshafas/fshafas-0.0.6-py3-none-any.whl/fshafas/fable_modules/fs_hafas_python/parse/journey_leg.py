...
from ...fs_hafas_python.types_raw_hafas_client import (RawMsg, RawSec, RawPltf, RawGis, RawJny, RawPoly, RawCrd, RawFreq, RawStop)
...
RemarkRange_reflection = expr_427
...
def apply_remark_range(remark_range: RemarkRange, common_data: CommonData, stopover: StopOver) -> StopOver:
    def arrow_428(remark_range: RemarkRange=remark_range, common_data: CommonData=common_data, stopover: StopOver=stopover) -> List[Any]:
...
def parse_journey_leg(ctx: Context, pt: RawSec, date: str) -> Leg:
    ...
    if True if (True if (pt.type == "WALK") else (pt.type == "TRSF")) else (pt.type == "DEVI"):
        def arrow_429(ctx: Context=ctx, pt: RawSec=pt, date: str=date) -> Leg:
    ...
    if pt.type == "JNY":
        def action(jny: RawJny, ctx: Context=ctx, pt: RawSec=pt, date: str=date) -> None:
            ...
            match_value_3 : Tuple[Optional[List[RawMsg]], Optional[List[StopOver]]] = (jny.msg_l, stopovers)
            (pattern_matching_result, msg_l, stopovers_1) = (None, None, None)
            if match_value_3[0] is not None:
                if match_value_3[1] is not None:
                    pattern_matching_result = 0
                    msg_l = match_value_3[0]
                    stopovers_1 = match_value_3[1]
            ...
            match_value_4 : Optional[List[RawMsg]] = jny.msg_l
            ...
            msg_l_2 = None if (match_value_4 is None) else filter(predicate_1, match_value_4)
            current_location : Optional[Location]
            match_value_5 : Optional[RawCrd] = jny.pos
            if match_value_5 is None:
                current_location = None
            
            else: 
                pos : RawCrd = match_value_5
            ...
            match_value_6 : Optional[RawFreq] = jny.freq
            if match_value_6 is None:
                cycle = None
            
            else: 
                freq : RawFreq = match_value_6
                match_value_7 : Tuple[Optional[int], Optional[int]] = (freq.min_c, freq.max_c)
                def arrow_430(jny: RawJny=jny) -> Optional[Cycle]:
                    max_c : int = match_value_7[1] or 0
                    min_c : int = match_value_7[0] or 0
                    return Cycle(min_c * 60, max_c * 60, freq.num_c)
                
                def arrow_431(jny: RawJny=jny) -> Optional[Cycle]:
                    min_c_1 : int = match_value_7[0] or 0
                    return Cycle(min_c_1 * 60, None, None)
                
                cycle = (arrow_430() if (match_value_7[1] is not None) else arrow_431()) if (match_value_7[0] is not None) else None
            ...
            def binder_4(freq_1: RawFreq, jny: RawJny=jny) -> Optional[List[Alternative]]:
                def mapping(array_2: List[RawJny], freq_1: RawFreq=freq_1) -> List[Alternative]:
                    def parse_alternative(a: RawJny, array_2: List[RawJny]=array_2) -> Alternative:
                        ...
                        match_value_8 : Optional[List[RawStop]] = a.stop_l
                        (pattern_matching_result_1, stop_l_1) = (None, None)
                        if match_value_8 is not None:
                            if len(match_value_8) > 0:
                                pattern_matching_result_1 = 0
                                stop_l_1 = match_value_8
                        ...
                        return Alternative(a.jid, a.dir_txt, Default_Alternative.location, line_1, Default_Alternative.stop, bind(binder, parsed_when), bind(binder_1, parsed_when), bind(binder_2, parsed_when), bind(binder_3, parsed_when), Default_Alternative.platform, Default_Alternative.planned_platform, Default_Alternative.prognosed_platform, Default_Alternative.remarks, Default_Alternative.cancelled, Default_Alternative.load_factor, Default_Alternative.provenance, Default_Alternative.previous_stopovers, Default_Alternative.next_stopovers, Default_Alternative.frames, Default_Alternative.polyline, Default_Alternative.current_trip_position)
