...
def parse_movement(ctx: Context, m: RawJny) -> Movement:
    def arrow_404(ctx: Context=ctx, m: RawJny=m) -> Optional[Location]:
    ...
    def arrow_405(ctx: Context=ctx, m: RawJny=m) -> Optional[List[Frame]]:
    ...
    def arrow_406(ctx: Context=ctx, m: RawJny=m) -> Optional[FeatureCollection]:
    ...
    return Movement(m.dir_txt, m.jid, get_element_at(m.prod_x, ctx.common.lines), arrow_404(), Profile__get_parseStopovers(ctx.profile)(ctx)(m.stop_l)(value_1(m.date)), arrow_405(), arrow_406())
