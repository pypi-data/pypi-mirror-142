...
def parse_line(ctx: Context, p: RawProd) -> Line:
    ...
    def arrow_401(s: str, ctx: Context=ctx, p: RawProd=p) -> Optional[str]:
        return slug(s)
    
    id = (None if (name is None) else slug(name)) if (match_value is None) else bind(arrow_401, default_arg(match_value.line_id, name))
    ...
    def arrow_402(ctx: Context=ctx, p: RawProd=p) -> Optional[str]:
        cat_out_2 : str = cat_out
        return cat_out_2.strip()
    
    product_name : Optional[str] = (arrow_402() if (cat_out != "") else None) if (cat_out is not None) else None
    def arrow_403(ctx: Context=ctx, p: RawProd=p) -> IEnumerable[str]:
    ...
    product : Optional[str] = try_head(arrow_403())
