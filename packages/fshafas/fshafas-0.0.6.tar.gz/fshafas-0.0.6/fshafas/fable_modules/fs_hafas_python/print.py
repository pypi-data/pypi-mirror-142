...
from ..fable_library.option import (to_array, value as value_2, map)
...
def printfn_arr_l(ident: int, prefix: str, arr: Optional[List[_A]]=None) -> str:
    ...
    if pattern_matching_result == 0:
        def arrow_490(ident: int=ident, prefix: str=prefix, arr: Optional[List[_A]]=arr) -> str:
...
def Location(ident: int, location: Location_1) -> str:
    def arrow_491(ident: int=ident, location: Location_1=location) -> str:
    ...
    return ((arrow_491() + print_lon_lat(ident, location.longitude, location.latitude)) + nl) + print_distance(ident + 2, location.distance)


def Stop(ident: int, stop: Stop_1) -> str:
    def arrow_492(ident: int=ident, stop: Stop_1=stop) -> str:
    ...
    return ((print_name_id(ident, stop.name, stop.id) + arrow_492()) + nl) + print_distance(ident + 2, stop.distance)
...
def Alternative(ident: int, alternative: Alternative_1) -> str:
    def arrow_493(ident: int=ident, alternative: Alternative_1=alternative) -> str:
    ...
    return (printfn_s(ident, "direction: ", alternative.direction) + printfn_s(ident, "when: ", alternative.when)) + arrow_493()
...
def StopOver(ident: int, so: StopOver_1) -> str:
    def arrow_496(ident: int=ident, so: StopOver_1=so) -> str:
        match_value : Optional[Any] = so.stop
        def arrow_494(__unit: Any=None) -> str:
            s_1 : Station_1 = value_2(match_value)
            return printf_s(ident, "origin: ", "") + Station(0, s_1)
        
        def arrow_495(__unit: Any=None) -> str:
            s : Stop_1 = value_2(match_value)
            return printf_s(ident, "origin: ", "") + Stop(0, s)
        
        return (arrow_494() if isinstance(value_2(match_value), Station_1) else arrow_495()) if (match_value is not None) else ""
    
    def arrow_497(ident: int=ident, so: StopOver_1=so) -> str:
...
def Leg(ident: int, leg: Leg_1, short: bool) -> str:
    def arrow_501(ident: int=ident, leg: Leg_1=leg, short: bool=short) -> str:
        match_value : Optional[Any] = leg.origin
        def arrow_498(__unit: Any=None) -> str:
            s : Stop_1 = value_2(match_value)
            return printf_s(ident, "origin: ", "") + Stop(0, s)
        
        def arrow_499(__unit: Any=None) -> str:
            s_1 : Station_1 = value_2(match_value)
            return printf_s(ident, "origin: ", "") + Station(0, s_1)
        
        def arrow_500(__unit: Any=None) -> str:
            l : Location_1 = value_2(match_value)
            return printf_s(ident, "origin: ", "") + Location(0, l)
        
        return (arrow_498() if isinstance(value_2(match_value), Stop_1) else (arrow_499() if isinstance(value_2(match_value), Station_1) else arrow_500())) if (match_value is not None) else ""
    
    def arrow_502(ident: int=ident, leg: Leg_1=leg, short: bool=short) -> str:
    ...
    def arrow_503(ident: int=ident, leg: Leg_1=leg, short: bool=short) -> str:
        ...
        if match_value_2 is None:
            ...
            return (replicate(ident, " ") + "currentLocation: ") + print_lon_lat(0, location.longitude, location.latitude)
        
    
    def arrow_504(ident: int=ident, leg: Leg_1=leg, short: bool=short) -> str:
    ...
    return (((((((arrow_501() + arrow_502()) + printfn_s(ident, "departure: ", leg.departure)) + printfn_s(ident, "arrival: ", leg.arrival)) + StopOvers(ident, leg.stopovers)) + printfn_b(ident, "cancelled: ", leg.cancelled)) + arrow_503()) + nl) + ("" if short else ((((arrow_504() + printfn_b(ident, "walking: ", leg.walking)) + printfn_b(ident, "transfer: ", leg.transfer)) + ProductOfLeg(ident, leg)) + Remarks(ident, leg.remarks)))


def Trip(ident: int, trip: Trip_1) -> str:
    def arrow_505(ident: int=ident, trip: Trip_1=trip) -> str:
    ...
    def arrow_506(ident: int=ident, trip: Trip_1=trip) -> str:
    ...
    def arrow_507(ident: int=ident, trip: Trip_1=trip) -> str:
    ...
    return ((((((((arrow_505() + arrow_506()) + printfn_s(ident, "departure: ", trip.departure)) + printfn_s(ident, "arrival: ", trip.arrival)) + printfn_arr_l(ident, "stopovers: ", trip.stopovers)) + arrow_507()) + printfn_b(ident, "cancelled: ", trip.cancelled)) + printfn_b(ident, "walking: ", trip.walking)) + printfn_b(ident, "transfer: ", trip.transfer)) + Remarks(ident, trip.remarks)
...
def Journey(ident: int, journey: Journey_1) -> str:
    ...
    def arrow_508(ident: int=ident, journey: Journey_1=journey) -> str:
    ...
    def arrow_509(ident: int=ident, journey: Journey_1=journey) -> str:
        match_value_1 : Optional[str] = journey.refresh_token
        return "" if (match_value_1 is None) else printfn_s(ident + 2, "refreshToken: \u0027", match_value_1 + "\u0027")
    
    return (((printfn_s(ident, "jouney:", "") + Legs(ident, journey.legs, False)) + price_1) + arrow_508()) + arrow_509()
...
def Duration(ident: int, duration: Duration_1) -> str:
    def mapping(d: int, ident: int=ident, duration: Duration_1=duration) -> str:
    ...
    return printfn_s(ident, "duration: ", map(mapping, duration.duration)) + fold_1(folder, "", duration.stations)
...
def Movement(ident: int, m: Movement_1, with_stopovers: bool) -> str:
    def arrow_510(ident: int=ident, m: Movement_1=m, with_stopovers: bool=with_stopovers) -> str:
    ...
    def arrow_511(ident: int=ident, m: Movement_1=m, with_stopovers: bool=with_stopovers) -> str:
    ...
    return (((printfn_s(ident, "tripId: ", m.trip_id) + printfn_s(ident, "direction: ", m.direction)) + arrow_510()) + arrow_511()) + ((printfn_arr_l(ident, "stopovers: ", m.next_stopovers) + StopOvers(ident, m.next_stopovers)) if with_stopovers else "")
