...
from ...fs_hafas_python.types_hafas_client import (Operator, Line, Icon, FeatureCollection)
...
def update_operators(ctx: Context, ops: List[Operator]) -> Context:
    def arrow_350(ctx: Context=ctx, ops: List[Operator]=ops) -> CommonData:
    ...
    return Context(ctx.profile, ctx.opt, arrow_350(), ctx.res)


def update_lines(ctx: Context, lines: List[Line]) -> Context:
    def arrow_351(ctx: Context=ctx, lines: List[Line]=lines) -> CommonData:
        input_record : CommonData = ctx.common
    ...
    return Context(ctx.profile, ctx.opt, arrow_351(), ctx.res)
