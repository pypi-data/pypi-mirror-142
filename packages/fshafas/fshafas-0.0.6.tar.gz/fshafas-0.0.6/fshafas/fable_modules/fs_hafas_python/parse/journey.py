...
from ...fs_hafas_python.types_hafas_client import (IndexMap_2, IndexMap_2__set_Item_541DA560, IndexMap_2__ctor_2B594, Leg, Cycle, Journey, FeatureCollection)
...
def parse_journey(ctx: Context, j: RawOutCon) -> Journey:
    ...
    def arrow_419(ctx: Context=ctx, j: RawOutCon=j) -> List[Any]:
        value : List[Any] = []
        return default_arg(msg_lto_remarks(ctx, j.msg_l), value)
    
    remarks : Optional[List[Any]] = arrow_419() if ctx.opt.remarks else None
    ...
    def arrow_422(ctx: Context=ctx, j: RawOutCon=j) -> Optional[Cycle]:
        ...
        if match_value_1 is None:
            ...
            def arrow_420(__unit: Any=None) -> Optional[Cycle]:
                max_c : int = match_value_2[1] or 0
                min_c : int = match_value_2[0] or 0
                return Cycle(min_c * 60, max_c * 60, freq.num_c)
            
            def arrow_421(__unit: Any=None) -> Optional[Cycle]:
                min_c_1 : int = match_value_2[0] or 0
                return Cycle(min_c_1 * 60, None, None)
            
            return (arrow_420() if (match_value_2[1] is not None) else arrow_421()) if (match_value_2[0] is not None) else None
        
    
    return Journey(Default_Journey.type, legs, j.ctx_recon, remarks, Default_Journey.price, arrow_422(), scheduled_days)


def distance_of_journey(j: Journey) -> float:
    def arrow_423(fc: FeatureCollection, j: Journey=j) -> float:
    ...
    class ObjectExpr426:
        ...
        def GetZero(self) -> Any:
            def arrow_424(__unit: Any=None) -> int:
                return 0
        ...
        @property
        def Add(self) -> Any:
            def arrow_425(x: float, y: float) -> float:
                return x + y
            
            return arrow_425
        
    return sum_by(arrow_423, choose(chooser, j.legs, None), ObjectExpr426())
