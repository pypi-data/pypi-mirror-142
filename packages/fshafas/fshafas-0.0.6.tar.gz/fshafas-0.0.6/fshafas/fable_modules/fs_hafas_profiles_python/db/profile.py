...
from ...fable_library.option import (bind, value as value_2, some)
...
HintByCode_reflection = expr_512
...
def parse_journey_leg_with_load_factor(parsed: Leg, ctx: Context, pt: RawSec, date: str) -> Leg:
    def arrow_513(parsed: Leg=parsed, ctx: Context=ctx, pt: RawSec=pt, date: str=date) -> Optional[MutableSequence[int]]:
    ...
    def arrow_514(parsed: Leg=parsed, ctx: Context=ctx, pt: RawSec=pt, date: str=date) -> Optional[List[RawTcoc]]:
        match_value_2 : Optional[RawCommon] = ctx.res.common
        return None if (match_value_2 is None) else match_value_2.tcoc_l
    
    match_value_3 : Tuple[Optional[MutableSequence[int]], Optional[List[RawTcoc]]] = (arrow_513(), arrow_514())
...
def parse_arr_or_dep_with_load_factor(parsed: Alternative, ctx: Context, d: RawJny) -> Alternative:
    def arrow_515(parsed: Alternative=parsed, ctx: Context=ctx, d: RawJny=d) -> Optional[MutableSequence[int]]:
    ...
    def arrow_516(parsed: Alternative=parsed, ctx: Context=ctx, d: RawJny=d) -> Optional[List[RawTcoc]]:
        match_value_2 : Optional[RawCommon] = ctx.res.common
        return None if (match_value_2 is None) else match_value_2.tcoc_l
    
    match_value_3 : Tuple[Optional[MutableSequence[int]], Optional[List[RawTcoc]]] = (arrow_515(), arrow_516())
    (pattern_matching_result, tcoc_l_1, tcoc_x_1) = (None, None, None)
    if match_value_3[0] is not None:
        if match_value_3[1] is not None:
            pattern_matching_result = 0
...
def parse_journey_with_price(parsed: Journey, raw: RawOutCon) -> Journey:
    ...
    if match_value is not None:
        def arrow_518(parsed: Journey=parsed, raw: RawOutCon=raw) -> bool:
    ...
    if pattern_matching_result == 0:
        ...
        if match_value_1 is not None:
            def arrow_517(parsed: Journey=parsed, raw: RawOutCon=raw) -> bool:
...
def format_loyalty_card(first_class: bool, data: LoyaltyCard) -> int:
...
def transform_journeys_query(opt: Optional[JourneysOptions], q: TripSearchRequest) -> TripSearchRequest:
    def arrow_519(v: JourneysOptions, opt: Optional[JourneysOptions]=opt, q: TripSearchRequest=q) -> Optional[bool]:
        return v.bike
    
    bike : bool = get_option_value(opt, arrow_519, Default_JourneysOptions)
    def arrow_520(v_1: JourneysOptions, opt: Optional[JourneysOptions]=opt, q: TripSearchRequest=q) -> Optional[bool]:
        return v_1.first_class
    
    first_class : bool = get_option_value(opt, arrow_520, Default_JourneysOptions)
    def arrow_521(opt: Optional[JourneysOptions]=opt, q: TripSearchRequest=q) -> Optional[int]:
        opt_2 : JourneysOptions = opt
        return format_loyalty_card(first_class, value_2(opt_2.loyalty_card))
    
    return TripSearchRequest(q.get_passlist, q.max_chg, q.min_chg_time, q.dep_loc_l, q.via_loc_l, q.arr_loc_l, append([bike_fltr], q.jny_fltr_l, None) if bike else q.jny_fltr_l, q.gis_fltr_l, q.get_tariff, q.ushrp, q.get_pt, q.get_iv, q.get_polyline, q.out_date, q.out_time, q.num_f, q.out_frwd, TrfReq(1 if first_class else 2, [TvlrProf("E", (arrow_521() if (opt.loyalty_card is not None) else None) if (opt is not None) else None)], "PK"))


req : RawRequest = RawRequest("de", [], RawRequestClient("DB", "19040000", "AND", "DB Navigator"), "DB.R20.12.b", "1.34", RawRequestAuth("AID", "n91dB8Z77MLdoR0K"))
