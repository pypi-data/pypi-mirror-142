...
class SDK:
    ...
    def cancel_order(self, order_result, **kwargs):
        ...
        unit = self.__core.get_volume_per_unit(order_result["stockno"])
        excuted_celqty = None
        ...
        if "celqty" not in kwargs and "celqty_share" not in kwargs:
            return loads(self.__core.modify_volume(order_result, 0))["data"]

        if "celqty" in kwargs:
            if apcode in (3, 4, 5):
                excuted_celqty = kwargs["celqty"] * unit
            else:
        ...
        if "celqty_share" in kwargs:
            if apcode in (3, 4, 5):
                ...
                if kwargs["celqty_share"] % unit != 0:
                    raise TypeError("must be multiplys of " + unit)
                excuted_celqty = int(kwargs["celqty_share"]/unit)
    ...
    def get_transactions(self, query_range):
        ...
        return loads(transactions_res)["data"]['matsums']

    def get_inventories(self):
        """get inventories data 庫存資訊"""
        inventories_res = self.__core.get_inventories()
        return loads(inventories_res)["data"]['stksums']
