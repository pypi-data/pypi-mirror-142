...
class SDK:
    ...
    def place_order(self, order: OrderObject):
        """place order"""
        return loads(self.__core.order(order))["data"]
    ...
    def cancel_order(self, order_result):
        """cancel_order"""
        return loads(self.__core.modify_volume(order_result, 0))["data"]

    def modify_volume(self, order_result, **kwargs):
        """modify_volume"""
        apcode = int(order_result["apcode"])
        excuted_celqty = None
        if "celqty" in kwargs and "celqty_share" in kwargs:
            raise TypeError("celqty or celqty_share, not both")

        if "celqty" in kwargs:
            if apcode in (3, 4, 5):
                excuted_celqty = kwargs["celqty"] * 1000
            else:
                excuted_celqty = kwargs["celqty"]

        if "celqty_share" in kwargs:
            if apcode in (3, 4, 5):
                excuted_celqty = kwargs["celqty_share"]
            else:
                if kwargs["celqty_share"] % 1000 != 0:
                    raise TypeError("must be multiplys of 1000")
                excuted_celqty = int(kwargs["celqty_share"]/1000)

        if not excuted_celqty:
            raise TypeError("must provide celqty or celqty_share")

        return loads(self.__core.modify_volume(order_result, excuted_celqty))["data"]

    def modify_price(self, order_result, targetPrice):
        """modify_price"""
        return loads(self.__core.modify_price(order_result, targetPrice))["data"]

    def get_order_results(self):
        """get order result data"""
        order_res = self.__core.get_order_result()
    ...
    def get_transactions(self, query_range):
        ...
        return loads(transactions_res)["data"]
    ...
    def get_settlements(self):
        ...
        return loads(settlements_res)["data"]
    ...
    def process_order_results(self, order_results):
        order_results["data"]["orderResults"] = list(map(self.process_order_result, order_results["data"]["orderResults"]))
        return order_results["data"]["orderResults"]
