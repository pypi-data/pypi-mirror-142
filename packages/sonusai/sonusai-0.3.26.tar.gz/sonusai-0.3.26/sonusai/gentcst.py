...
def gentcst(fold: str = '*',
    ...
    if config['truth_mode'] == 'mutex' and hierarchical:
        logger.exception('Multi-class truth is incompatible with truth_mode mutex')
        raise SonusAIError
...
def get_name_from_id(ontology_data: list, identity: str) -> list:
    ...
    for item in ontology_data:
        if item['id'] == identity:
            if found:
                logger.exception(f'id {identity} appears multiple times in ontology')
                raise SonusAIError
...
def get_id_from_name(ontology_data: list, name: str) -> str:
    ...
    for item in ontology_data:
        if name in item['name']:
            if found:
                logger.exception(f'name {name} appears multiple times in ontology')
                raise SonusAIError
...
def get_config(config_name: str) -> dict:
    if not exists(config_name):
        logger.exception('No confg.yml at top level')
        raise SonusAIError
    ...
    if 'feature' not in config.keys():
        logger.exception('feature not in top level config')
        raise SonusAIError

    if 'truth_function' not in config.keys():
        logger.exception('truth_function not in top level config')
        raise SonusAIError

    if 'truth_config' not in config.keys():
        logger.exception('truth_config not in top level config')
        raise SonusAIError

    if 'truth_mode' not in config.keys():
        logger.exception('truth_mode not in top level config')
        raise SonusAIError

    if config['truth_mode'] not in ['normal', 'mutex']:
        logger.exception('Invalid truth_mode in top level config')
        raise SonusAIError
