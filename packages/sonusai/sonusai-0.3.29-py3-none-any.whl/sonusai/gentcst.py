...
def gentcst(fold: str = '*',
    ...
    for file in use_files:
        ...
        if 'truth_function' in local_config:
            target['truth_function'] = local_config['truth_function']
        if 'truth_config' in local_config:
...
def get_config(config_name: str) -> dict:
    if not exists(config_name):
        logger.error('No config.yml at top level')
        raise SonusAIError

    with open(file=config_name, mode='r') as yaml_file:
    ...
    if 'truth_function' not in config:
        logger.error('truth_function not in top level config')
        raise SonusAIError

    if 'truth_config' not in config:
