...
def genft_cli(mixdb: dict,
    ...
    genft_global['noise_audios'] = build_noise_audio_db(mixdb=mixdb_out, show_progress=True)
    genft_global['target_audios'] = build_target_audio_db(mixdb=mixdb_out, show_progress=True)

    chunk_size = 100
    progress = tqdm(total=len(mixid), desc='genft')
    mixid = grouper(load_mixid(mixdb_out), chunk_size)
    ...
    for m in mixid:
        ...
        if o_frames != truth_f.shape[0]:
            logger.error(f'truth_f frames does not match feature frames: {truth_f.shape[0]} != {o_frames}')
            raise SystemExit(1)
...
def main():
    try:
        ...
        if not output:
            output = splitext(mixdb_name)[0] + '.h5'
        ...
        genft_cli(mixdb=mixdb, mixid=mixid, output=output, compute_segsnr=compute_segsnr)
