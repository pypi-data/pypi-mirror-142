...
class SafeCreate2TxBuilder:
    ...
    def build(
        ...
        estimated_gas: int = self._estimate_gas(
            safe_setup_data, salt_nonce, owners[0], payment_token, payment_receiver
        )
    ...
    def _calculate_refund_payment(
        ...
        if fixed_creation_cost is None:
            ...
            if (payment_token_eth_value == 0): return 0
    ...
    def _estimate_gas(
        ...
        gas: int = self.proxy_factory_contract.functions.createProxyWithNonce(
            self.master_copy_address, initializer, salt_nonce, owner
        ).estimateGas()
