...
class SequentialBpmnEngine(BpmnEngine):
    ...
    def invoke_by_event(self, event: Event, init_state: OptDict = None, headers: OptDict = None) -> List[State]:
        def invoke(subscription: NodeRef):
            ...
            state = State(init_state or {}, subscription, is_reentry, headers=headers)
    ...
    def resume_by_event(self, event: Event) -> List[State]:
    ...
    def invoke_by_id(self, ref: ProcessRef, init_state: OptDict = None, headers: OptDict = None) -> State:
        return self.invoke_by_event(NoneEvent(ref.group, ref.process_id), init_state, headers)[0]
