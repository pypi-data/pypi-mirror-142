...
class SequentialBpmnEngine(BpmnEngine):
    ...
    def invoke_by_event(self, event: Event, init_state: Optional[dict] = None) -> List[State]:
        def invoke(subscription: NodeRef):
            ...
            state = State(init_state or {}, subscription, is_reentry)

            result = self._handle_action(ContinueAction(subscription.node_id), state, environment)
            return result[0]

        # FIXME: consume if target is not a start event
        subscriptions = self.event_registry.get_subscriptions(event, consume=False)
    ...
    def invoke_by_id(self, ref: ProcessRef, init_state: Optional[dict] = None) -> State:
        return self.invoke_by_event(NoneEvent(ref.group, ref.process_id), init_state)[0]
    ...
    def _handle_action(self, action: Action, state: State, env: Environment) -> List[State]:
        ...
        # ************
            ...
            if action.event is not None:
                ...
                self.event_registry.create_subscription(action.event, state.reference)
