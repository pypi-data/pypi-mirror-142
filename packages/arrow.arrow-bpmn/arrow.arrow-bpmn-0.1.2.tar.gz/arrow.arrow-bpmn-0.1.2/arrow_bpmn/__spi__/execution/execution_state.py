...
class State:
    ...
    def with_is_reentry(self, is_reentry: bool):
        return State(self.properties, self.reference, is_reentry, self.parent_reference, self.headers)

    def with_parent_reference(self, parent_reference: Optional[NodeRef]):
        return State(self.properties, self.reference, self.is_reentry, parent_reference, self.headers)

    def with_reference(self, reference: NodeRef):
        return State(self.properties, reference, self.is_reentry, self.parent_reference, self.headers)
