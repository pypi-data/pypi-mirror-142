...
class BusinessLogic:
    ...
    """
    def __init__(self, login: str, url: str, password: str = None, session_id: str = None, is_guest: bool = False,
    ...
    def _convert_schedule_item(self, user_schedule: dict):
        """
        ...
        user_schedule["time_zone"] = self.server_codes["manager"]["timezone"][h_timezone]
    ...
    def create_sphere(self, cube_name: str, source_name: str, file_type: str, update_params: dict,
        ...
        """
        sql_params = dict() if sql_params is None else sql_params
        interval_borders = list() if interval_borders is None else interval_borders
        date_format = "%d.%m.%Y"
        ...
        try:
            param_types = [
                ...
                ["update_params", update_params, dict],
            ...
            error_handler.checks(
                self, self.func_name, update_params, file_type, sql_params, user_interval, source_name, cube_name,
                self.server_codes["manager"]["timezone"], increment_dim, interval_dim, interval_borders, date_format)
        except Exception as e:
            return self._raise_exception(ValueError, str(e))

        # определяем пользовательский планировщик
        user_schedule = update_params.get("schedule")
        ...
        if update_params["type"] != "ручное":
            for schedule_item in schedule_items_tmp:
                self._convert_schedule_item(schedule_item)
        ...
        # подготавливаем параметры для создания
        update_type = update_params["type"]
        command_params = {
        ...
        if update_type == "ручное":
            ...
            for dim in dims:
                if dim.get("name") == increment_dim:
                    ...
                    if current_type not in ("uint8", "uint16", "uint32", "uint64", "double", "date", "time", "datetime"):
    ...
    def update_cube(self, cube_name: str, update_params: Dict, user_interval: str = "с текущего дня",
                    delayed: bool = False, increment_dim: str = None) -> Dict:
        """
        ...
        if update_params["type"] != "ручное":
            ...
            update_params["schedule"]["type"] = UPDATE_PERIOD[user_period]
            ...
            Для типов графиков [1, "lines"], [2, "cylinders"], [3, "cumulative_cylinders"], [4, "areas"],
                ...
                {'OX': <value>, 'OY': <value>, 'short_format': <value>}, где
                    ...
                    division_value - цена деления;
                        ...
                        по-умолчанию "no"; актуально только для версии 5.7
