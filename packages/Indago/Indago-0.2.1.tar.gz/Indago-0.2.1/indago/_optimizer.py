...
class Optimizer:
    ...
    def __init__(self):
        ...
        self.it = 0
        self.eval = 0
        self.stalled_it = None
        self.stalled_eval = None
    ...
    def _init_optimizer(self):
        ...
        if self.monitoring == 'none':
            ...
            def update_progress_bar():
                ...
                if self.maximum_stalled_iterations:
                    progress.append(self.stalled_it)
                    report_str += f'\nStalled iterations: {self.stalled_it}'
                ...
                if self.maximum_stalled_evaluations:
                    progress.append(self.stalled_eval)
                    report_str += f'\nStalled evaluations: {self.stalled_eval}'
    ...
    def title(msg, line_length=60):
        """Utility function for consistently displaying titles in the output. 
        The text of the title is decorated and printed on the center of the screen.
        ...
        msg: str
            A text of the title.
        line_length: int
            A length (number of characters) of the line used for drawing the 
        decoration border and centering the title. Default is 60.
    ...
    def _stopping_criteria(self):
        ...
        if self.maximum_stalled_iterations:
            ...
            self.stalled_it = np.size(np.where(f_hist==f_hist[-1])) - 1
            if self.stalled_it > self.maximum_stalled_iterations:
                if self.monitoring == 'basic':
                    self.log(f'stalled for the last {self.maximum_stalled_iterations} iterations, optimization stopped')
                return True
        ...
        if self.maximum_stalled_evaluations:
            f_hist = np.array([r[2].f for r in self.results.cHistory])
            self.stalled_it = np.size(np.where(f_hist==f_hist[-1])) - 1
            self.stalled_eval = self.eval - self.results.cHistory[-1-self.stalled_it][1]
            if self.stalled_eval > self.maximum_stalled_evaluations:
                if self.monitoring == 'basic':
                    self.log(f'stalled for the last {self.maximum_stalled_evaluations} evaluations, optimization stopped')
                return True
    ...
    def collective_evaluation(self, candidates):
        ...
        # Record to history
        self.results.cHistory.append((self.it, self.eval, self.best))
        # some algorithms evaluate more than once per iteration
        if self.it >= 1 and self.results.cHistory[-2][0] == self.it:
            self.results.cHistory.pop(-2)
