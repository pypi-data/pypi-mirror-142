...
def calculate_VSH(df: pd.DataFrame, **kwargs) -> pd.DataFrame:  # noqa: C901
    ...
    non_shale_window_threshold: float = kwargs.get("non_shale_window_threshold", 0.01)
    system_dict = {
        "Nordland": ["NORDLAND"],
        "Hordaland": ["HORDALAND"],
        "Rogaland": ["ROGALAND"],
        "preCretaceous": ["UNKNOWN"],
    ...
    def _calculate_VSH(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if f_col is not None:
            mask = df[f_col].str.contains("DRAUPNE")
            if mask.sum() > 0:
                last_group = df.loc[mask, g_col].unique()[-1]
        ...
        for group_name, group_series in df.groupby(g_col, dropna=False, sort=False)[
            ...
            for i, sub_group_series in sub_groups:
                ...
                rolling_quantiles = sub_group_series.rolling(
                    min(rolling_window_size, sub_group_series.size // 2),
                    center=True,
                ).quantile(quant_sh)
                ...
                if i == 1:
                    df.loc[rolling_quantiles.index, "INDEX_GROUPS"] = group_name
                else:
                    df.loc[
        ...
        if non_shale_window_threshold is not None:
            ...
            def sign_change(start: int, end: int) -> Tuple[int, int]:
                # Find start and end by looking at sign change of first
                # derivative of GR_sh_Gp_f
                with np.errstate(invalid="ignore"):
                    try:
                        before = df.loc[:start, "GR_sh_Gp_f"][::-1]
                        first_sign_change = (
                            np.where(
                                np.diff(
                                    pd.Series(np.sign(np.gradient(before)))
                                    .replace(0, np.nan)
                                    .fillna(method="ffill")
                                )
                                != 0
                        ...
                        inter_start = before.index[first_sign_change]
                    except ValueError:
                        if start == df.index[0]:  # Cover edge cases
                            inter_start = df.index[0]
                        else:
                            raise

                    try:
                        after = df.loc[end:, "GR_sh_Gp_f"]
                        first_sign_change = (
                            np.where(
                                np.diff(
                                    pd.Series(np.sign(np.gradient(after)))
                                    .replace(0, np.nan)
                                    .fillna(method="ffill")
                                )
                                != 0
                        ...
                        inter_end = after.index[first_sign_change]
                    except ValueError:
                        if after == df.index[-1]:  # Cover edge case
            ...
            df["Threshold"] = df["GR_ss"] / (
                df["GR_sh_Gp_f_NS"] * (df["GR_sh_Gp_f_NS"] - df["GR_ss"])
            )
            mask = df["Threshold"] >= non_shale_window_threshold

            # Define areas where non_shale_window_threshold was violated
            counter = (mask != mask.shift(1)).cumsum()
            threshold_violations = (
                counter.index.to_series()
                .groupby(counter, sort=False)
                .agg(["first", "last"])
            )
            for _, row in threshold_violations.iterrows():
                ...
                if not mask.loc[start:end].unique()[0]:
                    # Does not contain threshold violation
                    continue
                elif len(groups) == 1:
                    # Threshold violation applies to only one group so apply
                    # group specific logic
                    group = df.loc[df["INDEX_GROUPS"] == groups[0]]
                    ...
                    if start == group_start and end == group_end:
                        ...
                        # group. This accounts for step changes
                        ...
                        try:
                            inter_start, inter_end = sign_change(start, end)
                        except ValueError:
                            # Not enough data points to calculate a gradient in this
                            # threshold violation window. Ignore it
                            continue
                        # Don't allow inter_start, inter_end to go beyond
                        # right before and right after the group
                        if inter_start < group_start:
                            inter_start = group_start - 1
                        if inter_end > group_end:
                            inter_end = group_end + 1
                ...
                # Constrain inter_start to within the bounds of the df to prevent
                # index errors
        ...
        # Drop unused columns
        df = df.drop(
            columns=["Age", "GR_ss", "GR_sh_Gp_f", "GR_sh_Gp_f_NS"], errors="ignore"
        )
    ...
    # Standardize g_col
    df[g_col] = df[g_col].apply(utilities.standardize_group_formation_name)
    ...
    if f_col is None:
        ...
        df[f_col] = df[f_col].apply(utilities.standardize_group_formation_name)
    # Process per well
    well_names = df[id_column].unique()
    dfs = []
