...
from marketmanager.models import (
    Order, PublicConfig, Structure, TypeStatistics, WatchConfig,
)
...
def update_all_type_statistics():
    ...
    # This task is not optimized. Dont run it
    for eve_region in EveRegion.objects.all():
        logger.debug(f"update_all_type_statistics Region {eve_region.name}")
        for eve_type in EveType.objects.filter(eve_market_group__isnull=False):
            logger.debug(f"update_all_type_statistics Region {eve_region.name} EveType {eve_type.name}")
            try:
                type_statistic = TypeStatistics.objects.update_or_create(
                    eve_type = eve_type,
                    eve_region = eve_region,
                    defaults = {
                        'buy_5_percent' : fifth_percentile(eve_type = eve_type, eve_region = eve_region, buy_order = True),
                        'sell_5_percent': fifth_percentile(eve_type = eve_type, eve_region = eve_region, buy_order = False),
