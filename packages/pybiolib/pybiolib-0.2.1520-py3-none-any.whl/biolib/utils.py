...
DownLoadChunkReturnTuple = Tuple[ByteRangeTuple, bytes]


def _download_chunk(input_tuple: DownloadChunkInputTuple) -> DownLoadChunkReturnTuple:
    ...
    for retry_attempt in range(max_download_retries):
        ...
        try:
            ...
            if response.ok:
                ...
                return byte_range, return_value
            else:
                logger_no_user_data.warning(
...
def download_presigned_s3_url(presigned_url: str, output_file_path: str) -> None:
    ...
    def get_chunk_input_iterator() -> Iterator[DownloadChunkInputTuple]:
    ...
    chunk_input_iterator = get_chunk_input_iterator()

    # use 16 cores, unless less is available
    parallel_download_processes_count = min(16, multiprocessing.cpu_count() - 1)
    chunk_collection_size = parallel_download_processes_count
    chunk_collection_count = math.ceil(chunk_count / chunk_collection_size)

    with multiprocessing.Pool(processes=parallel_download_processes_count) as process_pool:
        with open(output_file_path, 'ab') as output_file:
            bytes_written = 0

            for chunk_collection in range(0, chunk_collection_count):
                # The 'imap' method of a process pool consumes its input iterable with no regards for the consumption
                # of the result iterator that it returns.
                # To avoid keeping more than (parallel_download_processes_count * chunk_size) bytes in memory at once
                # we split in chunk collection to let a potentially slow file system write to disk

                last_chunk_in_current_collection = (chunk_collection + 1) * chunk_collection_size
                chunk_input_iterator = itertools.islice(chunk_input_iterator, last_chunk_in_current_collection)
                logger_no_user_data.debug(
                    f'PresignedUrlDownloader: Created chunk input iterator: {last_chunk_in_current_collection}'
                )

                return_tuple: DownLoadChunkReturnTuple  # for typing hinting
                for return_tuple in process_pool.imap(func=_download_chunk, iterable=chunk_input_iterator):
                    (start, end), data = return_tuple

                    logger_no_user_data.debug(f'Writing part {start}:{end} to file...')
                    output_file.write(data)

                    bytes_written += chunk_size
                    approx_progress_percent = min(bytes_written / file_size * 100, 100)
                    logger_no_user_data.debug(
                        f'Wrote part {start}:{end} of {file_size} to file, '
