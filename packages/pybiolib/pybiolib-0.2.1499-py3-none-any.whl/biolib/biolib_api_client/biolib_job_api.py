...
class BiolibJobApi:
    ...
    @staticmethod
    def get_job_storage_result_upload_url(job_id) -> str:
        config = CloudUtils.get_webserver_config()
        response = requests.get(
            f'{BiolibApiClient.get().base_url}/api/jobs/{job_id}/storage/results/upload/',
            auth=BearerAuth(BiolibApiClient.get().access_token),
            headers={
                'Compute-Node-Auth-Token': config["compute_node_info"]["auth_token"]  # pylint: disable=unsubscriptable-object
            }
        )
        ...
        presigned_s3_upload_link_response: PresignedS3UploadLinkResponse = response.json()
        return presigned_s3_upload_link_response['presigned_upload_url']
