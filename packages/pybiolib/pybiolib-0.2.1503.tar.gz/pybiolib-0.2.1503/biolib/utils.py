...
def download_presigned_s3_url(presigned_url: str, output_file_path: str) -> None:
    ...
    chunks = [((i, i + chunk_size - 1), presigned_url) for i in range(0, file_size, chunk_size)]
    ...
    with multiprocessing.Pool(processes=min(16, multiprocessing.cpu_count() - 1)) as process_pool:
        chunk_iterator = process_pool.imap(_download_chunk, chunks)

        bytes_written = 0
        with open(output_file_path, 'ab') as output_file:
            return_tuple: DownLoadChunkReturnTuple  # for typing hinting
            for return_tuple in chunk_iterator:
