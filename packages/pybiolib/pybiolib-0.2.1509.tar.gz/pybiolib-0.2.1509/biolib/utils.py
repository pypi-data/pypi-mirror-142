...
def download_presigned_s3_url(presigned_url: str, output_file_path: str) -> None:
    ...
    sub_chunk_size = parallel_download_processes_count

    with multiprocessing.Pool(processes=parallel_download_processes_count) as process_pool:
        with open(output_file_path, 'ab') as output_file:
            bytes_written = 0

            for sub_chunk in range(sub_chunk_size, chunk_count * sub_chunk_size, sub_chunk_size):
                ...
                # we split in sub chunks to let a potentially slow file system write to disk
                logger_no_user_data.debug(f'PresignedUrlDownloader: Creating sub chunk input iterator: {sub_chunk}')
                sub_chunk_input_iterator = itertools.islice(chunk_input_iterator, sub_chunk)

                return_tuple: DownLoadChunkReturnTuple  # for typing hinting
                for return_tuple in process_pool.imap(func=_download_chunk, iterable=sub_chunk_input_iterator):
