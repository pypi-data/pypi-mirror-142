...
from AllInOneInstagramBot import __file__
from AllInOneInstagramBot.core.log import get_log_file_config
...
from AllInOneInstagramBot.core.storage import ACCOUNTS
from AllInOneInstagramBot.version import __version__
...
def update_available():
    ...
    if "b" not in __version__:
        version_request = "https://raw.githubusercontent.com/AllInOneInstagramBot/bot/master/AllInOneInstagramBot/version.py"
    else:
        version_request = "https://raw.githubusercontent.com/AllInOneInstagramBot/bot/develop/AllInOneInstagramBot/version.py"
    try:
        ...
        logger.error(
            f"There was an error retrieving the latest version of AllInOneInstagramBot: {e}"
        )
        return False, False
    if "b" not in __version__:
...
def check_if_updated(crash=False):
    ...
    if new_update:
        ...
        logger.warning(
            f"Version {version} has been released! Please update so that you can get all the latest features and bugfixes. Changelog here -> https://github.com/AllInOneInstagramBot/bot/blob/master/CHANGELOG.md"
        )
        logger.warning("HOW TO UPDATE:")
        logger.warning("If you installed with pip: pip3 install AllInOneInstagramBot -U")
    ...
    if not crash:
        logger.info(
            f"AllInOneInstagramBot v.{__version__}",
...
def show_ending_conditions():
    ...
    logger.info(
        "For more info -> https://github.com/AllInOneInstagramBot/docs/blob/main/configuration.md#ending-session-conditions",
...
def init_on_things(source, args, sessions, session_state):
    ...
    from AllInOneInstagramBot.core.interaction import _on_interaction
