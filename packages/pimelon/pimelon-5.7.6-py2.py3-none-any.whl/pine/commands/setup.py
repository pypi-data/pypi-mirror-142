...
def setup_nginx(yes=False):
	...
	make_nginx_conf(pine_path=".", yes=yes)
...
def setup_supervisor(user=None, yes=False, skip_redis=False):
	from pine.config.supervisor import update_supervisord_config, generate_supervisor_config
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	...
	setup_wildcard_ssl(domain, email, pine_path=".", exclude_base_domain=exclude_base_domain)
...
def setup_manager(yes=False, port=23624, domain=None):
	...
	if pine.conf.get("restart_supervisor_on_update") or pine.conf.get("restart_systemd_on_update"):
		# implicates a production setup or so I presume
		if not domain:
			print("Please specify the site name on which you want to host guipine using the 'domain' flag")
			sys.exit(1)
