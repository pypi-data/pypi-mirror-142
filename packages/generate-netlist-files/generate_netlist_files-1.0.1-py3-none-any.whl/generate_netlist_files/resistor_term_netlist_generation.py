def resistor_term_netlist_generation(zero_order_file):
    '''
    ...
    The netlist files will contain a 50 Ohm .
    ...
    for item in first_order_items: 
        with open(f"Order_{order_count:02d}_{item}.txt", "w") as f:
            ...
            if item == "sc": # Write series capacitor file
                ...
                f.write(f"Port:TermG1  N__{node_count+1} {gnd} Num=1 Z=50 Ohm Noise=yes\n")
                f.close()
                ...
                f.write(f"Port:TermG1  N__{node_count} {gnd} Num=1 Z=50 Ohm Noise=yes\n")
                f.close()
    ...
    print(f"All resistor terminated netlist files up to order {max_order:02d} have been created.")